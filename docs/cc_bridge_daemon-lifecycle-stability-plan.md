# CC_BRIDGE_DAEMON 生命周期稳定性与时序收敛方案

## 1. 文档定位

这份文档是项目级 `cc-bridge-daemon` 生命周期、keeper 启停权、socket 绑定、显式 `kill` 语义、以及 provider helper ownership 的详细设计方案。
...
- CLI 与 keeper 同时直接启动 `cc-bridge-daemon`
- `mounted` 出现在 socket ready 之前
- 显式 `cc-bridge kill` 被 `socket_unreachable` 的 `degraded` 状态阻断
...
1. `pid_alive=true`、`heartbeat_fresh=true`、`socket_unreachable=true`
   - 普通 `cc-bridge kill` 被错误阻断
   - `cc-bridge ask`/`cc-bridge` 只能看到 “进程活着但不可接管”的矛盾状态
2. provider helper/bridge 与 slot/runtime generation 解绑
   - `PPID=1` 的孤儿 bridge 长期堆积
   - 单个项目能反复拉起大量无主后台进程
   - 最终形成 CPU 调度风暴与 swap 压力

3. 冷启动 `cc-bridge ask` 把慢启动误判为失败
   - `ask` 首次请求本质上只是想等 control-plane ready 并提交任务
   - 但当前实现把 keeper 拉起、`cc-bridge-daemon` bootstrap、socket readiness、以及本地 CLI deadline 混在一条短预算链路里
...
一个 .cc-bridge 项目
  ...
  = 一个 keeper
  = 一个 authoritative cc-bridge-daemon generation
...
- keeper 是项目生命周期唯一推进者
- CLI 只能表达启动/停机意图，不能直接与 keeper 竞争 `cc-bridge-daemon` 启动权
...
- 显式 `cc-bridge kill` 是强管理动作，不能被 “degraded but heartbeat fresh” 拒绝
...
`cc-bridge-daemon` 是项目 control-plane 的执行者，不是生命周期 authority 的拥有者。
...
- 在交互式 `cc-bridge` 中执行 foreground attach

CLI 不再直接拥有 `spawn_cc-bridge-daemon_process()` 的 authority。
...
- 为 CLI/keeper/cc-bridge-daemon 提供统一事实来源
...
- 当前 `cc-bridge-daemon` 实例的 liveness authority
...
- `unmounted`
  - 当前没有 authoritative `cc-bridge-daemon`
  - 项目控制面完整停机
- `starting`
  - keeper 已拥有启动事务
...
keeper 是 `cc-bridge-daemon` 唯一启动者。

CLI 不再直接 spawn `cc-bridge-daemon`。CLI 只负责：
...
8. keeper spawn child `cc-bridge-daemon`
...
- `cc-bridge ask`、`ping`、`pend`、`watch`、`queue` 等 daemon RPC caller 只等待 control-plane readiness
- 交互式 `cc-bridge` 先等待 control-plane readiness，再等待 namespace UI readiness
...
- `rpc_probe_timeout_s`
  ...
  - keeper config-check / graceful-shutdown probe 与 spawned cc-bridge-daemon readiness probe 必须共享这层预算，不得再使用私有更短字面量
  - client 可在这层预算内部重试瞬时 connect 失败；重试只覆盖 `ENOENT` / `ECONNREFUSED` / `EAGAIN`，且不得在请求发送后重试
- `foreground_attach_timeout_s`
  - 只服务交互式 `cc-bridge` 的 namespace/UI attach 等待
  ...
  - foreground attach 的单次 `ping('cc-bridge-daemon')` 可使用稳定 operational RPC 预算，但 target-ready 总预算仍必须被 `startup_transaction_timeout_s` 夹住
...
`cc-bridge-daemon` bootstrap 必须拆成两层，而不是把全部重型初始化都压在 mounted 发布前：
...
`cc-bridge kill` 与 `cc-bridge kill -f` 必须共享同一个 lifecycle transaction，只在强制程度上不同，不再是两套互斥世界观。
...
每次 keeper 启动新 `cc-bridge-daemon` 时，都必须分配新的 `generation + startup_id`。
...
- `lease mounted` 但 `cc-bridge-daemon.sock` 已不可连
...
- 引入 `lifecycle.json`
- keeper 成为唯一 `cc-bridge-daemon` 启动者
...
- `lib/cc-bridge-daemon/lifecycle_runtime/models.py`
- `lib/cc-bridge-daemon/lifecycle_runtime/store.py`
- `lib/cc-bridge-daemon/lifecycle_runtime/transaction.py`
- `lib/cc-bridge-daemon/lifecycle_runtime/journal.py`
...
- `lib/cc-bridge-daemon/keeper_runtime/loop.py`
...
目标是把 `cc-bridge-daemon` 收窄为“当前 generation 的执行者”。

建议新增或收口模块：

- `lib/cc-bridge-daemon/backend_runtime/startup.py`
- `lib/cc-bridge-daemon/backend_runtime/readiness.py`
- `lib/cc-bridge-daemon/backend_runtime/shutdown.py`
...
- `lib/cc-bridge-daemon/socket_runtime/ownership.py`
- `lib/cc-bridge-daemon/socket_runtime/bind.py`
...
- CLI 已不再直接启动 `cc-bridge-daemon`
- keeper 是同项目唯一 `cc-bridge-daemon` 启动者
- `lifecycle.json` 已成为 phase authority
- 同项目双终端并发 `cc-bridge ask` 不会产生双 backend generation
...
- 普通 `cc-bridge kill` 可处理 `socket_unreachable + heartbeat_fresh`
...
- [docs/cc-bridge-daemon-lifecycle-test-plan.md](/home/bfly/yunwei/cc-bridge_source/docs/cc-bridge-daemon-lifecycle-test-plan.md)
...
- `cc-bridge kill` 在 degraded socket 状态下仍能于有界时间内收口

## 20. 验收矩阵

方案完成前，以下场景必须自动化并通过：

- 两个终端同时首次 `cc-bridge ask`
- `cc-bridge-daemon` 在 `starting` 期间再次收到 `cc-bridge ask`
- `socket_unreachable + heartbeat_fresh` 下普通 `cc-bridge kill`
- `cc-bridge kill` 与 keeper tick 并发
...
- 多项目并行运行，各自独立 `cc-bridge-daemon`，互不接管

## 21. 非漂移规则

未来实现若改变以下内容，本文必须同步更新：

- keeper 是否仍是唯一 `cc-bridge-daemon` 启动者
- `mounted` 的严格含义
