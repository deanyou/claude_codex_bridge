# CC_BRIDGE Release 封装与发布方案

## 1. 文档定位

本文定义 `cc-bridge_source` 面向真实用户交付时的 release 封装方案，重点回答以下问题：

- `cc-bridge` 应该以源码仓安装为主，还是以 release 分发为主
- release 应该封装到什么程度
...
- `docs/cc-bridge-daemon-windows-psmux-plan.md`
...
- `cc-bridge update` 目前优先尝试对安装目录做 `git pull` 或 `git checkout tag`
- 如果安装目录不是 git 仓，`cc-bridge update` 会退回到下载 tarball 再安装
...
- [install.sh](/home/bfly/yunwei/cc-bridge_source/install.sh)
- [lib/cli/management_runtime/install.py](/home/bfly/yunwei/cc-bridge_source/lib/cli/management_runtime/install.py)
- [lib/cli/management_runtime/commands_runtime/update.py](/home/bfly/yunwei/cc-bridge_source/lib/cli/management_runtime/commands_runtime/update.py)
- [README.md](/home/bfly/yunwei/cc-bridge_source/README.md)
...
- 现有 `cc-bridge update` 的 tarball 路径说明项目已经适合收口到 release-first 分发
...
- 用平台特判散落在 CLI、`cc-bridge-daemon`、provider 逻辑中
...
- 检查当前平台是否被正式支持
- 检查安装目录、版本信息、日志目录、`cc-bridge-daemon` 诊断信息
...
- `cc-bridge` 代码和 Python 包
- `cc-bridge-daemon` 控制面代码
...
- 与 `docs/cc-bridge-daemon-windows-psmux-plan.md` 对齐
...
- `cc-bridge-linux-aarch64.tar.gz`
- `cc-bridge-macos-universal.tar.gz` 或按架构拆分
- `cc-bridge-wsl.tar.gz`
- `cc-bridge-windows-x86_64.zip`
...
- 升级和回滚更清晰
- `cc-bridge version` 可以准确读取当前激活版本
- 诊断包更容易给出版本路径

### 8.3 命令入口

`~/.local/bin/cc-bridge` 应始终指向 `current` 下的统一入口，而不是散落链接到临时工作树。

## 9. 更新与回滚策略

### 9.1 更新策略

`cc-bridge update` 应逐步收口为：

- 默认只更新到正式 release
- 默认只跟踪稳定通道
- 可选显式指定版本，例如 `cc-bridge update 6.2.1`
...
- `cc-bridge update 6.2.0`
- 或未来显式增加 `cc-bridge rollback 6.2.0`

回滚只切换当前激活版本，不修改用户项目内的 `.cc-bridge` authority。
...
- 不污染当前已安装稳定版本
- 不让 `~/.local/bin/cc-bridge` 指向半安装状态
- 失败后仍能继续运行旧版本
...
`cc-bridge doctor` 与日志 bundle 应输出：
...
- `tmux` / `psmux` 检查结果
- provider CLI 探测结果
- `cc-bridge-daemon` 状态、socket、tmux namespace、最近错误摘要
...
- 单元测试
- CLI smoke test
- `cc-bridge` 启动/attach/kill smoke test
...
- `cc-bridge version` 能明确显示 release 版本与 build 信息
- `cc-bridge update` 至少能沿 release 路径更新
- `cc-bridge doctor` 能输出 release 安装信息和关键前置依赖状态
...
1. 用户下载 `cc-bridge-linux-x86_64.tar.gz`
2. 解压到临时目录
3. 运行 `./install.sh install`
4. 安装器将 release 内容安装到用户目录
5. `~/.local/bin/cc-bridge` 指向已安装版本
...
- `cc-bridge version` 优先读取 release 安装元数据
- 版本信息不再依赖当前目录是不是 git 仓
- 诊断包输出安装路径与激活版本

#### C. 更新路径收口

- `cc-bridge update` 默认走 release 通道
...
- `cc-bridge doctor` 输出 release 版本、build 信息、安装目录
...
- 安装后 `cc-bridge version`
- 安装后 `cc-bridge doctor`
- 新建项目 `cc-bridge` 启动 smoke
- `cc-bridge kill` smoke
- `cc-bridge ask` 异步投递与回收 smoke
- `cc-bridge update` 升级 smoke
- 升级失败不破坏旧安装
...
- `cc-bridge version` 能稳定报告 release 信息
- `cc-bridge doctor` 能判断关键缺失项
- `cc-bridge update` 能从一个 release 更新到另一个 release
...
- 安装目录/链接目录思路
- `tmux` 主运行时契约
- `cc-bridge update` / `doctor` / 版本元数据模型
...
- 以 `docs/cc-bridge-daemon-windows-psmux-plan.md` 为准
...
`cc-bridge` 不应继续把“源码仓安装”当成产品本体，而应把“明确版本的 release + 很薄的安装器 + 清晰外置依赖”作为正式封装模型。
