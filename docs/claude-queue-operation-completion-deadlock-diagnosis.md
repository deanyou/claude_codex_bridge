...
Date: 2026-07-13
Version observed: cc-bridge 8.0.19 (`60b3c3d`)
...
| REPL state when prompt arrives | entry in session JSONL | CC_BRIDGE structured_event parses it? |
|---|---|---|
...
5. `session_boundary` claude prompts use `wrap_claude_turn_prompt` (no CC_BRIDGE_BEGIN/CC_BRIDGE_DONE markers), so the text-based `maybe_append_turn_boundary(is_done_text(...))` fallback also can't fire.
6. The attempt stays `running`, the mailbox head stays `delivering`, all subsequent `task_reply`s queue up behind it, and the agent appears frozen until a human `cc-bridge clear`/`cc-bridge restart`.

## 2. Evidence

### 2.1 Runtime snapshot (project `MiniMES`, pm stuck job `job_006fe7f3bdad`)

- cc-bridge-daemon: healthy, generation 29, heartbeat fresh, fault list empty.
...
- cc-bridge-daemon persisted execution state for the job (`executions/job_006fe7f3bdad.json`): `status=incomplete, reason=in_progress, state.offset=30707195` — offset has advanced to end-of-file, no terminal decision recorded.

### 2.2 Quantitative: how often the bad path fires

Scanning the entire pm session log (`de07c1c5-…jsonl`, 10665 entries, 183 CC_BRIDGE jobs):

- Entry types carrying `CC_BRIDGE_REQ_ID:` —
  - `last-prompt`: 483 (meta, not data)
  - `user/text_string`: 175 (normal path)
  - `queue-op/enqueue`: **14** (bad path)
  - `user/tool_result`: 12 (CC_BRIDGE_REQ_ID appears inside a tool-result string, i.e. subagent mentions, not our prompt)
  - `attachment/queued_command`: 9 (companion to queue-op)
  - `attachment/file`: 8
- First entry per job (the path CC_BRIDGE first sees the anchor via):
...
Attempt-state history for the 14 queue-op jobs from `cc-bridge-daemon/attempts/attempts.jsonl`:
...
→ 9 out of 14 queue-op prompt deliveries failed to complete automatically; the 4 "completed" ones likely closed via non-anchor paths (e.g. pm output CC_BRIDGE_DONE by coincidence after subagent replies, or were rescued by later prompt injections). There is no reliable self-recovery.
...
- `lib/provider_backends/claude/protocol_runtime/prompt.py:20-22`
  `wrap_claude_turn_prompt()` (used for `session_boundary` providers when a completion dir is registered) emits only `CC_BRIDGE_REQ_ID: <id>\n\n<body>` — no CC_BRIDGE_BEGIN/CC_BRIDGE_DONE pair. The text-marker fallback in `maybe_append_turn_boundary(is_done_text(...))` therefore cannot close the turn either.
- `lib/provider_hooks/artifacts_runtime/transcript.py:219-227`
  ...
  ```
  This proves the record type is known to CC_BRIDGE, but the handling never made it into the live completion-detection pipeline.
...
- pm is almost never idle (it is constantly dispatching, reading tool results, sending `ask --silence`, etc.), so when CC_BRIDGE injects a new message via tmux paste-buffer, Claude REPL is almost always busy → the prompt *consistently* lands as `queue-operation`.
...
3. **Restart/resume does not repair anchor state.** When cc-bridge-daemon restarts and resumes a `session_boundary` attempt, `runtime_state.anchor_seen` is whatever it was persisted as (False in the bad path) and the log reader's `offset` is at EOF; there is no "catch up on anchor / last assistant uuid by re-scanning recent log" step.
...
3. **Add an attempt-level completion watchdog.**
   In the dispatcher's poll loop, for each active attempt older than `CC_BRIDGE_ATTEMPT_TIMEOUT_S` (default e.g. 600 s) with no TURN_BOUNDARY/ERROR/PANE_DEAD item ever produced, force-terminalize it with `status=incomplete, reason=turn_boundary_timeout, confidence=degraded`, and move on to the next queue item. Publish a fault entry and leave enough diagnostics (session_path, last_offset, last_seen_entry_type) to diagnose without re-running.

4. **Resume-time anchor catch-up.**
   When a `session_boundary` attempt is resumed, scan the last N KiB (or last ~200 entries) of the session log backward from the persisted offset to find (a) the anchor marker and (b) the latest assistant uuid, so a missed anchor across cc-bridge-daemon restart does not strand the job.

5. **Telemetry.**
   Emit a counter metric `cc-bridge.completion.anchor_missing_after_seconds` and fault-inject self-test for the queue-op path so this class of bug has regression coverage.
...
- `cc-bridge restart pm` (or the stuck claude agent) unblocks the queue but loses in-flight agent context.
- `cc-bridge repair` can in some cases advance the head event; unreliable for this specific bug because the underlying completion detector never sees the boundary.
