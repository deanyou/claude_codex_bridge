# CC_BRIDGE_DAEMON 人工测试问题台账

## 1. 目的

这个台账用于记录 `cc-bridge-daemon` 控制面人工测试过程中发现的问题，并把问题处理从“现象修补”约束为“根因收敛 + 系统性修复”。
...
- `bootstrap-contract`
  - 项目发现、`.cc-bridge/config.yaml`、上下文建立不一致
- `identity-authority`
  - project identity / runtime authority / binding generation 不一致
- `daemon-lifecycle`
  - `cc-bridge-daemon` 启停、lease、socket、heartbeat、takeover 状态机不一致
- `runtime-supervisor`
...
- 关键输出
- 相关文件
  - 例如 `.cc-bridge/cc-bridge-daemon/lease.json`
  - 例如 `.cc-bridge/agents/<agent>/runtime.json`
- 是否稳定复现
- 复现频率
...
- 状态：`fixed-retested`
- 标题：`cc-bridge 在非 tmux 终端下遇到 stale binding 时输出启动成功，但没有真正打开 agent runtime`
- 根因分类：`daemon-lifecycle`
- 测试场景：`在项目根目录直接执行 cc-bridge，当前终端不在 tmux 内，项目里保留旧 runtime/session binding`
- 最小复现：
  - 在项目根执行 `cc-bridge`
  - 输出 `start_status: ok`
  - 随后 `cc-bridge ps` 显示 agent 为 `degraded`
  - `tmux list-panes -a` 显示 `no server running`
- 预期结果：
  - `cc-bridge` 应真正拉起可用 runtime
  - 若无法拉起，应明确失败，而不是输出成功
- 实际结果：
...
- 根因：
  ...
  - `cc-bridge-daemon.start_flow` 继续把该 binding 写回 authority，导致 `start_status` 与真实 runtime 可用性脱节
- 系统性修复方案：
...
- 状态：`fixed-retested`
- 标题：`已有 .cc-bridge 持久状态但缺失 config.yaml 时，cc-bridge 会静默回填默认配置并错误重建项目启动面`
- 根因分类：`bootstrap-contract`
- 测试场景：`项目目录下已有 .cc-bridge/agents 或 .cc-bridge/cc-bridge-daemon 等持久状态文件，但 .cc-bridge/config.yaml 丢失`
- 最小复现：
  - 构造 `.cc-bridge/agents/demo/runtime.json`
  - 不提供 `.cc-bridge/config.yaml`
  - 在项目根执行 `cc-bridge`
- 预期结果：
  - `cc-bridge` 应明确报错，要求恢复 `.cc-bridge/config.yaml` 或清理旧状态
  - 不能把“已有状态的损坏项目”当成“空项目首次 bootstrap”
- 实际结果：
  - `start` 自动写入默认 `agent1/agent2/agent3` 配置
  - 随后直接拉起默认 agents
  - 原有 `.cc-bridge/agents/demo/*` 状态被混入新的 project authority
- 影响范围：
  - 所有 v2 `start` 链路
...
- 系统性修复方案：
  ...
  - 只有当 `.cc-bridge` anchor 中不存在任何持久状态文件时，才允许自动写入默认配置
...
- 复测结论：
  - 黑盒复测确认：带 `.cc-bridge/agents/demo/runtime.json` 且缺失 `config.yaml` 时，`cc-bridge` 现在直接失败，且不会再生成默认配置
...
- 根因分类：`read-path`
- 测试场景：`当前配置只包含 demo，但 .cc-bridge/agents/legacy/runtime.json 仍残留旧版或损坏记录`
- 最小复现：
  - 配置 `demo:fake`
  - 额外保留 `.cc-bridge/agents/legacy/runtime.json`
  - 执行 `cc-bridge` 后再执行 `cc-bridge kill -f`
- 预期结果：
...
- 影响范围：
  - agent 改名、配置收缩、旧状态残留
  - `cc-bridge-daemon stop_all` 服务端路径
  - CLI 本地 fallback `kill` 路径
- 初步判断：
  - 清理路径把“配置 authority”与“磁盘残留扫描”混在了一起
- 根因：
  - `cli.services.kill` 和 `cc-bridge-daemon.supervisor.stop_all` 遍历 `.cc-bridge/agents/*` 时，把所有目录都当作可严格解码的 runtime authority
...
- 复测结论：
  - 黑盒复测确认：存在 `.cc-bridge/agents/legacy/runtime.json` 时，`cc-bridge kill -f` 现在可正常返回 `kill_status: ok`
...
- 最小复现：
  ...
  - 执行一次 `cc-bridge-daemon heartbeat`
  - 随后观察 queue/start 行为
- 预期结果：
...
- 根因：
  - `lib/cc-bridge-daemon/services/health.py` 将“没有进一步异常”错误等价为“已经健康”
...
- 回归测试：
  ...
  - `test_cc-bridge-daemon_heartbeat_recovers_degraded_agent_and_drains_queue`
- 复测结论：
...
- 测试场景：`无项目 cc-bridge.config，使用 v7 内置默认 window1/window2 + every_window sidebar，在 /home/bfly/yunwei/test_ccb2 执行 cc-bridge`
- 最小复现：
  - 清理旧全局默认 `/home/bfly/.cc-bridge/cc-bridge.config`
  - 在 `/home/bfly/yunwei/test_ccb2` 启动 `cc-bridge`
  - 查看两个 tmux window 的 pane border 设置
- 预期结果：
  - 每个 CC_BRIDGE window 都应启用 `pane-border-status=top`
  - 每个 pane 顶部都应显示 CC_BRIDGE label
  - sidebar pane 使用固定灰色主题，不与 agent 分区色冲突
- 实际结果：
  - `window1` 正常显示 CC_BRIDGE pane label 和主题
...
- 初步判断：
  - 不是 tmux socket 隔离问题；`@cc-bridge_agent`、`@cc-bridge_label_style`、`@cc-bridge_border_style` 已正确写入每个 pane
  - 失败层级在 tmux window option/theme 应用时机
- 根因：
  - `materialize_topology()` 在所有 windows 创建完成前调用 `apply_project_tmux_ui()`
  - `apply_project_tmux_ui()` 只枚举当时已存在的 windows，因此只给首个 window 写入 CC_BRIDGE pane border format
  - `config/cc-bridge-border.sh` 在 sidebar active 时优先借用非 sidebar pane 的 active style，使 sidebar active border 不再稳定保持灰色主题
- 系统性修复方案：
  ...
  - `cc-bridge-border.sh` 对 sidebar active pane 使用 sidebar 自身 `@cc-bridge_active_border_style`，不再借用 agent 色
- 回归测试：
...
- 预期结果：
  ...
  - 没有 CC_BRIDGE running job 时，不应因历史 scrollback 显示 `pending`
- 实际结果：
...
- 影响范围：
  ...
  - 无 CC_BRIDGE job 的人工交互状态显示
  - sidebar 顶部 agent 状态灯
- 初步判断：
  - 不是 sidebar TUI 渲染问题；错误状态由 cc-bridge-daemon `project_view` 返回
...
- 根因分类：`runtime-supervision`
- 测试场景：`agent 在 heartbeat 中被后台恢复后，检查 .cc-bridge/agents/<agent>/runtime.json 与 .cc-bridge/cc-bridge-daemon/supervision.jsonl`
- 最小复现：
  - 将 agent 置为 `pane-dead`
  - 触发一次 `cc-bridge-daemon heartbeat`
  - 读取 `.cc-bridge/agents/<agent>/runtime.json`
  - 读取 `.cc-bridge/cc-bridge-daemon/supervision.jsonl`
- 预期结果：
...
- 回归测试：
  ...
  - `test_cc-bridge-daemon_socket_rejects_cmd_sender`
  - `test_mailbox_store_rejects_cmd_mailbox_owner`
  - `test_mailbox_kernel_ack_reply_rejects_cmd_mailbox_owner`
- 复测结论：
  - 自动化复测应确认：`message_bureau`、`cc-bridge-daemon socket`、`ask_cli`、`cc-bridge-daemon dispatcher` 均不再将 `cmd` 视为正常 caller mailbox
- 初步判断：
...
- 系统性修复方案：
  ...
  - 新增 `.cc-bridge/cc-bridge-daemon/supervision.jsonl` 作为 append-only supervision 事件流
...
- 最小复现：
  - 先让 agent 正常存在并入队
  - 删除 `.cc-bridge/agents/<agent>/runtime.json` 或将其置为 stopped/failed
  - 执行一次 `cc-bridge-daemon heartbeat`
...
- 回归测试：
  ...
  - `test_cc-bridge-daemon_heartbeat_starts_missing_agent_and_drains_queue`
...
- 状态：`fixed-retested`
- 标题：`cc-bridge-daemon 崩溃后缺少项目级外部 keeper，导致后台只能等到下一次 cc-bridge 命令才被动重启`
- 根因分类：`daemon-lifecycle`
- 测试场景：`项目已启动成功，cc-bridge-daemon 被外部 kill 或异常退出，然后观察后台是否会自行恢复`
- 最小复现：
  - 在带 `.cc-bridge/config.yaml` 的项目里执行 `cc-bridge`
  - 记录 `.cc-bridge/cc-bridge-daemon/lease.json` 中的 `cc-bridge-daemon_pid`
  - 对该 pid 发送 `SIGTERM`
  - 等待 1 秒后重新读取 `.cc-bridge/cc-bridge-daemon/lease.json`
- 预期结果：
  - 项目级 keeper 应自动拉起新的 `cc-bridge-daemon`
  - `lease.json` generation 应递增
  - `.cc-bridge/cc-bridge-daemon/keeper.json` 应记录 keeper pid、最近检查时间与 restart_count
  - `cc-bridge kill` 应先写 shutdown intent，再停止 keeper，且 kill 后不得反拉
- 实际结果：
  - 旧实现只有 `ensure_daemon_started()` 的“下次命令再重启”
  - `cc-bridge-daemon` 崩溃后项目会长时间处于 backend 缺失状态
...
- 根因：
  ...
  - `.cc-bridge/cc-bridge-daemon/` 下没有 keeper state / shutdown intent 记录
  - `ensure_daemon_started()` 与 `cc-bridge kill` 没有围绕 keepalive intent 建立闭环
- 系统性修复方案：
  - 新增 `.cc-bridge/cc-bridge-daemon/keeper.json` 与 `.cc-bridge/cc-bridge-daemon/shutdown-intent.json`
  - 引入 `ProjectKeeper` 作为唯一的 daemon keepalive 进程，只负责 `cc-bridge-daemon` 保活，不接管 runtime authority
  ...
  - `cc-bridge kill` 显式写入 shutdown intent，并等待 keeper 退出；必要时强制结束 keeper
...
- 复测结论：
  - 黑盒复测确认：`cc-bridge-daemon` 被 `SIGTERM` 后，keeper 会自动拉起新 pid，且 generation 递增
  - 黑盒复测确认：`cc-bridge kill` 后 `lease.json` 进入 `unmounted`，`keeper.json` 进入 `stopped`，且不会反拉
  - 黑盒复测确认：再次执行 `cc-bridge` 会清除 shutdown intent，并重新建立 keeper + daemon
...
- 根因分类：`daemon-lifecycle`
- 测试场景：`项目已启动成功，执行 cc-bridge kill 后检查 runtime authority 与 ps 输出`
- 最小复现：
  - 在 fake provider 项目中执行 `cc-bridge`
  - 再执行 `cc-bridge kill`
  - 读取 `.cc-bridge/agents/<agent>/runtime.json`
  - 再执行 `cc-bridge ps`
- 预期结果：
  ...
  - `cc-bridge ps` 不应继续把 agent 显示成 `idle/partial`
- 实际结果：
  - backend 已经 `unmounted`
  - 但 runtime authority 仍被最后一次 heartbeat 改回 `idle / desired_state=mounted`
  - `cc-bridge ps` 因此继续显示 agent 处于活动态
- 影响范围：
  - `cc-bridge kill` 走服务端 `stop_all` 路径的所有场景
...
- 回归测试：
  - `test_cc-bridge-daemon_stop_all_does_not_run_post_shutdown_heartbeat`
- 复测结论：
  - 黑盒复测确认：`cc-bridge kill` 后 runtime authority 现在落到 `stopped/stopped`
...
- 最小复现：
  - 在临时项目执行 `cc-bridge`
  - 记录 `.cc-bridge/agents/demo/runtime.json` 中的 `pane_id`
  - 执行 `tmux kill-pane -t <pane_id>`
  - 等待 2-3 秒后读取 `runtime.json` 和执行 `cc-bridge ps`
- 预期结果：
...
- 复测结论：
  - 真实黑盒复测确认：`codex` pane 被 kill 后，后台会恢复到新 pane，`runtime.json` 与 `cc-bridge ps` 都会显示新 pane
  - 自动化复测确认：pid 缺失不再抢先把可恢复 pane death 归类成 `orphaned`

### ISSUE-010

- 状态：`fixed-retested`
- 标题：`keeper 拉起的 cc-bridge-daemon 在被 SIGTERM 后可能残留 zombie pid，导致恢复场景里旧 pid 长时间可见`
- 根因分类：`keeper-lifecycle`
- 测试场景：`运行中的 fake execution 期间直接 kill 当前 cc-bridge-daemon pid，等待 keeper 重启并恢复执行`
- 最小复现：
  - 启动项目并提交一个运行中的 fake provider 任务
  - 读取 `.cc-bridge/cc-bridge-daemon/lease.json` 中的 `cc-bridge-daemon_pid`
  - 执行 `kill -TERM <cc-bridge-daemon_pid>`
  - 观察旧 pid 是否在 1-2 秒内真正消失
- 预期结果：
  - 旧 `cc-bridge-daemon` pid 应快速退出并从进程表消失
  - keeper 应随后重启新的 `cc-bridge-daemon`
  - 运行中的 execution 应由新的 daemon 恢复
- 实际结果：
  - keeper 是 `cc-bridge-daemon` 的直接父进程时，没有主动回收已退出子进程
...
- 根因：
  - `ProjectKeeper` 会直接 spawn `cc-bridge-daemon`
...
- 回归测试：
  ...
  - `test_cc-bridge_fake_provider_recovers_running_execution_after_cc-bridge-daemon_restart`
- 复测结论：
  - 定向复测确认：旧 `cc-bridge-daemon` pid 在被 kill 后会在 keeper 循环内被回收
...
- 根因分类：`terminal-runtime`
- 测试场景：`项目彻底 kill 后重新执行 cc-bridge，在 fresh project namespace 下观察 cmd pane 首屏`
- 最小复现：
  - 执行 `cc-bridge kill`
  - 在项目根执行 `CC_BRIDGE_NO_AUTO_OPEN=1 cc-bridge`
  - 执行 `tmux -S .cc-bridge/cc-bridge-daemon/tmux.sock capture-pane -p -t %1 -S -20`
- 预期结果：
...
- 复测结论：
  ...
  - 黑盒复测确认：`CC_BRIDGE_NO_AUTO_OPEN=1 cc-bridge` 后 `%1` capture 只剩单个 prompt，不再出现前导 `%`

### ISSUE-012

- 状态：`fixed-retested`
- 标题：`cc-bridge.config 变更后复用旧 namespace 并在 root pane 上继续局部 split，会让多列布局漂移`
- 根因分类：`terminal-runtime`
- 测试场景：`项目已存在旧 project namespace，随后增加 agents 或修改 .cc-bridge/cc-bridge.config 布局表达式，再执行 cc-bridge`
- 最小复现：
  - 先以较小布局启动项目并保留 namespace
  - 修改 `.cc-bridge/cc-bridge.config`，例如从两列扩到三列
  - 再执行 `cc-bridge`
  - 观察 pane 几何与 `cc-bridge.config` 不一致
- 预期结果：
  - `cc-bridge` 应根据新的可见布局整体重建 namespace，再重新投影 panes
...
- 影响范围：
  - `.cc-bridge/cc-bridge.config` 变更后的前台重启
  - 新增 agents / 修改左右上下分组
...
- 根因分类：`provider-facts`
- 测试场景：`在 /home/bfly/yunwei/test_cc-bridge 中由 agent1 ask agent2，agent2 实际回答但 cc-bridge execution 长时间 running`
- 最小复现：
  - 在 `/home/bfly/yunwei/test_cc-bridge` 启动项目
  - 配置包含多个 inplace Codex agents，例如 `agent1:codex` 与 `agent2:codex`
  - 执行 `agent1 ask agent2`
  - 观察 agent2 pane 已经输出答案 `2`
  - 检查 `.cc-bridge/cc-bridge-daemon/executions/job_d06cdffcf34a.json`
- 预期结果：
  - agent2 的 Codex 日志应写入 agent2 私有 managed home
  - execution reader 应在 agent2 私有 home 内发现 `CC_BRIDGE_REQ_ID: job_d06cdffcf34a`
  - job 应完成并进入 reply 流程
- 实际结果：
  - `.cc-bridge/cc-bridge-daemon/executions/job_d06cdffcf34a.json` 中 `log_path: null`、`session_path: ""`、`anchor_seen: false`
  - `.cc-bridge/.codex-agent2-session` 只有 `codex_session_root`，没有绑定 `codex_session_id/codex_session_path`
  - `.cc-bridge/agents/agent2/provider-state/codex/home/sessions/` 没有收到真实日志
  - 真实日志写到了 `/home/bfly/.codex/sessions/2026/04/19/rollout-2026-04-19T20-01-07-019da59d-ac36-7932-99ab-2b6801e160af.jsonl`
  - 该真实日志包含 `CC_BRIDGE_REQ_ID: job_d06cdffcf34a` 和最终答案 `2`
- 影响范围：
  - 使用真实 `codex-cli 0.121.0` 的 managed Codex agents
  - 多个 inplace Codex agents 共享同一 `work_dir` 的隔离正确性
  - 手动在项目目录运行 `codex` 时与 `cc-bridge` managed Codex 的对话隔离
...
- 系统性修复方案：
  ...
  - 每个 configured Codex agent 默认使用 `.cc-bridge/agents/<agent>/provider-state/codex/home/`
...
- 测试场景：
  - 在 `/home/bfly/yunwei/test_cc-bridge` 执行 `start -> kill -> start`
  - 在 `/home/bfly/yunwei/cc-bridge_test2` 对同一空 anchor 并发执行两次 `cc-bridge`
- 最小复现：
  - `/home/bfly/yunwei/test_cc-bridge`：
    - 执行 `/home/bfly/yunwei/cc-bridge_source/cc-bridge`
    - 执行 `/home/bfly/yunwei/cc-bridge_source/cc-bridge kill`
    - 再执行 `/home/bfly/yunwei/cc-bridge_source/cc-bridge`
    - 读取 `.cc-bridge/cc-bridge-daemon/lifecycle.json`
    - 读取 `.cc-bridge/agents/agent1/runtime.json`
    - 读取 `.cc-bridge/agents/agent1/helper.json`
  - `/home/bfly/yunwei/cc-bridge_test2`：
    - 对同一目录并发执行两次 `/home/bfly/yunwei/cc-bridge_source/cc-bridge`
    - 读取 `.cc-bridge/cc-bridge-daemon/lifecycle.json`
    - 读取 `.cc-bridge/agents/agent1/runtime.json`
- 预期结果：
...
- 实际结果：
  - `/home/bfly/yunwei/test_cc-bridge/.cc-bridge/cc-bridge-daemon/lifecycle.json` 显示 `generation: 2`
  - 但 `/home/bfly/yunwei/test_cc-bridge/.cc-bridge/agents/agent1/runtime.json` 中新 `runtime_pid=4125663` 的同时，`started_at` 仍是上一代首次启动时间 `2026-04-22T00:37:35.631517Z`
  - `/home/bfly/yunwei/test_cc-bridge/.cc-bridge/agents/agent1/helper.json` 中新 `leader_pid=4125736`、`runtime_generation=2`，但 `started_at` 仍为上一代时间，且 `owner_daemon_generation` 仍为 `null`
  - `/home/bfly/yunwei/cc-bridge_test2/.cc-bridge/cc-bridge-daemon/lifecycle.json` 仍为 `generation: 1`
  - 但 `/home/bfly/yunwei/cc-bridge_test2/.cc-bridge/agents/agent1/runtime.json` 已出现 `binding_generation: 2`，同时 `daemon_generation: 1`、`runtime_generation: 1`
- 2026-04-22 继续复测：
  - `/home/bfly/yunwei/cc-bridge_test2` 执行 `cc-bridge kill -> cc-bridge -> cc-bridge ask agent1 "只回复：RESTART_OK"` 后，`doctor` 显示 `cc-bridge-daemon_generation: 3`
  - 但 `.cc-bridge/agents/agent1/runtime.json` 变为 `binding_generation: 4`、`daemon_generation: 3`、`runtime_generation: 3`
  - `.cc-bridge/agents/agent1/helper.json` 为 `runtime_generation: 3`、`owner_daemon_generation: 3`
  - 同目录下 `agent2`、`agent3` 也同样出现 `binding_generation: 4`、`daemon_generation: 3`
  - `/home/bfly/yunwei/test_cc-bridge` 在正常 mounted 状态下未出现该偏差，`binding_generation/daemon_generation/runtime_generation` 仍保持 `5/5/5`
- 影响范围：
...
- 回归测试：
  ...
  - 待补：同项目双 `cc-bridge` 并发时，最终 authority 不出现 `binding_generation > daemon_generation`
- 复测结论：
  - 待补
...
- 根因分类：`daemon-lifecycle`
- 测试场景：`在 mounted 项目中并发执行 cc-bridge kill 与 cc-bridge ask agent1`
- 最小复现：
  - 在 `/home/bfly/yunwei/test_cc-bridge` 保持项目 mounted
  - 并发执行：
    - `/home/bfly/yunwei/cc-bridge_source/cc-bridge kill`
    - `/home/bfly/yunwei/cc-bridge_source/cc-bridge ask agent1 "只回复：RACE_OK"`
  - 随后执行 `/home/bfly/yunwei/cc-bridge_source/cc-bridge doctor`
- 预期结果：
...
- 实际结果：
  ...
  - 事后 `/home/bfly/yunwei/test_cc-bridge/.cc-bridge/cc-bridge-daemon/lifecycle.json` 维持 `generation: 2`、`phase: unmounted`
...
- 回归测试：
  - 待补：`cc-bridge kill` 与新的 `cc-bridge ask` 并发时，`ask` 返回确定性错误且不产生新 generation
- 复测结论：
  - 2026-04-22 黑盒复测：在 `/home/bfly/yunwei/test_cc-bridge` 并发执行 `cc-bridge kill` 与 `cc-bridge ask agent1 "只回复：RACE_OK"` 时，`ask` 不再暴露 `Connection reset by peer`
  - 当前稳定返回：`error: project cc-bridge-daemon is unmounted; run \`cc-bridge\` first`
...
- 最小复现：
  - 在 `/home/bfly/yunwei/test_cc-bridge` 保持项目 mounted
  - 读取 `.cc-bridge/cc-bridge-daemon/lifecycle.json`，确认 `generation: 5`
  - 对其中 `owner_pid` 执行 `kill -9`
  - 执行 `/home/bfly/yunwei/cc-bridge_source/cc-bridge doctor`
  - 读取 `.cc-bridge/cc-bridge-daemon/lifecycle.json`
  - 读取 `.cc-bridge/agents/agent1/runtime.json`
  - 读取 `.cc-bridge/agents/agent1/helper.json`
  - 再执行一次 `/home/bfly/yunwei/cc-bridge_source/cc-bridge`，确认是否自愈
- 预期结果：
...
- 实际结果：
  - `doctor` 显示 keeper 已恢复项目，并进入 `cc-bridge-daemon_generation: 6`
  - `cc-bridge-daemon_startup_last_trigger: daemon_boot`
  - `.cc-bridge/cc-bridge-daemon/lifecycle.json` 显示 `generation: 6`
  - 但 `.cc-bridge/agents/agent1/runtime.json` 仍为 `binding_generation: 5`、`runtime_generation: 5`，只有 `daemon_generation` 被推进到 `6`
  - `.cc-bridge/agents/agent1/helper.json` 仍为 `runtime_generation: 5`，仅 `owner_daemon_generation: 6`
  - 随后执行一次 `cc-bridge` 返回 `cc-bridge-daemon_started: false`，上述 authority 仍不自愈
  - 同时 `cc-bridge ask agent1 "只回复：AFTER_SIGKILL"` 可以完成，说明运行面可用，但 authority 已失真
- 影响范围：
...
- 回归测试：
  ...
  - 待补：keeper 接管后重复执行 `cc-bridge` 不会留下旧代 authority
- 复测结论：
  - 待补
...
- 最小复现：
  - 执行 `CC_BRIDGE_LINUX_SOAK_SECONDS=90 CC_BRIDGE_LINUX_SOAK_KILL_EVERY=2 CC_BRIDGE_LINUX_SOAK_STUB_DELAY=0.2 CC_BRIDGE_LINUX_SOAK_ASK_WAIT_TIMEOUT_S=90 bash test/system_linux_soak.sh`
...
- 预期结果：
  ...
  - 新 session log 中出现 `CC_BRIDGE_REQ_ID`、assistant chunk、turn boundary 后，job 应进入 terminal
- 实际结果：
...
- 最小复现：
  - 5 分钟 Linux soak 中 `doctor-20.out` 同时包含 `cc-bridge-daemon_state: mounted` 与 `cc-bridge-daemon_health: healthy`
...
- 状态：`fixed-targeted`
- 标题：`Claude scheduled task Stop 复用旧 CC_BRIDGE_REQ_ID，导致 ask 被误标 completed 且 self 误判 idle`
- 根因分类：`provider-facts`
...
- 最小复现：
  ...
  - 用户手动 interrupt 后，再提交带 `CC_BRIDGE_REQ_ID` 的 ask
  - 后续 scheduled task 的 `Stop` hook 触发
- 预期结果：
  - scheduled task 的 `Stop` 不能 terminalize 旧 CC_BRIDGE job
  - project view / heartbeat 应把 `Running scheduled task`、`1 shell still running` 识别为 provider pane active
- 实际结果：
  - `cc-bridge-provider-finish-hook` 从 transcript 最新 `CC_BRIDGE_REQ_ID` / `last-prompt` 归因，写入旧 job 的 completion artifact
  - `job_86550847f237` 被 `hook_stop` 标成 completed，但 pane 仍显示 scheduled task、旧 CC_BRIDGE 输入块和 `1 shell still running`
  - `cc-bridge ps` 显示 clauder idle，`cc-bridge_self` 基于控制面证据没有定位到 hook 误归因
- 根因：
...
- 系统性修复方案：
  ...
  - 沿 `parentUuid` 链回溯，跳过 tool-result user record，只接受真实 prompt user 的 outer `CC_BRIDGE_REQ_ID`
  - scheduled task、用户中断、无 CC_BRIDGE anchor 的 provider turn 不写 completion artifact
  - project view 将 `running scheduled task`、`shell still running` 纳入 provider pane active marker
- 回归测试：
  - `test_current_turn_req_id_ignores_scheduled_task_after_interrupted_cc-bridge_prompt`
  - `test_provider_finish_hook_ignores_claude_scheduled_task_after_stale_cc-bridge_prompt`
  - `test_activity_resolver_claude_scheduled_task_shell_running_after_prompt`
- 复测结论：
  - 2026-06-11 targeted 通过：
    `pytest -q test/test_maintenance_heartbeat.py test/test_provider_activity_artifacts.py test/test_v2_config_loader.py test/test_provider_hook_transcript.py test/test_provider_finish_hook_script.py test/test_cc-bridge-daemon_project_view.py`
    -> `184 passed`
