# CC_BRIDGE 安装与运行环境问题修复分析文档

本文档说明 CC_BRIDGE 在 macOS、Volta、多 Python 环境下安装后出现启动失败、命令路径漂移、Claude/Codex agent 启动不一致等问题的前因后果，并给出可执行的修复方案和验收标准。
...
- `cc-bridge` 在 `chat2image` 中启动失败，报错为 `cc-bridge-daemon exited before ready with code 1`。
- `cc-bridge-daemon.stderr.log` 中出现 Python 类型语法相关错误：
...
- `Claude Code` agent 重建后停在首次交互确认界面，导致 CC_BRIDGE 投递任务处于 `running` / `delivering` 状态但不返回。
...
但是 source/dev 安装模式下，`~/.local/bin/cc-bridge` 会作为符号链接指向源码仓库中的入口文件：

```text
~/.local/bin/cc-bridge -> /Users/yuanfeijie/Desktop/project/claude_code_bridge/cc-bridge
...
因此用户执行 `cc-bridge` 时，实际解释器不是安装脚本检查过的 `python`，而是重新由 `/usr/bin/env python3` 解析。
...
运行时：python3 = Python 3.9，cc-bridge-daemon 崩溃
```

CC_BRIDGE 新版代码中使用了 Python 3.10+ 语法，例如 `A | None` 类型联合。Python 3.9 无法完整支持这些运行时类型表达式，因此 `cc-bridge-daemon` 在 import 阶段崩溃。

### 2.2 `cc-bridge-daemon` 和 `keeper` 继承了错误解释器

CC_BRIDGE 启动后会继续启动后台控制进程：

- `keeper_main.py`
- `cc-bridge-daemon/main.py`

这两个进程不是独立选择 Python，而是通过当前 `cc-bridge` 进程的 `sys.executable` 启动。

因此如果 `cc-bridge` 本身由 `/usr/bin/python3` 启动，那么后续 `keeper` 和 `cc-bridge-daemon` 也会使用 Python 3.9。

这就是为什么只在安装脚本中检查 Python 不足以保证运行时正确。必须确保 `cc-bridge` 入口本身、`keeper`、`cc-bridge-daemon` 三者使用同一个满足版本要求的 Python。

### 2.3 source/dev 模式优先开发便利，不保证运行时闭环

source/dev 模式的优点是全局命令直接链接当前源码，便于开发 CC_BRIDGE 本身。
...
因此 source/dev 模式适合 CC_BRIDGE 开发者，不适合作为多 Python macOS 用户的默认稳定安装方式。
...
但 CC_BRIDGE 不只在当前交互式 shell 中执行。它还会创建：
...
在重建 `.cc-bridge` 运行态后，Claude Code 可能出现首次确认界面，例如：

- 是否信任当前工作目录。
- 是否使用当前 `ANTHROPIC_API_KEY`。

如果 Claude pane 停在这些交互界面，CC_BRIDGE 仍可能认为 provider runtime 已经启动，但投递进去的任务不会被模型处理，表现为：
...
这不是 CC_BRIDGE 路由失败，而是 provider 处于交互确认状态。
...
```bash
cc-bridge wait-all --timeout 300 <job_id>
```
...
- `cc-bridge` 和 `ask` 不再是指向源码的 symlink，而是 wrapper：

```bash
exec "/Users/yuanfeijie/.local/share/codex-dual/.venv/bin/python" "/Users/yuanfeijie/.local/share/codex-dual/cc-bridge" "$@"
```

这样即使系统默认 `python3` 仍是 Python 3.9，也不会影响 CC_BRIDGE 运行。
...
```text
/Users/yuanfeijie/.local/bin/cc-bridge
...
如需清理旧运行态并保留 `.cc-bridge/cc-bridge.config`，使用：

```bash
cc-bridge -n
```

注意：`cc-bridge -n` 会要求交互确认。它会清理 `.cc-bridge` 下可重建运行态，但保留 `.cc-bridge/cc-bridge.config`。
...
CC_BRIDGE 还涉及 daemon、tmux、provider runtime，这些上下文未必读取同一份 shell 初始化文件。

因此仅修改 shell PATH 不是完整修复。

### 4.3 不推荐继续使用 source/dev 安装作为普通用户默认安装

source/dev 模式适合开发 CC_BRIDGE 本身。

普通使用更适合 managed release 模式，因为它能固定 Python 解释器和依赖环境。

## 5. 上游应实施的永久修复方案

以下方案面向 CC_BRIDGE 项目本身，目标是让安装器在多 Python、多 PATH、多 provider CLI 环境下具备闭环自检能力。

### 5.1 修复 Python 解释器闭环

安装器必须保证“安装时检查的 Python”和“运行时 `cc-bridge` 使用的 Python”一致。
...
4. `keeper` 和 `cc-bridge-daemon` 继续使用 `sys.executable` 派生即可，因为 wrapper 已确保入口解释器正确。
...
exec "/opt/homebrew/opt/python@3.12/libexec/bin/python" "/Users/yuanfeijie/Desktop/project/claude_code_bridge/cc-bridge" "$@"
```
...
```bash
"$BIN_DIR/cc-bridge" version
```

并增加一个机器可读的 runtime 检查，例如：

```bash
"$BIN_DIR/cc-bridge" doctor --runtime
```

输出必须包含：

```text
cc-bridge_executable: /Users/.../.local/bin/cc-bridge
cc-bridge_install_path: /Users/.../.local/share/codex-dual
...
必须在 CC_BRIDGE 实际启动 provider 的环境中检查：
...
如果检测到 `~/.volta/bin` 存在，并且用户当前 shell 的 `codex` 或 `claude` 来自 Volta，则 CC_BRIDGE 启动 provider 时应确保 `~/.volta/bin` 在 PATH 前部。

推荐策略：

1. 不全局覆盖 PATH。
2. 只在 CC_BRIDGE provider runtime 环境中 prepend `~/.volta/bin`。
3. 保留用户原 PATH 的其余部分。
...
```bash
CC_BRIDGE_DROID_AUTOINSTALL=1 ./install.sh install
```

### 5.7 检测 Claude Code 首次确认状态

CC_BRIDGE 启动 Claude provider 后，不应只判断 pane 存活。
...
如果检测到这些界面，应在 `cc-bridge ps` 或 `cc-bridge doctor` 中明确输出：
...
旧 `ask --wait` 同步行为不恢复，也不在 `ask` 错误提示中重新引入 wait/timeout 迁移文案。当前普通未知参数错误符合 submit-only 的 ask 表面约束；需要等待聚合结果时使用独立的 `cc-bridge wait-*` 命令。
...
- source/dev 安装不再直接 symlink `cc-bridge` 和 `ask`。
...
```bash
tmux -S .cc-bridge/cc-bridge-daemon/tmux.sock list-panes ...
```
...
- Claude 首次确认时，`cc-bridge ps` 或 `doctor` 显示 blocked，而不是只显示 idle/healthy。

验收：

- 清理 Claude managed home。
- 启动 CC_BRIDGE。
- Claude 停在 trust/API key prompt 时，`cc-bridge doctor` 明确提示人工确认。
...
如果只是使用 CC_BRIDGE，不开发 CC_BRIDGE 本身，执行以下命令：
...
本问题的本质不是 Python、Volta、Claude Code、Codex CLI 某一个工具单独损坏，而是 CC_BRIDGE 安装器在 source/dev 模式下没有形成完整运行时闭环。
...
Claude Code 首次安全确认会阻塞 CC_BRIDGE 任务处理。
...
```text
使用 release/managed 安装模式，让 ~/.local/bin/cc-bridge 和 ~/.local/bin/ask 调用 managed Python 3.12 venv。
...
安装后必须验证真实 cc-bridge 入口使用的 sys.executable。
...
只要完成这些改动，CC_BRIDGE 就能在多 Python、多 Node CLI 管理器、多后台运行层的 macOS 环境中稳定安装和运行。
