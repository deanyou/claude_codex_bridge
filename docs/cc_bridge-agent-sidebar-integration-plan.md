# CC_BRIDGE Agent Sidebar Integration Plan

## 1. Document Position

This document is the planning baseline for deep-forking `hiroppy/tmux-agent-sidebar` into a CC_BRIDGE-native project console.

The target is not a generic tmux plugin. The target is a built-in CC_BRIDGE namespace UI that solves the current "many agents squeezed into one foreground split layout" problem by combining:
...
- a compact CC_BRIDGE communication feed

The sidebar must consume `cc-bridge-daemon` authority. It must not become a second authority for project identity, agent identity, window layout, pane ownership, runtime lifecycle, worktree lifecycle, or message/job state.

Related authority documents:

- [docs/cc-bridge-daemon-startup-supervision-contract.md](cc-bridge-daemon-startup-supervision-contract.md)
- [docs/cc-bridge-daemon-project-namespace-lifecycle-plan.md](cc-bridge-daemon-project-namespace-lifecycle-plan.md)
- [docs/cc-bridge-config-layout-contract.md](cc-bridge-config-layout-contract.md)
- [docs/cc-bridge-daemon-diagnostics-contract.md](cc-bridge-daemon-diagnostics-contract.md)

This plan proposes a future `.cc-bridge/cc-bridge.config` topology extension. When implementation starts, [docs/cc-bridge-config-layout-contract.md](cc-bridge-config-layout-contract.md) must be updated in the same patch to promote the new grammar from plan to contract.

## 2. Product Decision

The fork should become `cc-bridge-agent-sidebar`: a CC_BRIDGE built-in project console.

Confirmed decisions:

- It is built into CC_BRIDGE project namespaces, not installed as a user-global TPM plugin.
...
- It is deeply coupled to `cc-bridge-daemon` state.
...
- It is distributed as part of the CC_BRIDGE release, not as a separately installed user tool.
- Sidebar focus/switch operations go through `cc-bridge-daemon` RPC, not direct Rust-side tmux control.
- The release bundles the runtime binary at `bin/cc-bridge-agent-sidebar`.
...
- provider hook enrichment beyond existing CC_BRIDGE completion/runtime state
...
Upstream topology is a toggle-created sidebar pane in the current tmux window. That does not satisfy CC_BRIDGE's target because the sidebar disappears when switching to a different window unless every window is separately toggled. CC_BRIDGE needs a stable project sidebar projected into managed windows.

## 4. Config Model

### 4.1 Target Shape

The next config model should make `windows` first-class. The examples below use the readable target shape, but Phase 1 implementation should keep using `.cc-bridge/cc-bridge.config` and the existing rich-config parser path rather than introducing a separate `.cc-bridge/config.yaml`.
...
[windows]
main = "agent1:codex, agent2:codex, agent3:claude, cc-bridge_self:codex"
```
...
- Window names are logical names managed by `cc-bridge-daemon`.
- `entry_window` controls the default selected window when `cc-bridge` creates or attaches to the project namespace.
- `cmd` is removed from the new grammar.
- Manual user-created panes are outside CC_BRIDGE and are not represented in config, sidebar state, diagnostics rows, or restore guarantees.
...
The sidebar polls `cc-bridge-daemon` `project_view`, so `project_view` is an online UI read
...
The old `cmd` leaf is ignored for the new window topology because command shells are user-managed panes, not CC_BRIDGE-managed config leaves.
...
If `.cc-bridge/cc-bridge.config` is missing, bootstrap should use the built-in default:
...
4. select `entry_window` for the user-facing `cc-bridge` entry path
...
- CC_BRIDGE-owned panes should set `@cc-bridge_window`
- agent panes should set `@cc-bridge_role=agent` and `@cc-bridge_slot=<agent>`
- sidebar panes should set `@cc-bridge_role=sidebar` and `@cc-bridge_sidebar_instance=<window_name>`
...
- a missing configured agent pane is not directly an `offline` UI state while `cc-bridge-daemon` owns recovery
- if `cc-bridge-daemon` is recovering or reflowing a missing configured agent pane, ProjectView should report `pending` with a reason such as `pane_missing_recovering`
...
`cc-bridge-daemon` remains the only owner of the project namespace.
...
- Manual panes must not enter CC_BRIDGE authority.
...
Pane identity for CC_BRIDGE-owned panes should use CC_BRIDGE-scoped tmux options:
...
- `@cc-bridge_sidebar_instance`

Generic upstream keys such as `@pane_status` are compatibility facts only and must not become CC_BRIDGE authority.
...
```text
CC_BRIDGE project-name                       cc-bridge-daemon ●
▾ main
  ● agent1 codex
  ○ agent2 claude
▾ research
...
Phase 1 status is an `agent activity state`. It is not only CC_BRIDGE job state, because users may manually give work to a provider pane outside the CC_BRIDGE job path.

State resolver priority:

```text
blocking runtime/reconcile facts > current CC_BRIDGE job > provider/session signal > pane/process liveness
```

`current CC_BRIDGE job` remains the strongest work-intent signal. The only things that override it are facts that prove the configured agent cannot currently do that work, such as recovery failure, missing pane with no recovery owner, or runtime fault.

| Symbol | Color | UI State | Meaning |
| --- | --- | --- | --- |
| `●` | green | `active` | current CC_BRIDGE job is running, runtime is busy, or provider/session activity is advancing |
...
| `✕` | red | `failed` | current CC_BRIDGE job failed/incomplete/cancelled, provider reported abort/failure, pane recovery failed, or runtime fault blocks the current work |
...
The bottom panel should initially show a compact CC_BRIDGE communication feed:
...
Do not show raw `cc-bridge-daemon` logs in Phase 1. The feed should use normalized CC_BRIDGE job/message events.
...
- configured windows and agents from `.cc-bridge/cc-bridge.config`
- namespace state from `.cc-bridge/cc-bridge-daemon`
- runtime state from `.cc-bridge/agents/<agent>/runtime.json`
- job records and job events
- completion snapshots/decisions
- dispatcher state
- provider/session activity signals normalized by `cc-bridge-daemon`
- pane liveness inside the project-owned tmux socket/session

### 8.1 ProjectView Generation Pipeline

`cc-bridge-daemon` should generate ProjectView from one authoritative snapshot per request:
...
3. Inspect only the project-owned tmux socket/session for configured CC_BRIDGE panes. Do not scan arbitrary tmux sessions.
...
6. Build a provider overlay from provider/session activity signals already normalized by `cc-bridge-daemon`.
...
10. Build `comms` from compact CC_BRIDGE job/message records, not raw logs.
...
If one input source is temporarily unavailable, `cc-bridge-daemon` should still return the best snapshot it can, with conservative status:
...
This resolver intentionally treats a missing desired pane as a supervision problem, not as normal offline state. CC_BRIDGE ask/job state is not an agent activity source; queued, running, completed, failed, retry, and callback status belongs in the Comms list. A job id may be used only as a correlation hint for Comms recoverability, never as authority for the top agent row.
...
- Claude has richer provider/session signals, but Phase 1 should still render the same simplified CC_BRIDGE task states.
- Codex is accurate for CC_BRIDGE tasks via job state, protocol-turn completion, session event logs, and runtime/pane health.
...
- Manual provider interactions may be shown as weak provider/session activity when `cc-bridge-daemon` can observe user/assistant/session progress. They must not become job authority.
...
The sidebar must consume a normalized `ProjectView` exposed by `cc-bridge-daemon` RPC rather than assembled independently by the Rust TUI from many files.
...
```json
{
  ...
  "project": {
    "id": "abc123",
    "root": "/home/bfly/yunwei/cc-bridge_source",
  ...
  "namespace": {
    "epoch": 4,
    "socket_path": "/home/bfly/yunwei/cc-bridge_source/.cc-bridge/cc-bridge-daemon/tmux.sock",
    "session_name": "cc-bridge-abc123",
    "active_window": "main",
    "active_pane_id": "%12",
    "entry_window": "main",
    "sidebar": {
  ...
  "agents": [
    {
      ...
      "workspace_path": "/home/bfly/yunwei/cc-bridge_source"
    }
  ],
  "comms": [
    {
...
- `activity_state`, `activity_symbol`, `activity_color`, and `activity_reason` are computed by `cc-bridge-daemon`, not by the Rust sidebar.
...
- Top agent rows must not display CC_BRIDGE ask job ids or queue depth. Those are Comms fields.
...
- reply-delivery folding should prefer structured CC_BRIDGE reply/attempt/message-origin records; parsing display body text such as `job=...` is only a compatibility fallback.
...
- `comms.status` follows CC_BRIDGE job status values: `accepted`, `queued`, `running`, `completed`, `cancelled`, `failed`, or `incomplete`.
...
- `execution_phase_reason` is a stable diagnostic reason, and `execution_evidence` contains the compact job/attempt/inbound/mailbox/lease/completion/provider/reply identity facts used by `cc-bridge-daemon`; the sidebar must not rederive a phase from those facts.
...
The TUI may inspect tmux only for its own pane identity or terminal sizing when needed. State rendering and focus changes should go through `cc-bridge-daemon`; the TUI should not scan arbitrary tmux sessions to create state.

## 10. ProjectView RPC Contract

### 10.1 Socket Operation

Phase 1 should add a read-only `cc-bridge-daemon` socket operation:
...
- `cc-bridge-daemon.state`
- `cc-bridge-daemon.health`
- `namespace.epoch`
...
- `cc-bridge-daemon.generation`
- `cc-bridge-daemon.last_heartbeat_at`
...
- It should reuse the mounted project socket path already known to `cc-bridge`/sidebar startup.
- It returns one snapshot for the current project only.
- It should fail with the normal `cc-bridge-daemon` RPC failure envelope when the backend is unavailable or the request is malformed.
...
The Rust sidebar may either call the socket protocol directly or use a small CC_BRIDGE adapter process/library, but it must treat the returned payload as the source of truth.
...
- the sidebar should feel current without putting constant pressure on `cc-bridge-daemon`
- `cc-bridge-daemon` heartbeat/reconcile already runs independently
- Phase 1 does not need long-lived watch subscriptions

`cache.sequence` is a monotonic `cc-bridge-daemon` ProjectView sequence number for UI diffing. It increments when the generated view content changes. The TUI may skip redraw when the sequence is unchanged.
...
- mark the header as degraded, for example `cc-bridge-daemon ✕`
...
- focus operations change tmux focus only; they do not mutate CC_BRIDGE job, runtime, config, or namespace ownership state.
...
| `unmanaged_target` | target exists in tmux but is not CC_BRIDGE-managed | reject silently or show a short status message; do not focus it |
...
Error response shape should fit the existing `cc-bridge-daemon` RPC failure envelope, but the error string or payload must preserve the stable code. Recommended payload shape if the protocol is extended:
...
- `cc-bridge` starts/attaches the project namespace
- the sidebar pane runs inside managed project windows
- the sidebar talks to `cc-bridge-daemon` through internal RPC

If a debug command is added later, prefer a hidden or explicit diagnostic form such as `cc-bridge debug project-view --json` rather than a main workflow command.
...
- `r`: deliberately restart configured agent panes through `cc-bridge-daemon`
- `Ctrl-L`: force an immediate ProjectView refresh

Header controls:

- `⚙`: launch the current project's loopback-only `cc-bridge config ui`
...
- the built `cc-bridge-agent-sidebar` binary is shipped in the CC_BRIDGE release at `bin/cc-bridge-agent-sidebar`
- Phase 1 release automation publishes the Linux x86_64 helper tarball and ships a macOS universal helper inside `cc-bridge-macos-universal.tar.gz`
...
- `--cc-bridge-daemon-socket` points at the project-owned `cc-bridge-daemon` socket
...
cc-bridge-daemon focus handler layer
  execute project-socket-only focus/window commands inside cc-bridge-daemon handlers
...
- initialize terminal
- create the CC_BRIDGE client and app state
- run the event loop
- restore terminal on exit

`args.rs`:

- parse exactly `--cc-bridge-daemon-socket`, `--project-root`, and `--pane-window`
...
- avoid reading local CC_BRIDGE files or tmux state
...
- degraded `cc-bridge-daemon` header case
- empty Comms feed
...
- release packaging includes `bin/cc-bridge-agent-sidebar`
...
- when the config topology slice is implemented, promote the relevant config grammar from this plan into [docs/cc-bridge-config-layout-contract.md](cc-bridge-config-layout-contract.md)
- when namespace materialization or focus RPC is implemented, update [docs/cc-bridge-daemon-startup-supervision-contract.md](cc-bridge-daemon-startup-supervision-contract.md)
- when updating startup/supervision contract for this work, align command wording with the current user-facing entry path: `cc-bridge` creates/attaches the project namespace; do not introduce `cc-bridge open` as a required surface for Phase 1
- when ProjectView becomes a supported diagnostics/read path, update [docs/cc-bridge-daemon-diagnostics-contract.md](cc-bridge-daemon-diagnostics-contract.md) if `doctor` or support bundles expose it
...
- `cc-bridge-daemon.services.project_namespace`
- `cc-bridge-daemon.start_flow_runtime`
...
- tag managed panes with `@cc-bridge_role`, `@cc-bridge_window`, `@cc-bridge_slot`, `@cc-bridge_sidebar_instance`, and `@cc-bridge_namespace_epoch`
...
9. launch sidebar pane with `bin/cc-bridge-agent-sidebar --cc-bridge-daemon-socket <path> --project-root <path> --pane-window <name>`
...
```text
lib/cc-bridge-daemon/services/project_namespace_runtime/materialization.py
  NamespaceMaterializationPlan
  WindowMaterializationPlan
  build_namespace_materialization_plan

lib/cc-bridge-daemon/services/project_namespace_runtime/window_ops.py
  create_or_select_window
  materialize_window_layout
  tag_managed_panes

lib/cc-bridge-daemon/services/project_namespace_runtime/sidebar.py
...
- launch command uses `bin/cc-bridge-agent-sidebar` and exactly the documented args
...
- panes get `@cc-bridge_role`, `@cc-bridge_window`, and slot/sidebar options
...
- new `cc-bridge-daemon` ProjectView service/module
- `cc-bridge-daemon.handlers`
- `CcbdClient`
- socket tests
...
- build minimal `comms` from CC_BRIDGE job/message records
...
- optional provider/session signals already normalized by `cc-bridge-daemon`
...
- `cc-bridge-daemon.state` and `cc-bridge-daemon.health` come from mounted/backend inspection
...
```text
lib/cc-bridge-daemon/project_view/models.py
  ProjectView
  WindowView
  AgentView
  CommsView
...
lib/cc-bridge-daemon/project_view/activity.py
  AgentActivityFacts
  resolve_agent_activity

lib/cc-bridge-daemon/project_view/sequence.py
  ProjectViewSequenceCache

lib/cc-bridge-daemon/handlers/project_view.py
...
- resolve targets by CC_BRIDGE-managed pane options, not by visible pane title alone
- verify `@cc-bridge_project_id`, `@cc-bridge_role`, `@cc-bridge_window`, and `@cc-bridge_slot`
...
```text
lib/cc-bridge-daemon/project_focus/models.py
  FocusErrorCode
  FocusSuccess
...
lib/cc-bridge-daemon/handlers/project_focus.py
...
- build `tools/cc-bridge-agent-sidebar` as part of release
- package it as `bin/cc-bridge-agent-sidebar`
- resolve release-bundled helper path at runtime
- launch sidebar panes with `--cc-bridge-daemon-socket`, `--project-root`, and `--pane-window`
- mark sidebar panes as managed sidebar panes
- ensure `cc-bridge kill` tears down sidebar panes with the namespace

Runtime path resolution:

- release install path should expose `bin/cc-bridge-agent-sidebar`
- development checkout resolves repository `bin/cc-bridge-agent-sidebar` or an explicit `CC_BRIDGE_AGENT_SIDEBAR_BIN`
...
```text
<helper> --cc-bridge-daemon-socket <cc-bridge-daemon_socket_path> --project-root <project_root> --pane-window <window_name>
```

Pane setup:

- sidebar pane starts as a managed CC_BRIDGE pane, not a user shell
- pane options include `@cc-bridge_role=sidebar`
- pane options include `@cc-bridge_sidebar_instance=<window_name>`
- pane options include `@cc-bridge_window=<window_name>`
...
release/build scripts
  build tools/cc-bridge-agent-sidebar
  copy binary to bin/cc-bridge-agent-sidebar
```

Tests:

- release artifact contains `bin/cc-bridge-agent-sidebar`
...
- sidebar panes are tagged with `@cc-bridge_role=sidebar`
- sidebar panes are tagged with `@cc-bridge_sidebar_instance`
...
- `docs/cc-bridge-config-layout-contract.md` documents the promoted grammar
...
- `lib/cc-bridge-daemon/services/project_namespace_runtime/topology_plan.py` derives a pure namespace topology plan from normalized `ProjectConfig.windows` and `ProjectConfig.sidebar`.
- The plan emits config-ordered window plans, logical user layouts, realized layouts with projected sidebar leaves, and sidebar launch arguments for `cc-bridge-agent-sidebar --cc-bridge-daemon-socket ... --project-root ... --pane-window ...`.
...
- `ProjectNamespaceController.ensure(topology_plan=...)` materializes configured tmux windows in config order, renames the first window, creates additional managed windows, projects sidebar panes for `every_window`, tags sidebar panes with `@cc-bridge_role=sidebar`, `@cc-bridge_window`, and `@cc-bridge_sidebar_instance`, and selects `entry_window`.
...
- CC_BRIDGE pane identity now carries optional `@cc-bridge_window`; reused project-namespace bindings remain backward compatible with older panes that lack this option.
...
- Sidebar pane respawn now resolves the helper from `CC_BRIDGE_AGENT_SIDEBAR_BIN`, repository/install `bin/cc-bridge-agent-sidebar`, or `PATH`; if unavailable, it respawns a keepalive shell with a visible short error instead of crashing namespace creation.
...
- Focused coverage currently lives in `test/test_cc-bridge-daemon_namespace_topology_plan.py` plus startup-flow regressions in `test/test_v2_cc-bridge-daemon_start_flow.py`.
- The dedicated release workflow builds the Rust helper with `bin/build-cc-bridge-agent-sidebar`, uploads `cc-bridge-agent-sidebar-linux-x86_64.tar.gz`, and attaches it to GitHub Releases for tag builds.
- The main release-artifacts workflow builds the macOS helper for `x86_64-apple-darwin` and `aarch64-apple-darwin`, combines them with `lipo`, verifies the result is a universal binary, and ships it inside `cc-bridge-macos-universal.tar.gz`.
- A PTY-backed `cc-bridge open` smoke against an isolated temporary project attaches to the real tmux namespace with `TERM=xterm-256color`, times out only because the UI remains attached, and confirms sidebar panes keep rendering the window/agent tree after attach.
- `bin/package-cc-bridge-agent-sidebar-release` stages the Linux x86_64 helper tarball and checksum, and a temporary-repo dry-run test verifies the tarball contains `bin/cc-bridge-agent-sidebar`.

Primary write scope:

- `lib/cc-bridge-daemon/services/project_namespace_runtime/`
- `lib/cc-bridge-daemon/services/project_namespace_state_runtime/models.py`
- `lib/cc-bridge-daemon/start_flow_runtime/`
...
- new focused tests if needed: `test/test_cc-bridge-daemon_namespace_windows.py`
...
- `project_view` is registered as a read-only cc-bridge-daemon socket operation.
- `CcbdClient.project_view(schema_version=1)` is available.
- `lib/cc-bridge-daemon/project_view/` provides the activity resolver, stable sequence cache, and ProjectView service.
- The payload includes project metadata, cc-bridge-daemon lease state, namespace summary, config-window rows, ordered agent rows, five-state activity fields, and compact current comms rows from active/queued jobs.
...
- `project_focus_window` and `project_focus_agent` are registered as cc-bridge-daemon socket operations.
- `CcbdClient.project_focus_window(...)` and `CcbdClient.project_focus_agent(...)` are available.
- Focus RPC validates logical targets against config, checks optional namespace epoch, uses only the project namespace tmux socket/session, resolves managed agent panes by CC_BRIDGE pane options, and never creates panes.
- Focused coverage currently lives in `test/test_cc-bridge-daemon_project_view.py` and `test/test_cc-bridge-daemon_project_focus.py`.
- Current limitations:
...
- `lib/cc-bridge-daemon/project_view/`
- `lib/cc-bridge-daemon/project_focus/`
- `lib/cc-bridge-daemon/handlers/project_view.py`
- `lib/cc-bridge-daemon/handlers/project_focus.py`
- `lib/cc-bridge-daemon/handlers/__init__.py`
- `lib/cc-bridge-daemon/app.py`
- `lib/cc-bridge-daemon/socket_client.py`

Primary tests:

- `test/test_v2_cc-bridge-daemon_socket.py`
- `test/test_cc-bridge-daemon_socket_client.py`
- new focused tests if needed: `test/test_cc-bridge-daemon_project_view.py`
- new focused tests if needed: `test/test_cc-bridge-daemon_project_focus.py`
...
Status: initial CC_BRIDGE-native Rust crate skeleton implemented.

Current implementation notes:

- `tools/cc-bridge-agent-sidebar` is an independent Rust crate, not a root Cargo workspace.
- The binary parses `--cc-bridge-daemon-socket`, `--project-root`, and `--pane-window`.
- The client sends cc-bridge-daemon RPC requests using the existing newline-delimited JSON protocol and reads `project_view`.
...
- Keyboard navigation supports `j`/`k`/arrows, deliberate `r` pane restart,
  `Ctrl-L` refresh, `Enter` focus through cc-bridge-daemon RPC, `Tab`
  return-to-current-window focus through cc-bridge-daemon RPC, and `q`/`Esc` exit for
...
- ProjectView RPC failures keep the last good window/agent tree visible, mark the header as `cc-bridge-daemon ✕`, show stale/unavailable status in the Comms panel, and use the Phase 1 2s/5s retry backoff.
...
- `bin/cc-bridge-agent-sidebar` exists in source installs as a wrapper that delegates to the built Rust binary or shows a visible keepalive error. `install.sh` builds/copies the real Rust binary over that path when Cargo or a prebuilt release binary is available.
- `bin/build-cc-bridge-agent-sidebar` is the explicit release/developer build entrypoint for compiling `tools/cc-bridge-agent-sidebar` and replacing `bin/cc-bridge-agent-sidebar` with the real binary.
- `.github/workflows/test.yml` runs Rust sidebar tests and the build script on Ubuntu.
- `.github/workflows/release-sidebar.yml` builds and publishes the Linux x86_64 sidebar helper tarball on manual dispatch and tag pushes, with a SHA-256 checksum and GitHub Release attachment for tagged releases. Artifact staging goes through `bin/package-cc-bridge-agent-sidebar-release`.
- `cargo test --manifest-path tools/cc-bridge-agent-sidebar/Cargo.toml` passes with Rust sidebar coverage. `cargo fmt` could not be run on the current host because the `rustfmt` component is not installed.

Primary write scope:

- `tools/cc-bridge-agent-sidebar/Cargo.toml`
- `tools/cc-bridge-agent-sidebar/src/`
- `tools/cc-bridge-agent-sidebar/README.md`
- `tools/cc-bridge-agent-sidebar/LICENSE.upstream`

Primary tests:

- Rust unit tests under `tools/cc-bridge-agent-sidebar`
- Rust fixture tests under `tools/cc-bridge-agent-sidebar/src/tests/fixtures`
...
- release artifact includes `bin/cc-bridge-agent-sidebar`
- namespace materialization launches the bundled helper with documented args
- missing helper failure is clear
- `cc-bridge kill` tears down sidebar panes with the namespace
...
- Do not add `.cc-bridge/config.yaml`; Phase 1 extends `.cc-bridge/cc-bridge.config`.
- Do not make `cmd` part of new `windows` topology.
- Do not let manual panes enter ProjectView or sidebar navigation.
- Do not let Rust TUI scan tmux or local `.cc-bridge` files to derive state.
- Do not let focus RPC create panes or trigger provider/reconcile work.
- Do not let provider reader empty-poll paths crash `cc-bridge-daemon`; a temporary no-match must preserve cursor state and let ProjectView remain conservative.
...
- Do not introduce `cc-bridge open` as a required entry path for this feature.
...
- CC_BRIDGE namespace/layout authority constraints identified.
...
- Multi-window namespace creation, per-window agent pane materialization, sidebar pane projection, and `@cc-bridge_window` pane tagging implemented for the first tmux slice.
...
- Codex reader empty-match crash fixed so delayed provider output does not restart `cc-bridge-daemon`.
...
- Installer tests now exercise the prebuilt release-binary path, proving `install.sh` replaces the source wrapper with an existing `tools/cc-bridge-agent-sidebar/target/release/cc-bridge-agent-sidebar` binary when present.
- Live tmux smoke tests now use isolated temporary projects and project-scoped tmux sockets to verify both namespace materialization and a real `cc-bridge` start flow: `cc-bridge-daemon` materializes multiple managed windows, projects one sidebar pane into each managed window, preserves user-root pane tagging, launches agents in their configured windows, protects sidebar panes from orphan cleanup, and keeps a sidebar present after switching windows.
- A manual non-interactive helper smoke with `CC_BRIDGE_AGENT_SIDEBAR_BIN` pointed at the locally built Rust binary confirms real sidebar panes execute `cc-bridge-agent-sidebar` and render the window/agent tree plus empty Comms panel under tmux capture.
- A PTY-backed `cc-bridge open` smoke confirms the real attach path enters the project tmux namespace and that sidebar panes continue rendering after attach.
...
- A dedicated release workflow now publishes a Linux x86_64 `cc-bridge-agent-sidebar` tarball and checksum, and attaches them to GitHub Releases on tag builds.
- A scripted release dry-run now packages the Linux x86_64 sidebar tarball in a temporary repo and verifies the archive contains `bin/cc-bridge-agent-sidebar`.
...
1. Integrate provider-native manual-work signals for sidebar status using the
   plan-tree root in
   [docs/plantree/plans/sidebar-provider-activity/README.md](plantree/plans/sidebar-provider-activity/README.md).
2. Dogfood the integrated sidebar in normal CC_BRIDGE project usage and decide which
...
- `.cc-bridge/cc-bridge.config` can describe multiple managed windows.
...
- `cc-bridge-daemon` materializes every managed window in the project namespace.
...
- the sidebar shows only configured CC_BRIDGE agents, not manual panes.
...
- the release includes a runnable `cc-bridge-agent-sidebar` binary used by managed sidebar panes.
- the runtime launcher can invoke `bin/cc-bridge-agent-sidebar` with the documented launch arguments.
- `cc-bridge kill` destroys sidebar panes with the project namespace.
- manual user-created split panes are not tracked or restored by CC_BRIDGE.
