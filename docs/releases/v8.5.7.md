...
- CC_BRIDGE now guarantees the built-in `cc-bridge-clear` and `cc-bridge-diagnose` Skills for
  every supported managed Agent, even when optional Skill inheritance is
  disabled. Run `$cc-bridge_diagnose <agentname>` to diagnose exactly one mounted
  Agent without clearing its conversation or restarting the whole project.
- `cc-bridge-diagnose` combines daemon/runtime authority, active job lineage, queue,
...
- Recovery remains bounded to supported CC_BRIDGE control-plane actions and is
...
- PR #288 adds self-healing for expired mailbox deliveries without replaying
  business work, forwards Provider no-terminal timeout settings into `cc-bridge-daemon`,
...
- No configuration or conversation migration is required. Install with
  `npm install -g @seemseam/cc-bridge@8.5.7`, restart CC_BRIDGE, and invoke
  `$cc-bridge_diagnose <agentname>` from a supported managed Agent when diagnosis is
  needed.
- `cc-bridge-diagnose` is an inherited Agent Skill rather than a standalone shell
  command. Continue to use `cc-bridge clear <agentname>` for explicit context clears.
...
- All seven packaged `cc-bridge-diagnose` copies passed Skill validation and have an
...
- 即使关闭可选 Skill 继承，CC_BRIDGE 也会为所有受支持的托管 Agent 保证内置
  `cc-bridge-clear` 和 `cc-bridge-diagnose`。运行 `$cc-bridge_diagnose <agentname>` 可诊断一个
  已挂载 Agent，无需清除其对话，也不会重启整个项目。
- `cc-bridge-diagnose` 会联合 daemon/runtime 权威状态、当前 job lineage、队列、inbox、
  trace、Provider 日志和强制的有界 Pane 检查，在选择修复动作前区分正常推进、等待
  输入、提示符停滞、Provider 错误、Pane 死亡或空白以及布局异常。
- 恢复动作只使用 CC_BRIDGE 支持的有界控制面操作，并根据 runtime、lineage、队列和 Pane
...
- PR #288 增加过期 mailbox 投递的自恢复，不会重放业务任务；Provider 无终态超时
  配置现在可传入 `cc-bridge-daemon`；Kiro 通过隔离的 `KIRO_HOME` 继承设置，同时不会复制
...
- 无需迁移配置或对话。使用 `npm install -g @seemseam/cc-bridge@8.5.7` 安装并重启
  CC_BRIDGE；需要诊断时，在受支持的托管 Agent 中运行
  `$cc-bridge_diagnose <agentname>`。
- `cc-bridge-diagnose` 是继承到 Agent 内的 Skill，不是独立 shell 命令。需要明确清除
  上下文时，继续使用 `cc-bridge clear <agentname>`。
...
- 7 份打包的 `cc-bridge-diagnose` 均通过 Skill 校验，并具有相同的 workflow digest。
