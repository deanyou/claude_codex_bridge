...
- CC_BRIDGE Mobile now shows the selected Agent's Provider, active/configured/pending
...
- Managed Agents receive the built-in `cc-bridge-compact` Skill. `cc-bridge compact`
...
```bash
npm install -g @seemseam/cc-bridge@8.6.1
```

Existing release-package installations can use `cc-bridge update`. Android users can
install `cc-bridge-mobile-v8.6.1.apk` from this Release. No configuration or
...
- Account quota is available only when CC_BRIDGE has an authoritative managed Codex
  or Claude credential. Missing quota remains explicitly unavailable.
- `cc-bridge compact` fails closed for Providers without a verified native command;
...
- CC_BRIDGE Mobile 现在会显示所选 Agent 的 Provider、当前/已配置/待生效的模型与
...
- 托管 Agent 新增内置 `cc-bridge-compact` Skill。`cc-bridge compact` 会先检查权威的未完成
...
通过 GitHub Release 包安装的用户可运行 `cc-bridge update`。Android 用户可从本 Release
安装 `cc-bridge-mobile-v8.6.1.apk`。无需迁移配置或对话历史。

### 已知限制

- Codex 和 Claude 的模型/thinking 修改需要重启受影响的托管 Agent；本版本不宣称
  支持实时切换 Provider。
- 只有 CC_BRIDGE 持有权威的托管 Codex 或 Claude 凭据时，账号额度才可用；缺失额度会
  明确显示为 unavailable，不会估算。
- 对于没有经过验证的原生命令的 Provider，`cc-bridge compact` 会保持 fail-closed；
