...
- CC_BRIDGE now resolves Provider authority per dimension. An API key, token, URL,
  route, account selection, or Provider profile explicitly present in
  `.cc-bridge/cc-bridge.config` wins for that dimension; dimensions without explicit CC_BRIDGE
...
- CC_BRIDGE keeps one stable conversation identity across authority generations.
...
- CC_BRIDGE now bundles and automatically arms `codex-reconnect` for managed Codex
...
- No configuration or data migration is required. Do not run `cc-bridge clear` to
  apply an account or API change. After changing inherited shell environment
  values, restart the CC_BRIDGE/backend process so it reads the new environment;
...
- Install the npm package with
  `npm install -g @seemseam/cc-bridge@8.5.6`. Android users can install the signed
  CC_BRIDGE Mobile v8.5.6 APK from the v8.5.6 release.

### Known limitations

- Native fork/import is capability-gated by the installed Provider CLI. When a
  CLI cannot import or fork, CC_BRIDGE preserves a linked continuation and does not
...
- An inspectable external source-runtime project re-read changed inherited
  ...
  stable CC_BRIDGE conversation id, and created a native fork whose user-message
...
- CC_BRIDGE 现在按维度解析 Provider 权威状态。在 `.cc-bridge/cc-bridge.config` 中显式配置的
  API key、token、URL、route、账号选择或 Provider profile 优先用于对应维度；
  没有显式 CC_BRIDGE 权威的维度，才从当前外部 Provider 环境或登录状态继承。
- 停止后的 Provider generation 会根据新的外部快照和 CC_BRIDGE 私有 projection
  重新构建。整个流程是单向的：托管 Provider 的 refresh、logout、cleanup 和
  restart 不会写入用户 shell、Provider home、IDE、OS keyring 或远程登录。
- CC_BRIDGE 在不同 authority generation 之间保持一个稳定的 conversation identity。
...
- 只有在旧 session id 和路径证明属于该 Agent 时，才恢复已撤回 v8.5.5 的
  ...
  record 和历史。
- CC_BRIDGE 现在会在托管 Codex 形成明确 thread binding 后自动安装并 arm
...
- v8.5.5 已于 2026-08-05 从 GitHub 撤回。托管会话不要安装该版本，请使用
  v8.5.6 或 v8.5.4。
- 无需迁移配置或数据。账号或 API 变化后不要执行 `cc-bridge clear`。如果改变了继承的
  shell 环境变量，应重启 CC_BRIDGE/backend 进程，让新进程读取新的环境；随后按需重启
  受影响的托管 Agent。Provider 文件和支持的 keyring 状态会单向重新读取，会话
  连续性保持不变。
- npm 安装命令为 `npm install -g @seemseam/cc-bridge@8.5.6`。Android 用户可从
  v8.5.6 Release 安装签名后的 CC_BRIDGE Mobile v8.5.6 APK。

### 已知限制

- 原生 fork/import 由所安装的 Provider CLI 能力决定。如果 CLI 不支持导入或分叉，
  CC_BRIDGE 会保留 linked continuation，不会声称远程上下文已导入。
...
- 在可检查的外部 source-runtime 项目中，Codex 重新读取了已变化的继承权威，
  authority generation 正常递增，同时稳定 CC_BRIDGE conversation id 保持不变；新建的
