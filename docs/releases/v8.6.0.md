...
CC_BRIDGE 8.6.0 publishes the normal stable Linux, macOS, npm, Sidebar, and Android
artifacts and adds a native Windows x64 ZIP to the same GitHub Release. The CC_BRIDGE
version is stable; the Windows support tier remains beta while real
WezTerm/Herdr and provider qualification continues.

- Native Windows PE launchers are included for `cc-bridge`, `ask`, `autonew`, and
  `ctx-transfer`.
...
- Pi now restores the exact validated native JSONL session after a CC_BRIDGE restart.
...
- Terminal backend exports now initialize lazily, preventing the Windows Herdr
  integration from creating a circular import when mobile or cc-bridge-daemon services load.
- Non-interactive Windows installation now honors `-Yes` and
  `CC_BRIDGE_INSTALL_ASSUME_YES=1` when Herdr is absent, while preserving the warning
  that normal CC_BRIDGE startup still requires Herdr.

### Upgrade

```bash
npm install -g @seemseam/cc-bridge@8.6.0
```

Existing release-package installations can use `cc-bridge update`. No configuration
or conversation migration is required.

For Windows testing, download `cc-bridge-windows-x86_64.zip` and its `.sha256`
...
- `cc-bridge update` is diagnostic-only on the Windows beta path.
...
CC_BRIDGE 8.6.0 会正常发布稳定版 Linux、macOS、npm、Sidebar 与 Android 产物，
并在同一个 GitHub Release 中新增原生 Windows x64 ZIP。CC_BRIDGE 版本是正式稳定版；
在真实 WezTerm/Herdr 与 Provider 验收完成前，Windows 支持等级仍保持 beta。

- 新增 `cc-bridge`、`ask`、`autonew`、`ctx-transfer` 的 Windows 原生 PE 启动器。
...
- CC_BRIDGE 重启后，Pi 现在会恢复经过校验的准确原生 JSONL 会话；新上下文和显式
  Pi 会话参数仍保持原有语义。
- 后端缓存复用后会重新绑定项目专属 tmux socket，修复 Windows 集成期间引入的
  Linux、macOS 与 WSL 生命周期回归。
- Terminal backend 导出改为按需初始化，避免 mobile 或 cc-bridge-daemon 服务加载时被
  Windows Herdr 集成触发循环导入。
- Windows 非交互安装在缺少 Herdr 时会正确识别 `-Yes` 与
  `CC_BRIDGE_INSTALL_ASSUME_YES=1`，同时继续明确提示：正常启动 CC_BRIDGE 仍然需要 Herdr。
...
通过 GitHub release 包安装的用户可运行 `cc-bridge update`。本次升级不需要迁移配置或
对话历史。

Windows 测试用户请从本 Release 下载 `cc-bridge-windows-x86_64.zip` 和对应的
...
- Windows beta 路径中的 `cc-bridge update` 目前只提供诊断。
