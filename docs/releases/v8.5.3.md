...
- CC_BRIDGE now keeps a process-local latest-state view of the append-only callback
...
- Non-API variables from `agents.<name>.env`, such as proxy settings and tool
  configuration paths, now reach managed Claude processes. CC_BRIDGE's API
...
- Restart already-running CC_BRIDGE projects after upgrading so the daemon and
  managed Claude processes load the new behavior.
- npm-managed installations can run
  `npm install -g @seemseam/cc-bridge@8.5.3`.
- Android users can install the signed CC_BRIDGE Mobile v8.5.3 APK from this release.
...
- CC_BRIDGE 现在为只追加的 callback edge ledger 维护进程内最新状态视图；
  检测到外部追加、替换或截断后会安全重建。
...
- `agents.<name>.env` 中的非 API 变量（例如代理设置和工具配置路径）现在
  会传入托管 Claude 进程。CC_BRIDGE 对 API 凭据、endpoint 和托管 runtime-home
  的优先级保持不变（PR #284）。

### 升级说明

- 无需迁移配置或数据。
- 升级后请重启已经运行的 CC_BRIDGE 项目，使 daemon 和托管 Claude 进程加载
  新行为。
- npm 安装可执行：
  `npm install -g @seemseam/cc-bridge@8.5.3`。
- Android 用户可从本 Release 安装签名后的 CC_BRIDGE Mobile v8.5.3 APK。
