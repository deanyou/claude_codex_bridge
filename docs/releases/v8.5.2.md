...
- Consecutive unstable recoveries back off for 30 seconds, 60 seconds,
  120 seconds, 5 minutes, 10 minutes, and 30 minutes. After six attempts CC_BRIDGE
...
- A stale CC_BRIDGE-managed Claude `--continue` binding can be removed without
...
- CC_BRIDGE Rich WezTerm launches now use a new process session with stdin, stdout,
...
- Restart already-running CC_BRIDGE projects after upgrading so they load the new
  recovery and managed-memory behavior.
- Existing Rich users should run `cc-bridge update rich` once to regenerate the
...
- If an Agent reaches `recovery-circuit-open`, repair its Provider/session
  state and then run `cc-bridge restart <agent>` or remount the project.
- npm-managed installations can run
  `npm install -g @seemseam/cc-bridge@8.5.2`.
- Android users can install the signed CC_BRIDGE Mobile v8.5.2 APK from this release.
...
- 连续不稳定恢复依次退避 30 秒、60 秒、120 秒、5 分钟、10 分钟和
  30 分钟。第六次仍不稳定时，CC_BRIDGE 会打开恢复熔断，并停止继续 respawn、
...
- 每个 Provider runtime 只保留最新 50 份 pane crash 日志及对应原因记录。
  ...
  写盘。
- CC_BRIDGE 可以移除失效的 Claude `--continue` 绑定，同时不改动 Provider
...
- CC_BRIDGE Rich WezTerm 现在使用独立进程会话启动，并断开与调用 shell 的
...
- 无需迁移现有配置。
- 升级后请重启已经运行的 CC_BRIDGE 项目，使其加载新的恢复和托管记忆行为。
- 已启用 Rich 的用户应执行一次 `cc-bridge update rich`，重新生成隔离 launcher
  和 Wayland cursor 兼容 overlay。
- 如果 Agent 进入 `recovery-circuit-open`，请先修复其 Provider/session
  状态，再执行 `cc-bridge restart <agent>` 或重新挂载项目。
- npm 安装可执行：
  `npm install -g @seemseam/cc-bridge@8.5.2`。
- Android 用户可从本 Release 安装签名后的 CC_BRIDGE Mobile v8.5.2 APK。
