...
- CC_BRIDGE now requires an external, exact-value approval before a project-local
  `.cc-bridge/cc-bridge.config` can execute a tool-window command or a custom Provider
  command template. Interactive startup can request the approval, and
  `cc-bridge config approve-commands` provides an explicit non-startup workflow.
...
- Managed Codex recovery preserves conversation continuity when the current
  session record is corrupt. CC_BRIDGE identifies the latest valid recoverable
...
- CC_BRIDGE Mobile adds multi-session host terminals and negotiates Provider-control
...
- Native Windows process liveness and command-line inspection are more robust;
  `cc-bridge kill` tears down the persisted Herdr namespace, and background launcher
...
- OMP managed homes inherit the intended Provider configuration while retaining
  CC_BRIDGE's managed-home isolation boundaries.
...
```bash
npm install -g @seemseam/cc-bridge@8.6.2
```

Existing release-package installations can use `cc-bridge update`. Android users can
install `cc-bridge-mobile-v8.6.2.apk` from this Release.
...
through the interactive prompt or `cc-bridge config approve-commands`. Changing an
...
- CC_BRIDGE 现在要求项目本地 `.cc-bridge/cc-bridge.config` 中的 tool-window 命令或自定义
  Provider command template 在执行前取得项目外部、精确值匹配的批准。交互式启动
  可请求批准，也可通过 `cc-bridge config approve-commands` 明确完成非启动流程。
  该安全边界修复了 Issue #299，不再把项目内容默认视为可信代码。
- 托管 Codex 当前 session 记录损坏时，CC_BRIDGE 会选择最近的有效可恢复 session，
  保持对话连续性，不再静默清空上下文或绑定到格式错误的状态。
- Cursor 任务可通过可见 Provider Pane 执行，并使用 transcript anchor、稳定空闲
  检查、busy 延迟和派发后的终态证据判断任务生命周期。
- CC_BRIDGE Mobile 新增主机多 session 终端，并通过 Relay 协商 Provider 控制能力；旧版
...
- 原生 Windows 的进程存活与命令行检查更加可靠；`cc-bridge kill` 会销毁持久化 Herdr
  namespace，后台 launcher 路径也避免弹出多余控制台窗口和 venv 重定向失败。
- OMP 托管 home 会继承预期的 Provider 配置，同时保持 CC_BRIDGE 的托管 home 隔离边界。
