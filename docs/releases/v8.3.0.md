...
- Managed Claude, Gemini, Droid, Qwen, and Copilot homes inherit supported
  extensions and plugins only when CC_BRIDGE can prove ownership, preserving
...
- Marker-only CC_BRIDGE worktrees are removed only after proving that no user payload
...
- CC_BRIDGE 8.3.0 requires no manual data migration. Existing installations can use
  their normal update path; npm-managed installations can run
  `npm install -g @seemseam/cc-bridge@8.3.0`.
...
- Qoder 现已按照真实 CLI 的会话、恢复、更新、存储与完成协议运行，其中包括
  PR #270 完成的集成修复。
- 托管的 Claude、Gemini、Droid、Qwen 与 Copilot 主目录仅在 CC_BRIDGE 能证明资产
...
- 由 npm 管理的安装会把升级交还给 npm，确保已安装包与元数据原子更新；该项
  修复解决了 Issue #268。
- 只有在确认不存在用户数据后，CC_BRIDGE 才会删除仅含标记的工作树；存在歧义或来自
...
- CC_BRIDGE 8.3.0 不需要手工迁移数据。现有安装可继续使用原有更新方式；由 npm
  管理的安装可执行 `npm install -g @seemseam/cc-bridge@8.3.0`。
