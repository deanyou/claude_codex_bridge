...
- CC_BRIDGE now supports the official DeepSeek Harness under the separate Developer
...
- CC_BRIDGE refreshes AGY's provider-recognized keyring-unavailable marker only
...
- DSH `0.1.0-rc.6` is Developer Preview software. CC_BRIDGE validated its official
...
- CC_BRIDGE 以独立 Developer Preview provider key `dsh` 接入官方 DeepSeek
...
- CC_BRIDGE 只在 AGY 的受管 HOME 内刷新 provider 自身识别的 keyring-unavailable
...
- DSH `0.1.0-rc.6` 仍是 Developer Preview。CC_BRIDGE 已验证官方 loopback carrier、
