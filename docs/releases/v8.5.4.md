...
- No configuration or data migration is required. Restart already-running CC_BRIDGE
...
- npm-managed installations can run
  `npm install -g @seemseam/cc-bridge@8.5.4`.
- Android users can install the signed CC_BRIDGE Mobile v8.5.4 APK from this release.
...
- 无需迁移配置或数据。升级后请重启已经运行的 CC_BRIDGE 项目，使托管 ask skill
  和 Config UI 加载新版本。
...
- npm 安装可执行：
  `npm install -g @seemseam/cc-bridge@8.5.4`。
- Android 用户可从本 Release 安装签名后的 CC_BRIDGE Mobile v8.5.4 APK。
