# CC_BRIDGE_DAEMON Ask Control Plane 收敛方案

## 1. 文档定位

这份文档定义 `cc-bridge ask` / `cc-bridge-daemon submit` 控制面快路径与命令面去重的统一收敛方案。
...
本文合并了原 `docs/cc-bridge-daemon-command-surface-simplification-plan.md` 的内容，避免两份半重叠方案长期并存。
...
1. 真失败
   - 例如 `project cc-bridge-daemon is unmounted; run cc-bridge first`
   - 消息未进入队列
   - 可安全重发
...
- compatible probe
- `client.ping('cc-bridge-daemon')`

而当前 `ping('cc-bridge-daemon')` 会先执行：
...
- `ping('cc-bridge-daemon')` 从 deep health 改为 light ping
...
> `cc-bridge-daemon` 把 submit/ping 这类 control-plane fast path，与 heartbeat/health/runtime recovery/mailbox refresh 这类 maintenance slow path 混在同一个同步执行模型里。
...
| P1 | 收窄 fast path | `ping('cc-bridge-daemon')` 改 light ping；submit 移除 `refresh_mailbox()`；submit 后同步 maintenance 从 double tick 收窄为 single tick | 直接降低 submit ack timeout | 中 |
...
| P3 | mailbox 视图增量化 | mailbox head/summary 改增量索引或异步维护；`read_all()` 退出控制面热路径；详细方案见 `docs/cc-bridge-daemon-p3-p4-mailbox-cli-plan.md` | 彻底消除日志历史增长对 ack 的影响 | 中 |
| P4 | 命令面收敛 | observer group / diagnostics group / recovery group 收敛；详细方案见 `docs/cc-bridge-daemon-p3-p4-mailbox-cli-plan.md` | 减少认知负担和误用 | 中 |
...
| P3 | mailbox 增量 summary 机制 | mailbox backlog 增长不再线性拖慢 submit；summary 写入 ownership 与版本/CAS 规则见 `docs/cc-bridge-daemon-p3-p4-mailbox-cli-plan.md` |
| P4 | CLI 帮助文案、parser alias、render 收敛 | 主帮助只突出 `ask` / `doctor`；observer group 弱化与后续命令面收敛见 `docs/cc-bridge-daemon-p3-p4-mailbox-cli-plan.md` |
...
| 目标 | 主要模块 | 预期改动 |
|---|---|---|
| light ping | `lib/cc-bridge-daemon/handlers/ping_runtime/handler.py` | 去掉默认 `health_monitor.check_all()`，改读 cached summary |
...
| submit maintenance 减负 | `lib/cc-bridge-daemon/socket_server_runtime/server.py`, `lib/cc-bridge-daemon/socket_server_runtime/loop.py` | submit 后同步 maintenance 从 double tick 收窄为 single tick |
| lazy restore handoff | `lib/cc-bridge-daemon/services/dispatcher_runtime/routing.py`, `lib/cc-bridge-daemon/services/runtime.py`, `lib/cc-bridge-daemon/services/dispatcher_runtime/lifecycle_start_runtime/*` | 保留 stopped agent lazy restore 语义，但把恢复从 submit path 后移 |
| shutdown / stop_all contract | `lib/cc-bridge-daemon/app_runtime/lifecycle.py`, `lib/cc-bridge-daemon/handlers/shutdown.py`, `lib/cc-bridge-daemon/handlers/stop_all.py`, `lib/cc-bridge-daemon/socket_server_runtime/protocol.py`, `lib/cc-bridge-daemon/socket_server_runtime/loop.py` | response-before-teardown，两阶段 stop contract |
| background maintenance fail-fast | `lib/cc-bridge-daemon/app_runtime/policy.py`, `lib/cc-bridge-daemon/supervisor_runtime/namespace.py`, `lib/cc-bridge-daemon/services/project_namespace_runtime/*` | 后台 namespace/tmux/recovery 走 non-blocking bounded probes |
| stop_all state gate | `lib/cc-bridge-daemon/app_runtime/lifecycle.py`, `lib/cc-bridge-daemon/handlers/stop_all.py`, `lib/cc-bridge-daemon/services/runtime.py`, `lib/cc-bridge-daemon/services/dispatcher.py` | `stop_all` finalize 后 runtime/job state 不回退，作为 lane 分离前置门禁 |
| maintenance lane 分离 | `lib/cc-bridge-daemon/socket_server_runtime/loop.py`, `lib/cc-bridge-daemon/app_runtime/lifecycle.py`, `lib/cc-bridge-daemon/services/dispatcher.py` | request worker 只做 request/response；maintenance worker 处理 tick/health/reconcile |
...
- `test_cc-bridge-daemon_socket_codex_protocol_turn_completes_via_tracker`
- `test_cc-bridge-daemon_socket_codex_protocol_turn_handles_interrupted_abort`
- `test_cc-bridge-daemon_socket_roundtrip_and_shutdown`
- `test_cc-bridge-daemon_stop_all_does_not_run_post_shutdown_heartbeat`
- `test_cc-bridge-daemon_stop_all_force_terminalizes_running_jobs_before_restart_restore`
- `test_cc-bridge-daemon_start_flow_writes_runtime_authority_via_rpc`
...
1. keep the phase table and status tracking aligned around:
   ...
   - `P3` complete
   - `P4` complete
2. use `docs/cc-bridge-daemon-lifecycle-test-plan.md` section 11 as the next verification
...
5. extend real Linux / macOS / WSL verification before declaring the ask/cc-bridge-daemon
   stabilization branch release-ready

The concrete real-test matrix is now tracked in
`docs/cc-bridge-daemon-lifecycle-test-plan.md` section 11. That matrix is the authoritative
