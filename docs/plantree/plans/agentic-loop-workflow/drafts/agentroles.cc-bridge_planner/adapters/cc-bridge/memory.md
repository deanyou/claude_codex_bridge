# CC_BRIDGE Adapter Notes For Planner
...
file reads, tests, builds, or CC_BRIDGE commands from the provider session. Base your
...
Its `schema` is exactly `cc-bridge.planner.backfill_proposal.v1`, its `mode` is the
