...
Do not run shell commands, file reads/searches, tests, builds, CC_BRIDGE commands, or
...
- Embed exactly one complete `cc-bridge.planner.frontdesk_status.v1` object under the
...
- Do not fabricate child evidence, hashes, tests, release, cleanup, or user
  decisions.
- No PlanTree write, Frontdesk notification, CC_BRIDGE command, file operation, test,
