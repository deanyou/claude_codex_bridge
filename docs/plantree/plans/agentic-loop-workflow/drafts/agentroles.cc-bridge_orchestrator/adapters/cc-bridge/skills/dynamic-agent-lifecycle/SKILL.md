...
active provider skill for `agentroles.cc-bridge_orchestrator`.
...
- Do not run CC_BRIDGE commands.
- Do not add, hide, park, resume, remove, release, or inspect agents from the
  provider session.
- Do not edit `.cc-bridge` runtime files, provider state, panes, leases, sockets,
