# CC_BRIDGE Workflow Plan Tree Skill Overlay
...
Define a CC_BRIDGE-owned Plan Tree skill for the agentic-loop workflow without
changing the public Plan Tree skill or making workflow behavior the default for
ordinary CC_BRIDGE agents. The CC_BRIDGE version may retain the useful public document
...
CC_BRIDGE ships one complete workflow-specific effective skill under the logical
skill name `plan-tree`. CC_BRIDGE projects that replacement only into the managed
...
```text
public Plan Tree behavior                CC_BRIDGE workflow contracts
  ...
  retrieval / resume                       proposal schemas and fences
                  \                       /
                   complete CC_BRIDGE effective skill
                             |
...
agent slot == "planner"
role id == "agentroles.cc-bridge_planner"
...
V2, ordinary CC_BRIDGE agents, non-Planner Config V3 roles, and standalone provider
...
The CC_BRIDGE Planner RolePack adapter owns this projection. It should use a
...
```toml
[adapters.cc-bridge.skill_overlays.plan_tree]
...
CC_BRIDGE must never replace, delete, or rewrite the source-home skill. The managed
home is rebuildable runtime state. Switching away from Config V3 or changing
the Planner Role removes the CC_BRIDGE-owned projection and rematerializes the normal
...
The specialized skill may reason about CC_BRIDGE workflow fields supplied by the
...
| Runtime job, node, round, provider, release, cleanup | CC_BRIDGE runtime/controller state | Treat supplied facts as evidence only |
...
- Projection cleanup is marker-owned and may remove only the managed-home
  target created by the matching CC_BRIDGE projection.
...
| :--- | :--- |
| Config V2 with public `plan-tree` | Public source/managed behavior remains unchanged; no CC_BRIDGE workflow overlay marker |
...
- Do not expose the CC_BRIDGE workflow overlay to ordinary CC_BRIDGE agents.
