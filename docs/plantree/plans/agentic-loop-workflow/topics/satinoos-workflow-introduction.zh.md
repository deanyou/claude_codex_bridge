...
Satinoös 是 CC_BRIDGE 上的一种脚本约束型多 Agent 工作流方案。它的目标不是让一组
...
Satinoös 面向需要长期规划、分工执行和可审查结果的复杂任务。它把 CC_BRIDGE 从
“前台人工协调多个 Agent”推进到“由文档状态和脚本边界驱动的半自动工作流”。

这个方案保留 CC_BRIDGE 的可见 Agent、`ask`、运行时状态和 pane/window 管理能力，
...
- 文档状态保存长期事实，运行时文件保存短期事实。低频结论进入 plan-tree，
  高频执行细节留在 `.cc-bridge/runtime` 和 artifact 记录中。
...
3. `orchestrator` 读取任务包和执行合同后做四路分流：
   ...
   - `needs_detail`：按需激活 `cc-bridge_task_detailer`，补齐任务局部细节后再回到
     `orchestrator`。
...
| `cc-bridge_planner` | 宏观目标、plan brief、task packet、验收和风险摘要 | 维护详细实现正文、直接调度运行时 |
| `cc-bridge_orchestrator` | 路由判断、任务拆分、请求 mount topology、组织 ask-first 协作 | 直接改 tmux、runtime、task status |
| `cc-bridge_task_detailer` | 按需细化任务、做局部技术调研、产出 detail packet | 成为默认 planner、直接改 roadmap 或调度 worker |
| `coder` | 完成有边界的实现或分析工作 | 降低验收标准、隐藏失败 |
| `code_reviewer` | 审查实现、设计/确认验证证据、拒绝假成功 | 接管产品范围或把 partial 改成 done |
| `cc-bridge_round_reviewer` | 审查整轮结果和证据，协助形成稳定 round summary | 直接修代码或直接写 task status |
| CC_BRIDGE scripts | artifact 导入、状态提交、topology apply/release、锁和校验 | 语义判断和产品取舍 |

`cc-bridge_task_detailer` 是按需角色。即使它在某些运行时布局中可见或 parked，它也只在
...
- `needs_detail`：先由 `cc-bridge_task_detailer` 细化，再进入执行轮。
