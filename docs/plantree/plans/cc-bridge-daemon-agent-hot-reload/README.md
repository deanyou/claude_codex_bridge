# CC_BRIDGE_DAEMON Agent Hot Reload Plan

Date: 2026-05-28

## Purpose

Plan true agent dynamic loading for `cc-bridge-daemon`: after `.cc-bridge/cc-bridge.config` changes,
...
General `cc-bridge-daemon` throughput optimization is not the primary feature goal here,
...
- `cc-bridge reload` or equivalent daemon RPC for reloading `.cc-bridge/cc-bridge.config`.
- `cc-bridge reload --dry-run` for diffing and validation before mutation.
...
- General `cc-bridge-daemon` performance slimming.
