...
- Force unload must be explicit and must record that it may interrupt provider
  work.
- `.cc-bridge/agents/<agent>` is preserved unless a separate cleanup command removes
  safe residue.
...
apply, CC_BRIDGE uses the existing managed pane as assigned pane evidence, stops the
...
namespace/runtime/publish mutation. Retrying `cc-bridge reload` after the agent
