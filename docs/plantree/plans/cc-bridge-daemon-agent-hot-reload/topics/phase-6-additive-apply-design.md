...
Phase 6a was design-only. It did not enable non-dry-run `cc-bridge reload`, call
...
`build_cc-bridge-daemon_service_graph()` is the right way to build a new config-bound
bundle. `publish_cc-bridge-daemon_service_graph()` is the only publish step and must be the
...
- `apply_cc-bridge_pane_identity()` for `@cc-bridge_project_id`, role, slot, window,
  namespace epoch, and `managed_by=cc-bridge-daemon` options;
...
- `build_reload_service_graph(app, new_config, version=...)`, a reload wrapper
  around `build_cc-bridge-daemon_service_graph()` that reuses startup dependencies and does
  not publish.
...
4. Splitting exactly one new pane per appended agent from the anchor/previous
   new pane, then applying CC_BRIDGE pane identity to the new pane.
...
4. If sidebar is enabled, split the root pane, respawn the sidebar command, and
   apply sidebar CC_BRIDGE identity for the new window only.
5. Materialize only the new window's agent layout under the new window user
   root, applying CC_BRIDGE identity to each new agent pane.
...
process checks the mounted daemon by calling `ping('cc-bridge-daemon')` and comparing the
...
`ping('cc-bridge-daemon')` still reports the old graph signature, and the keeper requests
...
- best-effort rollback may kill only panes/windows created by this patch and
  ...
  role, slot, and `managed_by=cc-bridge-daemon`;
...
- `preserved_before`: agent -> pane id from current CC_BRIDGE tmux identity options
  before patch;
...
The first implementation step was not enabling `cc-bridge reload`. It added a fake
...
- Append-only `add_agent` requires the new layout tree to expand the last
  ...
  writes CC_BRIDGE identity evidence for the new pane only.
...
- The fake-backend tests cover new window/sidebar/agent pane creation,
  append-only agent pane creation, `managed_by=cc-bridge-daemon` identity evidence,
...
`project_reload_config(dry_run=false)` and plain `cc-bridge reload`.

Current status:

- It runs under the existing app maintenance lock, then reads the current
  service graph and current namespace once.
- It builds the same dry-run plan used by `cc-bridge reload --dry-run` and rejects
...
- `project_reload_config(dry_run=false)` loads `.cc-bridge/cc-bridge.config` once and calls
  `run_additive_reload_apply(...)`.
- Plain `cc-bridge reload` sends `dry_run=false`; `cc-bridge reload --dry-run` remains
...
- Successful publish invalidates the current project-view cache so the next
  `project_view` and `ping('cc-bridge-daemon')` read the newly published graph/config.
...
- Sidebar's refresh control and `r` shortcut now submit `project_restart_panes`
  ...
  refresh. Config changes remain explicit CLI actions through `cc-bridge reload`; the
