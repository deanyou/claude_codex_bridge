...
- Add CLI `cc-bridge reload --dry-run`.
- Load and validate `.cc-bridge/cc-bridge.config`.
...
- Phase 3 implementation status:
  - `project_reload_config` rejects non-dry-run requests before updating reload
    metrics.
  - `cc-bridge reload --dry-run` calls the mounted daemon and does not bootstrap or
    write a missing `.cc-bridge/cc-bridge.config`.
...
- Added `cc-bridge-daemon.reload_drain` as pure state machinery with `DrainIntent`,
...
- `DrainQueueStore` persists only explicit state-machine calls to
  `.cc-bridge/cc-bridge-daemon/reload-drain.json`; heartbeat and request steady state do not scan
  it.
...
- At the Phase 4 checkpoint, non-dry-run `project_reload_config` / `cc-bridge reload`
...
- Record the project id, socket, session, namespace epoch, window, role,
  `slot_key`, and `managed_by=cc-bridge-daemon` proofs future mutation must satisfy.
...
- Keep future CC_BRIDGE-owned tmux settings project/session-scoped.
...
- Added `cc-bridge-daemon.reload_patch` as a pure namespace patch planner.
...
- At the Phase 5 checkpoint, non-dry-run `project_reload_config` / `cc-bridge reload`
...
- `project_reload_config(dry_run=false)` and plain `cc-bridge reload` now call the
  orchestrator, but only the accepted additive classes may publish.
- `cc-bridge reload --dry-run` remains no-mutation and keeps the same planning
  payload shape.
...
- Plain `cc-bridge reload` writes a short TTL `reload-handoff.json` before submitting
...
- Explicit `cc-bridge reload` now accepts the first idle unload slice. Removing an
...
- Remove managed pane only after runtime is idle/no-outstanding. Completed,
  ...
  follow-up.
- Preserve `.cc-bridge/agents/<agent>` history as residue/audit data, not configured
  authority.
