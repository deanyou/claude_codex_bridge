...
Current `cc-bridge-daemon` is not built around in-process config hot reload. It treats the
...
The keeper compares the live daemon's `ping('cc-bridge-daemon')` config signature with the
...
- `.cc-bridge/cc-bridge.config` defines the desired agent mount set and foreground pane
  layout for the backend.
- `cc-bridge-daemon` continuously keeps configured agents mounted and healthy.
- Pane ids are evidence, not logical identity.
- Runtime authority writes must go through explicit authority paths.
- Unknown `.cc-bridge/agents/*` directories are residue unless present in current
  config.
