...
- Reload dry-run:
  ...
  - Phase 3 covered classes: `no_change`, `view_only_change`, `add_agent`,
    ...
    and `invalid_config`;
  - `cc-bridge reload --dry-run` renders daemon payloads and returns non-zero for
    invalid-config dry-run results;
  - plain `cc-bridge reload` is not a dry-run and is allowed only for the gated
...
- Namespace patch planning:
  ...
  - additive planning requires verified project id, tmux socket, session,
    namespace epoch, window, role, slot key, and `managed_by=cc-bridge-daemon` proof before
    future apply;
...
- Phase 6b namespace additive patch API:
  ...
  - new pane identities include project id, role, slot key, window, namespace
    epoch, and `managed_by=cc-bridge-daemon`;
...
- Phase 6b user path gate:
  - `project_reload_config(dry_run=false)` and plain `cc-bridge reload` invoke the
  ...
  - after successful non-dry-run publish, `ping('cc-bridge-daemon')` and `project_view`
...
- Bounded drain/retire state:
  ...
  - `project_reload_config` and CLI reload rendering expose active drain count,
    agent, phase/status, busy state, bounded deadlines, and `cc-bridge reload` as the
    explicit retry path.
...
- Handler graph routing:
  ...
  - wrapper dispatch does not reload `.cc-bridge/cc-bridge.config` or rebuild the graph per
    request;
...
- Add agent to existing window:
  ...
  - new agent pane created with correct CC_BRIDGE tmux identity options;
...
- Existing agent removed from `[windows]`:
  ...
  - subsequent `cc-bridge reload --dry-run` / `cc-bridge reload` payloads show the active
...
- Existing agent provider/workspace/model/key/url change after automatic busy
  ...
  - idle-ready replace drains should execute the same same-slot replacement
    path without requiring a manual `cc-bridge reload`;
...
- Keeper signature continuity:
  ...
  - saving `.cc-bridge/cc-bridge.config` before explicit reload does not make keeper or CLI
    compatibility restart a modern mounted daemon. The old daemon signature is
    a reload-pending state until `cc-bridge reload` applies or rejects the change.
...
- 2026-06-28 live provider smoke evidence:
  ...
  - `/home/bfly/yunwei/test_ccb2/dynamic-layout-live-codex-arrange-window-latest.json`
    ...
    restore the fixed-column target geometry with `cc-bridge agent arrange`, preserve
...
- Phase 3 dry-run checks:
  - start a mounted project and run `cc-bridge reload --dry-run` with no config
    changes; expect `plan_class: no_change`;
  - edit `.cc-bridge/cc-bridge.config` to add one agent to an existing window; dry-run
  ...
  - break TOML or config validation; dry-run should report `invalid_config`
    while `cc-bridge ping cc-bridge-daemon` still shows the old config signature.
- Phase 5 dry-run namespace patch checks:
  ...
  - run `cc-bridge reload --dry-run` before plain `cc-bridge reload` and confirm the
...
- Phase 6+ mutating checks:
  ...
  - after the added agent is idle, remove it from `.cc-bridge/cc-bridge.config`, run
    `cc-bridge reload`, and verify only that agent pane disappears, `cc-bridge ping cc-bridge-daemon`
    no longer lists it, and old pane ids for the remaining agents are
    unchanged;
  - submit a real `ask` to a remaining agent after unload and confirm cc-bridge-daemon still
  ...
  - run `cc-bridge reload --dry-run` before each mutating manual test and verify it
...
- `cc-bridge kill` and normal cold start behavior remain unchanged.
