...
The user edits `.cc-bridge/cc-bridge.config` to add, unload, or eventually replace an
...
1. CLI sends `cc-bridge reload --dry-run` or `cc-bridge reload` to the mounted daemon.
2. Handler acquires `app.start_maintenance_lock`.
3. Load and validate `.cc-bridge/cc-bridge.config` from disk.
...
- prove the current session/window identity still matches the authoritative
  namespace state;
- list existing managed agent/sidebar panes by CC_BRIDGE tmux user options;
...
- write `@cc-bridge_project_id`, `@cc-bridge_role`, `@cc-bridge_slot`, `@cc-bridge_window`,
  `@cc-bridge_managed_by`, and epoch options for each new pane;
...
- `cc-bridge-daemon.reload_patch` produces deferred `create_window`,
...
- CC_BRIDGE-owned tmux behavior remains project/session-scoped; reload must not rely
...
hint: run cc-bridge restart or cc-bridge kill && cc-bridge
```
