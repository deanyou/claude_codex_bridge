# CC_BRIDGE_DAEMON Agent Hot Reload Roadmap

Date: 2026-06-28

## Done

- Confirmed current daemon initialization loads `.cc-bridge/cc-bridge.config` once and
...
- Added Phase 3 dry-run reload planning: `project_reload_config` accepts only
  `dry_run=true`, `cc-bridge reload --dry-run` renders the no-mutation plan, and the
...
- Wired the Phase 6b additive apply orchestrator into the explicit user path:
  `project_reload_config(dry_run=false)` and plain `cc-bridge reload` now attempt
...
- Added the Phase 6b keeper handoff guard: plain `cc-bridge reload` and accepted
...
- Enabled the first Phase 7 idle unload path for explicit `cc-bridge reload`:
...
- Changed keeper and CLI daemon compatibility checks so modern
  ...
  restart trigger. Saving `.cc-bridge/cc-bridge.config` no longer interrupts the mounted
  daemon; explicit `cc-bridge reload` or sidebar reload owns materializing the new
  panes/windows.
- Added the dynamic agent overlay command surface on top of the explicit reload
  transaction: `cc-bridge agent add ... --window/--window-class/--loop-id/--node-id`
  writes runtime lifecycle records without rewriting `.cc-bridge/cc-bridge.config`, then
...
- Added safe dynamic agent release on top of the idle `remove_agent` reload
  path: `cc-bridge agent remove ... --policy unload --idle-only` and
  `cc-bridge agent release ... --policy auto|hide|park|unload` retain busy agents,
...
- Added long-lived dynamic role parking semantics: `cc-bridge agent hide`,
  `cc-bridge agent park`, and `cc-bridge agent resume` now update dynamic lifecycle state;
...
- Verified the dispatch-disabled slice with `150 passed` across
  ...
  `test/test_v2_cc-bridge-daemon_dispatcher.py`; reload-related focused tests passed with
...
- Closed the startup-to-hotload pane identity bridge for compact configs and
  ...
  `@cc-bridge_window=main`, existing-window `add_agent`, new-window `add_window`,
...
- Added and verified batch dynamic lifecycle release coverage:
  `cc-bridge agent release --agents ...` can release a group of idle dynamic agents
  ...
  (`231e30b8`).
- Added and verified batch move placement by window class: `cc-bridge agent move
  ...
  `.cc-bridge/cc-bridge.config`, and the smoke flow proves source-window cleanup plus ask
  reachability after the move (`ec9d488d`).
- Added and verified execution-node batch move smoke: `cc-bridge agent move
...
- Added and verified mixed move-plus-add explicit `[windows]` reload: one
  `cc-bridge reload` transaction can create a new target window, move existing panes
...
- Extended live-provider evidence to explicit arrange/reflow recovery:
  ...
  fixed-column target geometry with `cc-bridge agent arrange`, preserve pane ids and
...
- Added the first bounded busy-unload bridge: non-dry-run `remove_agent` still
  ...
  `.cc-bridge/cc-bridge-daemon/reload-drain.json`, exposes drain diagnostics in the blocked
  ...
  `pytest -q test/test_cc-bridge-daemon_reload_drain.py test/test_cc-bridge-daemon_reload_apply.py test/test_v2_cc-bridge-daemon_dispatcher.py`
  (`78 passed`).
- Exposed active busy-unload drain status through `project_reload_config` /
  `cc-bridge reload --dry-run` / `cc-bridge reload`: reload payloads now include
  `reload_drains.active_records`, the CLI renders active drain count, agent,
  phase/status, bounded deadlines, and the explicit retry command (`cc-bridge
  reload`). Focused verification passed with
  `pytest -q test/test_cc-bridge-daemon_reload_drain.py test/test_cc-bridge-daemon_reload_apply.py test/test_v2_cli_render.py`
  (`76 passed`) and
  `pytest -q test/test_cc-bridge-daemon_reload_dry_run.py test/test_cc-bridge-daemon_reload_patch.py`
  (`42 passed`).
- Re-ran an external source-wrapper fake provider lifecycle smoke from
  `/home/bfly/yunwei/test_ccb2` using current source `cc-bridge_test`:
...
- Exposed active reload drain state in `project_view` for sidebar/orchestrator
  ...
  verification passed with `pytest -q test/test_cc-bridge-daemon_project_view.py`
  (`67 passed`) plus the reload/drain/dispatcher slice
  `pytest -q test/test_cc-bridge-daemon_reload_drain.py test/test_cc-bridge-daemon_reload_apply.py test/test_v2_cli_render.py test/test_v2_cc-bridge-daemon_dispatcher.py::test_dispatcher_rejects_targets_with_active_reload_drain`
  (`77 passed`).
- Added a fake-provider external smoke for the explicit `[windows]`
  ...
  project, submits a long-running fake job to `agent2`, edits `.cc-bridge/cc-bridge.config`
  to remove `agent2`, proves `cc-bridge reload` blocks and records an active drain,
  ...
  `cc-bridge reload`, and proves `agent2` disappears from project view. The smoke is
...
- Added daemon heartbeat auto-retry for ready active unload drains: after
  ...
  restores the agent in `.cc-bridge/cc-bridge.config` before auto-retry. Verification
  passed with `pytest -q test/test_cc-bridge-daemon_reload_drain_auto_retry.py
...
- Closed the first sidebar drain visibility gap: the Rust sidebar helper now
  ...
  --manifest-path tools/cc-bridge-agent-sidebar/Cargo.toml --check`,
  `cargo test --manifest-path tools/cc-bridge-agent-sidebar/Cargo.toml` (`73
...
- Added a repeatable true tmux sidebar pane capture to the busy-drain smoke:
  ...
  --manifest-path tools/cc-bridge-agent-sidebar/Cargo.toml` so the source wrapper
...
- Added the first replace-agent apply boundary: non-dry-run `replace_agent`
  ...
  passed with `pytest -q test/test_cc-bridge-daemon_reload_apply.py
  test/test_cc-bridge-daemon_reload_drain.py test/test_cc-bridge-daemon_reload_dry_run.py
  test/test_v2_cli_render.py` (`99 passed`) and `python -m py_compile
  lib/cc-bridge-daemon/reload_apply_plan.py lib/cli/render_runtime/reload_view.py`.
- Landed the first idle `replace_agent` mutation slice: pure existing-agent
  ...
  Verification passed with `PYTHONPATH=lib pytest -q test/test_cc-bridge-daemon_reload_apply.py
  test/test_cc-bridge-daemon_reload_patch.py test/test_cc-bridge-daemon_reload_drain.py
  test/test_cc-bridge-daemon_reload_dry_run.py test/test_cc-bridge-daemon_reload_runtime_mount.py
  test/test_cc-bridge-daemon_reload_transaction.py test/test_v2_cli_render.py
  ...
  /home/bfly/yunwei/test_ccb2 --cc-bridge-test
  /home/bfly/yunwei/cc-bridge_source/cc-bridge_test --provider fake --project-prefix
...
- Background file watching of `.cc-bridge/cc-bridge.config`.
- General `cc-bridge-daemon` control-plane performance optimization.
