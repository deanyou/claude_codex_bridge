...
Issue261 asks CC_BRIDGE to correct work that is already executing without creating a
...
same turn is still active. If the provider finishes between the last CC_BRIDGE check
...
`clientUserMessageId`, which can carry CC_BRIDGE's durable follow-up identity.
...
expected-active-turn request to CC_BRIDGE. Decision 002 therefore remains useful for
queued ordinary jobs but does not qualify pane input as active-job correction.

## Decision

`cc-bridge followup <job_id> --message <text>` addresses one exact running job. It
...
- `accepted`: CC_BRIDGE durably accepted the request and has not yet obtained a
  provider result.
...
- `too_late`: the addressed job was already terminal when CC_BRIDGE evaluated it.
- `terminal`: CC_BRIDGE accepted the record, but the provider's exact-turn
  precondition or a concurrent CC_BRIDGE terminal transition won before injection.
...
pending provider decision removes the active submission before CC_BRIDGE can inject.
...
| CC_BRIDGE fake adapter | supported for deterministic contract/restart tests | in-memory exact job/turn compare-and-record keyed by `followup_id` |
...
or matching active submission fails closed. CC_BRIDGE does not fall back to pane
...
mechanism, expected turn reference, and timestamps. `cc-bridge trace` accepts a
