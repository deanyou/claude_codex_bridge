...
3. Provider-writable marketplace, cache, plugin-data, and runtime state remains
   agent-local.
4. CC_BRIDGE replaces or removes only targets carrying a valid matching CC_BRIDGE ownership
...
| Provider | Official capability/storage evidence | Current CC_BRIDGE behavior | Result | Follow-up |
| :--- | :--- | :--- | :--- | :--- |
| Codex | Current plugin layouts include immutable bundle authority plus writable marketplace/cache state observed by PR257. | CC_BRIDGE isolates `CODEX_HOME`; merged PR269 keeps `.tmp/plugins` content-addressed and converts `.tmp/marketplaces` plus `plugins/cache` to marker-owned local seeds. | Fixed and merged in R1 (`06e1a46a`, merge `aed27abf`). | Retain rollback, missing-source, upgrade, and two-agent regressions. |
