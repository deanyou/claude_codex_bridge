# CC_BRIDGE Self Skill Draft Review And Test Evidence

Date: 2026-06-09

## Draft Paths

Production target, after review: `/home/bfly/yunwei/agent-roles-spec/roles/cc-bridge-self/`
or the equivalent accepted catalog path for `agentroles.cc-bridge_self`.

Current reviewable draft in this CC_BRIDGE plan:

- [drafts/agentroles.cc-bridge_self/role.toml](../drafts/agentroles.cc-bridge_self/role.toml)
- [drafts/agentroles.cc-bridge_self/memory.md](../drafts/agentroles.cc-bridge_self/memory.md)
- [cc-bridge-self-diagnose](../drafts/agentroles.cc-bridge_self/skills/codex/cc-bridge-self-diagnose/SKILL.md)
- [cc-bridge-self-recover](../drafts/agentroles.cc-bridge_self/skills/codex/cc-bridge-self-recover/SKILL.md)
- [cc-bridge-self-chain](../drafts/agentroles.cc-bridge_self/skills/codex/cc-bridge-self-chain/SKILL.md)
- [cc-bridge-config](../drafts/agentroles.cc-bridge_self/skills/codex/cc-bridge-config/SKILL.md)
- [config-contracts.md](../drafts/agentroles.cc-bridge_self/skills/codex/cc-bridge-config/references/config-contracts.md)
- [runtime-authority.md](../drafts/agentroles.cc-bridge_self/references/runtime-authority.md)
- [recovery-runbooks.md](../drafts/agentroles.cc-bridge_self/references/recovery-runbooks.md)
- [tmux-cc-bridge-quickstart.md](../drafts/agentroles.cc-bridge_self/references/tmux-cc-bridge-quickstart.md)
- [tools/doctor.py](../drafts/agentroles.cc-bridge_self/tools/doctor.py)
...
- Uses CC_BRIDGE control-plane diagnostics and read-only tmux evidence first.
...
- Provider/API recovery flow is exactly edit config -> `cc-bridge config validate` ->
  `cc-bridge reload --dry-run` -> `cc-bridge reload` -> re-check affected agents -> guarded
...
- Private built-in `agentroles.cc-bridge_self` skill, not a global inherited skill.
- Owns `.cc-bridge/cc-bridge.config` design/edit/validate/reload readiness.
- Runs `cc-bridge config validate` after every edit and `cc-bridge reload --dry-run` before
  reload.
- May run `cc-bridge reload` only after gates and materialization intent.
- Outputs affected agents for `cc-bridge-self-recover`; it does not run restart,
...
| --- | --- | --- | --- |
| coworker | All four skill drafts plus test plan | Completed; artifact `/home/bfly/yunwei/cc-bridge_source/.cc-bridge/cc-bridge-daemon/artifacts/text/completion-reply/job_d8a3fa6086ee-art_3ee15861074c44cc.txt` | Processed below |
| reviewer3 | All four skill drafts plus test plan | Completed; artifact `/home/bfly/yunwei/cc-bridge_source/.cc-bridge/cc-bridge-daemon/artifacts/text/completion-reply/job_7b7db3db986f-art_026de24551dd4813.txt` | Processed below |
...
- whether tests prove fast CC_BRIDGE/tmux/agent failure resolution or correctly mark
  current blockers.
...
- D2: removed source-checkout hardcoded `/home/bfly/yunwei/cc-bridge_source` and
  `/home/bfly/yunwei/test_ccb2` paths from distributable
  `cc-bridge-self-diagnose/SKILL.md`.
- D3/D5: added `cc-bridge ping cc-bridge-daemon` and `cc-bridge doctor logs <agent>` to diagnose.
- D4/R1: added `cc-bridge fault list` diagnostic evidence and guarded
  `cc-bridge fault clear <rule_id|all>` handling for known test residue.
- R2/R3/R4/R5: clarified the then-missing restart fallback, linked `cc-bridge clear`
  to pre-mutation gates, gated `cc-bridge roles update agentroles.cc-bridge_self`, and
  kept restart behind the documented command contract.
- C1/C2/C4: added `project_shutdown` handling, `cc-bridge cancel <job_id>` before
...
- CF1/CF4/CF5: added config-validate failure handling, explicit `[windows]`
  ...
  same-name `cc-bridge-config` skill.
- X1: replaced placeholder `tools/doctor.py` with a real read-only JSON helper
  that runs `ping cc-bridge-daemon`, `doctor`, `ps`, `queue --detail all`, `fault list`,
  `config validate`, and `reload --dry-run`.

Rejected or narrowed with evidence:

- D1: rejected as a blocker. `cc-bridge queue --detail all` was already validated in
...
- C3: rejected as a blocker. Parser and CLI help confirm
  ...
  `cc-bridge repair resubmit <message_id>`, and
  `cc-bridge repair ack <agent_name> [inbound_event_id]`.
- CF2/CF3: rejected for this draft. The private `cc-bridge-config/SKILL.md` already
  uses only `cc-bridge config validate`; there is no inline
...
- CF5 name-change suggestion: rejected for now because
  ...
  explicitly keeps the canonical name `cc-bridge-config`. The accepted mitigation is
...
- CR1: added the missing `cc-bridge cancel <job_id>` failure branch to
  `cc-bridge-self-chain/SKILL.md`. If cancel fails or reports a blocking state, the
...
- CG1: added an explicit dated `.cc-bridge/cc-bridge.config` pre-edit backup step and
  restricted rollback to the backup created for the current edit.

Validated after review:

- RD1: `cc-bridge_test doctor logs codexer` returned `logs_status: ok`, proving the
  `cc-bridge doctor logs <agent>` form works in the isolated test project.
- RR1: `cc-bridge_test fault clear all` returned `fault_status: cleared` with
  `cleared_count: 0`, proving the `cc-bridge fault clear <rule_id|all>` form works
  without active rules. A follow-up `cc-bridge_test fault list` still reported
  `rule_count: 0`.

Accepted follow-up risks:

- CG2 remains a release-blocking implementation requirement outside skill text:
  role projection must isolate the private same-name `cc-bridge-config` skill from
  inherited/global same-name skills before materialization.
- `cc-bridge restart <agent>` and catalog installation remain contract/platform
  blockers, not skill quality blockers.

## Test Environment

All source-runtime validation was run from `/home/bfly/yunwei/test_ccb2`, not
from `cc-bridge_source`:
...
- `cc-bridge_test doctor logs codexer`: `logs_status: ok`,
  `runtime_ref: tmux:%1`, `log_count: 4`.
- `cc-bridge_test fault list`: `fault_status: ok`, `rule_count: 0`.
- `CC_BRIDGE_BIN=/home/bfly/yunwei/cc-bridge_source/cc-bridge_test python3
  drafts/agentroles.cc-bridge_self/tools/doctor.py`: JSON `status: ok`,
...
```text
draft_static_check: ok role=agentroles.cc-bridge_self skills=4 tools=1
```

Draft doctor helper check:

```bash
python3 docs/plantree/plans/cc-bridge-self-role/drafts/agentroles.cc-bridge_self/tools/doctor.py
```
...
The helper emitted valid JSON and ran only read-only CC_BRIDGE control-plane commands.

`cc-bridge config validate` in isolated project:
...
`cc-bridge reload --dry-run` in isolated project:
...
`cc-bridge fault clear all` in the isolated project:
...
Follow-up `cc-bridge fault list` still reported `rule_count: 0`.
...
`cc-bridge roles show agentroles.cc-bridge_self`:

```text
roles_status: failed
error: unknown role: agentroles.cc-bridge_self
```
...
Interpretation: `cc-bridge restart <agent>` is implemented in the source CLI surface
...
1. Mounted daemon and pane evidence:
   - Commands: `cc-bridge_test doctor`, `cc-bridge_test ps`.
...
2. Queue/inbox evidence:
   - Commands: `cc-bridge_test queue --detail all`,
     `cc-bridge_test pend --inbox --detail codexer`.
...
3. Config drift check:
   - Commands: `cc-bridge_test config validate`, `cc-bridge_test reload --dry-run`.
   - Result: project config valid and reload plan is `no_change`. This proves
     the diagnose skill can route config drift to `cc-bridge-config` without mutation.
...
1. Provider/config recovery gate:
   ...
   - Expected skill behavior: only after a real edit plus user materialization
     intent may `cc-bridge-config` run `reload`; recover then rechecks affected agents.
2. Guarded restart contract:
   - Command: `cc-bridge_test restart codexer`.
   - Result: `restart_status: ok`; busy gate passed for an idle current-graph
     agent and old/new runtime evidence was reported.
   - Expected skill behavior: use `cc-bridge restart <agent>` only after
...
3. Busy/pending precheck:
   - Commands: `cc-bridge_test ps`, `cc-bridge_test queue --detail all`,
     `cc-bridge_test pend --inbox --detail codexer`.
...
1. Incomplete job retry candidate:
   - Command: `cc-bridge_test trace job_d5c0c10467fc`.
...
2. Artifact-backed reply with missing full file:
   - Command: `cc-bridge_test trace job_6fa5edd440cc`.
...
3. Repair command semantics:
   - Commands: `cc-bridge_test repair retry --help`,
     `cc-bridge_test repair resubmit --help`, `cc-bridge_test repair ack --help`.
...
1. Validation gate:
   - Command: `cc-bridge_test config validate`.
   - Result: project `.cc-bridge/cc-bridge.config` loaded as `project_config` and is valid.
   - Expected skill behavior: every edit must run this before reload discussion.
2. Reload dry-run gate:
   - Command: `cc-bridge_test reload --dry-run`.
...
3. Role package readiness:
   - Command: `cc-bridge_test roles show agentroles.cc-bridge_self`.
   - Result: `unknown role`.
   - Expected skill behavior: give `cc-bridge roles install agentroles.cc-bridge_self`
...
- Catalog fixture for `agentroles.cc-bridge_self` in the role package test suite.
- Role projection tests proving only `cc-bridge_self` receives the full private
