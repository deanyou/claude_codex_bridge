...
`cc-bridge_self` self-supervision should inspect the real CC_BRIDGE-owned pane view when
...
1. control-plane snapshot to identify target agent, active job, queue state,
   and expected pane;
2. read-only pane metadata from the current CC_BRIDGE tmux namespace;
...
5. bounded CC_BRIDGE-owned pane screenshot or equivalent visual artifact only when
   text capture cannot classify the state;
6. semantic assessment by `cc-bridge_self`;
7. schedule follow-up, report, or user escalation through CC_BRIDGE control-plane
   policy.
...
- `cc-bridge ps` could report a bound/alive pane;
...
- configured CC_BRIDGE agent panes;
- CC_BRIDGE sidebar panes;
...
- screenshot fallback for CC_BRIDGE-owned panes/windows when text is insufficient.
...
- whether captured text or screenshot may contain sensitive UI so `cc-bridge_self`
  should avoid quoting raw content.

## Observation Result

`cc-bridge_self` should return a compact structured result:
...
- `cc-bridge_pane_capture_text`: bounded `tmux capture-pane` text capture for a
  configured CC_BRIDGE target, with options for bottom/current screen and scrollback
  depth.
- `cc-bridge_pane_activity_sample`: compare pane text/metadata across a short
  interval.

Fallback tool surfaces:

- `cc-bridge_pane_screenshot`: capture a CC_BRIDGE-owned pane/window/sidebar/tool target
  to a CC_BRIDGE-owned artifact path when text is insufficient.
- `cc-bridge_visual_inspect`: summarize a screenshot artifact with OCR or vision
  when available.
...
tick. It should escalate to `cc-bridge_self` with a reason such as
...
2. heartbeat dispatches `ask --silence` to `cc-bridge_self` with bounded snapshot and
   target references;
3. `cc-bridge_self` uses sanctioned read-only pane observation tools, starting with
   `tmux capture-pane` style text capture;
4. `cc-bridge_self` uses screenshot fallback only if text evidence cannot classify
   the visible state;
5. `cc-bridge_self` schedules a follow-up or asks the user when the pane state is
   still ambiguous.

This keeps the heartbeat cheap and independent while letting `cc-bridge_self` inspect
what the agent actually sees.

## Safety Rules

- Capture only CC_BRIDGE-owned panes/windows/tool targets resolved from current CC_BRIDGE
  authority.
- Store capture artifacts under project-owned CC_BRIDGE artifact storage.
...
- `cc-bridge_self` has a documented pane-view self-supervision skill or runbook.
- Heartbeat assessor contract passes enough target references for `cc-bridge_self`
