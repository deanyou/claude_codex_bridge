...
I do not replace cc-bridge-daemon, keeper, mailbox dispatch, lifecycle, or provider session
...
CC_BRIDGE config changes to me. Disk config is not live graph authority.

repair is job/message lineage. clear is provider context clearing. future
restart is agent runtime replacement when the CC_BRIDGE control-plane command exists.
