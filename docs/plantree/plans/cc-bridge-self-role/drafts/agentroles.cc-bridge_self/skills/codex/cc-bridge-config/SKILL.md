---
name: cc-bridge-config
description: Private built-in CC_BRIDGE configuration skill for agentroles.cc-bridge_self. Design, edit, validate, and prepare reloads for .cc-bridge/cc-bridge.config, role bindings, providers, windows, workspaces, tool windows, sidebar, and provider startup inputs. Use only inside cc-bridge_self; non-self agents should delegate CC_BRIDGE config changes to cc-bridge_self.
---

# CC_BRIDGE Config

This is the private `agentroles.cc-bridge_self` built-in CC_BRIDGE configuration skill. It
owns `.cc-bridge/cc-bridge.config` design, editing, validation, reload readiness, and
...
The canonical skill name remains `cc-bridge-config`; role projection must keep this
...
- Edit project `.cc-bridge/cc-bridge.config`.
...
- Run `cc-bridge reload --dry-run` before reload materialization.
- Execute `cc-bridge reload` when validation passed, dry-run was reviewed, the plan
...
- Do not edit `.cc-bridge/cc-bridge_memory.md`, `.cc-bridge/agents/<agent>/memory.md`, provider
  homes, installed role stores, generated memory, lifecycle, lease, runtime,
  mailbox, provider session, or tmux state.
- Do not execute `cc-bridge restart`, `cc-bridge kill`, `cc-bridge clear`, or `cc-bridge repair` from
  this skill.
...
1. Resolve config source and target. Project config `.cc-bridge/cc-bridge.config` is the
   normal target; user config `~/.cc-bridge/cc-bridge.config` is out of scope unless the
...
4. Before editing an existing project config, create one dated pre-edit backup
   ...
   `cp .cc-bridge/cc-bridge.config .cc-bridge/cc-bridge.config.bak.$(date +%s)`. Restore only from the
...
10. Execute `cc-bridge reload` only when gates pass and materialization intent is
   explicit.
...
Reason: ...
Suggested next skill: cc-bridge-self-recover
```

## Role Binding

Use canonical Role Pack ids such as `agentroles.archi` and
`agentroles.cc-bridge_self`. The project-local agent name remains the ask target.
...
`cc-bridge config validate` style warnings as cleanup work before reload.

If validation reports a missing installed role, tell the user to install it:

```bash
cc-bridge roles install agentroles.cc-bridge_self
```

Do not copy role memory or skills into `.cc-bridge` manually.
...
- affected agents
- blocked runtime actions for `cc-bridge-self-recover`
