---
name: cc-bridge-comm-reply-recover
description: Diagnose and recover CC_BRIDGE communication and reply delivery stalls. Use when a user reports a missing CC_BRIDGE_REPLY, stuck ask, agent stuck busy/delivering, queued work behind an active job, cancelled/incomplete reply, empty artifact, callback not continuing, duplicate retry after success, or a CC_BRIDGE mailbox/communication backend that appears stuck.
---

# CC_BRIDGE Comm Reply Recover
...
Mutations must go through CC_BRIDGE control-plane commands. Do not directly edit
...
1. Identify the target from the user's evidence:
   ...
   - `cc-bridge queue --detail all` when neither id nor agent is clear.
...
4. Cross-check runtime evidence when a job is `running` or `delivering` longer
   ...
   - `cc-bridge doctor logs <agent>` when provider/pane failure is plausible.
   - read-only tmux pane capture only from the socket and pane id reported by
     `cc-bridge ps`, and only as evidence.
...
- `provider_pane_stale`: CC_BRIDGE reports the runtime healthy, but pane text/logs
...
- If trace shows the blocking job is still in flight, run
  `cc-bridge cancel <job_id>` first when the user supplied maintenance intent. If
  cancel fails or reports a blocker, stop.
- Prefer `cc-bridge repair retry <job_id|attempt_id>` when the same work should run
  again and the original lineage remains valid.
- Prefer `cc-bridge repair resubmit <message_id>` when the old execution lineage is
  stale, context-corrupted, semantically wrong, or no longer suitable.
- Use `cc-bridge repair ack <agent_name> [inbound_event_id]` only when the reply is
...
- Hand off to `cc-bridge-self-recover` for `cc-bridge restart <agent>` only after chain
...
- `cc-bridge trace <old_job_or_message>` proves the old path is terminal, completed,
  or intentionally cancelled.
- `cc-bridge queue --detail <agent>` and `cc-bridge pend --inbox --detail <agent>` show no
...
death, but `cc-bridge ps` still showed a bound pane. The correct repair was:
...
- Do not submit a second concurrent path for the same work while the original
  ...
  task.
- Do not treat observer snapshots as terminal authority; use `cc-bridge trace`.
- Do not infer from artifact preview text when the full artifact is required.
- Do not mutate tmux directly or write CC_BRIDGE runtime authority files.
