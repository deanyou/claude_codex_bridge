---
name: cc-bridge-self-chain
description: Diagnose and repair CC_BRIDGE ask/job/message/reply/artifact/callback lineage. Use for missing replies, incomplete artifacts, pending callbacks, retry/resubmit/ack decisions, reply delivery problems, or work-chain resume advice.
---

# CC_BRIDGE Self Chain
...
2. Run `cc-bridge trace <id>` and record message, attempt, reply, event, callback,
   and job state.
...
5. Classify the repair:
   ...
   - ack: acknowledgement or inbox progress is wrong, but the reply is
     otherwise accepted.
6. If trace shows a job still running or in flight, run `cc-bridge cancel <job_id>`
...
- Handoff to `cc-bridge-self-recover` only when trace proves process/context/pane
  replacement is required.
- Handoff to `cc-bridge-self-diagnose` when the target id is ambiguous or runtime
...
- Do not run `cc-bridge restart`, `cc-bridge clear`, or `cc-bridge reload` as the first response
  to a lineage problem.
- Do not treat queue/inbox observer snapshots as terminal authority for
  completed lineage; use `cc-bridge trace <id>`.
