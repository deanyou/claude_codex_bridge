# CC_BRIDGE Self Role Plan

Date: 2026-06-09

## Purpose

Plan the `agentroles.cc-bridge_self` Role Pack: a CC_BRIDGE-specific self-maintenance and
expert agent that helps users and other agents understand CC_BRIDGE architecture,
use CC_BRIDGE features, diagnose CC_BRIDGE runtime state, repair tmux/provider/message
faults, and keep CC_BRIDGE knowledge current as new features land.

`cc-bridge_self` is an auxiliary role. It does not own product or coding task
...
- [topics/skills-and-tools.md](topics/skills-and-tools.md): `cc-bridge_self`
...
- [topics/cc-bridge-expert-knowledge-role.md](topics/cc-bridge-expert-knowledge-role.md):
  next-stage design for making `cc-bridge_self` the project-local CC_BRIDGE expert across
...
- [topics/pane-view-self-supervision.md](topics/pane-view-self-supervision.md):
  self-supervision direction that uses real CC_BRIDGE-owned pane text view, especially
...
- [decisions/001-auxiliary-self-agent.md](decisions/001-auxiliary-self-agent.md):
  decision that `cc-bridge_self` is a failure-contained auxiliary agent, not runtime
  authority.
- [decisions/002-built-in-cc-bridge-config-skill.md](decisions/002-built-in-cc-bridge-config-skill.md):
  decision to make `cc-bridge-config` a built-in `agentroles.cc-bridge_self` skill instead
  of shipping it as a universally inherited skill.
- [decisions/003-bounded-autonomy.md](decisions/003-bounded-autonomy.md):
  decision to give `cc-bridge_self` stronger bounded autonomy for maintenance tasks.
- [decisions/004-default-recommended-install.md](decisions/004-default-recommended-install.md):
  decision to install or refresh `agentroles.cc-bridge_self` by default, include
  `cc-bridge_self` in the built-in blank-project default, and keep existing project
  config binding explicit.
- [decisions/005-expert-knowledge-database.md](decisions/005-expert-knowledge-database.md):
  decision to make `cc-bridge_self` a CC_BRIDGE expert through compact memory routing,
...
- [decisions/006-future-modification-guardrails.md](decisions/006-future-modification-guardrails.md):
  decision that future `cc-bridge_self` changes must preserve the canonical
  `agentroles.cc-bridge_self` id and keep maintenance heartbeat disabled unless
  manually enabled.
- [decisions/007-unified-diagnose-entrypoint.md](decisions/007-unified-diagnose-entrypoint.md):
  decision to expose one `cc-bridge_diagnose <agentname>` workflow that diagnoses,
  performs bounded recovery, verifies, and requests authorization before
  submitting a redacted GitHub issue.
- [drafts/agentroles.cc-bridge_self/](drafts/agentroles.cc-bridge_self/): reviewable draft
  Role Pack payload for `agentroles.cc-bridge_self`; production content should move
  to the role catalog or an accepted local role source after review.
- [../../../manuals/cc-bridge-self-expert-guide.md](../../../manuals/cc-bridge-self-expert-guide.md):
  Markdown expert guide for `cc-bridge_self` covering CC_BRIDGE architecture, command
...
- [../../../cc-bridge-daemon-lifecycle-stability-plan.md](../../../cc-bridge-daemon-lifecycle-stability-plan.md)
...
- [../../../cc-bridge-daemon-pane-recovery-continuous-attach-plan.md](../../../cc-bridge-daemon-pane-recovery-continuous-attach-plan.md)
...
- Role identity, memory, skills, and reference docs for `agentroles.cc-bridge_self`.
- CC_BRIDGE expert knowledge support: source architecture navigation, command and
...
- Private CC_BRIDGE configuration ownership through the built-in `cc-bridge-config` skill:
...
- Controlled use of existing CC_BRIDGE commands such as `doctor`, `ps`, `logs`,
...
- Autonomous low-risk and gated maintenance actions when the user asks
  `cc-bridge_self` to diagnose, fix, recover, maintain, or apply a CC_BRIDGE config change.
- Read-only MCP tools for CC_BRIDGE/tmux runtime snapshots and lineage tracing.
- Read-only screen evidence tools, including text capture first and bounded
  screenshot capture for CC_BRIDGE-owned panes/windows when visual state matters,
  especially when text capture cannot classify ambiguous agent progress.
- Mutating MCP tools only when they call CC_BRIDGE control-plane commands and satisfy
  the bounded-autonomy policy.
- User-facing tmux quickstart reference for CC_BRIDGE-managed sessions.

Out of scope:

- Making `cc-bridge_self` mandatory for daemon startup or agent supervision.
- Letting `cc-bridge_self` replace `cc-bridge-daemon` as lifecycle authority.
- Letting `cc-bridge_self` complete the original coding/product task after another
  agent failed.
- Treating `cc-bridge_self` as a general product engineer for non-CC_BRIDGE business
  features.
- Duplicating the entire CC_BRIDGE source tree or long implementation docs into role
  memory instead of using compact expert references and source-backed lookup.
- Keeping CC_BRIDGE config design/edit skills universally inherited by every agent.
- Using the built-in `cc-bridge-config` skill to execute `cc-bridge reload` without
  validation, dry-run review, and maintenance intent.
- Using the built-in `cc-bridge-config` skill to execute restart or kill; those
...
- Raw destructive tmux operations exposed to the role, including `kill-pane`,
  ...
  pane creation.
- Arbitrary desktop screenshots or screenshots outside the current CC_BRIDGE project
  namespace.
- Reading provider secrets or auth material.
- Treating `.cc-bridge/agents/*`, tmux pane facts, provider session files, or pid
  files as authority.

## Naming

- Stable role id: `agentroles.cc-bridge_self`
- Default project-local agent name: `cc-bridge_self`
...
name `cc-bridge_self` unless the user explicitly binds the role to another name.
