...
Several narrow maintenance skills would expose CC_BRIDGE internals to users and
...
2. classify the likely stall or failure and select the least disruptive
   supported repair;
3. execute only bounded CC_BRIDGE control-plane actions for that agent when the
...
The internal implementation may reuse `cc-bridge-self-diagnose`,
`cc-bridge-self-chain`, `cc-bridge-comm-reply-recover`, and `cc-bridge-self-recover`; those are
implementation modules, not additional user-facing choices.

## Agent Normality Contract

The distilled command must preserve `cc-bridge_self`'s existing evidence model. A
successful CLI invocation is not proof that the target agent is healthy.

For `cc-bridge_diagnose <agentname>`, evaluate in this order:
...
1. resolve the pane id and tmux socket from the current runtime record and
   verify that the pane belongs to the target CC_BRIDGE slot;
...
issue. Raw pane text may remain only in the local CC_BRIDGE-owned diagnostic artifact
when the storage contract permits it. If text is blank or cannot distinguish a
layout/visual failure, use the bounded CC_BRIDGE-owned screenshot fallback; never
...
The result should map to the existing `cc-bridge_self` report shape:
...
`drafts/agentroles.cc-bridge_self/tools/doctor.py` is therefore only a raw evidence
...
- `cc-bridge_diagnose <agentname>` targets exactly one agent.
