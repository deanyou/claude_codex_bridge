...
This note records the current `cc-bridge-daemon` communication-state and retry behavior so
...
`project_view` builds Comms rows in `lib/cc-bridge-daemon/project_view/service.py`:
...
- CC_BRIDGE messages are delivered by simulating provider user input, so a manual turn
  and a CC_BRIDGE-managed turn should share the same provider-native activity path.
- A CC_BRIDGE job can be `running` while the provider has already failed, returned to
  idle, or is waiting for user action.
- A provider can be actively working with no CC_BRIDGE job when the user typed
...
CC_BRIDGE job/message/attempt records remain useful metadata for lineage, retry,
