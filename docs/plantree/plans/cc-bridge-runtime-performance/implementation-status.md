...
`cc-bridge_test --print-version` boundary performs no startup transaction
...
- Earlier resource/harness unit matrix: `50 passed` across
  `test_perf_process_resources.py` and `test_perf_cc-bridge_startup.py`; lifecycle
...
- Isolated source-runtime project:
  ...
  windows, 10 Codex-stub agents, isolated `HOME` / `CC_BRIDGE_SOURCE_HOME`, and the
...
- Worker1 smoke checks from `/home/bfly/yunwei/test_ccb2`:
  `/tmp/cc-bridge_runtime_profile_startup_diagnose_scoped.json` and
  `/tmp/cc-bridge_runtime_profile_load_sleep_scoped.json`.
...
- Main focus fast-path review:
  `PYTHONPATH=lib python -m pytest -q
  test/test_cc-bridge-daemon_project_focus.py test/test_sidebar_click.py` passed with
  `15 passed`.
...
- Project_view dirty-state regression:
  `PYTHONPATH=lib python -m pytest -q
  test/test_cc-bridge-daemon_project_view.py test/test_cc-bridge-daemon_service_graph.py` passed with
...
- Project_view pending-refresh blocker fix:
  `PYTHONPATH=lib python -m pytest -q
  test/test_cc-bridge-daemon_project_focus.py test/test_sidebar_click.py
  test/test_cc-bridge-daemon_project_view.py test/test_cc-bridge-daemon_service_graph.py` passed with
  `81 passed`.
...
- Sidebar single-RPC working-tree slice:
  ...
  test/test_cc-bridge-daemon_socket_client.py test/test_cc-bridge-daemon_service_graph.py` passed with
...
- Source wrapper smoke after runtime helper change:
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test --diagnose` and
  `cc-bridge_test config validate` passed from `/home/bfly/yunwei/test_ccb2`.
- Shell/system bucket split:
  ...
  High-load artifact: `/tmp/cc-bridge_runtime_shellsplit_profile_v2.json`.
  Startup artifact: `/tmp/cc-bridge_runtime_shellsplit_startup_profile.json`.
- Worker report artifact:
  `.cc-bridge/cc-bridge-daemon/artifacts/text/completion-reply/job_21a7c0c0b62a-art_19c8d2c809734472.txt`
...
- Do not use the source checkout as a live runtime directory and do not mutate
  its installed-release `.cc-bridge` runtime state during source validation.
