...
- Captured a real lifecycle CPU profile from an isolated source runtime under
  `/home/bfly/yunwei/test_ccb2` using `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test`.
  Evidence:
...
- Established the current optimization priority: shell/tmux/subprocess
  orchestration first, then provider lifecycle policy, then CC_BRIDGE core only if it
...
- Added a working-tree interactive-latency slice for sidebar clicks:
  `cc-bridge __sidebar-click` can now focus through one daemon RPC
...
- Added a source-wrapper/Python bootstrap trace and attributed `94.664%` of the
  ...
  measured as `211.349 ms` from `cc_bridge.py` entry through eager imports.
...
- Closed the S0 CLI-only hot-path slice with an explicit no-startup-transaction
  contract.  The measured command is exactly `cc-bridge_test --print-version`; it
...
- Full CC_BRIDGE core rewrite or broad Rust migration.
