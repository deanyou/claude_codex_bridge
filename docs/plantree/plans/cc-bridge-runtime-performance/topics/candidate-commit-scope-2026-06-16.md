...
Include the source-runtime-safe defaults, project-cwd `cc-bridge_test` invocation,
...
- `test/test_cc-bridge-daemon_project_focus.py`
...
- `lib/cc-bridge-daemon/metrics.py`
...
- `docs/plantree/plans/cc-bridge-runtime-performance/`
...
- Production Rust/helper entry points that need a separate risk/benefit gate:
  - `lib/jobs/store.py` helper paths behind `CC_BRIDGE_RUST_JSONL_STORE`,
    `CC_BRIDGE_RUST_PROJECT_VIEW_RECENT_JOBS`, and `CC_BRIDGE_RUST_JOB_SUMMARY_TAIL`
  - `lib/storage/jsonl_store.py` helper path behind `CC_BRIDGE_RUST_JSONL_STORE`
  - `lib/cc-bridge-daemon/project_view/service.py` helper parser path behind
...
- `PYTHONPATH=lib python -m pytest -q test/test_perf_runtime_lifecycle_profile.py test/test_cli_runtime_launch_tmux_panes.py test/test_v2_runtime_launch.py test/test_cc-bridge-daemon_project_focus.py test/test_sidebar_click.py test/test_cc-bridge-daemon_project_view.py test/test_cc-bridge-daemon_service_graph.py`
