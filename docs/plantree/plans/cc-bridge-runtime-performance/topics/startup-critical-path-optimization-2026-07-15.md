...
Domain: `cc-bridge start`, `cc-bridge-daemon` startup, managed provider mount, tmux namespace
Read when: changing startup reuse, provider preparation, topology discovery, or
startup readiness
Related:
[startup supervision contract](../../../../cc-bridge-daemon-startup-supervision-contract.md),
[configuration layout contract](../../../../cc-bridge-config-layout-contract.md),
[baseline runtime flows](../../../baseline/runtime-flows.md)

## Objective

Reduce a reported warm `cc-bridge` startup from more than 20 seconds to a predictable
...
`243.986 ms`, dominated by `211.349 ms` from `cc_bridge.py` entry through eager
...
- `main` is tmux window `@0` and contains `main` and `cc-bridge_self`.
...
a pane whose `@cc-bridge_window` or tmux window name matches the configured logical
...
- `lib/cc-bridge-daemon/start_flow_runtime/service.py`
- `lib/cc-bridge-daemon/start_preparation.py`
- `lib/cc-bridge-daemon/start_runtime/binding_runtime/common.py`
- `lib/cc-bridge-daemon/services/project_namespace_pane.py`
...
- Pane recovery and `cc-bridge kill` semantics remain unchanged.

Source-runtime validation must use `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test` from
`/home/bfly/yunwei/test_ccb2` with isolated `HOME` and `CC_BRIDGE_SOURCE_HOME` unless
