...
without changing provider semantics or weakening CC_BRIDGE daemon ownership rules.
...
4. Continue Rust/helper optimization only for measured CC_BRIDGE-local hot paths.

## Startup Optimization

Current evidence: startup CPU is dominated by `shell-system` at 56.0%, followed
by provider processes at 24.1%, while CC_BRIDGE core is 16.5%.
...
- Startup cumulative CPU split by CC_BRIDGE core, provider, tmux, shell wrappers, and
  terminal frontend.
...
CC_BRIDGE core is 17.3% and provider is 9.3% in the Codex-targeted test.
...
## When To Optimize CC_BRIDGE Core Further

Reopen broad CC_BRIDGE-core optimization only when a refined profile shows one of:

- CC_BRIDGE core stays above 25% of sampled CPU after shell/tmux/provider policy
  reductions.
- A specific CC_BRIDGE-local path appears in click-to-focus or ask latency p95.
