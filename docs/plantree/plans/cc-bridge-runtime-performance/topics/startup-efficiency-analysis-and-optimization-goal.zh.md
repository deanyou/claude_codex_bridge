...
Authority: subordinate to CC_BRIDGE startup, lifecycle, provider-session, and
diagnostics contracts

Domain: `cc-bridge` CLI、keeper、`cc-bridge-daemon`、项目 tmux namespace、Agent runtime、
...
- [CC_BRIDGE_DAEMON Startup And Supervision Contract](../../../../cc-bridge-daemon-startup-supervision-contract.md)
- [CC_BRIDGE_DAEMON Lifecycle Stability Plan](../../../../cc-bridge-daemon-lifecycle-stability-plan.md)
...
建立一套可重复、可解释、可回退的 CC_BRIDGE 启动效率分析与优化程序，在不降低
生命周期、Provider session、Agent 通讯和持久化权威正确性的前提下：

1. 把 CLI、keeper、`cc-bridge-daemon`、tmux、Provider 和前台 UI 的启动耗时拆到可归因
   的阶段及 Agent 子阶段。
...
1. `docs/cc-bridge-daemon-startup-supervision-contract.md` 及 Provider session isolation
...
`cc-bridge_test --print-version`：wrapper `exec` 后只覆盖 Python/import/早期 CLI
...
生灭的瞬时进程。无子进程边界由 `cc-bridge_test` 的 `execvpe`、CLI version 分支在
...
| S1 | Warm attach | keeper、`cc-bridge-daemon`、namespace、全部 Agent runtime 健康且可复用 |
| S2 | Daemon restart reuse | 新 `cc-bridge-daemon` generation，现有 namespace/Provider 可安全复用 |
...
S4/S5 只能在一次性外部测试项目中执行。不得通过删除 `.cc-bridge`、直接 kill tmux、
...
| 时间点 | 含义 |
| :--- | :--- |
| `T0_cli_entry` | `cc_bridge.py` 入口；更早的 wrapper/Python bootstrap 单独计量 |
...
- 一个 `.cc-bridge` anchor 仍只能有一个 keeper、一个 authoritative `cc-bridge-daemon`
  generation 和一个项目 namespace。
- `.cc-bridge/cc-bridge.config` 仍定义完整 desired-agent set；`requested_agents` 只能决定
...
- 源码验证只使用 `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test`，并从
  `/home/bfly/yunwei/test_ccb2` 下的外部项目运行。
- 除非测试明确验证继承环境，否则设置隔离的 `HOME` 和 `CC_BRIDGE_SOURCE_HOME`。
- 不使用 installed `cc-bridge` 验证当前源码，不把 `cc-bridge_source` 当 source runtime
  project，不设置 `CC_BRIDGE_SOURCE_RUNTIME_OK=1` 绕过隔离。
- 真冷启动只使用正式 `cc-bridge_test kill`/测试控制面路径；不直接删除 `.cc-bridge` 或
...
- 新增独立启动 benchmark harness，建议入口为
  `dev_tools/perf_cc-bridge_startup.py`；不得把 lifecycle CPU sampler 的采样点误当
  成重复启动。
- 每一轮从外部测量 CLI wall time，并在下一轮覆盖前立即复制、解析和校验
  `.cc-bridge/cc-bridge-daemon/startup-report.json`；报告的 trigger、generated time、config
...
- 校验绝对 `cc-bridge_test` 的 realpath/source sha、允许测试根、隔离 HOME；检测到
  `CC_BRIDGE_SOURCE_RUNTIME_OK` 时拒绝运行。
- 非交互 latency run 强制 `CC_BRIDGE_NO_ATTACH=1`。冷启动仅调用正式 `cc-bridge_test kill`
...
```bash
python /abs/source/dev_tools/perf_cc-bridge_startup.py \
  --project-root /home/bfly/yunwei/test_ccb2/startup-perf-<uuid> \
  --cc-bridge-test /abs/source/cc-bridge_test \
...
- `cc-bridge kill`、reload、generation replacement 必须能取消或接管 background
...
- 回退不要求删除 `.cc-bridge`、清 Provider history、重建 worktree 或降级 state schema。
...
- `test/test_v2_cc-bridge-daemon_start_matrix.py`
- `test/test_cc-bridge-daemon_start_preparation.py`
- `test/test_cc-bridge-daemon_start_agent_runtime.py`
- `test/test_cc-bridge-daemon_start_binding.py`
- `test/test_cc-bridge-daemon_startup_pane_snapshot.py`
- `test/test_v2_runtime_launch.py`
- `test/test_v2_runtime_launch_session_files.py`
- `test/test_cli_runtime_launch_tmux_panes.py`
- `test/test_cc-bridge-daemon_supervisor_lifecycle.py`
- `test/test_cc-bridge-daemon_supervisor_namespace.py`
- `test/test_cc-bridge-daemon_tmux_namespace.py`
- `test/test_v2_cc-bridge-daemon_keeper.py`
- `test/test_v2_daemon_startup_wait.py`
- `test/test_v2_start_service.py`
- `test/test_v2_start_foreground.py`
- `test/test_v2_cli_kill.py`
- `test/test_cc-bridge_restart.py`
- `test/test_perf_cc-bridge_startup.py`
...
- `test/test_v2_cc-bridge-daemon_dispatcher.py`
...
env -u CC_BRIDGE_SOURCE_RUNTIME_OK \
...
HOME=/home/bfly/yunwei/test_ccb2/source_home \
CC_BRIDGE_SOURCE_HOME=/home/bfly/yunwei/test_ccb2/source_home \
/home/bfly/yunwei/cc-bridge_source/cc-bridge_test config validate --json
```

启动 benchmark 统一使用 `dev_tools/perf_cc-bridge_startup.py`。临时 shell 循环、旧
