...
- `lib/cc-bridge-daemon/services/project_namespace_pane.py` gives authoritative topology
  matching an explicit `require_alive` policy;
- `lib/cc-bridge-daemon/services/project_namespace_runtime/materialize_topology.py` includes
...
- `lib/cc-bridge-daemon/services/project_namespace_runtime/ensure.py` retains those exact
...
`dev_tools/perf_cc-bridge_startup.py` now implements `mixed-recovery` as a serial,
...
- the measured constructor invokes official single-Agent `cc-bridge_test restart`,
...
external fixture below, isolated `HOME`/`CC_BRIDGE_SOURCE_HOME`, deterministic Codex
