...
- runtime wrapper: `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test`
...
recognize `/path/cc-bridge ask` as `ask-cli-subprocess`, and report top commands per
bucket.
...
Artifact: `/tmp/cc-bridge_runtime_shellsplit_profile_v2.json`

Workload:

- Project: `/home/bfly/yunwei/test_ccb2/cc-bridge_perf_hotload`
- Load: 80 `cc-bridge ask` submissions, concurrency 16, target `agent_codex`
...
| `cc-bridge/cc-bridge-daemon/main` | 30.6% | 16.88% | 1 |
| `provider/opencode` | 2.4% | 1.30% | 1 |
| `provider/codex` | 1.6% | 0.90% | 3 |
| `other-system` | 1.6% | 0.90% | 13 |
| `tmux-server` | 0.7% | 0.40% | 1 |

Interpretation: high-load submission CPU is dominated by repeated `cc-bridge ask`
Python subprocess creation plus daemon-side job ingestion. Tmux is not the main
CPU owner in this workload.

## Startup

Artifact: `/tmp/cc-bridge_runtime_shellsplit_startup_profile.json`

Workload:

- Project: `/home/bfly/yunwei/test_ccb2/cc-bridge_perf_shellsplit_startup_20260616`
- Config copied from the four-provider `cc-bridge_perf_hotload` project.
- Startup sampled for up to 12s; the new test project was cleaned with
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test kill` after profiling.
...
- High-load first target: reduce one-Python-process-per-ask overhead through a
  persistent or batched submission path into `cc-bridge-daemon`.
