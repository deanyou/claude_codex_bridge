...
- warm priming invoked official `cc-bridge_test kill` before it captured `before`, so
...
`dev_tools/perf_cc-bridge_startup.py` now treats scenario construction as a
...
- S5a: no cc-bridge-daemon or Agent runtime state, empty isolated source home, clean process
...
`test/test_cc-bridge-daemon_reload_apply.py`; there is no new startup, scenario,
diagnostics, readiness, resource, provider, or cleanup failure.

## Retained External Evidence

All stateful validation used the absolute source `cc-bridge_test` wrapper from
external owner-marked projects, isolated `HOME`/`CC_BRIDGE_SOURCE_HOME`, the Codex
...
- before/ready proved no cc-bridge-daemon directory, no Agent directory, zero runtime
