# CC_BRIDGE Runtime Performance Plan

Date: 2026-07-17

## Purpose

Track startup, high-load, and interactive-latency performance work for CC_BRIDGE
across the full lifecycle. This plan is broader than the Rust helper plan: it
attributes cost across CC_BRIDGE core, shell/tmux orchestration, provider processes,
...
- Provider mount, idle, and lifecycle policies when they dominate CPU or
  memory.
- Shell/tmux subprocess and polling overhead inside CC_BRIDGE orchestration.
- Cross-reference to Rust helper evidence where it changes a measured hot path.

Out of scope for the first phase:

- A full rewrite of `cc-bridge-daemon` or the Python control plane.
...
`cc-bridge_test --print-version` command creates no startup transaction or RPC,
...
The first real lifecycle profile shows CC_BRIDGE core is not the dominant CPU cost:

- Startup: CC_BRIDGE core `16.5%`, provider `24.1%`, shell/tmux/system `56.0%`.
- High load: CC_BRIDGE core `17.3%`, provider `9.3%`, shell/tmux/system `72.6%`.

This points first to shell/tmux/subprocess overhead, provider lifecycle policy,
and interactive refresh isolation rather than broad CC_BRIDGE-core rewrites.
