# CC_BRIDGE Manuals Roadmap

Date: 2026-06-10

## Done

- Created a dedicated plan root for producing the CC_BRIDGE developer and user
  manuals.
...
- Added an initial source inventory for the communication chapter covering CLI
  ask parsing/submission, cc-bridge-daemon submit handling, dispatcher submission, message
...
- Added a Markdown `cc-bridge_self` expert guide at
  `docs/manuals/cc-bridge-self-expert-guide.md`, focused on role mission, authority
...
4. Optional role-pack integration pass: decide whether the Markdown
   `cc-bridge_self` guide should be copied or summarized into the distributable
   `agentroles.cc-bridge_self` Role Pack after role asset review.
...
- Markdown guide checks found no trailing whitespace in
  `docs/manuals/cc-bridge-self-expert-guide.md` and the manuals root index.
