...
- The central communication path is:
  CLI ask parsing -> `MessageEnvelope` -> cc-bridge-daemon submit handler -> dispatcher
...
- `.cc-bridge/cc-bridge-daemon/artifacts/text/completion-reply/job_8906713f6caf-art_4cc0a0677a114bcb.txt`
- SHA256:
  `72fcb60cb8cced7db8447abec8ec1ea9e392630f3ec2b5c7b8e5978f3712288b`

Accepted findings and changes:

- Fixed `cc-bridge maintenance schedule`: it is now documented as
  `cc-bridge maintenance schedule --after <duration> [--reason TEXT]`, not as a
  read-only schedule view.
- Documented `cc-bridge maintenance tick --force` and
  `cc-bridge maintenance tick --no-dispatch`.
- Documented that `cc-bridge maintenance enable` and
  `cc-bridge maintenance disable` currently return a config-authority hint; users
...
- Strengthened `cc-bridge clear` and `cc-bridge restart` safety-boundary descriptions.
...
## CC_BRIDGE Self Expert Markdown Guide
...
- Provide a Markdown operating manual for `agentroles.cc-bridge_self` so the role can
  act as a CC_BRIDGE architecture, usage, config, communication, diagnosis, and
  bounded recovery expert.

Source inputs:

- `docs/plantree/plans/cc-bridge-self-role/README.md`
- `docs/plantree/plans/cc-bridge-self-role/topics/cc-bridge-expert-knowledge-role.md`
- `docs/plantree/plans/cc-bridge-self-role/topics/recovery-runbooks.md`
- `docs/plantree/plans/cc-bridge-manuals/topics/communication-source-inventory.md`
- `docs/plantree/plans/cc-bridge-manuals/topics/command-config-inventory.md`
- existing developer and user manual source trees under `docs/manuals/`

Accepted content:

- Role boundary: `cc-bridge_self` is a CC_BRIDGE expert and maintenance assistant, not
  `cc-bridge-daemon`, keeper, lifecycle authority, or business-task owner.
...
- Runtime safety: source validation for this repository stays outside
  `cc-bridge_source` and uses the dedicated `cc-bridge_test` wrapper from
...
- Recovery guidance: the guide gives bounded playbooks for missing replies,
  ...
  drift, and `cc-bridge_self` itself being broken.

Verification:

- Markdown source was checked for trailing whitespace.
- `wc -l docs/manuals/cc-bridge-self-expert-guide.md` reported 1052 lines.
