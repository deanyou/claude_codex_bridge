...
2. Source code: current files under `lib/`, `config/`, `cc-bridge`, `cc-bridge_test`,
...
3. Generated analysis: `.architec/*`, current `.hippos/*`, legacy
   ...
   work.
4. Runtime/CLI evidence: command help output, focused `cc-bridge_test` validation
...
- cc-bridge-daemon request handlers.
...
- set isolated `HOME` and `CC_BRIDGE_SOURCE_HOME` unless intentionally testing real
  inherited provider config;
- do not run source runtime commands from the `cc-bridge_source` checkout itself.
