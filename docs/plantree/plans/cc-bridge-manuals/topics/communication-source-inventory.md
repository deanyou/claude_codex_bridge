...
- `message_with_reply_guidance` preserves its compatibility name but leaves
  ...
  `CC_BRIDGE_REPLY_MODE` line unless explicit output requirements are detected.
- request bodies can be artifact-backed explicitly or automatically spilled
  when larger than 4 KiB.
- the CLI submits a `MessageEnvelope` through the mounted cc-bridge-daemon client.
...
- plain nested asks from an active CC_BRIDGE task are rejected unless the child result
...
- on child completion, CC_BRIDGE records the child reply without delivering it to the
...
- `watch` polls `client.watch(target, cursor=...)` with defaults
  `CC_BRIDGE_WATCH_TIMEOUT_S=10.0` and `CC_BRIDGE_WATCH_POLL_INTERVAL_S=0.1`, yielding
...
- cc-bridge-daemon handlers validate the minimal payload fields and delegate to dispatcher
...
- `lib/cc-bridge-daemon/services/dispatcher_runtime/lifecycle.py`
- `lib/cc-bridge-daemon/services/dispatcher_runtime/cancellation.py`
- `lib/cc-bridge-daemon/services/dispatcher_runtime/finalization_retry.py`
- `lib/cc-bridge-daemon/services/dispatcher_runtime/finalization_retry_runtime/`
...
- `lib/cc-bridge-daemon/services/dispatcher_runtime/finalization_runtime/artifacts.py`
- `lib/cc-bridge-daemon/services/dispatcher_runtime/reply_delivery.py`
- `lib/cc-bridge-daemon/services/dispatcher_runtime/reply_delivery_runtime/`
- `lib/cc-bridge-daemon/services/dispatcher_runtime/artifact_maintenance.py`
...
- runtime recovery path in `lib/cc-bridge-daemon/services/dispatcher_runtime/comms_recover.py`.
