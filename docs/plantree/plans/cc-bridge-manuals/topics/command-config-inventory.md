...
- `roles add` locates the nearest project `.cc-bridge` anchor and calls
...
- The answer to "how an Archi role loads into CC_BRIDGE" belongs in both manuals:
...
- The managed tool stores state under XDG data/state/cache roots in
  `cc-bridge/tools/workbench`.
- `CC_BRIDGE_WORKBENCH_PROFILE`, `CC_BRIDGE_WORKBENCH_FORCE_RICH`,
  `CC_BRIDGE_WORKBENCH_THEME`, and `CC_BRIDGE_RICH_AUTO_START`
...
- Project config: `.cc-bridge/cc-bridge.config`.
...
- Legacy rich TOML that repeats matching `provider` or default workspace mode
  still loads, but `cc-bridge config validate` reports style warnings.
...
- `maintenance.heartbeat` supports `enabled`, `assessor`, `interval_s`,
  ...
  `startup_ensure`.
- Defaults are disabled heartbeat, assessor `cc-bridge_self`, `interval_s = 3600`,
