...
Design a CC_BRIDGE-native terminal workbench that becomes the first visible tmux
...
The workbench is a client of `cc-bridge-daemon` and the scripted workflow state machine. It
must not become an Agent, a workflow runner, a second task-state authority, or
a replacement for `.cc-bridge/cc-bridge.config`.

## Current Source Findings

- `tools/cc-bridge-agent-sidebar` already ships a Rust `ratatui`/`crossterm` client,
...
- Config V3 currently compiles resident `frontdesk` and `planner` into
  `cc-bridge-user` and `cc-bridge-plan`, and fixes `entry_window` to `cc-bridge-user`.
- `cc-bridge-daemon` already supports `from_actor=user` submission, job watch/cancel, and
...
- One managed `client` window named `cc-bridge-workbench` at topology order zero.
...
- Making `cc-bridge config` a workflow dashboard.
...
- [003-standalone-ratatui-client-and-cc-bridge-daemon-authority.md](decisions/003-standalone-ratatui-client-and-cc-bridge-daemon-authority.md)
...
- [CC_BRIDGE config and layout contract](../../../cc-bridge-config-layout-contract.md)
