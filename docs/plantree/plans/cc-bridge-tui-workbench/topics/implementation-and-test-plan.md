...
```text
tools/cc-bridge-workbench-tui/
bin/cc-bridge-workbench-tui
CLI entry: cc-bridge tui
```

Do not add the conversation/workflow application to the existing
`tools/cc-bridge-agent-sidebar/src/tui.rs`; that file already owns a compact sidebar
...
The existing `cc-bridge-workbench` name belongs to the optional Rich/Yazi/LazyVim
...
- cc-bridge-daemon service graph, handlers, socket client endpoints, and API models;
...
- new Ratatui reducer, screens, widgets, input modes, and cc-bridge-daemon client;
- Rust workspace/build scripts and release artifact assembly;
- install/update smoke and generated executable links;
- CLI parser/router for `cc-bridge tui` and managed launch arguments.
...
| Reload/recovery | TUI restart, cc-bridge-daemon restart, missing window recovery, no task cancellation, no duplicate submission. |
...
`/home/bfly/yunwei/test_ccb2` and `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test`:
...
8. Restart the TUI and cc-bridge-daemon separately; confirm state restoration and no
...
- Rollback disables the compiled client surface and restores `cc-bridge-user` as the
