...
```text
0  cc-bridge-workbench  kind=client  entry=true   sidebar=false
1  cc-bridge-user       kind=agents  frontdesk
2  cc-bridge-plan       kind=agents  planner
...
workbench is a built-in, versioned CC_BRIDGE client with a constrained socket command
...
window name, order, and entry selection. When disabled, current `cc-bridge-user` and
`cc-bridge-plan` compilation remains unchanged.
...
- `entry_window` resolves to `cc-bridge-workbench` for the enabled workflow profile;
...
- `cc-bridge-daemon` remains alive independently of the TUI process.
