# CC_BRIDGE TUI Workbench Roadmap
...
- Frozen the first-window, two-region interaction, scoped clarification, and
  cc-bridge-daemon-client authority decisions.
...
- Prove that the projection does not require the client to scan `.cc-bridge/runtime`
...
- Add a separate Rust binary behind `cc-bridge tui`; do not reuse the existing
  `cc-bridge-workbench` rich-tool binary name.
- Add the managed `client` topology kind and Config V3 workbench opt-in.
- Materialize `cc-bridge-workbench` first, without a duplicate Sidebar; shift
  `cc-bridge-user`, `cc-bridge-plan`, and dynamic Agent windows after it.
...
Gate: queue and cancellation survive cc-bridge-daemon/TUI restart, are idempotent, and do
...
- Run final visible opened-project acceptance from
  `/home/bfly/yunwei/test_ccb2` with the source `cc-bridge_test` wrapper and the
