...
- OpenCode 1.16.2 natively discovers project `AGENTS.md` while also loading
  configured `instructions`; CC_BRIDGE therefore excludes project `AGENTS.md` from
...
1. Audit OpenCode project `AGENTS.md` loading. Completed for OpenCode 1.16.2.
   ...
   - Determine whether project `AGENTS.md` is loaded by OpenCode itself,
     loaded only through `.cc-bridge/runtime/memory/<agent>.md`, or both.
...
2. Classify Codex source-home `AGENTS.md`. Classified as provider user memory.
   - Confirm whether `source_home / "AGENTS.md"` is true account memory, old
     CC_BRIDGE install output, or normally absent.
...
3. Inventory released install markers and legacy sections. Initial inventory
   ...
   - Decide how to treat Claude route-mode
     `~/.claude/rules/cc-bridge-config.md`: stop writing it, classify it as native
     rules residue, or provide a migration/removal path.
4. Decide old `.cc-bridge/cc-bridge_memory.md` seed migration. Completed with a
...
- `docs/claude-session-isolation-contract.md`
  ...
  - State that inherited provider user memory is filtered for CC_BRIDGE install
    residue.
...
- `docs/cc-bridge-project-shared-memory-plan.md`
...
- Keep `lib/project_memory/renderer.py` focused on rendering selected sources
  and the single CC_BRIDGE runtime coordination section.

Exit criteria:

- Policy decides source inclusion by provider and source kind.
- Filters never run on `.cc-bridge/cc-bridge_memory.md`, agent-private memory, or
...
- New `.cc-bridge/cc-bridge_memory.md` files do not duplicate ask protocol text.
- Existing edited `.cc-bridge/cc-bridge_memory.md` files are not overwritten.
- Existing unedited seeded `.cc-bridge/cc-bridge_memory.md` files upgrade to the current
  template.
- Rendered bundles still contain CC_BRIDGE ask protocol exactly once from the
  renderer.
...
- `test/test_project_memory_real_context.py`
  - Build a realistic temporary CC_BRIDGE project shape and run the real Claude,
...
- `test/test_provider_memory_external_context.py`
  ...
  - Default skip unless `CC_BRIDGE_REAL_PROJECT_MEMORY_CHECK=1` is set.
...
- Use `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test` only from the dedicated external
  source-test project `/home/bfly/yunwei/test_ccb2`, unless another external
  project is explicitly allowed with `CC_BRIDGE_TEST_ROOTS` or
...
- Do not run source runtime validation from `cc-bridge_source` itself.

Exit criteria:

- Unit tests covering source policy, filtering, and provider projection pass.
- The realistic temporary project fixture passes.
- The external project context check passes after `cc-bridge_test` has regenerated the
  managed provider memory files from the source-under-test runtime.
- External `cc-bridge_test` runtime validation confirms generated memory size and
...
- Template migration must never overwrite user-edited `.cc-bridge/cc-bridge_memory.md`, so
