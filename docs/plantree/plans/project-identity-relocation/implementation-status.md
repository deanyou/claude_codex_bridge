...
- Added durable `.cc-bridge/project.identity.json` identity and stable slug records.
...
- Reconciled stale lifecycle and lease identity before running intent and
  keeper startup.
- Preserved project identity through `cc-bridge -n`.
...
- `python -m pytest -q test/test_project_id.py
  ...
  test/test_cc-bridge-daemon_runtime_identity.py test/test_v2_cc-bridge-daemon_keeper.py
  test/test_cc-bridge-daemon_startup_fence.py test/test_v2_cc-bridge-daemon_start_flow.py
...
- External source-runtime validation from `/home/bfly/yunwei/test_ccb2` with
  ...
  - final `cc-bridge_test kill` reported unmounted state and a dead backend PID.
