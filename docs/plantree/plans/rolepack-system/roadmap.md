...
- Accepted CC_BRIDGE role-id shorthand and role-id ask alias semantics in
  [decisions/004-role-id-shorthand-resolves-to-agent-name.md](decisions/004-role-id-shorthand-resolves-to-agent-name.md).
- Chose `agent-roles-spec` as the role catalog authority; CC_BRIDGE consumes the
...
- Added the first source-tree `roles/cc-bridge.archi` package, role manifest parsing,
  system-store install, `cc-bridge roles list/show/install/add/doctor`, config
  `role` parsing, project role locks, role memory inclusion, and Codex/Claude
  role skill projection.
- Implemented CC_BRIDGE role-id shorthand in config loading and role-id ask alias
...
- Added `agent-roles-spec` catalog discovery with production `roles/`
  ...
  catalog digest status, and no production fallback to CC_BRIDGE source-tree role
  packages.
- Changed `cc-bridge update` role handling so it updates already installed catalog
...
- Added runtime stale-lock protection: if an installed role's `current`
  version or digest no longer matches a project's `.cc-bridge/role-lock.json`, CC_BRIDGE
...
- Added a legacy compatibility alias from `cc-bridge.archi` to `agentroles.archi`
...
- Removed source-tree `roles/cc-bridge.archi` content and excluded the source-tree
  `roles/` directory from release artifacts; production role content is now
  expected from `agent-roles-spec`.
- Added GitHub-backed `agent-roles-spec` catalog fallback through a CC_BRIDGE-owned
  user cache under `$XDG_CACHE_HOME/cc-bridge/role-catalogs/agent-roles-spec`, with
...
- Added user-level system role sources at `~/.cc-bridge/roles` and `~/.roles`.
  These local editable role sources take precedence over the remote catalog,
  are visible in `cc-bridge roles list`, and can be snapshotted into the installed
  store when added to a project. Project-level `.roles` is deferred.
- Added explicit `cc-bridge roles sync [path]` for local source edits. With no path,
...
- Completed the production `agentroles.archi` PR in `agent-roles-spec` as
  GitHub PR #1, with production role content removed from `cc-bridge_source`.
...
- Made duplicate catalog diagnostics visible in `cc-bridge roles list`, including
...
- Completed real `/home/bfly/yunwei/test_ccb2` validation for catalog list,
  ...
  path/default behavior, provider memory/skill projection, `cc-bridge` startup,
  `cc-bridge reload`, and runtime doctor. See
...
- Accepted the long-term boundary that `agent-roles-spec` should own `.roles`
  package management while CC_BRIDGE wraps those operations for project/runtime
  integration. See
...
- Added the first executable `agent-roles` package-manager slice in
  ...
  `cc-bridge.archi -> agentroles.archi` alias.
- Added the CC_BRIDGE compatibility bridge: `cc-bridge roles install/update/sync` delegates
...
- Added automatic legacy installed-role migration from
  `$XDG_DATA_HOME/cc-bridge/roles` into `.roles/installed` at CC_BRIDGE role-management
  boundaries. The legacy store is migration input only; `.roles/installed` is
  the single runtime read path.
- Removed the CC_BRIDGE-private installed role writer and
  `CC_BRIDGE_AGENT_ROLES_MANAGER` rollback switch so role payload writes have one
  owner.
- Documented and tested multiple project-local agents bound to the same role
  ...
  `cc-bridge roles add <role-id>:<provider> --agent <name>` creates explicit
...
1. Add provider fresh-start implementation behind `cc-bridge restart <agent>` for
   sessions whose launch role digest differs from installed current.
2. Add a doctor/cleanup command for legacy `.cc-bridge/role-lock.json` residue.
3. Move legacy store migration ownership into `agent-roles` once the package
   manager exposes a stable migration command/API.
4. Decide whether CC_BRIDGE should keep calling `agent-roles` through subprocess JSON
...
- Automatic background update checks outside explicit `cc-bridge update`.
