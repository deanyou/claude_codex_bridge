...
```text
source = "systemroles:/home/user/.cc-bridge/roles/<role>"
...
CC_BRIDGE source-tree role directories are not a production distribution source.
They may exist temporarily as migration fixtures, but role package content
belongs in `agent-roles-spec`.

## Agent Roles Catalog

`agent-roles-spec` is the first remote catalog authority. CC_BRIDGE should discover
user-level system role sources first, then environment/default local
`agent-roles-spec` paths, then a CC_BRIDGE-owned GitHub cache at
...
roles to list available roles, install selected roles into the local CC_BRIDGE role
...
runtime authority. A role added from `~/.cc-bridge/roles` or `~/.roles` is first
...
The CC_BRIDGE-owned cache is not a role authoring workspace. CC_BRIDGE may create it with
...
into `agent-roles-spec` itself. In that model, CC_BRIDGE keeps the same user-facing
role commands but delegates role package sync/install/update/doctor operations
to the spec-owned package manager, then applies CC_BRIDGE project locks and provider
projection. See
...
During `cc-bridge update`, CC_BRIDGE should refresh the catalog first. It should update
...
- Role updates must not change project locks without an explicit project update
  or `cc-bridge roles update --apply-lock`.
...
CC_BRIDGE PRs should not add production role package content. They should add or
change CC_BRIDGE adapter code, catalog consumption, install/update behavior,
