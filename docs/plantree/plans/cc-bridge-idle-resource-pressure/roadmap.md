...
- Removed callback-ledger idle read amplification (2026-08-01): callback
  ...
  about 26.8 seconds to about 0.25 ms and sampled idle `cc-bridge-daemon` at 0.4% CPU with
  no physical reads.
- Diagnosed and bounded a production pane-crash write storm (2026-07-30):
  one 70.6-hour `cc-bridge-daemon` had written about 71.9 GB while two provider panes
...
- Implemented and corrected the Codex diagnostic SQLite policy in the working
  ...
  are backed up, CC_BRIDGE no longer pre-creates Codex-owned SQLite schema, trigger
...
- Source runtime smoke under
  ...
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test config validate`, `cc-bridge_test -s`, and
  `cc-bridge_test doctor` mounted one Codex agent without `table logs already exists`;
  `logs_2.sqlite` pointed to temp storage, Codex migration records existed, the
  CC_BRIDGE insert-block trigger was installed, and `cc-bridge_test kill -f` stopped the
  smoke runtime.
...
- Review and harden PR234, now rebased cleanly onto `origin/main` `v7.6.19`.
  ...
  optional Rust runtime accelerator, Codex idle polling reduction, cc-bridge-daemon/keeper/
...
- Package/CI hardening for the runtime accelerator has been added during review:
  release artifacts build and ship `bin/cc-bridge-runtime-accelerator`, CI tests the
...
2. Finish PR234 full regression gates after the packaging and callback-hot-work
   fixes: targeted provider/accelerator tests, cc-bridge-daemon idle tests, Rust
...
- `cc-bridge kill`, `cc-bridge -n`, project restart, and callback continuation retain their
  current semantics.
