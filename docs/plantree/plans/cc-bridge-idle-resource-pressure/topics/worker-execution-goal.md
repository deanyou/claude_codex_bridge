...
Plan, land, and verify the remaining CC_BRIDGE idle resource pressure reductions in
...
- Source runtime validation must use `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test`
  from `/home/bfly/yunwei/test_ccb2`, not the installed `cc-bridge` command.
...
- Add observable counters or debug evidence when behavior becomes less frequent
  or less visible.
- Keep compatibility with existing startup, shutdown, `cc-bridge kill`, `cc-bridge -n`,
...
- Runtime-state root resolver with `/run/user/$UID/cc-bridge`, `/dev/shm/cc-bridge-$USER`,
  and project `.cc-bridge` fallback.
- A project `.cc-bridge` ref file or diagnostic path that keeps runtime-root
  discoverable.
...
- Integration/source-runtime test for startup, project restart, `cc-bridge kill`, and
...
```text
Goal: Execute the next phase of docs/plantree/plans/cc-bridge-idle-resource-pressure,
following topics/worker-execution-goal.md as the authority for scope,
landing discipline, and tests.

Read first:
- docs/plantree/plans/cc-bridge-idle-resource-pressure/README.md
- docs/plantree/plans/cc-bridge-idle-resource-pressure/roadmap.md
- docs/plantree/plans/cc-bridge-idle-resource-pressure/topics/idle-resource-pressure-solution.md
- docs/plantree/plans/cc-bridge-idle-resource-pressure/topics/worker-execution-goal.md

Deliver:
- A scoped implementation for the assigned phase only.
- Focused automated tests.
- Source runtime validation from /home/bfly/yunwei/test_ccb2 with
  /home/bfly/yunwei/cc-bridge_source/cc-bridge_test when daemon/provider/ask behavior
  changes.
- Plan-tree updates recording landed evidence, remaining risks, and next steps.

Constraints:
- Do not use installed/global cc-bridge for source validation.
- Do not break ask submit, queueing, callback continuation, reply delivery,
  cancellation, resubmit, project restart, cc-bridge -n, or cc-bridge kill.
