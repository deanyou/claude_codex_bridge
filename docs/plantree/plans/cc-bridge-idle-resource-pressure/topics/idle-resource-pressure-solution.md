...
- Socket request handling stays hot. A project may reduce maintenance work while
  idle, but the `cc-bridge-daemon` socket server must keep accepting submit/get/ps/cancel
...
```text
CC_BRIDGE_IDLE_HEARTBEAT_INTERVAL_S=15
CC_BRIDGE_DEEP_IDLE_HEARTBEAT_INTERVAL_S=120
CC_BRIDGE_HEARTBEAT_PERSIST_MAX_S=60
```
...
- `cc-bridge-daemon/services/registry.py`
- `cc-bridge-daemon/services/health_monitor_runtime/*`
- `cc-bridge-daemon/services/dispatcher_runtime/runtime_state.py`
...
- `cc-bridge-daemon/app_runtime/lifecycle.py`
- `cc-bridge-daemon/socket_server_runtime/loop.py`
- `cc-bridge-daemon/services/dispatcher.py`
- `cc-bridge-daemon/services/health.py`
- `cc-bridge-daemon/supervision/loop.py`

### 4. Provider Runtime Tmpfs Split

Current issue:

- Provider runtime residue is mixed with project `.cc-bridge` state. Many files are
...
```text
CC_BRIDGE_RUNTIME_STATE_HOME=/run/user/$UID/cc-bridge
fallback: /dev/shm/cc-bridge-$USER
fallback: project .cc-bridge when runtime root unavailable
```

The project `.cc-bridge` can keep a small ref file pointing to the runtime root.
...
```text
CC_BRIDGE_CODEX_RUNTIME_IDLE_TIMEOUT_S=600
CC_BRIDGE_PROVIDER_SUSPEND_ENABLED=0
CC_BRIDGE_PROVIDER_SUSPEND_AFTER_S=900
CC_BRIDGE_PROVIDER_WAKE_TIMEOUT_S=60
```
...
cc-bridge cleanup --dry-run
cc-bridge cleanup --provider-state --older-than 30d
```
...
- The `logs_2.sqlite` diagnostic-log filter is a pressure mitigation, not a
  ...
  rebuildable diagnostic DB to a temp path. CC_BRIDGE must not create Codex-owned
  SQLite schema in that target; Codex owns its migration path.
- Trigger installation is a post-migration retry. CC_BRIDGE installs the insert-block
...
- The filter must be reversible: when `CC_BRIDGE_CODEX_DIAGNOSTIC_LOGS` is enabled,
  CC_BRIDGE removes the symlink redirect, restores the backed-up managed DB when one
...
- Keep project `.cc-bridge` refs for discoverability.
...
```bash
ps -eo pid,cmd | rg 'cc-bridge-daemon|provider_backends|/codex '
cat /proc/<pid>/io
find <project>/.cc-bridge -type f -mmin -2 -printf '%T@ %s %p\n'
du -h -d 5 <project>/.cc-bridge | sort -h | tail
```
...
- Regression test for `cc-bridge kill`, `cc-bridge -n`, callback continuation, and
  project restart.
