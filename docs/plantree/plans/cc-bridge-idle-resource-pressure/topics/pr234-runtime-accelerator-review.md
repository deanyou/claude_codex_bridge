...
2. Rust runtime accelerator:
   optional `cc-bridge-runtime-accelerator` Unix-socket sidecar for Codex active-job
...
4. cc-bridge-daemon idle write/churn reduction:
...
- Claude queue-operation callback anchors are parsed as prompt records, while
  forwarded body-only `CC_BRIDGE_REQ_ID` text is ignored.
- Claude send prompt now clears non-empty stale input at the visible prompt line
  before pasting the CC_BRIDGE request.
...
- Runtime accelerator socket placement now falls back to a short runtime socket
  root when a project path makes `<project>/.cc-bridge/runtime-accelerator/accelerator.sock`
...
| cc-bridge-daemon idle heartbeat/write churn | selected `test/test_v2_cc-bridge-daemon_socket.py`, `test/test_v2_cc-bridge-daemon_mount_ownership.py`, `test/test_v2_cc-bridge-daemon_keeper.py`, `test/test_cc-bridge-daemon_registry.py` |
| ProjectView idle cache freshness | `test/test_cc-bridge-daemon_project_view.py` |
...
| Rust sidecar | `cargo fmt --check -p cc-bridge-runtime-accelerator`, `cargo test -p cc-bridge-runtime-accelerator -- --test-threads=1`, and `cargo build -p cc-bridge-runtime-accelerator` from `rust/` |

## Evidence So Far

- Provider/accelerator focused suite: `54 passed`.
- cc-bridge-daemon idle/project-view/registry/keeper suite: `173 passed`.
...
- Final broadened regression:
  ...
  `test_codex_comm_io.py`, `test_v2_cc-bridge-daemon_socket.py`,
  `test_v2_cc-bridge-daemon_mount_ownership.py`, `test_v2_cc-bridge-daemon_keeper.py`,
  `test_cc-bridge-daemon_project_view.py`, `test_cc-bridge-daemon_registry.py`,
  `test_build_linux_release_script.py`, `test_install_script_sidebar.py`, and
  `test_v2_phase2_entrypoint.py::test_cc-bridge_long_running_job_keeps_heartbeat_and_doctor_healthy`:
  `279 passed`.
- Rust sidecar: `cargo fmt --check -p cc-bridge-runtime-accelerator`,
  `cargo test -p cc-bridge-runtime-accelerator -- --test-threads=1`, and
  `cargo build --release -p cc-bridge-runtime-accelerator` passed.
- Linux release preview: `scripts/build_linux_release.py --allow-dirty`
  produced `cc-bridge-linux-x86_64.tar.gz`; the tarball contains
  `bin/cc-bridge-runtime-accelerator`, and the unpacked binary returned a project
  `.cc-bridge/runtime-accelerator/accelerator.sock` path.
...
- Real `/home/bfly/yunwei/test_ccb2` source-runtime pressure test:
  ...
  `cc-bridge_test`, real provider home. Initial isolated-home run correctly exposed
...
- After socket fallback fix, the sidecar started at
  `/run/user/1000/cc-bridge-runtime/accelerator-3f3c486ccb4b2dfd.sock` and
  `cc-bridge-runtime-accelerator ping` returned status `ok`.
...
- Post-pressure doctor summary: `cc-bridge-daemon_health: healthy`,
  `cc-bridge-daemon_active_execution_count: 0`, `cc-bridge-daemon_pending_items_count: 0`,
  `cc-bridge-daemon_terminal_pending_count: 0`, and mailbox consistency `ok` for all 3
...
- Final Rust sidecar gate after the socket fallback fix:
  ...
  `cargo test -p cc-bridge-runtime-accelerator -- --test-threads=1`, and
  `cargo build --release -p cc-bridge-runtime-accelerator` passed with 9 Rust tests.
- Final release preview after the socket fallback fix:
  `scripts/build_linux_release.py --allow-dirty` produced the Linux tarball,
  included `bin/cc-bridge-runtime-accelerator`, and the unpacked binary returned a
...
- Extended real source-runtime integration on 2026-06-27:
  - Codex soak project
    ...
    Codex agents, PR234 `cc-bridge_test`, real provider home, and the Rust sidecar at
    `/run/user/1000/cc-bridge-runtime/accelerator-9d6cbed77176aff5.sock`.
  ...
  - After every soak round, `cc-bridge-runtime-accelerator ping` returned `ok`;
    `cc-bridge-daemon_health` stayed `healthy`; active execution, pending queue, and
  ...
  - Mixed provider project
    ...
    doctor after cancellation showed `cc-bridge-daemon_health: healthy`, active execution
