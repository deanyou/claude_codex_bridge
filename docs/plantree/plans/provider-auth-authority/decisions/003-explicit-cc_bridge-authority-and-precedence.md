# Explicit CC_BRIDGE Authority And Precedence
...
`.cc-bridge/cc-bridge.config`. Mixing that explicit authority with inherited external login
or API state creates ambiguous routing and can reintroduce external side
effects.

## Decision

Explicit Agent auth/API/route configuration is CC_BRIDGE-local authority and has
highest precedence in each authority dimension it owns. It applies only inside
CC_BRIDGE-managed process environment and Provider state.
