...
CC_BRIDGE should benefit from a user's existing Provider login and API configuration,
...
input. CC_BRIDGE may observe it, copy capability-qualified static state, or use a
...
that boundary, CC_BRIDGE leaves the Agent unauthenticated and requests explicit CC_BRIDGE
...
- CC_BRIDGE may report that an external account is logged in without reusing its
  mutable credential.
