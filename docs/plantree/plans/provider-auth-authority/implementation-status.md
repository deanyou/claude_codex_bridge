...
CC_BRIDGE v8.5.6 source commit
...
and npm package `@seemseam/cc-bridge@8.5.6`.
Explicit CC_BRIDGE
...
The source boundary is one-way, and a change of authority increments CC_BRIDGE's
...
- Issue #319 macOS Claude OAuth re-login repair was published in CC_BRIDGE `v8.6.10`
  ...
  `@seemseam/cc-bridge@8.6.10`. On stopped restart, CC_BRIDGE compares the inherited source
  payload with the prior CC_BRIDGE-owned `.credentials.json` projection and updates
...
- `cc-bridge restart <agent>` no longer executes persisted `session.start_cmd`.
...
- Session records now carry a stable `cc-bridge_conversation_id`, authority
...
- The v8.5.6 source is installed locally in source/dev mode. The global
  commands report CC_BRIDGE `8.5.6` from the source line containing implementation
...
- `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test --diagnose` passed from the external
...
- External source-runtime project
  ...
  generation 1 to 2 without changing `cc-bridge_conversation_id`, retained both
...
- Implementation commit `8b35d868f402e5f68929782a6c2df657a8750d21`
  ...
  installation reported CC_BRIDGE `8.5.6` and `codex-reconnect 0.3.5` from the
  external test root.
- Release Artifacts run `31060954687` and npm OIDC run `31060954935` completed
  ...
  archives, and the signed CC_BRIDGE Mobile APK/manifest.
