...
2. Should CC_BRIDGE add `key_env`, `token_env`, or owner-only file references so API
...
3. When explicit CC_BRIDGE authority is removed, should the next stopped launch
...
4. What is the supported meaning of `provider_profile.home`? Current code may
   ...
   all writable runtime state under CC_BRIDGE-owned `provider-state`; the alternative
...
7. Can visible and headless processes for one Agent share the Provider's native
   cross-process refresh lock safely, or must CC_BRIDGE serialize them behind one
   credential writer?
...
11. Should CC_BRIDGE provide a stopped-Agent maintenance command that launches a
   login-only process with private roots, or require an operator-controlled
   shell command documented per Provider?
12. How should CC_BRIDGE locally clear an Agent-owned credential when the Provider's
...
13. At upgrade, should existing cloned rotating OAuth credentials be blocked at
    ...
    enforcement?
14. How should CC_BRIDGE identify likely duplicate credential authority without
...
16. What should `cc-bridge restart` do for a legacy Provider session that has only a
...
17. After the old writer is quiesced, should a source-probe or projection
    failure retain the dead pane for diagnosis or replace it with a bounded CC_BRIDGE
...
20. When a visible Provider process is alive but its writer lease cannot be
    proven after daemon recovery, should CC_BRIDGE terminate it immediately or park
...
21. For each Provider and exact CLI version, which account/API/route changes
    ...
    session linked to the stable CC_BRIDGE conversation?
...
24. How should a crash-time continuation preserve an in-flight turn that the
    Provider may have accepted but CC_BRIDGE did not commit? Duplicate submission and
