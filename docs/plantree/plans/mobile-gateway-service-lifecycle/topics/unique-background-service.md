...
- `lib/cli/management_runtime/commands_runtime/update.py`
  - `cc-bridge update mobile` calls `run_mobile_update_onboarding()`.
...
- `lib/cli/services/mobile_update.py`
  - prints Tailscale/mobile setup guidance and a suggested
    `cc-bridge mobile serve --listen 127.0.0.1:8787 ...` command.
...
- `lib/cli/management_runtime/commands_runtime/install.py`
  - `cc-bridge install mobile` calls `prepare_server_mobile_gateway()` and then
...
- On 2026-07-01 the local blocker was a development gateway:
  `/tmp/cc-bridge-mobile-real-project-emulator/notifications-testccb2/gateway_runner.py`.
...
`cc-bridge update mobile` should become the primary host-owned startup/update command:

1. Acquire a host mobile service lock.
2. Load the current mobile service state.
3. If a prior CC_BRIDGE-managed mobile gateway is alive, stop it and wait for the
   loopback port to release.
4. If the state is stale, clear it.
5. If the requested loopback port is owned by a non-CC_BRIDGE process, fail with a
   clear diagnostic and do not kill it.
6. Spawn one background CC_BRIDGE-managed mobile gateway.
7. Wait until the gateway responds to `/v1/health`.
8. Write fresh service state and print the active gateway/pairing summary.

The command should be repeatable. A second `cc-bridge update mobile` should refresh
...
- `service.json`
  - CC_BRIDGE-owned service authority, not a user config file.
- `service.log`
...
{
  "schema_version": 1,
  "record_type": "cc-bridge_mobile_host_service",
  "pid": 12345,
  ...
  "state_dir": "/home/user/.local/state/cc-bridge/mobile",
  "started_at": "2026-07-01T00:00:00Z",
  "command_kind": "cc-bridge_mobile_host_serve",
  "entrypoint": "/home/user/.local/bin/cc-bridge"
}
```
...
- pid exists;
- process command line contains the CC_BRIDGE internal mobile-host serve command;
...
```bash
cc-bridge __mobile-host-serve \
  --listen 127.0.0.1:8787 \
  --public-url https://host.tailnet.ts.net:8787 \
  --route-provider tailnet \
  --state-dir ~/.local/state/cc-bridge/mobile \
  --generation 7 \
  --host-id host-...
```
...
- `cc-bridge mobile serve`
  ...
  - if it fails with address-in-use, it should point users to
    `cc-bridge update mobile` for the managed host service.

## Port And Route Boundaries

CC_BRIDGE owns only the loopback gateway process.
...
- If the loopback port is occupied by an external process, CC_BRIDGE reports the pid
...
4. CLI integration:
   - wire `cc-bridge update mobile`;
   - route `cc-bridge install mobile` through the same manager;
...
- logged-in `cc-bridge update mobile` onboarding injects a managed service refresh
  instead of only printing the foreground `cc-bridge mobile serve` instruction;
...
- from `/home/bfly/yunwei/test_ccb2`, run source `cc-bridge_test update mobile`
...
- Should `cc-bridge update mobile` always create a new pairing code, or should it
  optionally keep the current service running when configuration is unchanged?
- Should there be a public `cc-bridge mobile status` or `cc-bridge mobile stop` command in
