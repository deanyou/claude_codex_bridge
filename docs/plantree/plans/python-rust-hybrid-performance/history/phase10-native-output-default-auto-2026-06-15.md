...
- Preserved explicit controls:
  - `CC_BRIDGE_RUST_NATIVE_OUTPUT=0` forces Python fallback.
  - `CC_BRIDGE_RUST_NATIVE_OUTPUT=1|auto` attempts Rust and falls back to Python.
  - `CC_BRIDGE_RUST_NATIVE_OUTPUT=required` raises on helper missing/crash/bad
    payload.
  - `CC_BRIDGE_RUST_HELPERS=0` still disables default-auto helper attempts when no
