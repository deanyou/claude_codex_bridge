...
- Rust helper skeleton: `tools/cc-bridge-rs-helper/`
...
- Helper binary name: `cc-bridge-rs-helper`
- Schema version: `1`
...
- Helpers are disabled by default because an unset `CC_BRIDGE_RUST_HELPERS` falls
  back without discovery.
- `CC_BRIDGE_RUST_HELPERS=0` forces Python-only fallback.
- `CC_BRIDGE_RUST_HELPERS=auto` and `CC_BRIDGE_RUST_HELPERS=1` may attempt discovery, but
...
- No ProjectView, JSONL, storage, provider parsing, process cleanup, startup,
  or `cc-bridge-daemon` runtime path was wired to the helper.
