# CC_BRIDGE Config Skill Scope

Date: 2026-06-06

## Goal

Make the `cc-bridge-config` skill a precise `.cc-bridge/cc-bridge.config` assistant. It should
...
- read the current `.cc-bridge/cc-bridge.config` when present;
...
- remind the user to use `cc-bridge reload --dry-run`, `cc-bridge reload`, or a restart as
  appropriate.

The skill should not:

- edit `.cc-bridge/cc-bridge_memory.md` or `.cc-bridge/agents/<agent>/memory.md` during ordinary
  config work;
...
- write runtime state, provider-state homes, installed role stores, or generated
  memory files;
- run `cc-bridge`, `cc-bridge -s`, `cc-bridge kill`, or restart commands from inside the skill.
