...
2. Should WezTerm be a user-visible backend option in `.cc-bridge/cc-bridge.config`, or an
...
1. What is the stable namespace identity for one CC_BRIDGE project in WezTerm:
...
2. Can WezTerm expose enough pane metadata to replace tmux user options such as
   `@cc-bridge_agent`, `@cc-bridge_project`, and namespace epoch?
3. How should `cc-bridge kill` terminate native Windows provider process trees when
   the pane process spawns children?
4. Does live WezTerm `get-text` provide enough scrollback and screen fidelity
   for `cc-bridge_self` pane diagnostics compared with `tmux capture-pane`?
