...
This plan is intentionally separate from CC_BRIDGE's internal
...
package management. CC_BRIDGE consumes that package-management contract through an
adapter, while CC_BRIDGE project config, project locks, projection, reload, ask, and
sidebar behavior remain CC_BRIDGE-owned runtime concerns.
...
- [topics/host-adapter-strategy.md](topics/host-adapter-strategy.md): how
  Claude Code, Codex, CC_BRIDGE, Hive, and future hosts should be handled without
...
- A role catalog that CC_BRIDGE can discover and install from without vendoring role
  packages into the CC_BRIDGE source tree.
...
- Host adapter contracts for Claude Code, Codex, CC_BRIDGE, Hive, and future hosts.
...
- A security sandbox.
- A CC_BRIDGE runtime extraction.
