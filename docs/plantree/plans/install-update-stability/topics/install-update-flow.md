...
1. Detect language with `CC_BRIDGE_LANG`, `LANG`, `LC_ALL`, or `LC_MESSAGES`.
...
Managed update is routed by Python `cc-bridge update`, but mutation authority depends
on install provenance.

For an npm install, the outer `@seemseam/cc-bridge` package owns the vendored release:
...
2. Python accepts npm provenance only when the outer `package.json` matches and
   the executing release is below that package's `.cc-bridge-release` directory.
3. npm `postinstall` downloads the manifest-pinned release and calls only the
   ...
   creates and validates `.cc-bridge-release/<platform>/.venv`; it must not install
...
4. The npm runner validates the release-local managed Python before launching
   CC_BRIDGE and repeats the same idempotent bootstrap under the package install lock
...
5. A vendored payload is complete only when `VERSION` exactly matches the outer
   manifest, the `cc-bridge` entrypoint is executable, and the managed Python can
   import CC_BRIDGE's required TOML, Mobile, and Relay dependencies.
6. `cc-bridge update` prints `npm install -g @seemseam/cc-bridge@<target>` and does not
...
4. Run the staged release `install.sh install` with optional provisioning
   ...
   - `CC_BRIDGE_INSTALL_NEOVIM=0`
5. Verify the newly installed entrypoint and read new build metadata.
6. Print update outcome.
7. Run post-update provisioning through the newly installed `cc-bridge`, not through
...
`cc-bridge kill`. The explicit maintenance equivalent remains:
...
`$XDG_STATE_HOME/cc-bridge/provider-cache-cleanup.json`, falling back to
`~/.local/state/cc-bridge/provider-cache-cleanup.json`.

## Post-Update Runner

The post-update runner should be a CLI entrypoint in the newly installed CC_BRIDGE,
for example an internal command such as:

```text
cc-bridge __post-update --from-version <old> --to-version <new>
```
...
- Run provider update discovery only when the parent `cc-bridge update` explicitly
...
- Persist decline/update evidence and exact muted provider versions under
  `$XDG_STATE_HOME/cc-bridge/provider-updates.json` (falling back to
  `~/.local/state/cc-bridge/provider-updates.json`). A muted version becomes
...
roll back a successfully installed CC_BRIDGE release. CC_BRIDGE must not invoke `npm
...
new files and try to update a legacy `cc-bridge.archi` source that no longer exists.
...
temporary release simulation prefix under `/tmp/cc-bridge-v7.2.1-install-smoke` was
left as the user's bare `cc-bridge` authority. The real `~/.local/bin/cc-bridge` symlink
...
`CC_BRIDGE_ALLOW_TEMP_INSTALL_GLOBAL_BIN=1` only when intentionally writing from a
