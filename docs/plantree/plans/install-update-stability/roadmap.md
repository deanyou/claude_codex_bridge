...
- Existing installer detects `CC_BRIDGE_LANG`/locale and has a `msg` function for
  selected shell installer messages.
- Existing installer blocks accidental root installs unless the user confirms
  interactively or sets `CC_BRIDGE_ALLOW_ROOT_INSTALL=1`.
...
- Existing installer runs installed entrypoint smoke checks:
  `$BIN_DIR/cc-bridge --print-version` and `$BIN_DIR/ask --help`.
...
- Existing Role Pack implementation canonicalizes `cc-bridge.archi` input to
  `agentroles.archi` in most runtime and CLI paths.
- Captured the v7.2.9 post-update `cc-bridge.archi` failure in
  [history/v729-rolepack-update-failure-2026-06-04.md](history/v729-rolepack-update-failure-2026-06-04.md).
- `cc-bridge update` now delegates post-update Role Pack and Neovim provisioning to
  the newly installed `cc-bridge __post-update` entrypoint after tarball install and
...
- Added Role Pack legacy store canonicalization for installed `cc-bridge.archi`
...
- Added regression coverage that catalog `current` Role Packs do not call
  update hooks, and that inherited `cc-bridge-config` docs use `cc-bridge.archi` only as a
  legacy alias.
- `install.sh install` now refuses a temporary `CODEX_INSTALL_PREFIX` when
  ...
  `cc-bridge` wrapper by accident.
- npm runners now attest package ownership to the vendored Python process.
  Ordinary `cc-bridge update` and startup update acceptance validate that
  provenance, print the exact `npm install -g @seemseam/cc-bridge@<target>` action,
...
- Provider CLI updates are centralized under explicit `cc-bridge update`: managed
  ...
  31 release packaging/entrypoint tests, external `cc-bridge_test --diagnose`, and a
...
- Added bounded post-update cleanup for that retired cache route. The newly
  installed `cc-bridge` owns the migration; simultaneous update windows are
  ...
  are deferred to their next successful `cc-bridge kill`, and unsafe content is
  ...
  validation. An isolated external `cc-bridge_test` smoke reported
...
- Extend update-tarball smoke isolation beyond the shell installer gate. A
  2026-06-15 audit found real user `cc-bridge` pointing at
  `/tmp/cc-bridge-v7.2.1-install-smoke/prefix/cc-bridge`, with multiple live daemons using
...
5. Add update-level temporary-prefix isolation tests for release simulations:
   ...
   `~/.local/bin/cc-bridge`, shell startup files, or live project daemons.
6. Add a real upgrade runbook that starts from an older released version with
   installed legacy `cc-bridge.archi`, updates to the new release, and confirms no
