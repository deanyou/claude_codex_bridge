...
assessor, and let `cc-bridge_self` request a later follow-up through
`cc-bridge maintenance schedule`.

Real validation in `/home/bfly/yunwei/test_ccb2` showed the missing runtime
piece: after `cc-bridge_self` updated `next_run_at`, no background consumer invoked
...
Add a CC_BRIDGE-owned project-scoped maintenance heartbeat schedule consumer helper.

The helper is started or ensured by normal `cc-bridge` project startup when
`[maintenance.heartbeat].enabled = true`, `startup_ensure = true`, and the
configured assessor exists. It reads
`.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/schedule.json`, waits until the schedule is
due, and invokes the existing one-shot `cc-bridge maintenance tick` service path.
...
The first slice should implement a project-local CC_BRIDGE helper, not automatic host
OS scheduler installation. It is outside provider context and must not become a
provider-side loop. It is also not a replacement for cc-bridge-daemon's normal configured
agent supervision.
...
- `cc-bridge kill` and project shutdown need a clear stop/exit rule for the helper.
- `cc-bridge maintenance status` should expose runner state so users can distinguish
