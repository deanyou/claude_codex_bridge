...
The maintenance heartbeat should be available as a CC_BRIDGE runtime feature rather
...
Effective `cc-bridge.config` will provide the project-facing heartbeat enablement and
assessor policy. When heartbeat is enabled and the configured assessor exists,
normal `cc-bridge` project startup should ensure the independent heartbeat runner is
started or scheduled for that project.

The runner periodically reads cc-bridge-daemon diagnostics and CC_BRIDGE communication state,
...
`cc-bridge_self` when configured. The runner submits once and exits; it must not poll,
watch, or wait for the assessor reply.

If the assessor decides that a later diagnostic pass is needed, it registers a
CC_BRIDGE-owned follow-up through the sanctioned maintenance schedule surface. The
...
- `.cc-bridge/cc-bridge.config` grammar and validation must be updated before
...
- Delayed rechecks become schedule/follow-up state owned by CC_BRIDGE, not nested
