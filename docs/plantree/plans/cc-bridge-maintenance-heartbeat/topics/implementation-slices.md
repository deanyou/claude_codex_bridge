...
- `lib/cc-bridge-daemon/app_runtime/request_guard.py`
- `lib/cc-bridge-daemon/socket_server_runtime/server.py`
- `lib/cc-bridge-daemon/socket_client_runtime/endpoints.py`
- `lib/cc-bridge-daemon/app_runtime/service_graph.py`
...
- `lib/cc-bridge-daemon/handlers/ping_runtime/payloads.py`
...
- [../../../../cc-bridge-daemon-diagnostics-contract.md](../../../../cc-bridge-daemon-diagnostics-contract.md):
  define `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/`, schedule/status/lock schema,
  support-bundle inclusion, and separation from daemon lease heartbeat plus
  `.cc-bridge/cc-bridge-daemon/heartbeats/<subject-kind>/` job/subject evidence.
...
- [../../../../cc-bridge-daemon-startup-supervision-contract.md](../../../../cc-bridge-daemon-startup-supervision-contract.md):
  define startup ensure semantics, optional failure reporting, `cc-bridge kill`
...
2. Namespace, store, and read-only status.
   ...
   diagnostics bundle inclusion, and `cc-bridge maintenance status`. Keep
...
3. One-shot tick snapshot without dispatch.
   ...
   `test_cc-bridge-daemon_project_view.py`, message-bureau queue tests.
...
5. Startup ensure, schedule follow-up, dedup, and caps.
   ...
   `test_cc-bridge-daemon_start_handler.py`, `test_v2_cc-bridge-daemon_start_flow.py`, source-runtime
   smoke with `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test` from
...
- `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/` path helpers;
- schedule/status data model;
- read-only `cc-bridge maintenance status`.
...
- Added `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/` paths plus schedule/status models
  and corrupt/missing read results.
- Added read-only `cc-bridge maintenance status`; at this initial landing,
...
- From `/home/bfly/yunwei/test_ccb2`, isolated
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test maintenance status` exited 0, reported
  missing schedule/status, and did not create
  `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/`.
- From `/home/bfly/yunwei/test_ccb2`, isolated
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test maintenance enable` exited 2 with
...
- Added a pure heartbeat classifier for `project_view` and local `ps` fallback
  snapshots.
- `cc-bridge maintenance tick` now runs only when `[maintenance.heartbeat]` is
...
- Enabled tick reads mounted daemon `project_view` first, falls back to local
  ...
  `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/status.json` plus `schedule.json`.
...
- Added `maintenance-heartbeat` as an internal non-mailbox CC_BRIDGE sender identity.
- `cc-bridge maintenance tick` now respects persisted `schedule.json`, supports
...
- `cc-bridge maintenance schedule --after <duration> [--reason <text>]` persists the
  next CC_BRIDGE-owned heartbeat follow-up, enforcing `min_interval_s`.
- Unknown streaks that reach `unknown_streak_cap` back off to `interval_s` and
  set `needs_user=true`.
- Normal `cc-bridge` startup now performs non-fatal due-tick ensure when heartbeat is
...
- Added internal `cc-bridge maintenance runner` as the project-scoped schedule
...
- `cc-bridge` startup ensure now starts or reuses one detached runner when heartbeat
  is enabled and the assessor is present.
- `cc-bridge kill` best-effort signals the live runner and records stopped/stale
...
- Startup ensure, nonfatal runner failure, and kill concurrency:
  `test_cc-bridge-daemon_start_handler.py`, `test_v2_cc-bridge-daemon_start_flow.py`,
  `test_cc-bridge-daemon_socket_server_loop.py`.

## Risks And Blockers

- `cc-bridge maintenance enable/disable` cannot safely mutate behavior until config
  authority is decided. If config is authority, these commands either edit
  `.cc-bridge/cc-bridge.config` through an explicit config-editing policy or remain
...
- Internal `ask --silence` dispatch needs a CC_BRIDGE system sender identity such as
...
- Do not run source runtime validation from `cc-bridge_source`; use
