...
The heartbeat is a generic CC_BRIDGE feature with a configurable semantic assessor.
The default assessor is `cc-bridge_self`.

CC_BRIDGE owns:

- independent heartbeat runner and external tick entrypoint;
- effective-config heartbeat enablement and assessor selection;
- normal `cc-bridge` project-start integration that ensures the runner when enabled;
...
- cc-bridge-daemon and communication snapshot collection;
...
- semantic interpretation of CC_BRIDGE evidence;
...
- optional skill-triggered requests to invoke or reschedule the independent
  heartbeat runner through sanctioned CC_BRIDGE commands.

The assessor must not own:

- keeper lifecycle;
- cc-bridge-daemon generation or lease authority;
...
- logs show repeated provider/tool errors that are not yet expressed as a
  single terminal CC_BRIDGE failure;
- an agent is idle but the current plan or user request still implies unfinished
  work;
- diagnostics are degraded enough that CC_BRIDGE cannot decide whether the state is
  healthy.
...
1. An external scheduler, user command, or sanctioned assessor request
   ...
   tick.
2. The tick acquires a heartbeat lock that is independent from keeper and cc-bridge-daemon
   lifecycle locks, then reads CC_BRIDGE-owned schedule policy.
3. If heartbeat is disabled, too early, or another maintenance tick is active,
   it exits.
4. CC_BRIDGE collects a bounded snapshot from existing diagnostics and communication
...
5. If the snapshot is clearly idle and healthy, CC_BRIDGE records `last_ok`, advances
...
6. If the snapshot has risk, is unhealthy, or cannot be classified with enough
   confidence, CC_BRIDGE sends a bounded diagnostic package to the configured
   assessor, default `cc-bridge_self`. In v1 this dispatch uses `ask --silence` so
...
8. CC_BRIDGE validates any schedule recommendation, persists the accepted next
...
- user/manual: `cc-bridge maintenance tick`, `cc-bridge maintenance status`,
  `cc-bridge maintenance schedule --after <duration>`, `cc-bridge maintenance enable`, and
  `cc-bridge maintenance disable`;
- scheduler: cron, systemd timer, launchd, Windows Task Scheduler, or a future
  CC_BRIDGE installer-managed timer calls the same one-shot tick surface;
...
deduplicates active ticks, writes the schedule state, invokes CC_BRIDGE diagnostics,
and decides whether a provider turn is needed.

## Config And Startup

Heartbeat enablement belongs in effective `cc-bridge.config`, not in role memory and
not in provider state. A planned shape can be refined before implementation,
but the config needs at least:

- enabled/disabled;
- assessor target, defaulting to `cc-bridge_self` or `self` when configured;
- normal interval;
- minimum interval;
- unknown-streak cap;
- whether risk/unknown/unhealthy states should escalate by `ask --silence`.

Normal `cc-bridge` project startup should ensure the independent heartbeat runner
...
one-shot schedule for the project without making it keeper, cc-bridge-daemon, or provider
...
deeper evidence and, through sanctioned CC_BRIDGE commands, schedule the next
...
`cc-bridge_self` assessor should prefer a pane-text-first evidence ladder for
...
a bounded CC_BRIDGE-owned pane screenshot or equivalent visual artifact only when
...
loop alive. Instead, `self` should register a CC_BRIDGE-owned follow-up through a
...
This keeps "ask self later" as a CC_BRIDGE scheduling concern rather than a nested
provider conversation or a self-recursive ask chain.

## Scheduled Activation Abstraction

Heartbeat should use a generic CC_BRIDGE activation dispatcher rather than a
...
This abstraction should not bypass normal CC_BRIDGE message/job authority. It should
compile down to supported CC_BRIDGE delivery surfaces such as `ask --silence` in v1,
with callback or result-dependent delivery left for a later explicit design.

## Assessor Command Surface

For v1, the default `cc-bridge_self` assessor should have a narrow sanctioned surface:
...
- `run-now`: restricted; use only for explicit user requests or high-confidence
  concern/failing results after CC_BRIDGE validation.
...
The CC_BRIDGE control plane must validate this result before changing schedule or
running any command.

## Snapshot Contract

The snapshot sent to the assessor should be small, derived from existing CC_BRIDGE
...
- cc-bridge-daemon mounted/alive state, generation, degraded diagnostics, and recent
...
- target pane/window references sufficient for the assessor to request
  ...
  screenshot fallback through CC_BRIDGE-owned tools;
...
Use a dedicated heartbeat namespace under `.cc-bridge/cc-bridge-daemon/`, separate from keeper
...
`.cc-bridge/cc-bridge-daemon/heartbeats/`; that namespace is reserved for non-lease subject/job
heartbeat evidence.
...
reuse keeper or cc-bridge-daemon lifecycle locks.
...
- Project idle: idle means cc-bridge-daemon is mounted, no active jobs, no pending
...
- Stale heartbeat lock: detect via lock age and tick id; release only according
  to the heartbeat runner's own stale-lock rule, not keeper or cc-bridge-daemon lock rules.
