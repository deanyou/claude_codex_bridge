...
- Job heartbeat is a long no-progress diagnostics guard for running `ask` jobs.
  Its default role is to record internal diagnostics while CC_BRIDGE continues
...
- `cc-bridge ask` submits a `MessageEnvelope` to the mounted daemon.
...
- Each cc-bridge-daemon polling cycle asks the provider adapter to poll its active
  submission.
...
- Codex: protocol/session event stream under the managed Codex session root.
  Prompt delivery is not accepted until the active `CC_BRIDGE_REQ_ID` anchor appears
...
- Claude: hook artifact first when available, then session event log. The
  Claude hook must bind the current transcript turn to the actual CC_BRIDGE prompt and
...
- Provider activity evidence can override CC_BRIDGE job evidence before pane text is
