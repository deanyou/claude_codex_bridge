...
Improve fault detection without making `cc-bridge-daemon` overly aggressive or turning the
maintenance classifier into a large provider-specific rule tree.

The target split:

- `cc-bridge-daemon` remains conservative and deterministic. It owns authoritative job
  state, runtime lifecycle, explicit terminal failures, and no-progress fuses.
- CC_BRIDGE maintenance owns cheap read-only suspicion detection and dispatch.
- `cc-bridge_self` owns semantic diagnosis, deeper evidence gathering, explanation,
  bounded repair selection, and follow-up scheduling.

## Design Principle

CC_BRIDGE should not try to fully diagnose every gray-zone condition. It should detect
that evidence is inconsistent, degraded, stale, or ambiguous, then package a
small evidence envelope for `cc-bridge_self`.

This keeps the default path unified:

- no `mode = "aggressive"`;
- no parallel classifier behavior;
- no fine-grained diagnosis matrix inside cc-bridge-daemon;
...
Handled by `cc-bridge_self` through sanctioned commands and skills:
...
- `cc-bridge restart <agent>` for a known bad single agent slot after busy gates pass;
- `cc-bridge repair retry|resubmit|ack` for message/reply lineage recovery;
- `cc-bridge config validate`, `cc-bridge reload --dry-run`, `cc-bridge reload`, and targeted
  agent restart for config/provider fixes;
- `cc-bridge maintenance schedule --after ...` for follow-up;
...
- `allowed_actions`: bounded CC_BRIDGE actions that self may consider for this
  envelope.

Avoid putting large raw logs in the envelope. Use references and let `cc-bridge_self`
...
- `provider_work_without_control_work`: emitted when project-view shows
  provider/pane work for an agent but there is no current CC_BRIDGE job and no active
  communication targeting that agent. This covers the important false-healthy
  class where CC_BRIDGE has already gone idle/terminal but the pane still looks busy.
...
The activation message now tells `cc-bridge_self` to use only actions explicitly
...
- The classifier still does not inspect raw tmux, provider logs, or private
  provider state directly; those remain `cc-bridge_self` responsibilities.
...
- `ProjectViewService` attaches the matching snapshot to each agent row as
  `provider_runtime`.
- When a current CC_BRIDGE job exists, project-view attaches only the provider
...
- `provider_runtime_without_control_job`: execution runtime exists but the
  agent row has no current CC_BRIDGE job.
...
The envelope copies stable provider runtime evidence only. `cc-bridge_self` can
...
`cc-bridge_self` should follow a fixed workflow so it is autonomous but predictable:

1. Read the suspicion envelope and classify which evidence is missing.
2. Gather cheap CC_BRIDGE state: `ps`, `trace`, `maintenance status`, comms queue or
...
8. If repair is safe, run the sanctioned CC_BRIDGE command, validate outcome, and
...
- `evidence_gap`: CC_BRIDGE cannot access the evidence needed for a confident
  decision.
...
- `recovery_needed`: a terminal or blocked state has a known CC_BRIDGE repair surface.

This gives `self` enough context while keeping cc-bridge-daemon efficient and conservative.

## Acceptance Criteria

- Healthy idle projects do not wake `cc-bridge_self`.
- Known deterministic failures still terminalize or report through existing
  hard-authority paths.
- Gray-zone states wake `cc-bridge_self` with bounded evidence and deduplication.
- `cc-bridge_self` can diagnose a terminal-control-plane versus busy-pane conflict
  without requiring the user to manually inspect tmux.
- `cc-bridge_self` can choose watch-later, ask-user, or bounded repair without
