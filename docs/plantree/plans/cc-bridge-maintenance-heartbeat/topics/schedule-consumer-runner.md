...
consumption, or real `cc-bridge_test` validation.
Related:
...
execution. The landed v1 can write `next_run_at`, and `cc-bridge_self` can update it
through `cc-bridge maintenance schedule`, but nothing consumes the schedule after
startup unless a user manually runs `cc-bridge maintenance tick`.
...
- `cc-bridge_test maintenance tick --force` advanced `last_tick_at`, wrote
  `schedule.json`, and submitted a silent diagnostic activation to `cc-bridge_self`.
- `cc-bridge_self` completed the activation and changed `next_run_at` to a later
  follow-up time.
- After the new `next_run_at` passed, `last_tick_at` did not advance
  automatically.
- `cc-bridge-daemon` was mounted and healthy, queues were empty, and no
...
piece is an automatic CC_BRIDGE-owned schedule consumer.

## Target Behavior

When heartbeat is enabled for a project:

1. Normal `cc-bridge` project startup ensures exactly one heartbeat schedule
...
5. The one-shot tick collects a fresh CC_BRIDGE snapshot, updates heartbeat status
   and schedule, and may submit one bounded silent assessor activation.
6. If `cc-bridge_self` schedules a later follow-up, the same consumer observes that
...
Healthy idle projects should not wake `cc-bridge_self`. "Idle exit" means the tick
...
- add an internal command or service path such as `cc-bridge maintenance runner`
  or `cc-bridge maintenance watch`;
- start it from the existing startup ensure path when heartbeat is enabled;
- keep the public documented surface as `cc-bridge maintenance status|tick|schedule`
  until the runner is stable;
- record runner state under `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/`;
- use the existing heartbeat lock before each tick so manual ticks and the
  consumer cannot race.

Avoid for this slice:

- provider-side loops inside `cc-bridge_self`;
...
- making cc-bridge-daemon's internal daemon heartbeat loop the semantic maintenance
  scheduler.
...
- path: `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/runner.json`
...
- Startup: if enabled and `startup_ensure=true`, `cc-bridge start` ensures the
...
- Shutdown: `cc-bridge kill` must stop or cause clean exit for the consumer. Shutdown
...
- Crash tolerance: a crashed consumer leaves only diagnostics residue. The
  next `cc-bridge start` or manual ensure can replace it.
...
6. If `next_run_at` is due, invoke the same internal service as
   `cc-bridge maintenance tick` without bypassing schedule, lock, duplicate
...
- cc-bridge-daemon mounted and healthy: use normal `project_view` snapshot through the
  existing tick path.
- cc-bridge-daemon unmounted or project-view unavailable: let the one-shot tick use its
...
- `lib/maintenance_heartbeat/store.py`: runner state read/write and stale
  handling helpers.
- `lib/storage/paths_cc-bridge-daemon.py`: runner state path.
- `lib/cli/router.py` and parser files: hidden/internal runner command if
  needed.
- `docs/cc-bridge-daemon-startup-supervision-contract.md` and
  `docs/cc-bridge-daemon-diagnostics-contract.md`: runner lifecycle and diagnostics
  contract updates.
...
5. Expand isolated `cc-bridge_test` validation from `test_ccb2` to prove a
   `cc-bridge_self` scheduled follow-up is consumed without manual `tick`.

## Landed Implementation

Landed on 2026-06-11:

- Added `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/runner.json` path, model, store, and
  `maintenance status` rendering.
- Added internal `cc-bridge maintenance runner` action with bounded foreground test
  options and an unbounded project-scoped schedule-consumer loop for startup.
- `cc-bridge` startup now ensures one detached schedule consumer when heartbeat is
...
- `cc-bridge kill` now best-effort signals the live runner and records stopped/stale
...
- `python -m py_compile lib/maintenance_heartbeat/models.py
  ...
  lib/storage/paths_cc-bridge-daemon.py lib/cli/services/maintenance.py
...
- Isolated real validation from `/home/bfly/yunwei/test_ccb2` with
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test` and isolated `HOME` /
  `CC_BRIDGE_SOURCE_HOME` confirmed:
...
- Provider-side loops or long-lived `cc-bridge_self` turns.
- Direct schedule-file writes from assessor code.
- A cc-bridge-daemon-internal semantic maintenance scheduler that cannot diagnose cc-bridge-daemon
  unresponsiveness.
