...
Ship the smallest useful CC_BRIDGE maintenance heartbeat that is independently
invokable, diagnoses configured-agent health from cc-bridge-daemon and communication
evidence, exits when healthy/idle, and escalates risk, unknown, or unhealthy
states to the configured semantic assessor, defaulting to `cc-bridge_self`.

V1 must not perform automatic mutating repair.

## Included

- `cc-bridge maintenance tick`: one-shot runner entrypoint with project discovery and
  optional explicit project root.
- `cc-bridge maintenance status`: read current schedule and last tick state.
- `cc-bridge maintenance schedule --after <duration> --reason <text>`: validated
  schedule update.
- `cc-bridge maintenance enable` / `disable`: reserved user-facing policy commands
  only; v1 returns `not_implemented` and keeps `.cc-bridge/cc-bridge.config` as the
  enablement authority.
- Effective `cc-bridge.config` heartbeat enablement and configured assessor lookup.
- Normal `cc-bridge` startup ensures the independent runner when heartbeat is enabled
...
- Independent heartbeat lock under the heartbeat namespace, with stale-lock
  detection separate from keeper and cc-bridge-daemon lifecycle locks.
- Schedule state under a dedicated namespace such as
  `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/schedule.json`.
- Programmatic snapshot from existing cc-bridge-daemon diagnostics and CC_BRIDGE communication
  state.
...
- Rich `cc-bridge doctor` or sidebar UI integration beyond a minimal status summary.
...
- update [../../../../cc-bridge-config-layout-contract.md](../../../../cc-bridge-config-layout-contract.md)
  with heartbeat config fields, defaults, validation, and reload behavior;
- update [../../../../cc-bridge-daemon-diagnostics-contract.md](../../../../cc-bridge-daemon-diagnostics-contract.md)
  with `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/`, schedule schema, lock/status files,
...
- update [../../../../cc-bridge-daemon-startup-supervision-contract.md](../../../../cc-bridge-daemon-startup-supervision-contract.md)
  with startup ensure semantics, optional runner failure reporting, and `cc-bridge
  kill` interaction;
...
- `test_maintenance_heartbeat_paths_use_dedicated_namespace`: heartbeat state
  lives under `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/`.
...
- `test_reload_plan_classifies_maintenance_only_change`,
  ...
  config changes while cc-bridge-daemon is mounted update heartbeat policy through explicit
...
- `test_maintenance_enable_disable`: deferred until config editing policy
  allows these commands to mutate `.cc-bridge/cc-bridge.config`.
