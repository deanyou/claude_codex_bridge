...
`self` / `cc-bridge_self` for semantic diagnostics. The model must still avoid
...
- Dispatcher: CC_BRIDGE policy layer that validates target authority, deduplication,
...
- Target agent: a configured current-graph agent. V1 target is the configured
  assessor, defaulting to `self` / `cc-bridge_self`.
...
- `heartbeat_state_check`: the runner sees unfinished work, risk, unhealthy
  state, or unknown progress from cc-bridge-daemon diagnostics and communication state.
...
- `target_agent`: configured assessor, default `self` / `cc-bridge_self`
...
- `status=submitted` includes the submitted CC_BRIDGE `job_id`
...
- v1 target must be the configured assessor, default `self` / `cc-bridge_self`;
...
- payload must stay bounded or use CC_BRIDGE artifact storage;
...
- default target: `self` / `cc-bridge_self`;
