...
- `clauder` was shown as idle/completed by CC_BRIDGE.
...
- The hook event completed `job_86550847f237` after the user had interrupted
  the original CC_BRIDGE turn.
- `cc-bridge_self` could not diagnose the failure because the control-plane evidence
  was already false: the job looked terminal and the agent looked idle.

Root cause:

- Claude `Stop` hook attribution used the latest visible `CC_BRIDGE_REQ_ID` or
  `last-prompt` style evidence from the transcript.
- A later provider-side scheduled-task turn could therefore reuse an earlier CC_BRIDGE
...
- Claude hook completion now binds the current assistant stop to the actual
  ...
  prompt itself has an outer `CC_BRIDGE_REQ_ID`.
- Scheduled-task, user-interruption, auth/info, and other provider-side turns
  do not inherit an old CC_BRIDGE request id.
...
- `python -m pytest -q test/test_provider_hook_transcript.py test/test_provider_finish_hook_script.py test/test_claude_hook_results.py test/test_cc-bridge-daemon_project_view.py`
  passed with 67 tests.
- `python -m pytest -q test/test_maintenance_heartbeat.py test/test_provider_activity_artifacts.py test/test_v2_config_loader.py test/test_provider_hook_transcript.py test/test_provider_finish_hook_script.py test/test_cc-bridge-daemon_project_view.py`
  passed with 184 tests.
- `py_compile` passed for the touched hook, transcript, artifact, and activity
  modules.
- `git diff --check` passed.
- Read-only replay of the real stale Claude transcript returned no current CC_BRIDGE
...
- Isolated source-runtime validation on 2026-06-12 used
  ...
  `/home/bfly/yunwei/test_ccb2/cc-bridge_empty_reply_real` with
...
- Codex may have activity/profile hooks, but those hooks are not the terminal
  completion authority for CC_BRIDGE jobs.
- The equivalent Codex risks are not stale `Stop` hook reuse. They are:
  delivery accepted incorrectly, active `CC_BRIDGE_REQ_ID` anchor never observed,
...
- CC_BRIDGE says an agent is idle or a job is terminal, but pane text indicates
...
For active anomalies, the `cc-bridge_self` running-supervision skill should start with
read-only evidence:

1. CC_BRIDGE control-plane snapshot: `ps`, trace, job state, queue state, callback
...
The purpose is to let `cc-bridge_self` explain and recommend the next step quickly
when CC_BRIDGE's own health summary is overconfident.
