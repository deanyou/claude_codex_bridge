# CC_BRIDGE Maintenance Heartbeat Roadmap
...
- Accepted the boundary correction: heartbeat scheduling, wake policy, and
  next-run state belong to CC_BRIDGE rather than to the `cc-bridge_self` Role Pack.
- Promoted the external heartbeat idea from the general ideas inbox into this
  CC_BRIDGE-level planning root.
- Accepted the direction that the heartbeat should be an independent CC_BRIDGE
  program/helper that `cc-bridge_self` can trigger or reschedule through a sanctioned
  skill or control-plane command.
- Accepted the user's correction that the heartbeat is generic CC_BRIDGE
  infrastructure: it periodically diagnoses all configured agents from cc-bridge-daemon and
  communication evidence, then escalates risk, unknown, or unhealthy states to
  the configured semantic assessor, defaulting to `cc-bridge_self`.
- Accepted reviewer1's architecture feedback: avoid hard-coding `cc-bridge_self`,
  prefer a hybrid `cc-bridge maintenance ...` one-shot surface over cc-bridge-daemon/keeper
  internal ticks, keep the heartbeat lock independent from keeper/cc-bridge-daemon locks,
...
- Accepted the startup/config direction: heartbeat enablement belongs in
  effective `cc-bridge.config`; normal `cc-bridge` project startup should ensure the
...
- Landed the first safe implementation slice: `[maintenance.heartbeat]`
  config parsing/defaults, `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/` path helpers,
  schedule/status data models, read-only `cc-bridge maintenance status`,
  ...
  `git diff --check`, and isolated external `cc-bridge_test` smoke tests from
  `/home/bfly/yunwei/test_ccb2`.
- Landed the one-shot tick snapshot slice: `cc-bridge maintenance tick` now runs only
...
- Landed the activation/schedule/startup ensure slice: non-healthy due ticks
  ...
  `from_actor=maintenance-heartbeat`, then exit. `cc-bridge maintenance schedule`
  writes the next CC_BRIDGE-owned follow-up time with `min_interval_s` enforcement.
  `cc-bridge` startup now performs a non-fatal due-tick ensure when heartbeat is
...
- Completed v1 verification: full `python -m pytest -q` passed with
  ...
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test` smoke tests from
...
- Landed post-review hardening for the next release: Codex unusable-pane
  ...
  --check`, `py_compile`, and isolated `cc-bridge_test --diagnose`, `config
...
- Accepted the self-supervision refinement: ambiguous execution-quality
  diagnosis should use real CC_BRIDGE-owned pane observation, starting with
  ...
  insufficient. Heartbeat passes target references; the `cc-bridge_self` assessor
...
- Verified the schedule-consumption gap in `/home/bfly/yunwei/test_ccb2`:
  manual `maintenance tick --force` updates status, submits `cc-bridge_self`, and
  lets `cc-bridge_self` reschedule a follow-up, but a due `next_run_at` is not
...
- Landed the project-scoped schedule consumer runner: startup ensure now starts
  ...
  `cc-bridge kill` best-effort signals it. Verified with targeted tests, full pytest,
...
- Recorded the v7.4.1 manual-test incident where Claude `Stop` hook attribution
  reused a stale `CC_BRIDGE_REQ_ID` after an interrupted request and scheduled task.
...
- Clarified that Codex is not on the same `Stop` hook completion path. Codex
  ...
  binding, and pane/control-plane conflicts rather than stale hook `CC_BRIDGE_REQ_ID`
  reuse.
...
- Accepted the self-first supervision direction: cc-bridge-daemon should not become a
  fine-grained semantic judge. It should emit coarse, read-only suspicion
  envelopes for gray-zone evidence and activate `cc-bridge_self`, which performs
...
- Completed and reviewed the first suspicion-envelope slice in the default maintenance
  ...
  provider/pane evidence says an agent is active but CC_BRIDGE has no active job or
  ...
  `allowed_actions`; heartbeat activation messages instruct `cc-bridge_self` to use
...
- Validated the reviewed first suspicion-envelope slice with
  ...
  `python -m pytest -q test/test_provider_hook_transcript.py test/test_provider_finish_hook_script.py test/test_cc-bridge-daemon_project_view.py test/test_maintenance_heartbeat.py`
...
- Validated the final reviewed Step 2 state with
  `python -m pytest -q test/test_provider_execution_service_runtime.py test/test_v2_execution_service.py test/test_cc-bridge-daemon_project_view.py test/test_maintenance_heartbeat.py`
  (`160 passed`) and
  `python -m pytest -q test/test_provider_hook_transcript.py test/test_provider_finish_hook_script.py test/test_provider_activity_artifacts.py test/test_claude_hook_results.py test/test_cc-bridge-daemon_project_view.py test/test_maintenance_heartbeat.py`
...
- Completed isolated source-runtime validation from
  `/home/bfly/yunwei/test_ccb2/cc-bridge_empty_reply_real` using
  `/home/bfly/yunwei/cc-bridge_source/cc-bridge_test` with fake completion-family agents.
...
1. Define the `cc-bridge_self` running-supervision skill input/output contract as
...
3. Define config-edit policy if `cc-bridge maintenance enable/disable` should ever
   mutate `.cc-bridge/cc-bridge.config` instead of staying config-only.
...
- Automatic host OS scheduler installation; the next slice uses a CC_BRIDGE-owned
  project-scoped schedule consumer helper instead.
- Multi-assessor arbitration beyond the default `cc-bridge_self` assessor.
- Public scheduled activation to arbitrary target agents.
- `cc-bridge maintenance enable/disable` editing `.cc-bridge/cc-bridge.config`; v1 keeps
