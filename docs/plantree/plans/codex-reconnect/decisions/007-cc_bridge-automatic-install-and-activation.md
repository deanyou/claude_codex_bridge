# CC_BRIDGE Automatic Installation And Activation
...
CC_BRIDGE already vendors `codex-reconnect`, installs its command entrypoint, and
...
- Every CC_BRIDGE installation includes the bundled `codex-reconnect` command and
  its managed Codex control skill. No separate standalone installation is a
  prerequisite for CC_BRIDGE use.
- The CC_BRIDGE Codex bridge invokes the bundled command with `on` only after its
...
- Automatic activation uses the agent provider runtime's `reconnect/` state
  directory and the existing `CC_BRIDGE_SESSION_FILE` binding. It does not wrap the
...
- Standalone Codex sessions remain opt-in through `$reconnect on`; automatic
  activation is CC_BRIDGE integration policy, not a new standalone default.
- Reconnect startup failure is visible in bridge diagnostics but does not
  prevent the managed Codex session from operating.

## Acceptance

- A CC_BRIDGE install exposes the bundled command without a prior standalone install.
...
CC_BRIDGE-managed Codex receives reconnect protection by default. The bridge becomes
