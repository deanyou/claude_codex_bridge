# Decision 002: stable CC_BRIDGE with a Windows beta asset
...
Publish `v8.6.0` as a stable CC_BRIDGE release across the existing Linux, macOS, npm,
Sidebar, and Android routes. Attach `cc-bridge-windows-x86_64.zip` and its checksum to
the same GitHub Release through the isolated Windows workflow.

The stable CC_BRIDGE version does not promote the Windows support tier. Windows
