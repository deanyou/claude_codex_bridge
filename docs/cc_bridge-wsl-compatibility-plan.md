# CC_BRIDGE WSL 兼容性收敛方案

## 1. 文档定位

这份文档定义 `cc-bridge_source` 在 WSL 环境下的系统性兼容方案，目标不是为单个报错补分支，而是把平台兼容性收敛成稳定边界。

本文覆盖两类已经确认的 WSL 问题：

- 项目位于 `/mnt/<drive>/...` 时，`cc-bridge-daemon`/tmux Unix socket 绑定失败
...
- 在 `cc-bridge-daemon`、CLI、tmux、installer 各调用点零散添加 WSL 特判
- 仅按 socket 路径长度判断是否可用，而不判断底层文件系统是否支持 Unix socket
- 让安装脚本直接承担 CRLF 自救，而忽略脚本在解析前就可能失败
- 为兼容 WSL 而新增第二套 lifecycle authority
- 把 `.cc-bridge/cc-bridge.config`、session files、workspaces 和 provider-state 混成同一类“运行态”
...
放在项目内 `.cc-bridge/cc-bridge-daemon/` 下。
...
```text
项目 authority 仍然留在 .cc-bridge
  +
transport / execution artifact 可以脱离项目目录放置
  +
CLI / keeper / cc-bridge-daemon 不因为 WSL mounted fs 失去可用性
```

关键约束：

- `.cc-bridge` 仍是唯一项目 authority 根
...
- `.cc-bridge/workspaces/` 仍保持在 anchor 下，作为 workspace authority / worktree 结构的一部分
- provider session files 仍保持在 `.cc-bridge/` anchor 下，继续作为 discovery evidence 而不是运行态 authority

以下逻辑路径属于可迁移运行态；在 WSL mounted-drive 项目上，物理存储可以离开 anchor，但 bundle/doctor 仍按逻辑 `.cc-bridge` 路径呈现：
...
- 在 WSL mounted-drive 项目上，`cc-bridge-daemon/` 和 `agents/` 的物理存储可以放到本机 Linux state root
- `.cc-bridge/` 仍是项目 anchor，`cc-bridge.config`、session file、workspaces 仍从 anchor 读写
- 迁移后的 runtime root 必须由 marker/ref 文件显式记录，不能依赖隐式 symlink 作为 authority
- 诊断与 bundle 必须把 runtime-root 物理位置映射回逻辑 `.cc-bridge` 路径，避免误把迁移后的物理路径当成新 authority
...
1. 优先尝试当前 runtime root 下的 `cc-bridge-daemon/<stem>.sock`
...
5. `cc-bridge-daemon.sock` 与 `tmux.sock` 必须共用同一套选择逻辑
...
1. `XDG_RUNTIME_DIR/cc-bridge-runtime`
2. `/tmp/cc-bridge-runtime`
...
- `doctor`、`ping('cc-bridge-daemon')`、startup report 必须暴露实际 socket 路径
...
- 前台 start/reflow 与后台 heartbeat probe 必须区分 readiness budget：
  ...
  - 后台 heartbeat 的 `has-session` / root-pane probe 必须使用短预算并在 transient `no server running` 时返回 defer，而不是长时间阻塞 `cc-bridge-daemon` RPC loop
...
- 背景 heartbeat 内的 namespace/supervision 失败只能记为最近一次后台维护失败，不得直接把已 mounted 的 `cc-bridge-daemon` authority 打成 unmounted
...
- 不引入 CLI / cc-bridge-daemon / tmux 业务依赖

### 8.2 cc-bridge-daemon / diagnostics 层
...
- 当 relocated runtime-state root 下的 socket 路径仍超长时，`cc-bridge-daemon_socket_path` / `tmux_socket_path` 再回退到 runtime socket root
- 非 WSL 普通 Linux 项目路径仍优先使用 `.cc-bridge/cc-bridge-daemon/*.sock`
...
- `ping cc-bridge-daemon` / `doctor` 输出 runtime socket root 信息
...
- `cc-bridge update` 通过 staging 执行 release 包内 `install.sh`
...
- 修 `cc-bridge-daemon.sock` / `tmux.sock` 同步回退
- 先补 unit tests
