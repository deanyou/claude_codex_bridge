# CC_BRIDGE_DAEMON Diagnostics Contract

## 1. Purpose

This document defines the non-drifting diagnostics contract for project-scoped startup/shutdown reports, backend logs, and support-bundle export in `cc-bridge_source`.
...
- `.cc-bridge/cc-bridge-daemon/start-policy.json`
- `.cc-bridge/cc-bridge-daemon/lifecycle.jsonl`
- `.cc-bridge/cc-bridge-daemon/heartbeats/<subject-kind>/*.json`
- `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/`
- `.cc-bridge/cc-bridge-daemon/reload-drain.json`
- `.cc-bridge/cc-bridge-daemon/artifacts/text/`
- project-scoped backend log retention under `.cc-bridge/cc-bridge-daemon/`
- `cc-bridge doctor`
- `cc-bridge doctor ps`
- `cc-bridge doctor logs <agent>`
- `cc-bridge doctor --bundle`

The repo-local memory file [AGENTS.md](/home/bfly/yunwei/cc-bridge_source/AGENTS.md) must point to this document instead of duplicating the rules.
...
`cc-bridge doctor` includes a `windows_x64_release_surface` record loaded from the
...
- Diagnostics are scoped to one `.cc-bridge` anchor.
- All project diagnostics records must live under that anchor's logical `.cc-bridge/cc-bridge-daemon/`, even when the physical runtime state root is relocated on WSL mounted-drive projects.
- Runtime-root marker and reference files are part of the project diagnostics evidence chain and must be mapped back into the logical `.cc-bridge` archive path in support bundles.
- Project-local provider state under `.cc-bridge/agents/<agent>/provider-state/` is diagnostics evidence and should be exported when it is relevant to session isolation or binding analysis.
...
- optional `daemon_started`
  - whether the foreground `cc-bridge` command had to start a new daemon
...
- `inspection`
- `socket_placement`
  - at minimum preferred/effective socket paths plus root kind and fallback reason for both `cc-bridge-daemon` and project tmux socket selection
...
- `ping('cc-bridge-daemon')` and `doctor` must surface the latest startup timing map,
...
- source-benchmark process bootstrap timings are a separate optional surface;
  ...
  `popen_begin_to_cc-bridge_test_entry`, `cc-bridge_test_entry_to_pre_exec`,
  `cc-bridge_test_pre_exec_to_cc-bridge_py_entry`, `cc-bridge_py_entry_to_main`, and
  `cc-bridge_py_main_to_cli_start`
...
owned external test fixture, not into the live project's `.cc-bridge` authority.  A
...
- the implemented scenario constructors are S0 CLI-only hot path, S1 warm
  ...
  Its measured command is exactly `cc-bridge_test --print-version`: it emits no
  ...
  and S4 publish
  their `before` phase before invoking official `cc-bridge_test kill`; their ready
  ...
  process audit.  S5a additionally requires absent cc-bridge-daemon/Agent state and an
...
- S3 is available only for an explicitly owned external fixture with at least
  ...
  official single-Agent `cc-bridge_test restart` RPC has returned, then released to
...
- remote `cc-bridge kill` must finalize lifecycle state before recording the final shutdown report, so `inspection_after` reflects `phase=unmounted` / `desired_state=stopped` rather than a transient `stopping` state

### 3.4 Backend Logs

Project backend logs must remain under `.cc-bridge/cc-bridge-daemon/`:

- `cc-bridge-daemon.stdout.log`
- `cc-bridge-daemon.stderr.log`
- `keeper.stdout.log`
- `keeper.stderr.log`
...
- crash capture enforces online per-runtime retention of the newest 50
  ...
  deferred until a later `cc-bridge cleanup`
...
- `.cc-bridge/cc-bridge-daemon/lifecycle.jsonl`
- `.cc-bridge/cc-bridge-daemon/heartbeats/<subject-kind>/*.json`
- `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/schedule.json`
- `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/status.json`
- `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/runner.json`
- `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/lock.json`
- `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/activations.jsonl`
- `.cc-bridge/cc-bridge-daemon/reload-drain.json`
...
- `maintenance-heartbeat/` records CC_BRIDGE maintenance heartbeat schedule, status,
  ...
  - `status.json` records the latest maintenance heartbeat status summary from
    `cc-bridge maintenance tick`, including `last_tick_status`, `last_tick_at`,
  ...
  - `activations.jsonl` records `ActivationIntent` dispatch outcomes such as
    ...
    diagnostics/audit evidence for CC_BRIDGE-originated silent asks and are not
    mailbox, job, or daemon authority.
  - malformed or missing files must be visible to `cc-bridge maintenance status`
  ...
  - `cc-bridge maintenance tick` may write only `status.json`, `schedule.json`,
...
- `reload-handoff.json` records a short-lived explicit reload handoff while
  plain `cc-bridge reload` submits a non-dry-run request and, after daemon-side plan
...
- `artifacts/text/` stores oversized CC_BRIDGE agent-to-agent message and reply text. Request bodies, terminal replies, notices, and callback continuations larger than 4 KiB are written there as UTF-8 text artifacts; ledgers store only the short preview plus artifact path, byte count, and sha256 metadata. These artifacts are diagnostics/evidence and transport support, not scheduling authority.
...
- `ping('cc-bridge-daemon')` and `doctor` should surface start-policy summary fields when available
...
- `ping('cc-bridge-daemon')` and `doctor` must surface namespace summary fields such as epoch, tmux socket path, session name, and latest lifecycle event when available
- `ping('cc-bridge-daemon')` and `doctor` must surface current socket placement diagnostics, including preferred/effective socket path, root kind, fallback reason, and filesystem hint when known
- `ping('cc-bridge-daemon')` must remain available during the post-self-ping
...
- `cc-bridge layout status --json` may include best-effort tmux observation
  ...
  distinguished from configured agent panes by CC_BRIDGE identity metadata.
- `project_view` must surface active reload drain state when
  `.cc-bridge/cc-bridge-daemon/reload-drain.json` contains non-terminal records. The view should
...
- `cc-bridge-daemon` heartbeat may automatically retry a ready active unload drain by
...
`cc-bridge doctor` is the best-effort project diagnostics read path.
...
- startup failure diagnostics must retain chained cause detail in CLI output and in `cc-bridge-daemon_startup_last_failure_reason` when the backend recorded it
- Codex agent diagnostics should surface managed in-pane session-switch state
  when `.cc-bridge/agents/<agent>/provider-runtime/codex/session-switch.json`
...
- `.cc-bridge/cc-bridge-daemon/support/<bundle-id>.tar.gz`

The support bundle must include:

- a manifest
- a generated doctor snapshot
- current project config from `.cc-bridge/cc-bridge.config`
...
- persisted non-lease heartbeat state under `.cc-bridge/cc-bridge-daemon/heartbeats/` when present
- maintenance heartbeat schedule/status/runner/lock/activation files under
  `.cc-bridge/cc-bridge-daemon/maintenance-heartbeat/` when present
- oversized CC_BRIDGE text artifacts under `.cc-bridge/cc-bridge-daemon/artifacts/text/` when referenced by recent message/reply records
...
The keeper may directly spawn `cc-bridge-daemon`, but it must reap exited direct children.

Rule:

- a crashed or killed `cc-bridge-daemon` process must not remain visible as an unreaped zombie just because keeper is still alive
...
2. run `cc-bridge doctor`
3. run `cc-bridge doctor --bundle`
...
- Use [docs/cc-bridge-daemon-manual-test-issue-log.md](/home/bfly/yunwei/cc-bridge_source/docs/cc-bridge-daemon-manual-test-issue-log.md) for concrete incidents and repro findings.
