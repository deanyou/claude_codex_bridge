# CC_BRIDGE Config And Layout Contract

## 1. Purpose

This document defines the non-drifting user-facing contract for project configuration, pane layout, and tmux presentation in `cc-bridge_source`.

It is the authoritative design anchor for:

- `.cc-bridge/cc-bridge.config`
- `.cc-bridge/cc-bridge_memory.md` and `.cc-bridge/agents/<agent>/memory.md` project memory placement
...
- Effective config is resolved in three layers:
  ...
  2. user config at `~/.cc-bridge/cc-bridge.config`;
  3. project config at `.cc-bridge/cc-bridge.config`.
- Higher layers override lower layers by replacing the whole effective config; CC_BRIDGE does not merge partial config documents across layers.
- `.cc-bridge/cc-bridge.config` is the highest-priority user-facing project config file.
- `~/.cc-bridge/cc-bridge.config` is the user-level config file used only when project config is absent.
- CC_BRIDGE must not auto-create, reconstruct, or rewrite `.cc-bridge/cc-bridge.config`; it is a user-authored project file.
- When both `.cc-bridge/cc-bridge.config` and `~/.cc-bridge/cc-bridge.config` are absent, config loading must use the built-in default project config from code and report the source kind as `builtin_default`.
...
- `.cc-bridge/config.yaml` is not part of the contract and must not be read or written by current code.

### 2.1 Project Shared Memory Files

Project memory files are user context, not startup/layout authority.

- `.cc-bridge/cc-bridge_memory.md` under the project anchor is the shared CC_BRIDGE project memory file.
...
- `.cc-bridge/agents/<agent>/memory.md` is optional agent-private memory.
  - It is user-editable local data and must not be deleted or rewritten by
    normal startup.
  - `<agent>` is the normalized logical agent name from `.cc-bridge/cc-bridge.config`.
  - This path is anchored under the project `.cc-bridge/` directory and does not move
    with relocated runtime state.
- Generated project-memory metadata and runtime bundles are CC_BRIDGE-owned state:
  - `<runtime_state_root>/state/memory.seed.json` records template seed
    metadata for the initial `.cc-bridge/cc-bridge_memory.md`.
  - `<runtime_state_root>/runtime/memory/<agent>.md` is generated runtime
    memory for providers that need a stable file path.
  - `project_root/.cc-bridge/runtime/memory/<agent>.md` may be generated as a
...
- `.cc-bridge/` is local/runtime state by default. Projects that want to commit
  selected files such as `.cc-bridge/cc-bridge.config` must opt in with their own
  `.gitignore` whitelist.
...
- Built-in provider keys are currently `codex`, `claude`, `gemini`,
  ...
  the official DeepSeek Harness Web service with command `dsh`; CC_BRIDGE uses its
...
- Rich `cc-bridge.config` formats may define agents separately, but must still provide a `layout` compatible with the same leaf rules.

### 4.1 Compact Header With Agent Overlay

`cc-bridge.config` may combine a compact layout header with trailing TOML agent
overlays.
...
Rich or hybrid `cc-bridge.config` may define project-scoped maintenance heartbeat
policy:

```toml
[maintenance.heartbeat]
enabled = true
assessor = "cc-bridge_self"
interval_s = 3600
min_interval_s = 300
...
- `[maintenance.heartbeat]` is CC_BRIDGE runtime policy, not agent layout authority.
...
- `cc-bridge maintenance status` may read and report this policy.
- `cc-bridge maintenance tick` may use this policy to run a bounded one-shot
...
- `cc-bridge maintenance schedule --after <duration> [--reason <text>]` may write
  only the CC_BRIDGE-owned next heartbeat schedule. Requested delays shorter than
  `min_interval_s` are raised to `min_interval_s`.
- `cc-bridge maintenance enable` and `disable` must not edit `.cc-bridge/cc-bridge.config` until
  config editing policy is defined before those commands mutate behavior.
- `cc-bridge reload --dry-run` must classify a pure `[maintenance.heartbeat]` diff as
  `maintenance_change`, not `layout_change`.
- `cc-bridge reload` may publish a `maintenance_change` by refreshing the project
...
- `[tool_windows.<name>]` may declare a managed non-agent tmux window. Tool
  ...
  legacy `neovim`, `nvim`, and `cc-bridge-nvim` tool-window configurations are not
  supported.
...
- A project-authored `tool_windows.<name>.command` is executable authority. It
  ...
  materializes its pane. Built-in layout aliases are CC_BRIDGE-owned commands and do
...
- Tool windows must not appear in `cc-bridge ask` targets, provider runtime
...
- Runtime layout diagnostics may observe every tmux pane in a managed window,
  ...
  whose CC_BRIDGE identity matches that window's configured/effective `agent_names`;
...
- In `mode = "every_window"`, CC_BRIDGE treats `width` as a project-wide sidebar width regardless of left/right position. Topology refreshes must resize every managed sidebar pane to the same configured share of its tmux window so page/window switches do not leave sidebars at different widths. If the user drags a sidebar border, CC_BRIDGE stores that runtime column width in the project tmux session and applies it to every managed sidebar window until the session is recreated. If tmux later resizes a window because a terminal client attaches or changes size, CC_BRIDGE reapplies the stored runtime width instead of treating the auto-resized pane width as a new user preference.
...
- Legacy rich TOML that repeats `provider` or `workspace_mode` remains accepted
  when it does not conflict, but `cc-bridge config validate` reports style warnings
  and CC_BRIDGE-generated config must not introduce those redundant fields.
...
- Role id shorthand in `[windows]`, for example `agentroles.archi:codex`, is a
  ...
  `[agents.<name>].role`, or `cc-bridge roles add <role-id>:<provider> --agent <name>`.
- `cc-bridge ask <role-id> ...` is only an alias when exactly one configured agent is
...
- `[agents.<name>]` tables for names no longer referenced by `[windows]` are ignored as stale overlay residue and must not become configured agents or block startup.
  `cc-bridge config validate` reports them as style warnings so accidental misspelled
...
```toml
[tool_windows.files]
command = "CC_BRIDGE_WORKBENCH_PROFILE=rich CC_BRIDGE_WORKBENCH_FORCE_RICH=1 cc-bridge-workbench files"
label = "files"
```
...
or hybrid `cc-bridge.config` may use agent-local shortcut fields in the agent table:
...
- Changing API authority for an existing managed agent is provider-startup
  ...
  the user next starts CC_BRIDGE; that foreground start must create a fresh provider
...
- For Codex, CC_BRIDGE merges the overlay into the managed agent-local
...
- This is source configuration, not provider runtime state. Running providers
  may need the normal CC_BRIDGE reload/restart flow before a changed MCP set is
  visible.
...
hybrid `cc-bridge.config` may use an agent-local model shortcut:
...
- Pi model values may use the qualified `provider/model` form from the source
  user's `.pi/agent/models.json`; CC_BRIDGE passes the complete value through one
...
- DeepSeek V4 Pro and V4 Flash accept `off`, `high`, or `max`. CC_BRIDGE compiles
  these to the Deep Code CLI's `DEEPCODE_THINKING_ENABLED` and
  `DEEPCODE_REASONING_EFFORT` environment overrides.
- Other providers do not have a static CC_BRIDGE thinking mapping. Their `thinking`
...
For provider-specific launch wrappers, rich or hybrid `cc-bridge.config` may define an
...
- CC_BRIDGE first builds the normal provider command segment, including managed
  provider arguments, agent `startup_args`, and provider resume flags.
- CC_BRIDGE then replaces `{command}` with that provider command segment.
- The template wraps only the provider command segment. CC_BRIDGE-managed env prefixes,
...
`.cc-bridge/cc-bridge.config` cross this trust boundary. Configs without those fields,
user-level defaults, and CC_BRIDGE-owned layout aliases keep their existing behavior.
...
- Receipts live outside the repository under the user state root:
  `$XDG_STATE_HOME/cc-bridge/trust/project-commands` (or
  `~/.local/state/cc-bridge/trust/project-commands`) on Linux/macOS/WSL and
  `%LOCALAPPDATA%\CC_BRIDGE\trust\project-commands` on Windows.
- Interactive startup prints JSON-escaped field names and values and requires
  an explicit yes. `cc-bridge config approve-commands` provides the same deliberate
  review path.
- Missing, invalid, stale, or changed approval fails closed in non-interactive
  ...
  `cc-bridge config approve-commands`.
- `cc-bridge -s` continues to control provider auto-permission only; it neither grants
...
- `workspace_path` may be set on an agent with `workspace_mode =
  "git-worktree"` to point at an exact external worktree path. CC_BRIDGE validates the
...
- `workspace_group` may be set on an agent with `workspace_mode =
  "git-worktree"` to share a CC_BRIDGE-managed internal worktree with other agents in
  the same group. Group worktrees live under `.cc-bridge/workspaces/groups/<group>`
  and use branch `cc-bridge/group/<group>`.
- CC_BRIDGE may write an untracked root `.cc-bridge-workspace.json` binding into a managed
...
Rich or hybrid `cc-bridge.config` may define loop capacity policy under `[loop]`.
...
- `role` must be an installed Role Pack id in publisher-qualified form such as
  `agentroles.coder`.
- `provider` follows normal CC_BRIDGE provider naming. If `model` is set, the
...
- Loop profiles do not mount agents by themselves. They become runtime agents
  only when a CC_BRIDGE-owned command such as `cc-bridge loop capacity ensure` resolves
...
- Active loop capacity state is stored under the runtime-state root at
  ...
  loading as a CC_BRIDGE-owned runtime overlay while the loop is `ensured`.
- `config validate`, startup, and guarded reload see active loop-generated
  agents from that overlay, but the user-authored `.cc-bridge/cc-bridge.config` file must
  not be rewritten by `ensure`, `status`, or `release`.
- `cc-bridge loop capacity release --idle-only` marks loop-owned generated agents
...
- `.cc-bridge/cc-bridge.config` logical leaf names are the only authority for pane display names.
- Pane titles must be the exact logical names:
  - `cmd`
  - `agent1`, `agent2`, ...
- Pane border labels must show the logical pane name, not tmux pane numbers.
- Provider-specific pane markers such as `CC_BRIDGE-agent1-...` are internal runtime evidence only:
...
- Pane styling is session-scoped CC_BRIDGE UI state and must not permanently overwrite unrelated user tmux themes.
...
- Namespace creation or reuse must reapply session-scoped CC_BRIDGE tmux options on the project-owned socket.
- CC_BRIDGE-managed tmux sessions use vi copy/scroll mode by default (`mode-keys vi`) with a 10000-line scrollback history. Set `CC_BRIDGE_TMUX_HISTORY_LIMIT` to override the limit, including `v` to begin selection, `y` to copy and leave copy mode, and Vim-style pane navigation (`prefix+h/j/k/l`) plus pane resizing (`prefix+H/J/K/L`).
...
- Namespace state must also track the current visible layout signature derived from `.cc-bridge/cc-bridge.config` after foreground pruning.
...
- If `.cc-bridge/cc-bridge.config` grammar changes, update this document in the same patch.
