# CC_BRIDGE_DAEMON P3/P4 Mailbox And CLI Convergence Plan

## 1. Purpose

This document extracts `P3` and `P4` from
`docs/cc-bridge-daemon-ask-submit-fastpath-plan.md` into a narrower plan focused on:
...
- for `P3` and `P4`, this document supersedes the broad summary rows in
  `docs/cc-bridge-daemon-ask-submit-fastpath-plan.md`
...
- `ping('cc-bridge-daemon')` is already a light control-plane read path
...
- CLI `queue` / `inbox` now default to summary-first observer reads:
  - `cc-bridge queue <agent>` sends `detail=false`
  - `cc-bridge inbox <agent>` sends `detail=false`
...
- observer render/help wording is now aligned across the whole group:
  ...
  - terminal observer views still remain weaker than `ask` and redirect to
    `cc-bridge trace <id>` for lineage
...
- provider async-send hints no longer promote `cc-bridge pend` as a normal primary
...
- non-terminal observer notices redirect to `cc-bridge trace <id>`
...
- normal `cc-bridge queue <agent>` and `cc-bridge inbox <agent>` no longer linearly depend
...
| `P4a.2` | observer contract normalization | `lib/cli/services/*observer*`, `lib/mailbox_runtime/targets.py`, `lib/cc-bridge-daemon/socket_client_runtime/endpoints.py` |
