...
与“项目专属 tmux namespace + `cc-bridge`/`cc-bridge kill` 生命周期收敛”直接相关的详细实施方案，见：
...
- 每个 `.cc-bridge` 根目录只能启动唯一项目实例
- 每个 `.cc-bridge` 根目录只能存在唯一绑定权威
- 一个项目下可以管理任意多 agent
...
- `.cc-bridge` 已经是项目状态根
- `PathLayout(project_root)` 已经把大部分状态约束到 `.cc-bridge/` 下
- 项目配置已经收敛到项目本地，不再依赖全局 `~/.cc-bridge`
...
1. 项目身份双轨并存
   ...
   - 旧链路：`project_id.compute_cc-bridge_project_id(work_dir)`
...
3. 绑定真相不唯一
   - `.cc-bridge/.<provider>-session`
   - `.cc-bridge/agents/<agent>/runtime.json`
   - pane registry
   - pend/session path 回读链路

4. 启动和停止不是闭环
   - `cc-bridge start` 仍由 CLI 编排 workspace、pane、binding，再回写 askd
   - `cc-bridge kill` 仍依赖猜 pid、猜 tmux socket、猜 pane、猜 proc cwd
...
## 4. 命名决策：`askd` 重命名为 `cc-bridge-daemon`

结论：应该改。

原因不是“名字好听”，而是当前组件语义已经不是 ask-only daemon，而是项目级控制面。继续叫 `askd` 会持续误导架构边界。

### 4.1 为什么 `cc-bridge-daemon` 更合适

- `cc-bridge` 是项目级框架名
- 当前守护进程负责的不只是 ask，还包括启动、附着、绑定、恢复、状态、观测、停止
- 它的真实角色是 project control plane daemon
- 名字统一后，`.cc-bridge` 根目录下的控制面语义更清晰

### 4.2 命名切换的目标范围

本方案中的目标命名：

- `askd` 组件重命名为 `cc-bridge-daemon`
- `askd.sock` 重命名为 `cc-bridge-daemon.sock`
- `.cc-bridge/askd/` 重命名为 `.cc-bridge/cc-bridge-daemon/`
- 用户可见文档、状态输出、错误信息统一使用 `cc-bridge-daemon`
...
- 一个 `.cc-bridge` 根目录只有一个项目身份
- 一个 `.cc-bridge` 根目录只有一个控制面实例
...
- `project bootstrap` 只负责解析项目根、创建 `.cc-bridge`、准备配置
- `cc-bridge-daemon` 只负责控制面入口与协调
...
```text
cc-bridge doctor ps / cc-bridge ping / cc-bridge pend / cc-bridge doctor logs / cc-bridge pend --watch
  -> cc-bridge-daemon RPC
...
```text
cc-bridge kill
  -> cc-bridge-daemon shutdown/stop-all RPC
  -> runtime supervisor stops children
  -> supervisor clears bindings
  -> cc-bridge-daemon marks lease unmounted
...
- `.cc-bridge/config.yaml`

规则：

- 在当前目录执行 `cc-bridge`
- 如果当前目录有 `.cc-bridge`，直接使用
- 如果当前目录没有 `.cc-bridge`，向上找最近祖先 `.cc-bridge`
- 如果找到了祖先 `.cc-bridge`，当前目录视为项目子目录
- 如果没找到祖先 `.cc-bridge` 且当前目录不是项目子目录，自动创建 `.cc-bridge/config.yaml`

禁止项：

- `~/.cc-bridge/cc-bridge.config`
- `.cc-bridge/cc-bridge.config` 作为最终契约
- 全局回退
...
- `.cc-bridge/cc-bridge-daemon/` 是项目控制面域
- `.cc-bridge/agents/<agent>/` 是 agent authority 域
- `.cc-bridge/workspaces/` 是默认工作区物化域
...
| --- | --- | --- | --- | --- |
| P0 | 冻结新架构契约 | 确认 `askd -> cc-bridge-daemon`、`.cc-bridge/config.yaml`、唯一 runtime authority、唯一启动/停止闭环 | 继续新增旧路径设计 | 文档成为唯一执行基线 |
| P1 | 项目解析与配置收口 | 重写 project bootstrap；固定当前目录/祖先 `.cc-bridge`/本地初始化规则 | 全局 `~/.cc-bridge`、旧 `cc-bridge.config` 契约 | `cc-bridge` 在根目录、子目录、无 `.cc-bridge` 目录三种场景行为唯一且确定 |
...
| P3 | `cc-bridge-daemon` 控制面落地 | `askd` 重命名为 `cc-bridge-daemon`；调整 socket/lease/state path | `askd` 旧命名留在主输出和主路径中 | 启动/日志/错误/状态输出统一使用 `cc-bridge-daemon` |
| P4 | binding authority 落地 | 统一由 `cc-bridge-daemon` 写 `.cc-bridge/agents/<agent>/runtime.json` | provider session / registry 双重 authority | `ps/ping/pend/logs/kill` 只读 runtime authority 或 cc-bridge-daemon RPC |
| P5 | runtime supervisor 收口 | start/stop/reconcile 全部进 supervisor；CLI 变 thin client | CLI 本地 pane 编排和盲杀 | `cc-bridge kill` 不再依赖 blind pid/pane scan |
| P6 | 读路径 cutover | 删除 registry routing、ancestor session scan、daemon cwd fallback | `pane_registry_runtime` 主路由、resolution fallback | 所有读路径只走 `cc-bridge-daemon` 或 runtime authority |
...
- `.cc-bridge/config.yaml` 新契约
- 单一 project id 路径
- 旧 `project_id.py` 退出主链路

### 10.2 P3-P5 交付物

- `cc-bridge-daemon` 命名切换
- `runtime supervisor`
- `runtime authority store`
- `cc-bridge start/kill/ps/ping/pend/logs` 统一走控制面
...
- 固定项目发现规则
- 固定 `.cc-bridge/config.yaml` 新契约
- 结束 `cc-bridge.config` 作为最终契约的状态
...
1. 把 `CONFIG_FILENAME` 从 `cc-bridge.config` 改为 `config.yaml`
2. 重写默认配置模板与加载器
3. 明确三种入口语义
   - 当前目录已有 `.cc-bridge`
   - 当前目录无 `.cc-bridge` 但祖先有 `.cc-bridge`
   - 当前目录与祖先都无 `.cc-bridge`，则本地创建
4. 删掉对全局 `~/.cc-bridge/cc-bridge.config` 的任何依赖与文档表述

退出标准：

- 所有项目上下文都通过统一 bootstrap 规则建立
- 新项目初始化只生成 `.cc-bridge/config.yaml`
...
1. 盘点所有 `compute_cc-bridge_project_id` 调用点
2. 区分真正需要的是：
   - project id
   - workspace scope id
...
### 11.3 执行包 C：`askd -> cc-bridge-daemon` 命名切换
...
1. 先切用户可见命名
   ...
   - lease path
2. 再切目录命名
   - `.cc-bridge/askd/ -> .cc-bridge/cc-bridge-daemon/`
3. 最后切代码包命名
   - `askd` 包迁移到 `cc-bridge-daemon`

说明：

- 这一步可以分两次提交
- 但完成阶段退出前，不允许主输出里继续混用 askd/cc-bridge-daemon 两套名称

退出标准：

- 用户可见路径和输出统一只出现 `cc-bridge-daemon`

### 11.4 执行包 D：runtime authority 落地

目标：

- `.cc-bridge/agents/<agent>/runtime.json` 成为唯一 binding truth
...
- `lib/launcher/*`
- `lib/askd/app.py` 或未来 `lib/cc-bridge-daemon/app.py`
- `lib/askd/handlers/*`
...
- 从 `cc-bridge` 到 provider runtime 的主链只经过 control plane
...
- 正常 `cc-bridge kill` 不再读取 pane registry 或盲扫 pid 才能成功
...
2. 删除 registry 路由主路径
3. 把 `pend` 从 provider session 回推切到 authority / cc-bridge-daemon RPC
...
| 当前模块 | 当前问题 | 目标去向 |
| --- | --- | --- |
| `lib/project/resolver.py` | 仍带旧 bootstrap 语义残留 | 保留为 project bootstrap 主入口，按 `.cc-bridge/config.yaml` 新契约重写 |
...
| `lib/cli/services/kill.py` | 依赖 blind pid/tmux/proc scan | 改成纯 `cc-bridge-daemon stop-all` client |
| `lib/cli/services/daemon.py` | 仍是 askd 视角且夹杂 shutdown fallback | 重构为 `cc-bridge-daemon` client/bootstrap helper |
| `lib/askd/app.py` | 当前组件过重 | 重命名到 `lib/cc-bridge-daemon/app.py`，只保留控制面入口与 service wiring |
| `lib/askd/services/runtime.py` | binding authority 与 provider reload 混杂 | 拆成 runtime authority + runtime supervisor 协调层 |
| `lib/askd/services/ownership.py` | 语义正确但命名旧 | 迁移为 `cc-bridge-daemon` lease/ownership 模块 |
| `lib/provider_sessions/files.py` | session 查找仍做 ancestor scan | 降级为 provider artifact I/O，不再承担项目路由 |
| `lib/askd/client_runtime/resolution.py` | registry fallback、daemon cwd fallback、多头猜测 | 删除主路径，读路径统一改成 cc-bridge-daemon/runtime authority |
| `lib/pane_registry_runtime/*` | pane registry 参与项目路由 | 降为调试/观察用途，随后退出主架构 |
| `lib/pend_cli/session_paths.py` | 仍会从 session/registry 回推绑定 | 改成只读 cc-bridge-daemon/runtime authority |
...
- `test/test_v2_project_resolver_config_yaml.py`
  - 验证 `.cc-bridge/config.yaml` 新路径
  - 验证子目录向上解析
  - 验证无 `.cc-bridge` 时本地初始化
...
- 不再生成 `.cc-bridge/cc-bridge.config`
- 不再读取 `~/.cc-bridge/cc-bridge.config`
...
- `test/test_v2_cc-bridge-daemon_mount_ownership.py`
- `test/test_v2_cc-bridge-daemon_socket.py`

重点断言：

- 路径变为 `.cc-bridge/cc-bridge-daemon/*`
- 输出文案只出现 `cc-bridge-daemon`
...
- `test/test_v2_cc-bridge-daemon_start_flow.py`
  - 验证 `cc-bridge -> cc-bridge-daemon -> supervisor -> runtime authority`
...
- `test/test_v2_cc-bridge-daemon_stop_all.py`
...
- 在项目根执行 `cc-bridge`，存在 `.cc-bridge/config.yaml` 时直接启动
- 在项目子目录执行 `cc-bridge`，正确绑定到最近祖先 `.cc-bridge`
- 在无 `.cc-bridge` 的普通目录执行 `cc-bridge`，自动创建 `.cc-bridge/config.yaml`
- 不再从全局目录或根目录外 session 文件推导项目

### 13.2 唯一实例验收

- 同一 `.cc-bridge` 根并发执行两次 `cc-bridge`，只能存在一个 `cc-bridge-daemon`
- `lease.json + cc-bridge-daemon.sock` 状态一致
- `cc-bridge-daemon kill` 后 lease 状态明确变为 unmounted
...
- `cc-bridge kill` 不依赖 blind pane scan 仍可完成清理
- `cc-bridge kill -f` 只作为补充强制路径，不作为正常主路径
...
- `askd` 应重命名为 `cc-bridge-daemon`
- `cc-bridge-daemon` 是唯一项目控制面入口
- `.cc-bridge/agents/<agent>/runtime.json` 是唯一运行时绑定权威
- `.cc-bridge/config.yaml` 是唯一项目配置契约
- CLI 必须变成 thin client
- pane、registry、provider session 都必须从主 authority 退位

只有这样，`一个 .cc-bridge 根目录只能启动唯一 cc-bridge、唯一绑定、管理多 agent` 才不是口号，而是结构上成立的系统事实。
