# CC_BRIDGE Provider State Storage Boundary Plan

## 1. Purpose

This plan defines the code-level and layout-level boundary for storage under
`.cc-bridge/`, especially provider-managed state under:

```text
.cc-bridge/agents/<agent>/provider-state/<provider>/
.cc-bridge/provider-profiles/
.cc-bridge/shared-cache/
```
...
breaking `cc-bridge-daemon` / `ask` stability.

Superseding decision, 2026-07-23:

- CC_BRIDGE no longer creates or depends on project-scoped Claude/Gemini caches under
  `~/.cache/cc-bridge/projects/<project-id-prefix>/provider-cache/`.
- Managed Claude panes use the user-installed executable and disable Claude's
  self-updater. CC_BRIDGE does not copy provider binaries into project cache.
- Managed Gemini panes use one user-scoped rebuildable cache under
  `~/.cache/cc-bridge/provider-cache/gemini/`; cache identity is not project or
  session authority.
- Existing project-scoped caches are legacy residue. Startup detaches only
  recognized CC_BRIDGE-owned Claude cache links, while explicit stopped-project
  cleanup removes the current legacy cache. Cross-project orphan cleanup
  requires `cc-bridge cleanup --legacy-provider-caches` and a valid manifest whose
...
- [docs/cc-bridge-config-layout-contract.md](/home/bfly/yunwei/cc-bridge_source/docs/cc-bridge-config-layout-contract.md)
...
- [docs/gemini-session-isolation-contract.md](/home/bfly/yunwei/cc-bridge_source/docs/gemini-session-isolation-contract.md)
...
Online `cc-bridge-daemon` views are not allowed to depend on future storage cleanup to be
...
Observed local `.cc-bridge` shape:

```text
.cc-bridge/cc-bridge-daemon/                         small control-plane ledger
.cc-bridge/agents/<agent>/runtime.json    agent runtime authority
.cc-bridge/agents/<agent>/provider-state  managed provider home/state
.cc-bridge/provider-profiles/             currently may contain profile runtime homes
```

The control-plane portion is not the storage problem. In the observed project,
`.cc-bridge/cc-bridge-daemon` is small, while most disk usage comes from provider-managed homes:
...
- Codex runtime home data under `.cc-bridge/provider-profiles/<agent>/codex/`
...
- `agents_dir` is rooted at `runtime_state_root`, but `provider_profiles_dir` is
  rooted at the anchor `.cc-bridge`, so WSL relocation can split runtime state from
  provider profile state.
- provider profiles can become runtime homes for Codex when a profile has
  explicit env/home authority, which makes `.cc-bridge/provider-profiles/` hold
...
CC_BRIDGE must classify project storage before it attempts cleanup or deduplication.
...
- `.cc-bridge/agents/<agent>/agent.json`
...
- Codex `.cc-bridge-session-namespace.json` inside the managed home
...
- `.cc-bridge/agents/<agent>/provider-runtime/<provider>/`
- `<runtime_state_root>/runtime/memory/<agent>.md`
- `project_root/.cc-bridge/runtime/memory/<agent>.md` provider compatibility bridge
...
- Droid uses
  `.cc-bridge/agents/<agent>/provider-state/droid/home/` as its private OS home and
...
CC_BRIDGE-managed projection. If symlinks are unavailable, startup may fall back to a
marked copy.
...
- OpenCode generated `opencode.json` and generated ask skill instruction files
  under `.cc-bridge/runtime/skills/<agent>/opencode/`
...
marker with schema version 1, record type `cc-bridge_projected_asset`, the exact
...
- Codex auth sidecars such as `company-codex-api-key`,
  `company-codex.config.toml`, and `.cc-bridge-auth-projection.json`
...
- `.cc-bridge/history/` handoff/context-transfer documents
- `.cc-bridge/cc-bridge_memory.md` project shared memory, when present under the project anchor
- `.cc-bridge/agents/<agent>/memory.md` agent-private memory anchored under the
  project `.cc-bridge/` directory
...
- unknown `.cc-bridge/agents/<unknown-agent>/` directories
...
```text
~/.cache/cc-bridge/provider-cache/
  gemini/
    MANIFEST.json
    npm/
    xdg/
...
- `.cc-bridge/agents/<agent>/provider-state/<provider>/home` remains the default
  managed session boundary.
- Managed Grok may copy inherited system `.grok/auth.json` and `.grok/config.toml`
  into `.cc-bridge/agents/<agent>/provider-state/grok/home/.grok/` when profile
...
- `.cc-bridge/provider-profiles/` must not silently become a long-lived runtime home
  unless the user explicitly configures that path as an external provider home.
- `.cc-bridge/shared-cache/` contains only rebuildable cache and never conversation
...
- New Claude/Gemini startup paths must not create provider cache under
  `.cc-bridge/shared-cache/` or `~/.cache/cc-bridge/projects/`.
...
- classify paths under `.cc-bridge/cc-bridge-daemon`
- classify paths under `.cc-bridge/agents/<agent>`
- classify provider-state subtrees by provider-specific rules
- classify `.cc-bridge/provider-profiles`
...
- detect `.cc-bridge/provider-profiles/<agent>/codex/` trees that already contain
  sessions, auth, logs, plugin bundles, or other runtime-home data
- move or copy that runtime-home data into
  `.cc-bridge/agents/<agent>/provider-state/codex/home/` only when the target is
...
- after migration, non-explicit Codex profiles resolve to
  `.cc-bridge/agents/<agent>/provider-state/codex/home/`; only explicit
...
- prevent default profile materialization from creating session/log/cache data
  under `.cc-bridge/provider-profiles`
...
`cc-bridge cleanup`. `cc-bridge doctor storage` remains the inspection path; cleanup itself
...
- hold the same project startup/lifecycle lock used by `cc-bridge-daemon` startup while
  checking state and pruning, so cleanup cannot race a concurrent backend start
- never touch session roots or auth files
- refuse to run while `cc-bridge-daemon` is active or pending/running `ask` jobs exist
...
- detach only Claude symlinks that resolve exactly to a recognized legacy CC_BRIDGE
...
- after a successful project `cc-bridge kill`, retry removal of that project's
...
- `cc-bridge doctor storage` explains cache vs. authority before cleanup
- repeated cleanup is stable and idempotent
- provider startup and ask completion semantics are unchanged
- `cc-bridge cleanup` deletes only safe rebuildable cache and preserves authority,
...
`cc-bridge cleanup` command and must not broaden that command's deletion classes.
...
- managed Claude startup must not create, copy, hash, or select binaries under
  a CC_BRIDGE project cache
- managed Claude startup must disable provider self-update and detach
  recognized legacy CC_BRIDGE cache symlinks without deleting cache payload during
  startup
...
- user-scoped Gemini cache resolution must not recursively nest when a managed
  Gemini process invokes CC_BRIDGE with its own `XDG_CACHE_HOME`
...
- a new Claude/Gemini project creates no
  `~/.cache/cc-bridge/projects/<id>/provider-cache`
- two projects use the same user-scoped Gemini rebuildable cache
- Claude executes from the user/provider installation managed by `cc-bridge update`
...
- `.cc-bridge-session-namespace.json`
- project-scoped `.codex-<agent>-session`
...
- auth sidecars copied from the source Codex home, including
  ...
  `.cc-bridge-auth-projection.json` evidence manifest
...
- `.tmp/plugins/`; the real bundle may live under
  `.cc-bridge/shared-cache/codex/plugin-bundles/<sha>/`, with managed homes pointing
...
- managed `logs_2.sqlite` may be an agent-scoped symlink to the CC_BRIDGE temporary
...
- retired project cache under
  `~/.cache/cc-bridge/projects/<project-id-prefix>/provider-cache/claude/`
...
through CC_BRIDGE shared cache without a future provider-supported ownership design.
...
- `NPM_CONFIG_CACHE` and `npm_config_cache` routed to
  `~/.cache/cc-bridge/provider-cache/gemini/npm`
- `XDG_CACHE_HOME` routed to
  `~/.cache/cc-bridge/provider-cache/gemini/xdg`
...
`cc-bridge update` may persist non-secret, user-wide provider update metadata at:

```text
$XDG_STATE_HOME/cc-bridge/provider-updates.json
```

with `~/.local/state/cc-bridge/provider-updates.json` as the fallback. This file is
...
only the explicit CC_BRIDGE update flow owns discovery and mutation.
...
An explicit user-provided `--config-dir` overrides CC_BRIDGE's managed root and must
be preserved without adding a second option. CC_BRIDGE diagnostics may report that
...
- default behavior should continue to use project-local `.cc-bridge` when safe
...
- `cc-bridge doctor storage` must separately report the stopped current project's
  ...
  contain a large npm tree. These values are outside the `.cc-bridge` `total_bytes`
...
- classify `.cc-bridge/cc-bridge-daemon` authority vs events
- classify agent runtime files as authority/runtime
- classify Codex session roots as session authority
- classify Codex `.cc-bridge-session-namespace.json` as session authority
...
- cleanup refuses to run while cc-bridge-daemon or ask jobs are active
...
- `cc-bridge ask` still completes after storage audit
- `cc-bridge-daemon` restart still restores managed provider sessions
...
5. Implement `cc-bridge cleanup` as the single conservative provider-cache cleanup
   command.
6. Keep `cc-bridge doctor storage` as the provider-cache cleanup preview/audit
...
8. Retire project-scoped Claude/Gemini cache generation, migrate recognized
   ...
   installed post-update runner with active projects deferred to `cc-bridge kill`.
...
- Do not make `.cc-bridge/agents/<agent>/provider-state` globally shared.
...
- Do not run destructive provider-cache `cc-bridge cleanup` concurrently with an active backend:
  cleanup must acquire the project startup/lifecycle guard, confirm `cc-bridge-daemon` is
...
- add `cc-bridge doctor storage --json`
- add human render for `cc-bridge doctor storage`
- include `schema_version`, primary class, provider, agent, size, active flags,
  reclaimability, and reason fields in JSON output
- add tests with synthetic `.cc-bridge` trees
...
- `cc-bridge doctor storage` and `cc-bridge doctor storage --json` expose storage totals and
  per-entry class/provider/agent/size metadata.
- `cc-bridge doctor storage` reports `shared_cache_status=enabled` for usable project
...
- `cc-bridge doctor storage` emits `shared_cache_root` and
...
- Copilot `config.json` classifies as `SECRET` because it mixes authentication
  and application state. Its CC_BRIDGE-owned installed plugin trees and markers
...
- Codex `.cc-bridge-session-namespace.json` and Gemini `.gemini/tmp/` classify as
...
- `.cc-bridge/history/` classifies as `USER_CONTENT`, not provider session state.
- `.cc-bridge/workspaces/` classifies as `WORKSPACE`, not residue.
- Non-Codex `provider_profile.home` is rejected at config load and materializer
  ...
  `.cc-bridge/agents/<agent>/provider-state/<provider>/home` paths.
- Phase A.5 Codex migration moves old default
  `.cc-bridge/provider-profiles/<agent>/codex/` runtime-home data into
  `.cc-bridge/agents/<agent>/provider-state/codex/home/` only after session
...
- Non-explicit Codex resolved profile records leave `profile_root` unset so
  ...
  `.cc-bridge/provider-profiles/<agent>/codex/` runtime-home path.
...
- Startup preparation rejects duplicate effective provider runtime homes before
  provider launch.
- `cc-bridge config validate` rejects duplicate effective provider runtime homes
  before startup as well.
- `cc-bridge cleanup` is implemented as the single cleanup entrypoint. It refuses to
  run while `cc-bridge-daemon` is active or ask jobs are pending/running, prunes old local
  Claude version residue, removes rebuildable Claude/Gemini cache residue,
  safely detaches CC_BRIDGE-owned legacy Claude cache links, removes the stopped
...
- `cc-bridge cleanup --legacy-provider-caches` additionally removes provider caches
  for recorded project roots that no longer exist. It validates the CC_BRIDGE
...
- After a real version change, the newly installed `cc-bridge __post-update` runner
...
- Automatic migration does not perform a separate recursive byte-size walk,
  ...
  bounded result in `$XDG_STATE_HOME/cc-bridge/provider-cache-cleanup.json`.
- `cc-bridge update --no-cache-cleanup` skips this migration for one update. Cleanup
...
- `cc-bridge cleanup` holds the project `startup.lock` while re-checking backend/job
...
- storage diagnostics report the retired current-project cache as a separate
  ...
  beside `.cc-bridge` totals without slowing normal diagnostics.
- managed Claude startup exports `DISABLE_AUTOUPDATER=1`,
  `DISABLE_LOGIN_COMMAND=1`, and `DISABLE_LOGOUT_COMMAND=1`, consumes the
  user-installed executable, and does not create a CC_BRIDGE binary cache or writable
...
- managed Gemini startup disables update checks and routes npm/XDG cache to one
  user-scoped `~/.cache/cc-bridge/provider-cache/gemini/` tree without recursive
  cache nesting.
- 2026-07-23 cache-retirement validation:
  ...
  - an isolated external `cc-bridge_test kill` removed a synthetic current-project
...
- Linux real validation passed with the current Phase A-C implementation:
  ...
  - shortened Linux soak after the shutdown reply-delivery fix:
    `CC_BRIDGE_LINUX_SOAK_SECONDS=180 CC_BRIDGE_LINUX_SOAK_KILL_EVERY=3
  ...
  - real cleanup validation on the soak project passed after injecting Claude
    and Gemini cache residue: no pending jobs remained, `cc-bridge cleanup` removed
...
- The real-platform GitHub Actions workflow
  `.github/workflows/cc-bridge-daemon-real-platform.yml` now includes macOS and WSL
  `cc-bridge doctor storage --json` plus `cc-bridge cleanup` smoke steps. Those steps
...
- Remote macOS and WSL validation passed on GitHub Actions run
  ...
  - macOS real cc-bridge-daemon/ask smoke passed in `5m27s`, including lifecycle smoke,
    communication matrix, short soak, fastpath stress, and storage cleanup
    smoke.
  - WSL mounted-drive cc-bridge-daemon/ask smoke passed in `8m40s`, including lifecycle
