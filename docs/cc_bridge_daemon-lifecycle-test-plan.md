# CC_BRIDGE_DAEMON 生命周期稳定性测试方案

## 1. 文档定位

这份文档是 `docs/cc-bridge-daemon-lifecycle-stability-plan.md` 的独立测试执行方案。
...
- 同项目双终端并发 `cc-bridge` / `cc-bridge ask`
- `kill` 与 `start` / `ask` 并发
- 冷启动 `ask` 与交互式 `cc-bridge` 的 readiness 分层
...
- 同项目两个 CLI 几乎同时发起 `cc-bridge`，最终只产生一个 keeper 和一个 mounted generation
- 同项目两个 CLI 几乎同时发起 `cc-bridge ask`，第二个请求必须等待已存在的 `starting` 事务
- 冷启动事务耗时超过旧 5 秒预算但小于 `startup_transaction_timeout_s` 时，`cc-bridge ask` 仍能成功提交
- 已 mounted 热路径下的 `cc-bridge ask` / `ping` 不得接近 `startup_transaction_timeout_s`
- namespace/UI readiness 故意延迟时，`cc-bridge ask` 仍可在 control-plane mounted 后成功；交互式 `cc-bridge` 才继续等待 attach
- `phase=starting` 时 child 启动失败，系统稳定落到 `phase=failed`
- `phase=mounted` 时 config 改动，keeper 通过 orderly restart 完成 generation 切换
- socket 不可连但 pid 仍存活时，普通 `cc-bridge kill` 能经 keeper 收口，而不是被 degraded 拒绝
- `cc-bridge kill` 与新的 `cc-bridge ask` 并发时，新的 ask 只能等待停机结果，不得触发第二条启动链
- 从旧项目状态启动：
  - 只有 `lease.json`
  - 没有 `lifecycle.json`
  - 没有 `helper.json`
...
以下用例是固定的真实 `cc-bridge` 黑盒生命周期基线，必须通过 `cc-bridge` 入口拉起项目，而不是只调用后台 service：

- `test/test_v2_phase2_entrypoint.py::test_cc-bridge_v2_project_lifecycle`
- `test/test_v2_phase2_entrypoint.py::test_cc-bridge_ping_cc-bridge-daemon_recovers_from_stale_mount_and_bumps_generation`
- `test/test_v2_phase2_entrypoint.py::test_cc-bridge_long_running_job_keeps_heartbeat_and_doctor_healthy`
- `test/test_v2_phase2_entrypoint.py::test_cc-bridge_fake_provider_recovers_running_execution_after_cc-bridge-daemon_restart`
- `test/test_v2_cc-bridge-daemon_start_matrix.py::test_cc-bridge_start_restarts_dead_daemon_on_subsequent_start`

统一执行入口：

- `python -m pytest -q -m cc-bridge_lifecycle_smoke test/test_v2_phase2_entrypoint.py test/test_v2_cc-bridge-daemon_start_matrix.py`
...
- 多项目并行运行，各自 keeper/cc-bridge-daemon 独立
...
- `keeper` 数量
- `cc-bridge-daemon` generation 数量
...
- 多项目并发压测
  - 10 至 30 个项目并行 `cc-bridge` / `cc-bridge ask`
  - 验证互不接管、互不清理、互不复用 keeper
- 单项目高频 ask 压测
  - 1 个项目、多个 agent、循环 ask 1000 次以上
  - 观察 helper group 数量是否稳定
- kill/restart 抖动压测
  - 反复执行 `cc-bridge ask`、`cc-bridge kill`、`cc-bridge`
...
- `cc-bridge ask` 与交互式 `cc-bridge` 已验证等待不同 readiness 层
- 普通 `cc-bridge kill` 覆盖 degraded/socket_unreachable 场景
...
- 新项目首次 `cc-bridge`
- 已有项目 `cc-bridge` 恢复历史
- `cc-bridge ask agentX` 正常提交与回复
- `cc-bridge kill` 后当前项目不再保留 mounted backend
- `cc-bridge kill -f` 可清理异常状态
- 多项目同时运行互不影响
...
- 项目路径
- phase 变化
- keeper pid / cc-bridge-daemon pid
- helper group 数量前后变化
- 是否出现 orphan residue
...
本节是 `v6.0.29` 之后 cc-bridge-daemon / ask / mailbox observer 稳定化工作的真实验证方案。
...
真实验证按 7 层执行。越往后越接近用户真实环境，失败优先记录到 `docs/cc-bridge-daemon-manual-test-issue-log.md`，再决定是否补自动化。
...
| C. Lifecycle recovery | 验证 kill / crash / restart / authority 收口 | ask 中 kill、cc-bridge-daemon crash、socket 删除、tmux pane 死亡、stop_all 后 restart | 单 backend generation；runtime/job 不回退；queued jobs 收敛到 terminal 或可恢复状态 |
...
```bash
pytest -q
pytest -q test/test_v2_cc-bridge-daemon_socket.py test/test_cc-bridge-daemon_socket_server_loop.py test/test_v2_message_bureau_dispatcher_integration.py -k 'tracker or interrupted_abort or stop_all or roundtrip or start_flow'
...
以下场景应尽量用真实 `cc-bridge` 命令、真实 project `.cc-bridge`、真实 tmux session 执行：

- `cc-bridge ask` 已提交但 provider 未 terminal 时执行 `cc-bridge kill`
- 删除 project socket 后执行 `cc-bridge ping cc-bridge-daemon`、`cc-bridge ask`、`cc-bridge kill`
- kill cc-bridge-daemon pid 后执行 `cc-bridge ask`，确认 keeper / lifecycle 进入确定性恢复
...
- `cc-bridge kill` 能收口到无 project-owned backend / helper residue

macOS 最小必跑：

```bash
pytest -q -m cc-bridge_lifecycle_smoke test/test_v2_phase2_entrypoint.py test/test_v2_cc-bridge-daemon_start_matrix.py
bash test/system_comm_matrix.sh
pytest -q test/test_v2_cc-bridge-daemon_socket.py -k 'tracker or interrupted_abort or roundtrip'
```
...
- Linux native path，例如 `~/tmp/cc-bridge-wsl-native`
- Windows mounted drive path，例如 `/mnt/c/tmp/cc-bridge-wsl-mounted`

每类都要验证：

- `cc-bridge` cold start
- `cc-bridge ask <agent>` receipt
- `cc-bridge trace <id>` lineage inspection
...
- socket / cc-bridge-daemon runtime storage relocation 发生在 Linux-local state root
- logical `.cc-bridge` 呈现仍指向项目 anchor
- `doctor` 能显示 relocation reason，且区分 WSL mounted drive 与 ordinary long path fallback
- Unix shell 文件不因 CRLF 破坏执行

Windows PowerShell bootstrap 独立验证，不与本节 Linux/WSL cc-bridge-daemon runtime 测试混在同一轮中。
...
| S2 | 2 小时 | 多项目并行 ask / provider terminal / doctor | 项目间 keeper / cc-bridge-daemon / socket / runtime authority 不串扰 |
...
只有满足以下条件，才能把 `v6.0.29` 后 cc-bridge-daemon / ask 稳定化视为达到设计目标：
...
- `CC_BRIDGE_LINUX_SOAK_SECONDS=90 CC_BRIDGE_LINUX_SOAK_KILL_EVERY=2 CC_BRIDGE_LINUX_SOAK_STUB_DELAY=0.2 CC_BRIDGE_LINUX_SOAK_ASK_WAIT_TIMEOUT_S=90 bash test/system_linux_soak.sh`
  - 结果：8 轮 ask/wait/pend/doctor，4 次 kill/restart，全部通过
  - 指标：p50 `200ms`，p95 `244ms`，max `244ms`
  - artifacts：`/home/bfly/yunwei/test_cc-bridge_linux_soak_20260509183038-1091008/project`
- `CC_BRIDGE_LINUX_SOAK_SECONDS=300 CC_BRIDGE_LINUX_SOAK_KILL_EVERY=3 CC_BRIDGE_LINUX_SOAK_STUB_DELAY=0.2 CC_BRIDGE_LINUX_SOAK_ASK_WAIT_TIMEOUT_S=90 bash test/system_linux_soak.sh`
  - 结果：23 轮 ask/wait/pend/doctor，7 次 kill/restart，全部通过
  - 指标：p50 `204ms`，p95 `247ms`，max `270ms`
  - artifacts：`/home/bfly/yunwei/test_cc-bridge_linux_soak_20260509183857-1727628/project`
- `bash test/system_fastpath_stress.sh`
  ...
  - artifacts：`/home/bfly/yunwei/test_cc-bridge_fastpath_20260509185600-3149944/project`
