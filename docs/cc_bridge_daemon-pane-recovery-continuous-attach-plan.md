# CC_BRIDGE_DAEMON Pane 恢复与前台连续性详细方案

## 1. 文档定位

这份文档是 `cc-bridge-daemon` 启动/监督契约在 “pane 死亡恢复” 场景下的详细实施方案，目标是解决当前的核心结构问题：

- 杀死单个 pane 会导致整个 `cc-bridge` 前台退出
- daemon 已有恢复能力，但恢复层级选择过重
- `.cc-bridge/cc-bridge.config` 的 canonical layout 与前台 attach 连续性被错误耦合
...
当前在 `cc-bridge` 已打开的前台里杀死一个 agent pane，常见表现是：

- 当前 `cc-bridge` 前台直接退出
- 看起来像“没有自动恢复”
- 再次执行 `cc-bridge` 后项目可能重新拉起，但原 attach 已经断开
...
6. 当前前台 `cc-bridge` 只是一次 `tmux attach-session`
...
- 恢复 pane 到 `.cc-bridge/cc-bridge.config` 定义的 canonical 布局位置
- 维持前台 attach 连续性
...
1. 杀死单个 pane 时，当前前台 `cc-bridge` 不退出
2. pane 恢复后优先回到原 logical slot，而不是漂移到任意位置
3. `.cc-bridge/cc-bridge.config` 仍然是布局 authority
4. `cc-bridge-daemon` 仍然是唯一恢复 authority
5. `cc-bridge kill` 仍然是唯一允许销毁整个项目 tmux server 的正常入口
...
每个 `.cc-bridge` 项目采用以下 tmux 拓扑：

```text
project .cc-bridge
  -> one cc-bridge-daemon backend
  -> one project tmux server/socket
  -> one long-lived tmux session
      -> one hidden control window (__cc-bridge_ctl)
      -> one visible workspace window (cc-bridge)
          -> cmd / agent1 / agent2 / ...
```
...
`cc-bridge` 前台 attach 阶段的连续性必须建立在：
...
1. `.cc-bridge/cc-bridge.config`
2. `.cc-bridge/cc-bridge-daemon/lease.json`
3. `.cc-bridge/cc-bridge-daemon/project-namespace-state.json`
4. `.cc-bridge/agents/<agent>/runtime.json`
...
- `slot_key` 永远对应 `.cc-bridge/cc-bridge.config` 的逻辑叶子
...
3. 根据 `.cc-bridge/cc-bridge.config` 完整 materialize 新布局
...
- `cc-bridge kill` / 显式 destructive reset
...
`cc-bridge` 的 foreground attach 逻辑固定为：
...
- `lib/cc-bridge-daemon/services/project_namespace_runtime/workspace.py`
- `lib/cc-bridge-daemon/services/project_namespace_runtime/slot_assignment.py`
...
`lib/cc-bridge-daemon/supervision/loop_runtime.py`

- 将 `pane-dead` 的默认恢复从 namespace reflow 降级为 local recovery first
- `should_reflow_project_namespace()` 不再作为首选路径

`lib/cc-bridge-daemon/supervision/recovery_transitions.py`
...
`lib/cc-bridge-daemon/app_runtime/policy.py`

- 拆分 `remount_project_from_policy()`
- 让 policy 层显式表达恢复等级

`lib/cc-bridge-daemon/services/project_namespace_runtime/ensure_state.py`
...
- 杀死单个 pane 不再导致整个 `cc-bridge` 前台退出
...
- `cc-bridge` foreground attach 对 workspace reflow 透明
...
- `cc-bridge kill` 仍销毁 control window + workspace window + tmux server
...
1. 杀死任意 agent pane，当前前台 `cc-bridge` 不退出
2. 杀死 `cmd` pane，前台仍在同一 session 中
3. 恢复后的 pane 回到 `.cc-bridge/cc-bridge.config` 的原 logical slot
4. 只有 `cc-bridge kill` 或 backend 严重损坏才会销毁 tmux server
