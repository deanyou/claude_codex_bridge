...
This document is the authoritative CC_BRIDGE contract for the official DeepSeek
...
DeepSeek Harness is Developer Preview software.  CC_BRIDGE must fail closed if a
...
CLI for CC_BRIDGE purposes: the headless task path creates a fresh persisted Agent
per invocation and cannot provide Codex-like mounted-session continuity by
itself.  CC_BRIDGE therefore uses the structured Web session carrier for native
session reuse, history recovery, approvals, cancellation, and compaction.

## 2. Runtime Boundary

DSH is a service-backed harness, not an interactive terminal provider.  CC_BRIDGE
starts:
...
The current POSIX CC_BRIDGE launcher may keep the host wrapper inside the Agent's
...
- CC_BRIDGE never submits a DSH prompt with pane input;
...
- `cc-bridge clear` and `cc-bridge compact` do not send pane keys for DSH.
...
The DSH host binds only to loopback.  CC_BRIDGE rejects non-loopback, credentialed,
...
Each CC_BRIDGE Agent owns one isolated managed `DSH_HOME`.  The launcher projects
...
- optional user skills, Role skills, and required CC_BRIDGE control skills under
  `skills/`;
- the generated CC_BRIDGE memory bundle at `AGENTS.md`.

Projection is one-way.  CC_BRIDGE does not copy a user's native session store,
...
The Agent session binding records the exact CC_BRIDGE project, Agent, workspace,
...
1. CC_BRIDGE opens the DSH event WebSocket before prompt submission.
2. CC_BRIDGE creates or joins the exact managed DSH session.
3. Model/provider/reasoning selection is applied with the native session RPC
   when configured.
4. CC_BRIDGE submits `session.prompt` with RPC id `J`.
5. The prompt retains the leading `CC_BRIDGE_REQ_ID: J` correlation line even for a
   no-wrap CC_BRIDGE request.
...
1. a committed `user/message` has `source.kind == "user"` and
   `source.rpcId == J`;
2. its text begins with the exact `CC_BRIDGE_REQ_ID: J` line;
...
successful CC_BRIDGE completion.
...
cc-bridge-daemon restart it may reconnect an observer to that same request and page
...
CC_BRIDGE cancellation sends native `session.cancel` for the bound session when the
...
- DSH has native `/compact`.  `cc-bridge compact <agent>` invokes it through the
...
- DSH has no native `/clear`.  `cc-bridge clear <agent>` rotates the managed native
  session id and context generation without deleting old DSH logs, CC_BRIDGE jobs,
...
- Both operations use the existing CC_BRIDGE outstanding-work gate and must not
...
- DSH question requests are never guessed or automatically answered.  CC_BRIDGE
...
not CC_BRIDGE integration evidence.
...
- clear/compact tests proving DSH uses its API/session control path rather than
  pane input;
- an isolated official-host probe from the external CC_BRIDGE test project.  Without
