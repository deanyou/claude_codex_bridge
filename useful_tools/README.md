...
This directory contains optional tools that are useful for CC_BRIDGE users but are not
...
Then restart or relaunch the managed CC_BRIDGE agent so its isolated Codex home can
...
Then restart or relaunch the managed CC_BRIDGE agent so its isolated Claude home can
inherit the skill. This affects same-provider agents whose
`provider_profile.inherit_skills` is true.

## Agent-Specific Requests

`.cc-bridge/cc-bridge.config` currently has `provider_profile.inherit_skills`, which turns
provider-home skill inheritance on or off as a whole. It does not have a
per-skill allowlist.

For requests such as "inject plan-tree into agent2", use the `cc-bridge-config` skill
...
```bash
mkdir -p .cc-bridge/agents/<agent>/provider-state/codex/home/skills
cp -R useful_tools/codex_skills/plan-tree .cc-bridge/agents/<agent>/provider-state/codex/home/skills/
```

Temporary Claude runtime copy:

```bash
mkdir -p .cc-bridge/agents/<agent>/provider-state/claude/home/.claude/skills
cp -R useful_tools/claude_skills/plan-tree .cc-bridge/agents/<agent>/provider-state/claude/home/.claude/skills/
```

Replace `<agent>` with the configured CC_BRIDGE agent name. Before using temporary
