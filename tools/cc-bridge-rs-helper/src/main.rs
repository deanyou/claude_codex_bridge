...
const HELPER_NAME: &str = "cc_bridge-rs-helper";
...
fn classify_storage_relative(
    ...
    if parts.len() >= 2 && first == "cc_bridge_daemon" {
        return classify_cc_bridge_daemon_record(record, parts);
    }
...
fn classify_cc_bridge_daemon_record(record: &StorageInventoryRecord, parts: &[String]) -> StorageSummaryEntry {
    let Some(name) = parts.last() else {
        return storage_entry(record, "unknown", None, None, None, None, None, None, None);
    };
    let top = parts.get(1).map(String::as_str).unwrap_or("");
    if parts.len() == 2 && cc_bridge_daemon_authority_file(name) {
        return storage_entry(
            record,
            "authority",
            None,
    ...
    if cc_bridge_daemon_runtime_dir(top)
        || name.ends_with(".pid")
        || name.ends_with(".sock")
        || name.ends_with(".lock")
    {
    ...
    if name.ends_with(".jsonl") || name.ends_with(".log") {
        return storage_entry(
            ...
            None,
            None,
            Some("cc_bridge_daemon_event_log"),
        );
    }
...
fn classify_provider_home_record(
    ...
    if name.ends_with(".cc_bridge-projection.json") {
        return storage_entry(
            record,
            "projected_config",
            provider_ref,
...
fn cc_bridge_daemon_authority_file(name: &str) -> bool {
    matches!(
        name,
        "keeper.json"
            | "lease.json"
...
fn cc_bridge_daemon_runtime_dir(name: &str) -> bool {
...
fn unsafe_symlink_reason(path: &Path, cc_bridge_dir: &Path, runtime_state_root: &Path) -> Option<String> {
    let Ok(target) = path.canonicalize() else {
        return Some("symlink_target_missing".to_string());
    };
    if path_within(&target, cc_bridge_dir) || path_within(&target, runtime_state_root) {
        return None;
    }
...
fn is_marked_projected_symlink(path: &Path) -> bool {
    let marker_path = PathBuf::from(format!("{}.cc_bridge-projection.json", path.to_string_lossy()));
    ...
    if payload.get("record_type").and_then(Value::as_str) != Some("cc_bridge_projected_asset") {
        return false;
    }
...
mod tests {
    ...
    fn temp_test_dir(name: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge_rs_helper_{name}_{}_{}_{}",
            std::process::id(),
            nanos,
            serial
        ));
    ...
    fn storage_scan_summary_classifies_and_limits_entries() {
        let cc_bridge_dir = temp_test_dir("summary");
        let cc_bridge_daemon = cc_bridge_dir.join("cc_bridge_daemon");
        let agents = cc_bridge_dir.join("agents/talk1/provider-state/codex/home/sessions");
        std::fs::create_dir_all(&cc_bridge_daemon).unwrap();
        std::fs::create_dir_all(&agents).unwrap();
        std::fs::write(cc_bridge_dir.join("cc_bridge.config"), b"agents = talk1").unwrap();
        std::fs::write(cc_bridge_daemon.join("state.json"), b"{}").unwrap();
        std::fs::write(agents.join("session.jsonl"), b"abcd").unwrap();

        let payload = StorageSummaryPayload {
            roots: vec![StorageScanRoot {
                root_kind: "project".to_string(),
                path: cc_bridge_dir.clone(),
            }],
            cc_bridge_dir: cc_bridge_dir.clone(),
            runtime_state_root: cc_bridge_dir.join("runtime-state"),
    ...
    fn handle_request_returns_storage_scan_summary_payload() {
        let cc_bridge_dir = temp_test_dir("handle_summary");
        std::fs::write(cc_bridge_dir.join("cc_bridge.config"), b"agents = talk1").unwrap();
        ...
        let response = handle_request(HelperRequest {
            ...
            payload: json!({
                "roots": [{"root_kind": "project", "path": cc_bridge_dir}],
