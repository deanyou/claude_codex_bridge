...
fn request_unix_socket(
    ...
    let first_line = data
        ...
        .ok_or_else(|| "empty response from cc_bridge_daemon".to_string())?;
    ...
    if response.get("ok").and_then(|value| value.as_bool()) != Some(true) {
        let error = response
            .get("error")
            .and_then(|value| value.as_str())
            .unwrap_or("cc_bridge_daemon request failed");
        return Err(error.to_string());
    }
    let mut payload = response
        .as_object()
        .cloned()
        .ok_or_else(|| "cc_bridge_daemon response is not an object".to_string())?;
...
mod tests {
    ...
    #[test]
    fn project_view_round_trips_over_cc_bridge_daemon_socket_protocol() {
