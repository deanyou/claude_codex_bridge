...
impl SidebarTheme {
    ...
    pub fn from_env() -> Self {
        let profile = env::var("CC_BRIDGE_SIDEBAR_THEME_PROFILE")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .or_else(|| env::var("CC_BRIDGE_TMUX_THEME_PROFILE").ok());
        profile
            .as_deref()
...
fn theme_config_path() -> Option<PathBuf> {
    if let Some(config_home) = env::var_os("XDG_CONFIG_HOME").filter(|value| !value.is_empty()) {
        return Some(PathBuf::from(config_home).join("cc_bridge").join("theme.json"));
    }
    env::var_os("HOME")
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
        .map(|home| home.join(".config").join("cc_bridge").join("theme.json"))
}
...
fn system_sidebar_theme() -> SidebarTheme {
    for key in ["CC_BRIDGE_SYSTEM_THEME", "GTK_THEME", "QT_STYLE_OVERRIDE"] {
        if let Ok(value) = env::var(key) {
            let normalized = value.trim().to_ascii_lowercase();
            if normalized.contains("dark") {
                return SidebarTheme::default_dark();
            }
            if normalized.contains("light") || (key != "CC_BRIDGE_SYSTEM_THEME" && !normalized.is_empty())
            {
...
mod tests {
    ...
    fn saved_theme_profile_wins_over_stale_launch_fallback() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root =
            env::temp_dir().join(format!("cc_bridge-sidebar-theme-{}-{unique}", std::process::id()));
    ...
    fn missing_theme_file_uses_launch_fallback() {
        let resolver = RuntimeThemeResolver::with_config_path(
            "light",
            Some(env::temp_dir().join("cc_bridge-sidebar-missing-theme.json")),
        );
