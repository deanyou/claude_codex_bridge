...
const SIDEBAR_CC_BRIDGE_BIN_ENV: &str = "CC_BRIDGE_SIDEBAR_CC_BRIDGE_BIN";
...
pub fn run(args: Args) -> io::Result<()> {
    ...
    match action {
        ...
        ExitAction::KillProject => run_cc_bridge_kill(&args.project_root)?,
    }
    Ok(())
}
...
fn run_tui(args: &Args) -> io::Result<ExitAction> {
    ...
    let client = CcbdClient::new(args.cc_bridge_daemon_socket.clone());
    let cc_bridge_program = cc_bridge_program();
...
fn run_cc_bridge_kill(project_root: &Path) -> io::Result<()> {
    run_cc_bridge_kill_with_program(cc_bridge_program(), project_root)
}

fn run_cc_bridge_kill_with_program(program: PathBuf, project_root: &Path) -> io::Result<()> {
    ...
    Err(io::Error::other(format!(
        "cc_bridge kill failed with status {status}"
    )))
}

fn cc_bridge_program() -> PathBuf {
    if let Some(program) = env::var_os(SIDEBAR_CC_BRIDGE_BIN_ENV)
    ...
    env::current_exe()
        .ok()
        .and_then(|path| cc_bridge_program_for_sidebar(&path))
        .unwrap_or_else(|| PathBuf::from("cc_bridge"))
}
...
fn cc_bridge_program_for_sidebar(sidebar_exe: &Path) -> Option<PathBuf> {
    for ancestor in sidebar_exe.ancestors().skip(1).take(8) {
        let candidate = ancestor.join("cc_bridge");
        ...
        match header_action_at(column, row, areas.tree) {
            Some(HeaderMouseAction::Settings) => {
                if !self.config_ui_launch_is_active() {
                    match launch_config_ui(project_root, cc_bridge_program) {
...
mod tests {
    ...
    #[test]
    fn cc_bridge_kill_prefers_sibling_cli_binary() {
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-sibling-test-{}-{}",
            std::process::id(),
        ...
        let cc_bridge = dir.join("cc_bridge");
        std::fs::write(&cc_bridge, b"#!/bin/sh\n").unwrap();
        let sidebar = dir.join("cc_bridge-agent-sidebar");

        assert_eq!(cc_bridge_program_for_sidebar(&sidebar), Some(cc_bridge));
    ...
    fn config_ui_command_is_project_scoped() {
        ...
        let command = config_ui_command(project_root, Path::new("/opt/cc_bridge/bin/cc_bridge"));

        assert_eq!(command.get_program(), "/opt/cc_bridge/bin/cc_bridge");
        assert_eq!(
    ...
    #[test]
    fn cc_bridge_program_resolution_finds_release_root_above_bin() {
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-program-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let sidebar = dir.join("bin/cc_bridge-agent-sidebar");
        let cc_bridge = dir.join("cc_bridge");
        std::fs::create_dir_all(sidebar.parent().unwrap()).unwrap();
        std::fs::write(&sidebar, b"sidebar").unwrap();
        std::fs::write(&cc_bridge, b"cc_bridge").unwrap();
    ...
    fn config_ui_launch_reports_ready_url_and_late_failure() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-config-ui-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        std::fs::create_dir_all(&dir).unwrap();
        let ready = dir.join("ready-cc_bridge");
        std::fs::write(
            &ready,
        ...
        let manual = dir.join("manual-cc_bridge");
        std::fs::write(
            &manual,
        ...
        let failed = dir.join("failed-cc_bridge");
        std::fs::write(
            &failed,
    ...
    #[cfg(unix)]
    #[test]
    fn project_kill_runs_cc_bridge_kill_from_project_root() {
        use std::os::unix::fs::PermissionsExt;

        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-kill-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let cc_bridge = bin_dir.join("cc_bridge");
        std::fs::write(
            &cc_bridge,
            format!(
                "#!/bin/sh\nprintf '%s|%s\\n' \"$PWD\" \"$1\" > {}\n",
        ...
        .unwrap();
        let mut permissions = std::fs::metadata(&cc_bridge).unwrap().permissions();
        permissions.set_mode(0o755);
        std::fs::set_permissions(&cc_bridge, permissions).unwrap();

        run_cc_bridge_kill_with_program(cc_bridge, &project_root).unwrap();
    ...
    fn rpc_failure_keeps_last_good_project_view_and_marks_degraded() {
        let mut app = SidebarApp::new("main".into());
        app.apply_response(sample_response());
        app.set_error("connect /tmp/cc_bridge_daemon.sock: refused".into());

        let rendered = render_to_string(&app, 80, 36);

        assert!(rendered.contains("cc_bridge_daemon ✕"));
        assert!(!rendered.contains("repo · main"));
        assert!(rendered.contains("> main"));
        assert!(rendered.contains("◐* agent1 [codex]"));
        assert!(!rendered.contains("#job1"));
        ...
        assert!(rendered.contains("↻  X  ⌫  agent2 > agent1 #msg1 run"));
        assert!(!rendered.contains("connect /tmp/cc_bridge_daemon.sock"));
    }

    #[test]
    fn rpc_failure_without_last_good_view_renders_minimal_degraded_screen() {
        let mut app = SidebarApp::new("main".into());
        app.set_error("empty response from cc_bridge_daemon".into());

        let rendered = render_to_string(&app, 80, 14);

        assert!(rendered.contains("cc_bridge_daemon ✕"));
        assert!(!rendered.contains("CC_BRIDGE · main"));
        assert!(rendered.contains("cc_bridge_daemon unavailable"));
        assert!(!rendered.contains("agent1"));
        assert!(!rendered.contains("empty response from cc_bridge_daemon"));
    }
    ...
    fn header_buttons_are_right_aligned_and_kill_project() {
        let client = CcbdClient::new("/tmp/cc_bridge-sidebar-unused.sock");
        let mut app = SidebarApp::new("main".into());
        app.apply_response(sample_response());
        let area = Rect::new(0, 0, 24, 20);
        let controls = tree_controls_area(sidebar_areas(area, app.sidebar_view()).tree);
        let project_root = Path::new("/tmp");
        let cc_bridge_program = Path::new("/bin/true");

        assert_eq!(controls, Rect::new(20, 0, 3, 1));
        assert_eq!(
            app.handle_mouse_down(1, 0, area, &client, project_root, cc_bridge_program),
            None
        );
        assert_eq!(
            app.handle_mouse_down(controls.x, 0, area, &client, project_root, cc_bridge_program,),
            None
        );
        assert_eq!(
            header_action_at(
        ...
        assert!(!app.needs_refresh());
        assert_eq!(
            app.handle_mouse_down(controls.x + 2, 0, area, &client, project_root, cc_bridge_program,),
    ...
    fn header_settings_launch_failure_is_visible() {
        ...
        assert_eq!(
            app.handle_mouse_down(
                ...
                &client,
                Path::new("/tmp"),
                Path::new("/definitely/missing/cc_bridge"),
            ),
            None
        );
    ...
    #[cfg(unix)]
    #[test]
    fn clicking_retry_symbol_calls_cc_bridge_daemon() {
    ...
    #[cfg(unix)]
    #[test]
    fn clicking_cancel_symbol_calls_cc_bridge_daemon_cancel() {
        let seen = Arc::new(Mutex::new(Vec::new()));
    ...
    #[cfg(unix)]
    #[test]
    fn clicking_clear_symbol_dismisses_comms_through_cc_bridge_daemon() {
        let seen = Arc::new(Mutex::new(Vec::new()));
    ...
    #[cfg(unix)]
    #[test]
    fn recover_first_visible_comms_calls_cc_bridge_daemon() {
        let seen = Arc::new(Mutex::new(Vec::new()));
        let (socket_path, handle) = spawn_comms_recover_server(Arc::clone(&seen));
        let client = CcbdClient::new(socket_path);
        let mut app = SidebarApp::new("main".into());
    ...
    #[cfg(unix)]
    #[test]
    fn enter_on_window_row_focuses_window_through_cc_bridge_daemon() {
        let seen = Arc::new(Mutex::new(Vec::new()));
    ...
    #[cfg(unix)]
    #[test]
    fn tab_focuses_the_sidebar_window_through_cc_bridge_daemon() {
        let seen = Arc::new(Mutex::new(Vec::new()));
        let (socket_path, handle) = spawn_window_focus_server(Arc::clone(&seen));
        let client = CcbdClient::new(socket_path);
    ...
    fn spawn_stale_focus_server(
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-stale-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        std::fs::create_dir_all(&dir).unwrap();
        let socket_path = dir.join("cc_bridge_daemon.sock");
    ...
    fn spawn_target_missing_server(
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-target-missing-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
    ...
    fn spawn_window_focus_server(
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-window-focus-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
    ...
    fn spawn_comms_recover_server(
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-comms-recover-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
    ...
    fn spawn_cancel_server(
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-cancel-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
    ...
    fn spawn_dismiss_comms_server(
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-dismiss-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
    ...
    fn spawn_error_server(
        ...
        let dir = std::env::temp_dir().join(format!(
            "cc_bridge-agent-sidebar-error-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
