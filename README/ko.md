...
<p align="center">
  <img src="../assets/readme_v7/cc-bridge-hero-en-light.png" alt="CC_BRIDGE의 보이는 멀티 에이전트 CLI 작업 공간" width="960">
</p>
...
npm이 관리하는 CC_BRIDGE는 npm으로 설치하거나 업데이트합니다.

```bash
npm install -g @seemseam/cc-bridge@latest
```

GitHub release 패키지 또는 소스 설치에는 CC_BRIDGE의 트랜잭션 updater를 사용합니다.

```bash
cc-bridge update
```

npm 관리 설치에서 `cc-bridge update`는 동일한 npm 명령만 표시하며 vendored payload를 직접 변경하지 않습니다.
...
소스 설치는 전역 `cc-bridge` / `ask` 명령을 현재 checkout으로 연결합니다. 일반 사용자는 npm 패키지를 권장합니다.
...
시작 시 `.cc-bridge`를 자동으로 만들 수 없거나 프로젝트 앵커가 없다고 나오면 `.cc-bridge`를 직접 만듭니다.
...
빈 프로젝트는 가볍게 시작합니다. CC_BRIDGE는 `main` window 하나만 열고 머신에서 사용 가능한 첫 번째 지원 CLI를 선택해 `demo`라는 agent 하나를 만듭니다. 멀티 에이전트 팀은 더 이상 기본으로 마운트되지 않습니다.

CC_BRIDGE sidebar 왼쪽 위의 **⚙ 설정** 아이콘을 클릭하면 로컬 설정 제어판이 열립니다. `cc-bridge config ui` 명령으로도 실행할 수 있습니다.

<p align="center">
  <img src="../assets/readme_v7/config-control-panel.png" alt="기본 demo agent를 편집하는 CC_BRIDGE 설정 제어판" width="960">
</p>
...
고급 멀티 에이전트 토폴로지가 필요하면 화면에서 agent를 추가하거나 `.cc-bridge/cc-bridge.config`를 직접 만드세요. `,`와 `;`는 세로 쌓기와 가로 분할을 제어하며 `A,B;C,D`는 네 pane 배치에 가깝습니다.
...
workflow orchestration 중 agent가 `/ask`를 호출해 위임과 인계를 수행할 수도 있습니다. 지속적인 조율에는 agent memory 또는 프로젝트 공유 메모리 파일 `.cc-bridge/cc-bridge_memory.md`를 사용하세요.

<a id="mobile-app"></a>

## 모바일 원격 제어 (Android)

휴대폰에서 CC_BRIDGE를 제어하는 권장 방식은 모든 CC_BRIDGE 프로젝트에 연결하고, 각 agent를 제어하며, 음성 입력과 파일 전송을 사용할 수 있습니다.
...
<p align="center">
  <img src="../assets/readme_v7/mobile-control-chat.jpg" alt="CC_BRIDGE Mobile agent 대화" width="180">
  <img src="../assets/readme_v7/mobile-control-terminal.jpg" alt="CC_BRIDGE Mobile 터미널 제어" width="180">
  <img src="../assets/readme_v7/mobile-control-files.jpg" alt="CC_BRIDGE Mobile 파일 전송" width="180">
  <img src="../assets/readme_v7/mobile-control-pairing.jpg" alt="CC_BRIDGE Mobile 페어링과 연결" width="180">
</p>

<details>
<summary><b>Mobile App 세부 정보, 안전 경계, 소스</b></summary>

CC_BRIDGE 8.6.6은 Flutter CC_BRIDGE Mobile 소스를 [`mobile/`](../mobile/)에 포함하며 Android APK를 GitHub Releases로 배포합니다.

- [CC_BRIDGE Mobile v8.6.6 APK 다운로드](https://github.com/SeemSeam/claude_codex_bridge/releases/download/v8.6.6/cc-bridge-mobile-v8.6.6.apk)
- 앱 소스: [`mobile/app`](../mobile/app)
- 서버 gateway 소스: [`lib/mobile_gateway`](../lib/mobile_gateway)

휴대폰 앱은 서버에서 실행 중인 실제 CC_BRIDGE 프로젝트의 원격 컨트롤러입니다. server-wide mobile gateway에서 마운트된 프로젝트를 찾고, window/agent를 전환하며, agent 대화 컨텍스트를 표시하고, pane-native 입력으로 텍스트를 보내고, terminal view를 열며, 인증된 gateway로 이미지와 문서를 업로드/다운로드할 수 있습니다.

안전 경계:

- CC_BRIDGE gateway는 `127.0.0.1:8787` 같은 loopback에만 bind합니다.
- 원격 접근은 Tailscale Serve를 사용하며 Tailscale Funnel은 사용하지 않습니다.
- CC_BRIDGE는 Tailscale 비밀번호, OAuth token, admin API token을 저장하지 않고 tailnet ACLs/grants를 자동 수정하지 않습니다.
...
<p align="center">
  <img src="../assets/readme_v7/rich-workbench.png" alt="WezTerm에서 Yazi preview를 사용하는 CC_BRIDGE rich media workbench" width="860">
</p>
...
rich mode가 활성화되면 일반 `cc-bridge`는 이미 CC_BRIDGE-managed rich WezTerm 세션 안에서 실행 중인 경우를 제외하고 rich WezTerm launcher를 자동으로 엽니다. 일반 터미널 시작으로 돌아가려면 `cc-bridge uninstall rich`를 실행합니다.

<a id="agent-roles"></a>

## Agent Roles Spec 및 역할 카탈로그

CC_BRIDGE는 전문 agent를 패키징하기 위한 host-neutral 명세인 [Agent Roles Spec](https://github.com/SeemSeam/agent-roles-spec)을 지원합니다. skills, memory, tool dependencies를 설치 가능하고 마운트 가능하며 제거 가능한 Role Pack으로 묶을 수 있습니다. 해당 저장소는 공개 role catalog 역할도 합니다.

| Role | 목적 |
| :--- | :--- |
| `agentroles.cc-bridge_self` | CC_BRIDGE 자체 유지관리, 설정 지원, runtime 진단, 보호된 복구, workflow orchestration. |
...
| `agentroles.su_cc-bridge` | 요구사항 분석, 계획, dispatch, review gates, 보관, 복구를 포함한 SU-CC_BRIDGE workflow 운영. |

<a id="config-memory"></a>

## 설정과 공유 메모리

일반 프로젝트 설정에는 **⚙ 설정** 제어판을 사용하세요. Agent 기반 설정 지원과 runtime 진단이 필요하면 `cc-bridge_self`를 선택적 Role Pack으로 `cc-bridge roles add agentroles.cc-bridge_self:codex` 명령을 통해 추가할 수 있습니다.

`.cc-bridge/cc-bridge_memory.md`는 프로젝트 전체 공유 메모리 문서입니다. 팀 협업 규칙, 프로젝트 제약, 장기 컨텍스트, agent 인계 규칙을 기록하는 데 사용하세요. 여러 provider private memory에 같은 내용을 복사하기보다 안정적인 cross-agent 정보는 여기에 두는 편이 더 안정적입니다.
...
- Tailscale, private LAN, Relay 선택을 `cc-bridge update mobile`로 옮기고 휴대폰은 QR 스캔이나 코드 입력만 하도록 단순화했습니다.
...
- 지원되는 Provider 업그레이드를 `cc-bridge update`로 통합하고 정확한 버전 확인, 거절, 버전별 건너뛰기를 제공합니다. 활성 pane은 자동으로 재시작하지 않습니다.
...
- lifecycle 및 ownership 검증을 유지하면서 cc-bridge-daemon 시작 경로의 반복 작업을 줄였습니다.
...
- mobile host registry 테스트는 이제 임시 Unix sockets를 짧은 `/tmp/cc-bridge-sock-*` 경로 아래에 두어 macOS CI의 `AF_UNIX path too long` 실패를 피합니다.
- `cc-bridge update mobile`, README 링크, package metadata, mobile release manifest가 이제 8.0.12 APK를 가리킵니다.
- v8.0.12에서 공통 섹션 구조의 다국어 README 세트를 도입했습니다. 현재 지역화 파일은 `README/` 디렉터리에 있습니다.

</details>

<details>
<summary><b>v8.0.0</b> - CC_BRIDGE Mobile Monorepo 릴리스</summary>

- Flutter CC_BRIDGE Mobile 소스가 공식적으로 이 저장소에 들어왔고 Android APK가 GitHub Releases로 배포되었습니다.
...
- `cc-bridge update mobile`을 통합 Tailscale Tailnet onboarding 진입점으로 승격하면서 gateway는 loopback-only로 유지하고 Funnel을 쓰지 않으며 token을 저장하지 않고 ACLs/grants를 자동 수정하지 않습니다.
...
- release artifacts에 선택적 Rust `cc-bridge-runtime-accelerator`가 포함되며, sidecar가 예상되는 설치형 Codex agent가 Python hot path로 조용히 fallback하지 않습니다.
