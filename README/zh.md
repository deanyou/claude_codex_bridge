...
<p align="center">
  <img src="../assets/readme_v7/cc-bridge-hero-zh-light.png" alt="CC_BRIDGE 可见多 Agent CLI 工作台" width="960">
</p>
...
通过 npm 管理的 CC_BRIDGE 应继续使用 npm 安装或更新：

```bash
npm install -g @seemseam/cc-bridge@latest
```

通过 GitHub release 包或源码安装时，使用 CC_BRIDGE 自带的事务 updater：

```bash
cc-bridge update
```

在 npm 管理的安装中，`cc-bridge update` 只会显示等价的 npm 命令，不会原地修改 npm vendored payload。

CC_BRIDGE 托管的 provider pane 会关闭已知的 provider 原生启动更新提示。更新 CC_BRIDGE
后，或者 CC_BRIDGE 已经是最新版时，`cc-bridge update` 会统一检查已安装的 provider
CLI，并只提示一次可安全管理的更新。可使用 `--providers check`、
`--providers all` 或 `--providers none` 分别执行仅检查、非交互全部更新或
本次跳过。选择“暂不更新”后，下次 `cc-bridge update` 会再次提示；选择“跳过此
版本”只会静默当前检测到的准确版本。该流程不会自动重启正在运行的
provider pane；已接受的新版本会在 pane 下次启动或显式重启后生效。

版本发生更新后，新安装的 CC_BRIDGE 还会迁移旧的项目级 Claude/Gemini 缓存：
manifest 校验通过且项目已经删除的缓存会立即清理；当前项目已经停止时会
立即清理，仍在运行或属于其他现存项目的缓存会保留到对应项目下一次成功
执行 `cc-bridge kill` 后再清理。未知 Provider、损坏的 manifest、外来符号链接、
session/auth 和用户级 Gemini 缓存都不会被该迁移删除。单次更新可用
`cc-bridge update --no-cache-cleanup` 跳过。
...
源码安装会让全局 `cc-bridge` / `ask` 链接回当前 checkout。普通用户更建议使用 npm 包。
...
如果启动时提示无法自动创建 `.cc-bridge` 或找不到项目锚点，需要手动创建 `.cc-bridge` 作为项目锚点：
...
空白项目现在会轻量启动：CC_BRIDGE 只打开一个 `main` window，并根据本机实际可用的 CLI（依次优先 Codex、Claude、Gemini，再到其他 provider）创建一个名为 `demo` 的 agent，不再默认挂载多 Agent 团队。

点击 CC_BRIDGE sidebar 左上角的 **⚙ 设置** 图标即可打开本地配置控制面；也可以在项目目录运行 `cc-bridge config ui`。

#### 固化本地 Config UI 访问

Config UI 始终只绑定 loopback。若需要固定本地端口和 token，请在 `.cc-bridge/cc-bridge.config` 中配置 token 的**来源**，不要将 token 明文写入该文件：
...
`--port` 仍可覆盖单次启动端口。`token_file` 必须是项目内相对路径、不能是符号链接，并且在 POSIX 上应仅允许文件所有者读取（`chmod 600 .cc-bridge/config-ui.token`）。未配置 token 来源时，CC_BRIDGE 保持原有的随机 token 与临时端口行为。CLI 只输出 loopback URL 和 token 来源，不会输出 token 值。

<p align="center">
  <img src="../assets/readme_v7/config-control-panel.png" alt="CC_BRIDGE 配置控制面正在编辑默认 demo agent" width="960">
</p>

控制面可以配置 windows、pane 拆分、provider、模型、thinking 等级、API 覆盖、workspace、Rich 模式和 sidebar；保存前会先校验，并支持 reload dry-run 和受保护的热加载。保存后会生成 `.cc-bridge/cc-bridge.config`，将当前 provider 和拓扑固定为项目配置。

需要高级多 Agent 拓扑时，可以继续在控制面中可视化添加，或手动创建 `.cc-bridge/cc-bridge.config`。v2 `[windows]` 中的 `,` 和 `;` 分别控制 window 内的上下堆叠和左右分栏，例如 `A,B;C,D` 接近四宫格布局。
...
也可以在工作编排中让 agent 自动调用 `/ask` 完成委派和交接。建议通过修改 agent 记忆或项目共享记忆 `.cc-bridge/cc-bridge_memory.md` 进行编排。

<a id="mobile-app"></a>

## 手机远程控制（Android）

推荐使用手机控制 CC_BRIDGE：可以接入所有 CC_BRIDGE 项目，控制每个 agent，语音输入，并传递文件。
...
<p align="center">
  <img src="../assets/readme_v7/mobile-control-chat.jpg" alt="CC_BRIDGE Mobile agent 对话" width="180">
  <img src="../assets/readme_v7/mobile-control-terminal.jpg" alt="CC_BRIDGE Mobile 终端控制" width="180">
  <img src="../assets/readme_v7/mobile-control-files.jpg" alt="CC_BRIDGE Mobile 文件传输" width="180">
  <img src="../assets/readme_v7/mobile-control-pairing.jpg" alt="CC_BRIDGE Mobile 配对和连接" width="180">
</p>

<details>
<summary><b>Mobile App 详情、安全边界和源码</b></summary>

CC_BRIDGE 8.6.13 已把 Flutter 版 CC_BRIDGE Mobile 源码放入 [`mobile/`](../mobile/)，并在 GitHub Release 中发布 Android APK：

- [下载 CC_BRIDGE Mobile v8.6.13 APK](https://github.com/SeemSeam/claude_codex_bridge/releases/download/v8.6.13/cc-bridge-mobile-v8.6.13.apk)
- App 源码：[`mobile/app`](../mobile/app)
- 服务端 gateway 源码：[`lib/mobile_gateway`](../lib/mobile_gateway)

手机端定位是远程控制真实服务器上的 CC_BRIDGE 项目。它可以从 server-wide mobile gateway 获取所有已挂载项目，切换 window/agent，渲染 agent 对话上下文，以 pane-native 输入方式发送文本，打开 terminal 视图，并通过认证 gateway 上传/下载图片和文档附件。

安全边界：

- CC_BRIDGE gateway 默认绑定 loopback，例如 `127.0.0.1:8787`。
- 局域网直连时可绑定一个明确的私网网卡地址（拒绝通配地址和公网地址）：`cc-bridge install mobile --route-provider lan --listen 192.168.31.155:8787`。配对 URL 会从 `--listen` 自动推导，无需额外转发进程或 `--public-url`。
- 远程访问使用 Tailscale Serve，不启用 Tailscale Funnel。
- CC_BRIDGE 不保存 Tailscale 密码、OAuth token、admin API token，也不会自动修改 tailnet ACL/grants。
...
<p align="center">
  <img src="../assets/readme_v7/rich-workbench.png" alt="CC_BRIDGE rich 富媒体工作台在 WezTerm 中使用 Yazi 预览" width="860">
</p>
...
rich 启用后，普通 `cc-bridge` 会自动打开 rich WezTerm launcher，只有当当前已经处于 CC_BRIDGE 自己拉起的 rich WezTerm 中时才不会再次跳转；运行 `cc-bridge uninstall rich` 可退回普通终端启动。

<a id="agent-roles"></a>

## Agent Roles Spec 规范和角色库

CC_BRIDGE 支持 [Agent Roles Spec](https://github.com/SeemSeam/agent-roles-spec)：这是一个 host-neutral 的专业 agent 封装规范，可把 skills、记忆和工具依赖打包成可安装、可挂载、可卸载的 Role Pack。该仓库同时也是公开角色库。
...
| Role | 基本功能 |
| :--- | :--- |
| `agentroles.cc-bridge_self` | CC_BRIDGE 自维护、配置辅助、运行诊断、受保护恢复和工作流编排。 |
...
| `agentroles.su_cc-bridge` | SU-CC_BRIDGE 工作流操作，覆盖需求分析、计划、派发、审查 gate、归档和恢复。 |
...
普通项目配置推荐直接使用左上角的 **⚙ 设置** 控制面。如果希望由 Agent 辅助设计配置或诊断运行状态，`cc-bridge_self` 仍作为可选 Role Pack 提供，可以用 `cc-bridge roles add agentroles.cc-bridge_self:codex` 添加。

即使关闭可选 skill 继承，受支持的托管 Agent 也会获得内置 `ask`、`cc-bridge-clear`、`cc-bridge-compact` 与 `cc-bridge-diagnose` 控制 skill。使用 `$cc-bridge_diagnose <agentname>` 可结合权威 runtime/job 状态和实时 Pane 证据诊断一个 Agent，在安全时执行受限恢复，并在明确授权提交 GitHub issue 前先审阅脱敏草稿。托管 Codex 还会保留 `reconnect`。

`.cc-bridge/cc-bridge_memory.md` 是项目级共享记忆文档，适合记录团队协作规则、项目约束、长期上下文和 agent 交接约定。把跨 agent 的稳定信息放在这里，比把同一段说明复制到多个 provider 私有记忆里更可靠。
...
<p align="center">
  <img src="../assets/weixin.png?v=da517368" alt="CC_BRIDGE 微信技术群 2" width="240">
</p>
...
- Config V1/V2 角色选择仅显示通用角色和 `agentroles.cc-bridge_self`；workflow 专用 `cc-bridge_*` 角色保留给 V3，不再出现在早期 schema 中。
...
- 在 CC_BRIDGE Mobile 中显示 Pi 和 OMP 原生会话，同时保持 Provider 私有 session 路径与 transcript 边界。
...
- 升级后请重启受管 Codex Agent 与 CC_BRIDGE 项目。无需迁移项目、对话、配对或配置。
...
- CC_BRIDGE 凭据投影为 symlink 或私有 Keychain 检查异常时 fail closed。无需迁移项目、对话、配对或配置。
...
- 当前 Agent 在 `.cc-bridge/workspaces/&lt;agent&gt;/...` 中引用的普通文件，会转换为经过认证的 Mobile 下载附件。
- 安全边界保持 fail-closed：其他 Agent workspace、workspace 隐藏路径以及 `.cc-bridge` 内其余私有运行时状态仍不可下载。
...
- 项目配置中的 tool-window 命令或自定义 Provider command template 执行前，必须取得外部、精确匹配的批准；有意配置时可运行 `cc-bridge config approve-commands`。
...
- CC_BRIDGE Mobile 新增主机多 session 终端、经 Relay capability 协商的 Provider 控制，以及更严格的原生 Windows readiness 校验。
...
- CC_BRIDGE Mobile 显示所选 Agent 的 Provider 身份、已配置/当前/待生效模型与 thinking、Codex/Claude 原生 session 用量和可选账号额度。
- 受支持的模型/thinking 选择通过有保护、需要重启的主机配置持久化，不中断活跃任务，也不暴露 Provider 凭据。
- 可从 Mobile 首页直接打开项目 window 或 Agent 终端，并自定义终端快捷键的显示与顺序。
- 新增内置 `cc-bridge-compact` Skill 和 `cc-bridge compact` 命令，先检查未完成工作，并对未验证的 Provider 命令保持 fail-closed。
...
- 所有受支持的托管 Agent 继续内置 `cc-bridge-clear`，并新增必装的 `cc-bridge-diagnose` Skill。运行 `$cc-bridge_diagnose &lt;agentname&gt;` 可联合检查单个 Agent 的 daemon、lineage、队列、inbox、trace、Provider 日志和实时 Pane 证据。
...
<details>
<summary><b>v8.5.6</b> - 连续继承 Provider 状态且不清除 CC_BRIDGE 对话</summary>

- 每个 API、token、URL、route 和账号维度都优先使用 CC_BRIDGE 配置；未显式配置的维度才从当前外部 Provider 状态单向读取。
- 停止后的 Provider 新 generation 会重新读取继承状态；不会反向写入用户 shell、Provider home、IDE、keyring 或远程登录。
- 权威 generation 变化时保留稳定的 CC_BRIDGE conversation、workspace、队列和历史。同一权威使用原生 resume；Codex/Claude/Gemini 在能力允许时执行 fork/import，否则记录 linked continuation，不隐藏历史。
- 可恢复受撤回版 v8.5.5 迁移形状影响的旧会话，不会执行 clear；撤回的 v8.5.5 包不再复用。
- CC_BRIDGE 托管 Codex 自动安装并 arm 内置 `codex-reconnect`；在精确 pane 和 thread 可证明安全时，网络中断或选定模型容量错误最多自动提交一次 `continue`。
...
- 将 `agents.&lt;name&gt;.env` 中的非 API 变量传入托管 Claude，同时保留 CC_BRIDGE 对 API 凭据和 endpoint 的优先级控制（PR #284）。
...
- 只修复失效的 CC_BRIDGE 托管 Claude continuation 状态且不改登录信息；托管 Codex app server 不可用时安全停止。
- 把稳定的回复与取消规则放入托管项目记忆，普通 ask 不再重复注入提示段落，也不要求每一步轮询取消文件。
- CC_BRIDGE Rich WezTerm 与父 TTY 完全分离，并提供私有 Wayland XCursor overlay，同时保留用户选择的 cursor theme。
...
- 保持对 8.5.0 已持久化 Pi job 和显式 `CC_BRIDGE_PI_EXECUTION_MODE=headless` 回滚路径的兼容。
...
- 即使关闭可选 skill 继承，也会为受支持的托管 Agent 提供内置 `ask` 与 `cc-bridge-clear`；托管 Codex 还会保留 `reconnect`。
- 可选 skill 改为逐项投影，一个损坏的外部条目不会阻断 CC_BRIDGE 控制技能或其他有效 skill。
...
- 新增可持久化的 CC_BRIDGE 主题选择，包括跟随系统，并在 Config UI、Rich WezTerm 与 sidebar 重启后保持一致。
...
- 新增端到端加密的 CC_BRIDGE Mobile Relay，支持运营方签发的一次性邀请码、有边界的接入控制、多路复用流，以及官方或自建 Relay 两种部署方式。
- 将路由选择收敛到 `cc-bridge update mobile`：电脑端可选 Tailscale、经过校验的局域网私网地址、CC_BRIDGE Relay 或自建 Relay，手机端只需扫码或输入配对码。
- 新增可信的 Android 应用内更新流程；交给系统安装前，会校验 GitHub 官方 Release 元数据、APK 大小和 SHA-256。
- CC_BRIDGE 项目移动或改名后仍保持稳定身份，并让终端和配置界面支持跟随系统的浅色/深色主题。
...
- 将受支持的 Provider 升级统一到 `cc-bridge update`，提供准确版本检查、暂不更新和精确版本跳过，并且不会自动重启运行中的 pane。
...
- 减少 cc-bridge-daemon 启动关键路径的重复工作，同时保留 lifecycle 和 ownership 校验。
...
- 阻止 Codex 原生 subagent rollout 抢占 CC_BRIDGE 请求绑定，也不能替换权威父会话和父 turn。
- 内置 subagent 的活动、消息和完成事件只留在父 agent 的协作流程内，不再返回给 CC_BRIDGE caller。
- Python runtime 与 Rust accelerator 使用相同隔离规则，并通过真实登录态 `spawn_agent` 回归确认 caller 只收到父 agent 最终回复。
- 为每个受管 Grok home 独立注入原生 `ask` 与 `cc-bridge-clear` skills，并且只开放这两个命令所需的精确权限。
- 启动前刷新继承的系统 Grok 登录态，并通过每个 agent 的可见原生 Grok 会话处理 CC_BRIDGE 请求；双 Grok 真实测试已通过可见 ask、结果回收、指定目标 clear 和 clear 后隔离验证。
...
- 新增可视化项目配置控制面，可通过 sidebar 左上角的 **⚙ 设置** 或 `cc-bridge config ui` 打开，支持配置校验、差异审查、保存、reload dry-run 和受保护的热加载。
...
- 改进 CC_BRIDGE Mobile gateway profile 持久化、配对凭据保留、项目健康缓存、列表预热显示和 terminal UI 效率。
...
- `cc-bridge update mobile` 对 server-wide loopback `/v1/health` 使用更宽松的单次请求超时和总启动等待窗口，避免挂载项目较多时误判后台 gateway 不健康。
- 新增 regression test 覆盖健康响应晚于旧 0.5 秒请求超时时仍应启动成功。
- `cc-bridge update mobile` 默认 APK 下载链接、README、package metadata 和 mobile app version metadata 对齐到 8.0.19。
...
- 新增 `.cc-bridge-auth-projection.json` 证据 manifest，只记录来源/目标存在性、大小和 SHA256，不保存密钥明文。
- 显式 Codex API authority 会清理继承来的 auth sidecar，避免全局登录态和 agent-local API 配置混用。
- WSL 下 Codex 解析到 Windows interop 可执行文件时，`cc-bridge doctor` 会标记 `reason=wsl_windows_interop_executable`。
- server-wide mobile 项目发现会容忍 stale project 记录，避免单个失效项目拖垮整体列表。
- README 角色列表改为折叠显示，并更新联系方式中的微信图片。
- `cc-bridge update mobile` 默认 APK 下载链接、README、package metadata 和 mobile app version metadata 对齐到 8.0.18。
...
- Mobile frontdesk 消息提交改走 cc-bridge-daemon ask job，不再直接写 pane。
- `cc-bridge watch` 默认不再 10 秒超时，`cc-bridge update mobile` 等长等待路径可以等到真实终态；显式 `CC_BRIDGE_WATCH_TIMEOUT_S` 仍会生效。
- `cc-bridge update mobile` 默认 APK 下载链接、README、package metadata 和 mobile app version metadata 对齐到 8.0.17。
...
- CC_BRIDGE Mobile Terminal 模式增加自动重连诊断与恢复路径，断线时会显示 Reconnecting，并在恢复后继续使用当前 agent pane。
- mobile gateway 在 pane 输入路径记录项目活动，保证 mobile 项目列表最近使用排序能感知 Terminal 输入。
- `cc-bridge update mobile` 默认 APK 下载链接、README、package metadata 和 mobile release manifest 对齐到 8.0.16。
...
- mobile host registry 测试现在把临时 Unix socket 放到短的 `/tmp/cc-bridge-sock-*` 路径，避免 macOS CI 触发 `AF_UNIX path too long`。
- `cc-bridge update mobile`、README 链接、package metadata 和 mobile release manifest 对齐到 8.0.12 APK。
- v8.0.12 引入了结构一致的多语言 README；当前多语言文件统一放在 [`README/`](./) 目录，中文版本为 [`zh.md`](zh.md)，根目录 `README.md` 为英文首页。

</details>

<details>
<summary><b>v8.0.0</b> - CC_BRIDGE Mobile Monorepo 发布</summary>

- Flutter 版 CC_BRIDGE Mobile 源码正式进入本仓库，并在 GitHub Release 中发布 Android APK。
- 新增 server-wide mobile 项目发现、配对、认证 gateway 路由、pane-native 消息输入、对话上下文渲染、terminal 访问，以及图片/文档上传下载能力。
- 将 `cc-bridge update mobile` 提升为 Tailscale Tailnet onboarding 的统一入口，同时保持 gateway 仅监听 loopback，不启用 Funnel、不保存 token、不自动修改 ACL/grants。
...
- Release artifacts 现在会携带可选 Rust `cc-bridge-runtime-accelerator`，安装版 Codex agent 在预期存在 sidecar 时不再静默退回 Python 热路径。
