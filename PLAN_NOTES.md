...
- **phase0.1** (4b96d83) 通讯链路静默异常接入 `~/.cc-bridge/logs/comm.log`(`CC_BRIDGE_COMM_LOG_DIR` 可覆盖)。
...
1. **任务 2.2(硬打断)无需实施**:调查发现 `ExecutionService.cancel()` 已经通过
   ...
   向 pane 发送 C-c/Escape/C-u,`cc-bridge cancel <job_id>` CLI 也已存在
...
3. **测试环境注意**:套件运行期间禁止编辑 lib/ —— 测试会启动 cc-bridge-daemon 子进程实时导入源码,
   中途编辑造成过 6 个误报失败。另:套件会遗留 cc-bridge-daemon 守护进程,重跑前
   `pkill -9 -f cc-bridge-daemon/main.py`。
...
- 注意:test_v2_phase2_entrypoint.py 的 gemini 重启用例在全量并发下偶发失败
  (残留 cc-bridge-daemon 守护进程干扰),单独重跑稳定通过;遇到时先 pkill cc-bridge-daemon 再重跑该文件确认。
