param(
  ...
  [string]$SmokeProjectPath = "$env:USERPROFILE\Desktop\cc-bridge-smoke",
  [string]$LogPath = "",
  [switch]$SkipGit,
  [switch]$SkipPython,
  [switch]$SkipNode,
...
function Get-WorkingPythonCommand {
  ...
  Add-PythonCandidate $env:CC_BRIDGE_PYTHON_CMD
...
function Install-CCSwitch {
  ...
  $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("cc-bridge-ccswitch-" + [guid]::NewGuid().ToString("N"))
  New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
  try {
    $headers = @{ "User-Agent" = "cc-bridge-bootstrap" }
...
function Test-CCBInstalled {
  $candidate = Join-Path $InstallPrefix "bin\cc-bridge.cmd"
...
function Invoke-CCBInstall {
  ...
  if ((-not $Force) -and (Test-CCBInstalled)) {
    Write-Ok "CC_BRIDGE already installed at $InstallPrefix"
    if (-not (Test-CommandExists "cc-bridge")) {
      Ensure-UserPathContains (Join-Path $InstallPrefix "bin")
      Refresh-SessionPath
    }
    return
  }
  Write-Step "Installing CC_BRIDGE into $InstallPrefix"
  ...
  if (-not (Test-CommandExists "cc-bridge")) {
    Fail-Step "cc-bridge install completed but command 'cc-bridge' is still missing from PATH."
  }
  Write-Ok "cc-bridge available at $(Get-CommandLocation 'cc-bridge')"
}
...
function Write-SmokeProject {
  ...
  $configDir = Join-Path $ProjectPath ".cc-bridge"
  $configPath = Join-Path $configDir "cc-bridge.config"
  ...
  if (-not (Test-Path $readmePath) -or $Force) {
    $readmeLines = @(
      "# CC_BRIDGE Windows smoke project",
      "",
...
function Show-ProviderSummary {
  $providers = @("codex", "claude", "gemini", "opencode", "cc-bridge")
...
if (-not $SkipPython) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($workingPython)) {
    $env:CC_BRIDGE_PYTHON_CMD = $workingPython
    Write-Ok "Python 3.10+ already available"
...
if (-not $SkipSmokeProject) {
  ...
  Write-Host "3. Run: cc-bridge"
} else {
  Write-Host "2. Run CC_BRIDGE inside your target project: cc-bridge"
}
Write-Host ""
Write-Host "If you launched this bootstrap with 'powershell -File ...', open a new terminal before testing codex / claude / gemini / opencode / cc-bridge."

if ($LaunchSmoke -and -not $SkipSmokeProject) {
  Write-Step "Launching CC_BRIDGE smoke project"
