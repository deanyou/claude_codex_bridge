# Native Flutter CC_BRIDGE Blueprint
...
The app is a remote controller for CC_BRIDGE already running on a server.

It should not run providers locally on the phone. It should not become a
generic server management app. Its default home is CC_BRIDGE projects, favorites,
...
Server-side CC_BRIDGE
  |
  | control: cc-bridge-daemon project_view / focus / submit / content / lifecycle
  | terminal: tmux -S <project_tmux_socket> attach/control/poll
  v
Project tmux namespace and managed CC_BRIDGE panes
```
...
mode. LAN, tailnet, CC_BRIDGE Relay, and Cloudflare Tunnel should all expose the same
CC_BRIDGE project, agent, content, notification, and terminal-token API to the
Flutter app.
...
- calls to `cc-bridge-daemon` over project Unix sockets;
...
- CC_BRIDGE authority is centralized on the server;
...
- CC_BRIDGE project id/root or a resolver command;
...
- call a narrow `cc-bridge mobile inspect --json` wrapper over SSH;
- call `cc-bridge mobile project-view --json <project>` over SSH;
...
- easier to drift into a generic SSH/tmux client if the UI is not CC_BRIDGE-first.
...
- `gateway_url`;
- `host_id`;
- one-time CC_BRIDGE pairing token;
- server fingerprint;
...
- CC_BRIDGE device tokens and scopes remain mandatory;
- terminal tokens remain short-lived and target-bound;
- Cloudflare Access can be optional defense-in-depth but must not replace CC_BRIDGE
  device identity;
...
CC_BRIDGE-specific constraints:

- terminal target is selected by project/window/agent, not arbitrary session;
- terminal input requires `terminal_input` scope;
- multiline paste should be CC_BRIDGE/tmux paste-buffer based;
- destructive tmux operations are absent in normal CC_BRIDGE mode.
...
Terminal capture should not be the authoritative Markdown source. Use CC_BRIDGE
...
## Server-Side CC_BRIDGE Additions
...
Goal: create a separate `cc-bridge-mobile` Flutter workspace.
...
### Package N2: CC_BRIDGE Project Data Model

Goal: replace server/session-first state with host/project/agent state.

Work:

- add `CcBridgeHost`, `CcBridgeProject`, `CcBridgeAgent`, `CcBridgeWindow`, `CcBridgeTerminalTarget`;
...
- no generic tmux session picker is visible in CC_BRIDGE mode.

### Package N3: Socket-Aware Tmux Transport

Goal: terminal opens the CC_BRIDGE project tmux namespace.
...
- closing terminal view does not stop CC_BRIDGE.
...
- define `cc-bridge-mobile://pair?...` or HTTPS pairing link;
...
- support CC_BRIDGE Relay gateway profile for the first not-on-LAN release and keep
...
Goal: named agent switching from CC_BRIDGE authority.

Work:

- gateway or SSH wrapper calls `project_view`;
- app displays windows/agents/status;
- focus agent/window calls CC_BRIDGE endpoint/wrapper;
...
Goal: readable CC_BRIDGE content view.
...
- add wake/open/close/stop through CC_BRIDGE-owned lifecycle behavior;
...
3. Socket-aware tmux terminal attach to one CC_BRIDGE project.
4. ProjectView fixture side panel.
5. Agent focus through a fake CC_BRIDGE transport.
...
- content endpoint backed by CC_BRIDGE artifact validation;
