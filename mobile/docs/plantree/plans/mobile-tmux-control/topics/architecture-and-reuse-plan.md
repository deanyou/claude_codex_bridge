...
This is the architecture and open-source reuse gate for CC_BRIDGE Mobile
...
and notification patterns into a CC_BRIDGE-scoped native app.

## Architecture Rules

- The phone/iPad is a controller for CC_BRIDGE already running on a server.
...
- UI code depends on `MobileCcbRepository`, not directly on SSH, WebSocket,
  `tmux`, or CC_BRIDGE socket details.
- `GatewayTransport` and `SshTransport` are implementation details behind the
  repository boundary.
- `RouteProvider` is a sub-layer of `GatewayTransport`: LAN, tailnet, CC_BRIDGE
  Relay, and Cloudflare Tunnel must keep the same app-facing CC_BRIDGE schemas.
...
- Destructive tmux/session actions are absent from normal CC_BRIDGE mode unless a
  later CC_BRIDGE-owned admin endpoint explicitly permits them.
...
models/
  cc-bridge_host.dart
  cc-bridge_project.dart
  cc-bridge_window.dart
  cc-bridge_agent.dart
  cc-bridge_terminal_target.dart
  cc-bridge_content_item.dart
  cc-bridge_notification.dart
  cc-bridge_scope.dart
repository/
  mobile_cc-bridge_repository.dart
  fake_mobile_cc-bridge_repository.dart
transport/
...
## CC_BRIDGE Data Model

Minimum app model:

- `CcBridgeHost`: local host id, display name, transport mode, route provider,
  base URL or SSH profile reference, capabilities, health.
- `CcBridgeProject`: project id, display name, root for diagnostics, favorite
  state, lifecycle, health, last activity summary.
- `CcBridgeWindow`: window name, label, kind, order, active state, agent names,
  tmux window evidence.
- `CcBridgeAgent`: agent name, provider, window, order, active state, queue depth,
  callback/completion state, runtime health, pane evidence.
- `CcBridgeTerminalTarget`: project id, namespace epoch, tmux socket path/session
  evidence for direct mode, target kind, window/agent, current pane evidence,
  input scope.
- `CcBridgeContentItem`: project id, content id, kind, preview, format,
  render hints, source safety.
- `CcBridgeNotification`: host id, project id, event id, kind, severity, target
  deep link, acknowledgement state.
- `CcBridgeScope`: `view`, `content`, `focus`, `terminal_input`, `notify`, `ask`,
  `lifecycle`, `admin`.
...
- `GatewayTransport`: default for QR pairing, CC_BRIDGE Relay, Cloudflare Tunnel,
...
| Source | License | Reuse posture | Planned use | Required CC_BRIDGE adaptation |
| :--- | :--- | :--- | :--- | :--- |
| ServerBox | AGPL-3.0 | Fork only if AGPL is accepted. Otherwise use as reference only. | Flutter shell, SSH PTY terminal, xterm view, secure storage, reconnect, notifications, iOS/Android packaging. | Replace server dashboard with CC_BRIDGE project home; add socket-aware tmux commands; remove/gate generic tmux mutations; add ProjectView/content layers. |
...
| tmux-mobile | MIT | Reuse server/gateway patterns and tests, not native app base. | WebSocket terminal gateway, fake tmux/PTY tests, socket-aware CLI executor, grouped-session research. | Bind gateway PTY attach to CC_BRIDGE project socket/session; validate project id, namespace epoch, target, and scopes first. |
| Paseo | AGPL-3.0 | Protocol/pairing reference unless AGPL reuse is accepted. | QR pairing, daemon/client protocol, relay, terminal frames, reconnect concepts. | Replace daemon/workspace model with CC_BRIDGE `cc-bridge-daemon`, ProjectView, and tmux socket authority. |
| ConnectBot | Apache-2.0 | Android SSH reference only. | SSH UX and host-key handling ideas if needed. | Android-only; not CC_BRIDGE/tmux aware. |
| ttyd | MIT | Gateway diagnostic/reference only. | Minimal terminal server concepts. | Must wrap CC_BRIDGE identity/scopes; not a product UI. |
...
4. Decide whether code is copied, vendored, forked, or only reimplemented as
   a pattern.
5. Add tests around each adapted boundary before wiring it to live CC_BRIDGE state.
...
3. Add CC_BRIDGE models and fake ProjectView fixture mapper.
...
6. Add tests proving:
   ...
   - default `tmux attach` cannot be generated for a CC_BRIDGE project;
...
Required before live CC_BRIDGE terminal work:
...
- isolated CC_BRIDGE test project outside `/home/bfly/yunwei/cc-bridge_source` and outside
...
- verify `cc-bridge-daemon`, provider panes, and project tmux session remain alive.
...
- Never include `.cc-bridge/agents`, `.cc-bridge/cc-bridge-daemon`, logs, build output, tokens,
...
- Android debug APK build from the CC_BRIDGE fake-project scaffold;
- fake project agent tap opens a read-only `xterm` terminal screen backed by
  a `CcBridgeTerminalTarget` and socket-aware attach command;
...
- `TerminalTransport` and `SshTerminalTransport` implementation that reuses
  ...
  `CcBridgeTerminalTarget`, and forwards paste, resize, reconnect, and output
...
- record the CC_BRIDGE source ready-check for `cc-bridge mobile serve`, including runtime
...
- running the isolated CC_BRIDGE terminal harness against a started test project
