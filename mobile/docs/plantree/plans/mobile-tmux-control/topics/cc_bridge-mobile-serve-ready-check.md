# CC_BRIDGE Mobile Serve Ready-Check
...
`cc-bridge mobile serve` or mobile gateway code under
...
CC_BRIDGE source commit `bcee866e` landed the G1 loopback current-project gateway
skeleton in `/home/bfly/yunwei/cc-bridge_source`.

Landed scope:

- normal phase2 CLI route for `cc-bridge mobile serve`;
...
- mobile state path properties under `PathLayout(project_root).cc-bridge-daemon_dir /
  "mobile"`;
...
CC_BRIDGE source commit `55c26078` landed the first pairing/device-token foundation
in `/home/bfly/yunwei/cc-bridge_source`. Mobile app commit `d01a338` landed the
matching claim-response import and secure host-profile storage.

Landed source scope:

- startup `cc-bridge mobile serve` summary emits a short-lived pairing code;
- pairing codes are stored only as hashes in `.cc-bridge/cc-bridge-daemon/mobile/pairing-tokens.jsonl`;
- claimed devices are stored in `.cc-bridge/cc-bridge-daemon/mobile/devices.json` with device
...
- audit metadata is appended to `.cc-bridge/cc-bridge-daemon/mobile/audit.jsonl` without token
...
CC_BRIDGE source commit `a222446c` added the route metadata required for Cloudflare
Tunnel pairing:

- `cc-bridge mobile serve --public-url <http-or-https-url>`;
- `cc-bridge mobile serve --route-provider lan|tailnet|cloudflare_tunnel|relay`;
...
- `cc-bridge mobile devices` lists paired devices from local mobile state;
- `cc-bridge mobile revoke <device_id>` revokes lost devices locally without adding
...
- exits before starting disposable CC_BRIDGE runtime.
...
- both routes call existing cc-bridge-daemon `project_focus_agent` /
...
CC_BRIDGE source commit `dfcb7af7` landed the first terminal-open/token foundation:
...
CC_BRIDGE source commit `8ce445f1` landed the first gateway-owned terminal
...
`/home/bfly/yunwei/cc-bridge_source`: `project_focus_agent` now selects the pane
found by CC_BRIDGE pane options instead of first selecting a tmux window named after
the logical CC_BRIDGE window. This preserves the existing `focus-window` behavior
while allowing projects whose actual tmux window is still `cc-bridge` and whose CC_BRIDGE
logical window is stored as `@cc-bridge_window=main`.
...
- `tools/mobile_gateway_terminal_smoke.py` creates a disposable project under
  `/home/bfly/yunwei/test_ccb2`, starts source CC_BRIDGE with `mobile_probe`, starts
  `cc-bridge mobile serve --listen 127.0.0.1:0`, captures the pairing summary without
...
- cleanup: gateway process terminated and `cc-bridge kill -f` returned `kill_status:
  ok`
...
- `CcBridgeProjectView.terminalTargetForWindow` maps configured windows to stable
...
Read-only inspection was done against `/home/bfly/yunwei/cc-bridge_source` at CC_BRIDGE
source version `7.6.12`.

Relevant source facts:

- CLI entry is `cc_bridge.py` via `bin/cc-bridge.js` / `bin/cc-bridge`; phase2 commands flow
...
- `CcbdClient` in `lib/cc-bridge-daemon/socket_client.py` talks to the project cc-bridge-daemon Unix
  socket and binds endpoint methods from
  `lib/cc-bridge-daemon/socket_client_runtime/endpoints.py`.
- Existing cc-bridge-daemon handlers include `ping`, `project_view`,
  `project_focus_agent`, `project_focus_window`,
  `project_view_dismiss_comms`, lifecycle/start/stop, mailbox, queue, trace,
  and recovery operations.
- `project_view` returns `view.project`, `view.cc-bridge-daemon`, `view.namespace`,
...
- Focus handlers already reject stale namespace epochs through
  `ProjectFocusService`.
- `PathLayout(...).cc-bridge-daemon_dir` is the existing per-project runtime/state root,
...
- Foreground terminal attach already treats tmux attach as a UI client:
  ...
  from `ping('cc-bridge-daemon')`, then runs
...
First package: implement `cc-bridge mobile serve` as a CLI-managed gateway sidecar
process for the current project.

Do not mount the HTTP/WebSocket gateway inside `cc-bridge-daemon` for the first package.
Do not make `cc-bridge-daemon` supervise the gateway in the first package.

Rationale:

- `cc-bridge-daemon` is already the project authority and Unix-socket RPC server.
...
- Those concerns are not the same as cc-bridge-daemon's project reconciliation,
...
- Keeping the gateway as a separate process lets it bind loopback by default,
  be tunneled by Cloudflare later, and fail without stopping CC_BRIDGE.

The gateway must use `CcbdClient(context.paths.cc-bridge-daemon_socket_path)` to read and
mutate CC_BRIDGE-owned state. It must not write directly to provider runtime records
...
- cc-bridge-daemon remains the authority for project state, agent/window focus, namespace
  epoch, tmux socket/session, lifecycle, Comms, and jobs;
- mobile clients are gateway records in the first package, not cc-bridge-daemon runtime
  records.
...
Use existing cc-bridge-daemon endpoints directly through `CcbdClient`:

- `ping('cc-bridge-daemon')` for health, namespace attachability, and diagnostics;
...
Add gateway wrappers instead of cc-bridge-daemon handlers for:
...
- first content/event/lifecycle APIs until their exact CC_BRIDGE source authority is
  selected.
...
`cc-bridge mobile serve` should resolve the active project through the existing
`CliContext` and `PathLayout`, then return one project from
`GET /v1/projects`.

Do not scan recent `.cc-bridge` anchors, source checkouts, home directories,
...
- no cc-bridge-daemon lifecycle ownership changes.

## Candidate Source Files

Expected files to change or add in `/home/bfly/yunwei/cc-bridge_source` for G1:
...
- `lib/storage/paths_cc-bridge-daemon.py` for `cc-bridge-daemon_mobile_dir` and concrete mobile state
  paths
- new `lib/mobile_gateway/` package for gateway models, storage, service,
  HTTP server, cc-bridge-daemon adapter, and later terminal streaming
...
- focused additions or fixtures around `test/test_cc-bridge-daemon_project_view.py`;
...
- `cc-bridge mobile serve --help` or equivalent usage is routed through the normal
  CLI path;
...
- ProjectView route preserves CC_BRIDGE schema and route-provider independence;
- gateway process shutdown does not call `stop_all`, `shutdown`, raw
  `tmux kill-session`, or `tmux kill-server`;
- no `.cc-bridge/agents`, `.cc-bridge/cc-bridge-daemon` runtime state, token plaintext, logs, or build
...
- Should the future host-level registry be an explicit `cc-bridge mobile register`
  list, a cc-bridge-daemon-discovered recent-project list, or a user-managed gateway
  config?
- Which library or adapted open-source implementation should own the terminal
  WebSocket server once G3 starts?
- Should mobile lifecycle actions call existing CLI flows, new cc-bridge-daemon handlers,
  or gateway-managed wrappers?
- Which CC_BRIDGE content store becomes the first authoritative Markdown endpoint?
