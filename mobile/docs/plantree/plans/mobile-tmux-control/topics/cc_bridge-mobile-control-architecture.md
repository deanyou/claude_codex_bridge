# CC_BRIDGE Mobile Control Architecture

Date: 2026-06-17

## Core Position

CC_BRIDGE mobile control should be an agent-first remote client for server-side CC_BRIDGE
...
It should still be CC_BRIDGE-scoped, not a generic tmux browser. Generic tmux clients
organize around SSH hosts, tmux sessions, windows, and panes. CC_BRIDGE organizes
around projects, `cc-bridge-daemon` backends, configured agents, configured windows, Comms,
...
transport adapter
  |
  | GatewayTransport: HTTPS/WebSocket to cc-bridge mobile gateway
  | SshTransport: SSH exec/PTY fallback for direct tmux mode
  v
server-side CC_BRIDGE
  |
  | control: cc-bridge-daemon project_view / focus / content / lifecycle
  | terminal: tmux -S <project_socket> attach/control/capture
  v
project tmux namespace and managed provider panes
```

The mobile gateway can be a new `cc-bridge mobile serve` process or a `cc-bridge-daemon` sidecar.
The app may also keep an SSH direct transport for developer/fallback use. The
important boundary is that every normal product action is CC_BRIDGE-scoped and
project-aware, not an arbitrary host tmux session action.

## Project Discovery

The mobile home screen should list CC_BRIDGE projects. Opening a project should land
in an agent-first project workspace for that project.

Initial discovery options:

- explicit project registry written by `cc-bridge mobile serve`;
- recent `.cc-bridge` anchors known to the user account;
- manually paired project QR code from an active `cc-bridge` foreground;
- later: multi-host registry with host health.

Avoid blind filesystem scans by default. A project entry should resolve to a
`.cc-bridge` anchor, then to current `cc-bridge-daemon` lifecycle/socket/session facts.
...
- `cc-bridge-daemon` generation, health, heartbeat, namespace epoch;
...
Those endpoints already validate namespace epoch, find panes using CC_BRIDGE tmux
...
For this product direction, the primary mobile path is structured CC_BRIDGE control:
...
- type into CC_BRIDGE-managed panes;
...
exist is CC_BRIDGE-aware mobile control of projects, agents, content, and terminal
...
CC_BRIDGE already has capture helpers that can be wrapped by a `cc-bridge-daemon` endpoint.
...
- use a CC_BRIDGE-owned paste-buffer path for multiline text;
- keep destructive tmux operations behind CC_BRIDGE admin endpoints.
...
- per-client grouped sessions isolate focus, but should be deferred until
  `cc-bridge-daemon` explicitly owns mobile view-client state.
...
- focus agent/window through `cc-bridge-daemon`;
- submit ask/composer text;
- read pane snapshots;
- view logs/diagnostics summarized from CC_BRIDGE authority.
...
- session/window renaming that bypasses `.cc-bridge/cc-bridge.config`;
...
- TLS, trusted local network, tailnet, CC_BRIDGE Relay, or Cloudflare Tunnel for
  transport;
...
3. CC_BRIDGE Relay for the default not-on-LAN release;
...
Agent identity is the CC_BRIDGE slot/config identity, not the tmux pane number.
...
This matches CC_BRIDGE's multi-agent value better than a raw terminal grid.
...
Prototype the smallest native CC_BRIDGE tmux remote flow:

1. create a dedicated Flutter app/fork baseline;
2. pair or manually configure one CC_BRIDGE host/project;
...
5. list registered CC_BRIDGE projects and favorites through a fake then real CC_BRIDGE
   transport;
...
That proves the CC_BRIDGE-specific value without turning the product into a separate
