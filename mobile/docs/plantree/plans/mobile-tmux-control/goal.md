# CC_BRIDGE Mobile Landing Goal

Date: 2026-06-18

## Purpose

This document is the reusable goal prompt for landing the full CC_BRIDGE Mobile
phone/iPad project.
...
```text
读取并执行 `/home/bfly/yunwei/cc-bridge_source/mobile/docs/plantree/plans/mobile-tmux-control/goal.md`
作为当前长期 goal。

你要端到端落地 CC_BRIDGE Mobile 手机/iPad 开发项目。默认产品体验必须是
...
- 修改 `/home/bfly/yunwei/cc-bridge_source` 时，必须在 CC_BRIDGE 源码库独立检查、
  测试和提交；mobile 仓库和 source 仓库不要混合提交。
- 不提交 runtime state、secrets、build artifacts、`.cc-bridge/agents`、
  `.cc-bridge/cc-bridge-daemon`、日志、token 或本地 SDK 配置。
- 大包实现要拆分给 CC_BRIDGE worker/reviewer，并把结果回写为 plan-tree
  evidence。

不要把 goal 标记为 complete，直到 Android app、gateway route、
CC_BRIDGE project discovery/control、agent-first single-agent workspace、
...
```text
请读取并执行
`/home/bfly/yunwei/cc-bridge_source/mobile/docs/plantree/plans/mobile-tmux-control/goal.md`
作为当前长期 goal。

目标是端到端落地整个 CC_BRIDGE Mobile 手机/iPad 开发项目。默认 UI 目标是
...
开源库复用/二改方案。实现时优先复用成熟开源库和已有 CC_BRIDGE 源码能力，
...
可回滚，不提交 runtime state、secrets、build artifacts、`.cc-bridge/agents`、
`.cc-bridge/cc-bridge-daemon`。修改 `/home/bfly/yunwei/cc-bridge_source` 时必须在 CC_BRIDGE 源码库中
独立检查、测试和提交。
...
```text
端到端落地以单 Agent 工作页为默认体验的 CC_BRIDGE Mobile 手机/iPad 开发项目，并持续维护 plan tree、进度记录和阶段性 commit。
```

Call block:

```text
Read `/home/bfly/yunwei/cc-bridge_source/mobile/docs/plantree/plans/mobile-tmux-control/goal.md`
and execute it as the active project goal.

Deliver the whole CC_BRIDGE Mobile phone/iPad project incrementally as an
...
Lead owns orchestration only: split work into packages, dispatch CC_BRIDGE workers,
...
secrets, build artifacts, `.cc-bridge/agents`, or `.cc-bridge/cc-bridge-daemon`.
```
...
```text
Do not mark this goal complete until the Android app, gateway route, CC_BRIDGE project
...
目标：端到端落地以单 Agent 工作页为默认体验的 CC_BRIDGE Mobile 手机/iPad
开发项目，并持续维护
plan tree、进度记录和阶段性 commit。

项目目标：

在 `/home/bfly/yunwei/cc-bridge_source/mobile` 中落地一个原生 Flutter
Android/iOS/iPadOS App，作为 server-side CC_BRIDGE 项目的移动远程控制器。
App 默认进入 CC_BRIDGE-aware 单 Agent 工作页：顶部显示 agent 切换列表，主体
...
阅读、通知、CC_BRIDGE Relay 默认远程访问、Cloudflare Tunnel 高级远程访问和
后续 lifecycle 控制。
...
- 对 terminal、tmux、SSH/WebSocket、QR pairing、gateway、Markdown/math、
  ...
  才允许写 CC_BRIDGE 专用实现，并把原因写入 plan tree。
...
- App 必须是 CC_BRIDGE-aware remote controller，不是通用 SSH/tmux 客户端，
  也不在手机本地运行 provider/agent。
- 默认体验必须是 CC_BRIDGE-aware single-agent workspace，不是 tmux stream。
  顶部优先展示 agent 切换，主体只展示一个 selected agent；项目路径、
  gateway URL、pairing code、runtime id、诊断信息进入连接详情/设置，
  不常驻第一屏。
- 结构化 CC_BRIDGE content 是 Markdown/math 阅读的权威来源。不要从 tmux
...
- Selected-agent workspace 默认应优先展示可读化 agent 工作历史：结构化
  CC_BRIDGE content、Comms/replies/artifacts，以及来自 tmux scrollback 的
...
- 移动端代码放在 `/home/bfly/yunwei/cc-bridge_source/mobile`，首选路径 `app/`。
- 不把本仓库当作 CC_BRIDGE 源码库。
- CC_BRIDGE 源码只在需要且经过 ready-check 后，才在
  `/home/bfly/yunwei/cc-bridge_source` 中修改。
- CC_BRIDGE 权威来自现有 `project_view`、`project_focus_agent/window`、
  namespace epoch、tmux socket/session。
- `pane_id` 只能作为 evidence，不能单独授权输入、focus 或破坏性操作。
- CC_BRIDGE Relay 是普通用户默认非 LAN 远程路线；Cloudflare Tunnel 是
...
3. 写清 transport 边界：`GatewayTransport`、`SshTransport`、
   `RouteProvider`。
4. 写清 CC_BRIDGE 数据模型：Host、Project、Window、Agent、TerminalTarget、
...
6. 写清测试策略：model tests、tmux command tests、fake transport tests、
   Android emulator smoke test、isolated CC_BRIDGE project terminal slice。
...
5. 实现 CC_BRIDGE-first fake transport、ProjectView fixture、数据模型、顶部
...
7. 加 socket-aware tmux command layer 测试，确保生成
   ...
   `tmux attach`。
8. 完成隔离 CC_BRIDGE 测试项目 raw terminal vertical slice：输入、paste、
   resize、background/resume、reconnect；该 slice 是显式控制模式，不是
   默认项目页。
9. 再推进 QR pairing、GatewayTransport、CC_BRIDGE Relay、Cloudflare Tunnel
...
3. Dispatch：lead 把当前 coherent package 拆成 worker 指令，写清目标、
   ...
   CC_BRIDGE callback，不把实现细节留在 lead 侧临场补写。
...
- 每次 commit 前必须检查 `git status`，不要误提交 `.cc-bridge/agents`、
  `.cc-bridge/cc-bridge-daemon`、日志、build 产物、密钥、token、Android
  `local.properties` 等运行态文件。
- implementation commit 应包含对应 plan-tree 更新；纯计划更新也可以
  单独 commit。
- 修改 `/home/bfly/yunwei/cc-bridge_source` 时必须在 CC_BRIDGE 源码库内独立检查、
  测试、提交，不要混到 mobile 仓库 commit。

验收标准：

- Android emulator 能启动 App。
- 首页展示 CC_BRIDGE 项目/agent fixture，而不是通用服务器面板。
...
- Markdown/content reader 以结构化 CC_BRIDGE 内容为权威来源，并保留 raw
...
- tmux 命令测试覆盖 socket path、session name、agent/window target、
  multiline paste。
- 第一条真实 raw terminal slice 连接的是 CC_BRIDGE project tmux socket/session。
- App 关闭或断线重连不会停止 server-side CC_BRIDGE。
