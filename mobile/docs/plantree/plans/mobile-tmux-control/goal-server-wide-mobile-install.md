# CC_BRIDGE Mobile Server-Wide Install Goal
...
server-wide CC_BRIDGE Mobile install and multi-project gateway.

Use it when starting or resuming the work that changes CC_BRIDGE Mobile from a
...
all mounted/reachable CC_BRIDGE projects on that server, open any project, send to
...
```text
读取并执行
`/home/bfly/yunwei/cc-bridge_source/mobile/docs/plantree/plans/mobile-tmux-control/goal-server-wide-mobile-install.md`
作为当前长期 goal。

目标：先制定并校准方案，然后分包落地，最终打通 `cc-bridge install mobile`
服务器级安装/激活能力。完成后，手机只需配对一次服务器 gateway，就能在
App 第一页看到该服务器所有 mounted/reachable 的 CC_BRIDGE 项目；点任意项目后
进入该项目的真实 ProjectView；发送问题能到达该项目真实 CC_BRIDGE 后端并显示
...
3. 再落 `cc-bridge install mobile` package：服务器级安装/激活/配对入口，能在非
   CC_BRIDGE 项目目录执行。
4. 再落 app package：paired mode 首页先 `listProjects()`，展示服务器项目
   列表，点击项目后加载对应 ProjectView，fake demo 只作为显式 demo/fallback。
5. 最后落真实本地 AVD multi-project smoke：两个真实本地 CC_BRIDGE 项目、一个
   server gateway、一个 app profile、分别收发消息和文件，记录响应速度。

Source repo 规则：
- CC_BRIDGE source 在 `/home/bfly/yunwei/cc-bridge_source`，但实现必须使用明确 worktree；
...
- `ProjectListScaffold` 必须支持多个 `CcBridgeProject`，显示 health/root/activity。
...
真实验收：
- 至少两个本地 CC_BRIDGE 项目通过同一个 server gateway 出现在 App 首页。
...
- `cc-bridge install mobile` server-level path 已在 source worktree 落地并通过测试；
- source gateway 能列出所有 mounted/reachable CC_BRIDGE projects；
...
```text
端到端落地 `cc-bridge install mobile` 服务器级 mobile gateway：手机配对一次服务器，
首页列出所有 mounted/reachable CC_BRIDGE 项目，并能分别进入每个项目完成真实消息、
...
- CC_BRIDGE source worktree changes for server-level mobile install, project registry,
  host gateway routing, pairing metadata, and focused tests.
- CC_BRIDGE mobile app changes for paired gateway project list, project selection,
...
Goal: make the gateway able to discover and route to multiple local CC_BRIDGE
projects.
...
- `/v1/projects/{project_id}/view` calls the matching cc-bridge-daemon client;
...
### B. Source `cc-bridge install mobile`
...
- command works outside any CC_BRIDGE project root;
...
- existing `cc-bridge update mobile` behavior is either delegated to or clearly
...
- create two disposable real CC_BRIDGE project roots;
