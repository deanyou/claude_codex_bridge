...
void main() {
  group('CcBridgeConversationItem timing metadata', () {
    test('parses and serializes nullable timing fields as UTC ISO strings', () {
      final item = CcBridgeConversationItem.fromJson({
        'id': 'reply-1',
        'agent': 'lead',
        'kind': 'agent_reply',
        'title': 'Agent reply',
    ...
    test('copyWith preserves timing fields through state updates', () {
      ...
      final item = CcBridgeConversationItem.userMessage(
        id: 'local-lead-0',
        agentName: 'lead',
        body: 'hello',
        sentAt: sentAt,
      ...
      final sent = item.copyWith(state: CcBridgeConversationDeliveryState.sent);

      expect(sent.sentAt, sentAt);
      expect(sent.state, CcBridgeConversationDeliveryState.sent);
    });
    ...
    test('agent replies carry timing metadata from content items', () {
      final reply = CcBridgeConversationItem.agentReplyFromContent(
        agentName: 'lead',
        content: CcBridgeContentItem(
          id: 'content-1',
          kind: 'reply',
          format: 'markdown',
          text: 'done',
