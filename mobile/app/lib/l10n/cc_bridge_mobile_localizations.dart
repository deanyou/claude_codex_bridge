...
class CcBridgeMobileLocalizations {
  const CcBridgeMobileLocalizations(this.locale);

  final Locale locale;

  static const supportedLocales = <Locale>[Locale('en'), Locale('zh')];

  static CcBridgeMobileLocalizations of(BuildContext context) {
    return CcBridgeMobileLocalizations(Localizations.localeOf(context));
  }

  bool get isChinese => locale.languageCode.toLowerCase() == 'zh';

  String get appTitle => 'CC_BRIDGE Mobile';

  String get connectTitle => isChinese ? '连接 CC_BRIDGE Mobile' : 'Connect CC_BRIDGE Mobile';
  ...
  String lanGatewayUnreachableBody(String gatewayHost) {
    return isChinese
        ? '确认手机和电脑在同一 Wi-Fi，未使用访客/设备隔离网络，防火墙允许 $gatewayHost。若电脑 IP 已变化，请在电脑重新运行 cc_bridge update mobile 并扫码。'
        : 'Check that both devices use the same Wi-Fi, guest/client isolation is off, and the firewall allows $gatewayHost. If the computer IP changed, rerun cc_bridge update mobile and scan the new code.';
  }
  ...
  String get noCcbProjectsFound =>
      isChinese ? '未找到 CC_BRIDGE 项目' : 'No CC_BRIDGE projects found';
  ...
  String get chatBackgroundDescription =>
      isChinese
          ? '选择一张本机图片作为聊天和终端的全屏背景。图片只保存在此设备，不会上传到 CC_BRIDGE。'
          : 'Choose a local image as the full-screen background for chats and terminals. It stays on this device and is never uploaded to CC_BRIDGE.';
  ...
  String get mobileUpdates =>
      isChinese ? 'CC_BRIDGE Mobile 更新' : 'CC_BRIDGE Mobile updates';
  ...
  String get updateAvailableTitle =>
      isChinese ? '发现 CC_BRIDGE Mobile 更新' : 'CC_BRIDGE Mobile update available';
  ...
  String get providerHostUpdateRequired =>
      isChinese
          ? '电脑端 CC_BRIDGE 版本不支持模型控制。请在电脑执行 cc_bridge update，然后重新连接。'
          : 'The computer CC_BRIDGE version does not support model controls. Run cc_bridge update on the computer, then reconnect.';
