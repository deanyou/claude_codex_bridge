...
const _projectId = String.fromEnvironment('CC_BRIDGE_MOBILE_CC_BRIDGE_DAEMON_RESTART_PROJECT_ID');
const _projectName = String.fromEnvironment(
  'CC_BRIDGE_MOBILE_CC_BRIDGE_DAEMON_RESTART_PROJECT_NAME',
  defaultValue: 'test_ccb2_alpha',
);
const _agentName = String.fromEnvironment(
  'CC_BRIDGE_MOBILE_CC_BRIDGE_DAEMON_RESTART_AGENT',
...
void main() {
  ...
  testWidgets('explicit refresh recovers after project cc_bridge_daemon restart', (
    tester,
  ) async {
    if (_projectId.trim().isEmpty) {
      throw TestFailure('cc_bridge_daemon restart smoke project id is required');
    }
    ...
    // Host-side runner stops this selected project's real cc_bridge_daemon after this
    // marker. The server-wide mobile gateway and adb reverse stay up.
    // ignore: avoid_print
    print('CC_BRIDGE_CC_BRIDGE_DAEMON_RESTART_READY_STOP selected-agent');
    ...
    // Host-side runner starts the same test project's cc_bridge_daemon again. The stored
    ...
    // ignore: avoid_print
    print('CC_BRIDGE_CC_BRIDGE_DAEMON_RESTART_READY_START selected-agent');
    await tester.pump(const Duration(seconds: 2));
