...
const vendorRoot = path.join(root, ".cc_bridge-release");
const installLock = path.join(root, ".cc_bridge-install.lock");
const runtimeProbe = [
  "import sys",
...
function artifactForHost() {
  if (process.platform === "darwin") {
    return {
      directory: "cc-bridge-macos-universal",
      file: "cc-bridge-macos-universal.tar.gz",
    };
  }
  if (process.platform === "linux" && process.arch === "x64") {
    return {
      directory: "cc-bridge-linux-x86_64",
      file: "cc-bridge-linux-x86_64.tar.gz",
    };
  }
  throw new Error(
    `Unsupported platform for @seemseam/cc-bridge: ${process.platform}/${process.arch}. ` +
...
function executablePath(command = "cc-bridge") {
  const info = artifactForHost();
  const base = installDir(info);
  return command === "cc-bridge" ? path.join(base, "cc-bridge") : path.join(base, "bin", command);
}
...
function isReleaseInstalled(info) {
  ...
  const cc_bridgePath = path.join(dir, "cc_bridge");
  if (!fs.existsSync(versionFile) || !fs.existsSync(cc_bridgePath)) {
    return false;
  }
  try {
    fs.accessSync(cc_bridgePath, fs.constants.X_OK);
...
function bootstrapRuntime(info) {
  ...
  if (!isReleaseInstalled(info)) {
    throw new Error(`Cannot bootstrap an incomplete CC_BRIDGE release at ${dir}`);
  }
  if (!fs.existsSync(installerPath)) {
    throw new Error(`CC_BRIDGE runtime bootstrap installer not found at ${installerPath}`);
  }
  ...
  if (!isRuntimeReady(info)) {
    throw new Error(`CC_BRIDGE managed Python runtime is not usable at ${runtimePythonPath(info)}`);
  }
}
...
async function acquireInstallLock() {
  const configuredTimeout = Number(process.env.CC_BRIDGE_NPM_INSTALL_LOCK_TIMEOUT_MS);
  const timeoutMs =
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for CC_BRIDGE npm install lock: ${installLock}`);
      }
      await sleep(200);
      continue;
    }
...
async function downloadRelease(info) {
  const baseUrl =
    process.env.CC_BRIDGE_NPM_RELEASE_BASE_URL ||
    `https://github.com/SeemSeam/claude_codex_bridge/releases/download/v${version}`;
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cc_bridge-npm-"));
  ...
  try {
    ...
    if (!fs.existsSync(executablePath("cc_bridge"))) {
      throw new Error(`Installed CC_BRIDGE executable not found at ${executablePath("cc_bridge")}`);
    }
    console.log(`Installed CC_BRIDGE v${version} from ${info.file}.`);
  } finally {
...
async function install() {
  ...
  try {
    ...
    if (!isReleaseInstalled(info)) {
      if (process.env.CC_BRIDGE_NPM_SKIP_DOWNLOAD === "1") {
        console.warn("Skipping CC_BRIDGE release download because CC_BRIDGE_NPM_SKIP_DOWNLOAD=1.");
        return;
      }
