...
def check_local_files(root: Path, version: str, repo: str, issues: list[str], warnings: list[str]) -> None:
    ...
    files = {
        ...
        "cc_bridge": read(root / "cc_bridge"),
        "cc_bridge.py": read(root / "cc_bridge.py"),
    ...
    if f'VERSION = "{bare_version}"' not in files["cc_bridge"] and f'VERSION = "{bare_version}"' not in files["cc_bridge.py"]:
        fail(issues, f"cc_bridge.py does not contain VERSION = {bare_version!r}", fix=f'update cc_bridge.py to VERSION = "{bare_version}"')
    ...
    for readme_name in ("README.md", "README/zh.md"):
        ...
        if ".cc-bridge/cc_bridge_memory.md" not in body:
            fail(issues, f"{readme_name} does not mention .cc-bridge/cc_bridge_memory.md", fix="state that .cc-bridge/cc_bridge_memory.md is the project-wide shared memory document")
    ...
    if "CC_BRIDGE.md" in files["README.md"] or "CC_BRIDGE.md" in files["README/zh.md"]:
        fail(issues, "README mentions current CC_BRIDGE.md support; current design must only use .cc-bridge/cc_bridge_memory.md", fix="remove current-feature references to CC_BRIDGE.md; keep only .cc-bridge/cc_bridge_memory.md")
...
def check_active_skill_sync(root: Path, warnings: list[str]) -> None:
    source_dir = root / "dev_tools" / "skills" / "cc_bridge-github"
    if not source_dir.is_dir():
        return
    cc_bridge_dir = root / ".cc-bridge"
    if not cc_bridge_dir.is_dir():
        return
    active_dirs = sorted(cc_bridge_dir.glob("agents/*/provider-state/codex/home/skills/cc_bridge-github"))
    for active_dir in active_dirs:
        ...
        if mismatched:
            warn(
                warnings,
                f"Active cc_bridge-github skill copy differs from dev_tools at {active_dir}: {', '.join(mismatched)}",
            )
