---
name: cc-bridge-github
description: Maintain this CC_BRIDGE project's GitHub-facing release and npm publication surface. Use when preparing, publishing, auditing, or fixing CC_BRIDGE releases; updating README.md, README/zh.md, localized README files, CHANGELOG.md, VERSION, package.json, GitHub release notes/assets, repository description/topics, npm registry state, or GitHub Actions release/test status.
---

# CC_BRIDGE GitHub Release Maintainer
...
For CC_BRIDGE releases, npm publishing is part of the release surface by default. If `package.json`, npm wrapper scripts, README npm install snippets, or `.github/workflows/npm-publish.yml` are missing or stale during release preparation, create or repair them before the release commit and before creating or pushing the release tag. Do not create a tag first and add npm publish support in a later branch commit.
...
From the CC_BRIDGE repo root, run the bundled checker before and after release work:

```bash
CHECKER="dev_tools/skills/cc-bridge-github/scripts/check_release_state.py"
...
Use `--phase dev` for ordinary CC_BRIDGE development or maintainer tooling changes that are not intended to create a package release. Use `--wait-seconds 0` for lightweight commit/push work where the user did not ask to wait for GitHub Actions:
...
`--phase published` checks both release state and homepage state: GitHub latest release, release assets, `SHA256SUMS`, release workflows, branch validation workflows, and `README.md` / `README/zh.md` as rendered from the repository default branch. Use `--wait-seconds 1800` immediately after tagging so the checker waits for `Release Artifacts` and uploaded assets instead of reporting transient failures. The checker does not replace npm registry verification; for CC_BRIDGE releases, also verify `npm view @seemseam/cc-bridge version dist-tags --json`.
...
Use this for CC_BRIDGE development changes, including `dev_tools`, tests, docs, CI, and maintainer workflows.

1. Classify the change:
   - `dev_tools/`, tests, docs, and CI-only checks usually do not require a package release.
   - `lib/`, `cc-bridge`, `bin/`, installer scripts, release build scripts, `VERSION`, `package.json`, `.github/workflows/npm-publish.yml`, README release notes, or `CHANGELOG.md` may affect users and must be considered for release.
2. Run targeted tests first, then the smallest broad check that matches the risk.
3. Commit the development change.
4. Push the branch.
5. Run:
   ```bash
   python dev_tools/skills/cc-bridge-github/scripts/check_release_state.py --phase dev --repo SeemSeam/claude_codex_bridge --wait-seconds 0
   ```
...
- `lib/`, `cc-bridge-daemon`, provider runtime, startup, tmux, or shared behavior edits: run targeted pytest for the touched module and adjacent contract tests; broaden only when the change affects cross-module behavior.
...
- `VERSION`
- `cc_bridge.py` `VERSION = "..."` (or legacy Python-in-`cc-bridge` layouts)
...
- "Config Control" / "配置控制" must stay aligned with current `.cc-bridge/cc-bridge.config` behavior.
- Keep the shared memory wording concise: `.cc-bridge/cc-bridge_memory.md` is the project-wide shared memory document.
- Do not reintroduce root `CC_BRIDGE.md` support or mention it as a current feature.
...
- npm install snippets should point at `npm install -g @seemseam/cc-bridge` when npm is the recommended install path.

npm auto-completion requirements:

- Treat npm publishing as enabled for every CC_BRIDGE package release. If `package.json` is absent, add the root npm wrapper package instead of skipping npm.
- If npm wrapper scripts are absent, add or repair CLI bins for `cc-bridge`, `ask`, `autonew`, and `ctx-transfer` so the npm package installs the intended GitHub Release payload.
...
- `package.json` version must match `VERSION`, `cc-bridge` `VERSION`, and the release tag without the leading `v`.
- `package.json` must publish `@seemseam/cc-bridge` as a public package with CLI bins for `cc-bridge`, `ask`, `autonew`, and `ctx-transfer`.
...
```bash
pytest -q
python -m compileall -q lib cc-bridge
git diff --check
npm pack --dry-run
python scripts/build_linux_release.py --allow-dirty --output-dir dist-release-local
```

For startup, tmux, cc-bridge-daemon, provider auth, or release asset changes, add the relevant targeted tests or smoke commands before publishing.
...
2. Run:
   ```bash
   python dev_tools/skills/cc-bridge-github/scripts/check_release_state.py --phase prepare --repo SeemSeam/claude_codex_bridge
   ```
...
4. Merge the maintenance branch into the default branch so GitHub homepage README changes are visible:
   ...
   ```
5. Wait for default-branch `Tests`, `CC_BRIDGE_DAEMON Real Platform Smoke`, and `Cross-Platform Compatibility Test`.
6. Run:
   ```bash
   python dev_tools/skills/cc-bridge-github/scripts/check_release_state.py --phase published --repo SeemSeam/claude_codex_bridge --wait-seconds 1800
   ```
...
10. Let `Npm Publish` publish `@seemseam/cc-bridge@X.Y.Z` through npm Trusted Publishing.
11. Confirm `Release Artifacts` is green for the tag or a valid `workflow_dispatch` recovery on the release tag commit, confirm `Npm Publish` is green for the release tag commit, and confirm branch validation workflows for the release commit are green or consciously accepted as warnings:
   - `Tests`
   - `CC_BRIDGE_DAEMON Real Platform Smoke`
...
- Release assets missing: rerun the `Release Artifacts` workflow for the tag, then verify `cc-bridge-linux-x86_64.tar.gz`, `cc-bridge-macos-universal.tar.gz`, and `SHA256SUMS`.
...
- npm Trusted Publisher mismatch: configure npm package `@seemseam/cc-bridge` with owner `SeemSeam`, repository `claude_codex_bridge`, workflow filename `npm-publish.yml`, blank environment, and allowed action `npm publish`; then rerun `Npm Publish` for the release tag.
...
- `VERSION`, `cc-bridge`, changelog, badges, or release notes disagree.
- `package.json` version differs from `VERSION`, `cc-bridge` `VERSION`, or the release tag.
...
- npm `latest` does not resolve to `@seemseam/cc-bridge@X.Y.Z` after an npm-enabled release.
