...
## AI Collaboration
Use `/ask <agent>` to contact another CC_BRIDGE agent by name.
Use `/ping <agent|cc-bridge-daemon>` to inspect project control-plane health.
Use `/pend <agent|job_id>` to inspect mailbox/job replies.

Agent names come from `.cc-bridge/cc-bridge.config`. Providers are implementation details.

## Async Guardrail (MANDATORY)

When you run `ask` (via `/ask` skill OR direct `Bash(ask ...)`) and the output contains `[CC_BRIDGE_ASYNC_SUBMITTED`:
