...
def _build_start_handler():
    path = Path(__file__).resolve().parents[1] / 'lib' / 'cc_bridge_daemon' / 'handlers' / 'start.py'
    spec = importlib.util.spec_from_file_location('cc_bridge_daemon_start_handler_for_test', path)
...
def test_start_handler_defaults_missing_auto_permission_to_plain_cc_bridge_policy() -> None:
