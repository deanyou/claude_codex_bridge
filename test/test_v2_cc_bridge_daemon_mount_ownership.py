...
from cc_bridge_daemon.services.ownership import OwnershipConflictError, OwnershipGuard
from cc_bridge_daemon.services.project_namespace_state import ProjectNamespaceState, ProjectNamespaceStateStore
from cc_bridge_daemon.services.runtime import RuntimeService
...
class FakeTmuxBackend:
    def __init__(
        ...
        *,
        exists: bool = True,
        alive: bool = True,
        pane_title: str = 'CC_BRIDGE-codex-demo',
    ...
    def describe_pane(self, pane_id: str, *, user_options: tuple[str, ...] = ()) -> dict[str, str]:
        ...
        for name in user_options:
            if name == '@cc_bridge_agent':
                described[name] = str(self.owner_agent or '')
            elif name == '@cc_bridge_project_id':
...
def test_mount_manager_refresh_heartbeat_zero_interval_keeps_every_tick(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv('CC_BRIDGE_CC_BRIDGE_DAEMON_HEARTBEAT_WRITE_INTERVAL_S', '0')
...
def test_mount_manager_refresh_heartbeat_rejects_replaced_lease_holder(tmp_path: Path) -> None:
    ...
    assert lease.cc_bridge_daemon_pid == 654
...
def test_mount_manager_update_config_signature_requires_current_holder_and_generation(tmp_path: Path) -> None:
    ...
    assert updated.cc_bridge_daemon_pid == 321
...
def test_ownership_guard_blocks_healthy_lease_and_allows_stale_takeover(tmp_path: Path) -> None:
    ...
    manager.mark_mounted(project_id=ctx.project_id, pid=111, socket_path=layout.cc_bridge_daemon_socket_path, generation=3)
    ...
    with pytest.raises(OwnershipConflictError):
        healthy_guard.verify_or_takeover(project_id=ctx.project_id, pid=222, socket_path=layout.cc_bridge_daemon_socket_path)
    ...
    assert stale_guard.verify_or_takeover(project_id=ctx.project_id, pid=222, socket_path=layout.cc_bridge_daemon_socket_path) == 4
...
def test_ownership_guard_allows_unmounted_foreign_lease_takeover(tmp_path: Path) -> None:
    ...
    assert guard.verify_or_takeover(project_id=ctx.project_id, pid=222, socket_path=layout.cc_bridge_daemon_socket_path) == 10
...
def test_ownership_guard_blocks_mounted_foreign_lease(tmp_path: Path) -> None:
    ...
    with pytest.raises(OwnershipConflictError, match='lease project_id mismatch'):
        guard.verify_or_takeover(project_id=ctx.project_id, pid=222, socket_path=layout.cc_bridge_daemon_socket_path)
...
def test_ownership_guard_marks_fresh_socket_failure_as_degraded(tmp_path: Path) -> None:
    ...
    manager.mark_mounted(project_id=ctx.project_id, pid=111, socket_path=layout.cc_bridge_daemon_socket_path, generation=5)
    ...
    with pytest.raises(OwnershipConflictError):
        degraded_guard.verify_or_takeover(project_id=ctx.project_id, pid=222, socket_path=layout.cc_bridge_daemon_socket_path)
...
def test_ownership_guard_allows_takeover_when_socket_and_heartbeat_are_stale(tmp_path: Path) -> None:
    ...
    manager.mark_mounted(project_id=ctx.project_id, pid=111, socket_path=layout.cc_bridge_daemon_socket_path, generation=7)
    ...
    assert stale_guard.verify_or_takeover(project_id=ctx.project_id, pid=222, socket_path=layout.cc_bridge_daemon_socket_path) == 8
...
def test_health_monitor_marks_same_socket_detached_namespace_pane_foreign(tmp_path: Path) -> None:
    ...
    class NamespaceBackend(FakeTmuxBackend):
        def __init__(self) -> None:
            ...
            self._socket_path = str(layout.cc_bridge_daemon_tmux_socket_path)
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-repo',
        )
    )
    monitor = HealthMonitor(
        registry,
...
def test_cc_bridge_daemon_heartbeat_recovers_degraded_agent_and_drains_queue(tmp_path: Path, monkeypatch) -> None:
    ...
    session.data = {
        'agent_name': 'codex',
        'cc_bridge_project_id': ctx.project_id,
        'pane_id': '%77',
        'terminal': 'tmux',
    }
    binding = type(
...
def test_cc_bridge_daemon_heartbeat_does_not_proactively_mount_missing_agent_without_start_policy(tmp_path: Path, monkeypatch) -> None:
...
def test_cc_bridge_daemon_heartbeat_uses_persisted_start_policy_for_recovery_mount(tmp_path: Path, monkeypatch) -> None:
...
def test_cc_bridge_daemon_heartbeat_keeps_backend_mounted_on_background_supervision_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    while time.time() < deadline:
        ...
        raise AssertionError('cc_bridge_daemon did not reach mounted while serving')
    ...
    monkeypatch.setattr('cc_bridge_daemon.app_runtime.lifecycle.full_heartbeat_due', lambda app, started: True)
    ...
    while time.time() < deadline:
        ...
        raise AssertionError('cc_bridge_daemon did not record the background supervision failure')
...
def test_cc_bridge_daemon_request_shutdown_does_not_unmount_replaced_lease_holder(tmp_path: Path) -> None:
    ...
    assert lease is not None
    assert lease.mount_state is MountState.MOUNTED
    assert lease.cc_bridge_daemon_pid == app.pid + 1
    assert lease.daemon_instance_id == 'replacement-daemon'
    assert app.lifecycle_store.load() == old_lifecycle


def test_cc_bridge_daemon_foreign_pane_reflow_uses_persisted_start_policy(tmp_path: Path, monkeypatch) -> None:
