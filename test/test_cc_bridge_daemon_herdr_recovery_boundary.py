...
from cc_bridge_daemon.services.dispatcher_runtime.lifecycle_start_runtime.models import QueuedTargetSlot
from cc_bridge_daemon.services.dispatcher_runtime.lifecycle_start_runtime.tick import tick_jobs
from cc_bridge_daemon.services.runtime_recovery_policy import (
...
from cc_bridge_daemon.services.dispatcher_runtime.lifecycle_start_runtime.recovery import (
    refresh_slot_runtime_for_start,
)
from cc_bridge_daemon.services.runtime import RuntimeService
from cc_bridge_daemon.services.runtime_runtime.refresh import refresh_provider_binding
from cc_bridge_daemon.supervision.recovery import recover_runtime
from cc_bridge_daemon.supervision.recovery_events import append_recovery_event
from cc_bridge_daemon.supervision.loop_runtime import runtime_requires_recovery
...
def test_herdr_recovery_policy_keeps_cc_bridge_as_owner() -> None:
    policy = herdr_recovery_policy(_herdr_runtime())

    assert policy is not None
    assert policy["owner"] == HERDR_RECOVERY_OWNER == "cc_bridge"
...
def test_herdr_recovery_admits_cc_bridge_owned_runtime_healths(health: str) -> None:
...
def _runtime_requires_recovery(runtime) -> bool:
    ctx = SimpleNamespace(
        ...
        layout=SimpleNamespace(cc_bridge_daemon_tmux_socket_path=""),
