...
SCRIPT = Path(__file__).resolve().parents[1] / "dev_tools" / "skills" / "cc_bridge-github" / "scripts" / "check_release_state.py"


def _load_checker():
    spec = importlib.util.spec_from_file_location("cc_bridge_github_release_checker", SCRIPT)
...
def _init_repo_with_remote(tmp_path: Path) -> Path:
    ...
    _git(repo, "config", "user.email", "cc_bridge@example.invalid")
    _git(repo, "config", "user.name", "CC_BRIDGE Test")
...
def test_dev_change_classification_flags_release_and_dev_only_paths() -> None:
    ...
    assert checker.classify_dev_path("dev_tools/skills/cc_bridge-github/SKILL.md") == "dev_tools"
    assert checker.classify_dev_path("test/test_cc_bridge_github_skill.py") == "verification"
    ...
    assert checker.classify_dev_path("cc_bridge") == "runtime_package"
...
def test_check_local_files_accepts_cc_bridge_py_version_for_shell_launcher(tmp_path: Path) -> None:
    checker = _load_checker()
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "VERSION").write_text("9.9.9\n", encoding="utf-8")
    (repo / "cc_bridge").write_text("#!/usr/bin/env bash\nexec ./cc_bridge.py \"$@\"\n", encoding="utf-8")
    (repo / "cc_bridge.py").write_text('VERSION = "9.9.9"\n', encoding="utf-8")
...
def test_check_sha256sums_requires_entries_for_tarballs(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_run(cmd: list[str], cwd: Path, *, timeout: int = 60):
        ...
        (target_dir / "SHA256SUMS").write_text(
            "0" * 64 + "  cc_bridge-linux-x86_64.tar.gz\n",
            encoding="utf-8",
        )
    ...
    assert any("cc_bridge-macos-universal.tar.gz" in item for item in issues)
...
def test_active_skill_sync_warns_when_runtime_copy_differs(tmp_path: Path) -> None:
    checker = _load_checker()
    source = tmp_path / "dev_tools" / "skills" / "cc_bridge-github"
    active = tmp_path / ".cc-bridge" / "agents" / "agent4" / "provider-state" / "codex" / "home" / "skills" / "cc_bridge-github"
    ...
    assert any("Active cc_bridge-github skill copy differs" in item for item in warnings)
