...
def test_run_cli_entrypoint_prints_start_help_without_phase2() -> None:
    ...
    result = run_cli_entrypoint(
        ["--help"],
        version="5.2.8",
        script_root=Path("/tmp/cc_bridge"),
    ...
    assert result == 0
    assert "usage: cc_bridge [-s] [-n]" in stdout.getvalue()
    assert "Primary workflow:" in stdout.getvalue()
    assert "cc_bridge -s" in stdout.getvalue()
    assert "cc_bridge clear [agent...]" in stdout.getvalue()
    assert "cc_bridge maintenance status Show maintenance heartbeat config and stored status." in stdout.getvalue()
    assert "cc_bridge theme [system|dark|light|+|-|PRESET]" in stdout.getvalue()
    assert "Core commands:" in stdout.getvalue()
    assert "cc_bridge ask <agent> [from <sender>] <message>" in stdout.getvalue()
    assert "cc_bridge doctor" in stdout.getvalue()
    assert "Diagnostics-only control-plane status:" in stdout.getvalue()
    assert "Diagnostics-only observer:" in stdout.getvalue()
    assert "cc_bridge pend <agent|job_id> [N]" in stdout.getvalue()
    assert "cc_bridge pend --queue [--detail] <agent|all>" in stdout.getvalue()
    assert "Advanced views:" in stdout.getvalue()
    assert "cc_bridge queue [--detail] <agent|all>" in stdout.getvalue()
    assert "cc_bridge trace <id>" in stdout.getvalue()
    assert "Advanced recovery:" in stdout.getvalue()
    assert "cc_bridge repair <ack|retry|resubmit> ..." in stdout.getvalue()
    assert "cc_bridge install mobile" in stdout.getvalue()
    assert "cc_bridge rich" in stdout.getvalue()
    assert "cc_bridge update rich" in stdout.getvalue()
    assert "cc_bridge update mobile" in stdout.getvalue()
    assert "cc_bridge rich-install" not in stdout.getvalue()
    assert "cc_bridge watch <agent|job_id>" not in stdout.getvalue()
    assert "cc_bridge inbox [--detail] <agent>" not in stdout.getvalue()
    assert "cc_bridge ps | cc_bridge logs <agent>" not in stdout.getvalue()
...
def test_run_cli_entrypoint_rejects_removed_rich_install() -> None:
    ...
    assert "`cc_bridge rich-install` has been removed" in stderr.getvalue()
...
def test_run_cli_entrypoint_routes_install_mobile_before_phase2(monkeypatch) -> None:
    ...
    def _install(args, *, script_root):
        calls.append(args)
        assert script_root == Path("/tmp/cc_bridge")
        return 41
...
def test_run_cli_entrypoint_routes_internal_mobile_host_serve_before_phase2(monkeypatch) -> None:
    ...
    assert result == 37
    assert calls == [(["__mobile-host-serve", "--listen", "127.0.0.1:8787"], Path("/tmp/cc_bridge"))]
...
def test_run_cli_entrypoint_routes_rich(monkeypatch) -> None:
    ...
    assert stderr.getvalue() == ""
    assert calls == [(Path("/tmp/cc_bridge"), Path("/tmp/project"), stdout, stderr)]
...
def test_run_cli_entrypoint_routes_theme_without_project_discovery(monkeypatch, tmp_path: Path) -> None:
    ...
    assert result == 0
    payload = json.loads((config_home / "cc_bridge" / "theme.json").read_text(encoding="utf-8"))
    assert payload == {
        "palette": "latte",
        "schema_version": 1,
        "theme": "light",
...
def test_run_cli_entrypoint_prints_theme_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge theme" in stdout.getvalue()
    assert "cc_bridge theme +" in stdout.getvalue()
    assert "cc_bridge theme system" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_rich_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge rich" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_auto_launches_rich_for_plain_start(monkeypatch) -> None:
    ...
    assert result == 0
    assert calls == [(Path("/tmp/cc_bridge"), Path("/tmp/elsewhere"), ("--project", "/tmp/project", "-n"))]
...
def test_run_cli_entrypoint_prints_kill_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge kill [-f]" in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_clear_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge clear [agent_name|all]..." in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_compact_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge compact [agent_name|all]..." in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_ping_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge ping <agent|all|cc_bridge_daemon>" in stdout.getvalue()
    assert "Diagnostics-only control-plane status:" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_pend_help_as_supplementary_status() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge pend [--watch|--inbox|--queue] [--detail] <agent|job_id|all> [N]" in stdout.getvalue()
    ...
    assert "cc_bridge pend --watch <agent|job_id>" in stdout.getvalue()
    assert "cc_bridge pend --queue <agent|all>" in stdout.getvalue()
    assert "cc_bridge pend --queue --detail <agent>" in stdout.getvalue()
    assert "Use `cc_bridge trace <id>` for lineage when needed." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_watch_help_with_project_prefix() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge watch <agent|job_id>" in stdout.getvalue()
    ...
    assert "Prefer `cc_bridge pend --watch <agent|job_id>` as the converged observer entrypoint." in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_ps_help_as_runtime_diagnostics_view() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge ps" in stdout.getvalue()
    assert "Runtime diagnostics compatibility view:" in stdout.getvalue()
    assert "Prefer `cc_bridge doctor ps` as the converged diagnostics entrypoint." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_logs_help_as_runtime_diagnostics_view() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge logs <agent>" in stdout.getvalue()
    assert "Runtime diagnostics compatibility view:" in stdout.getvalue()
    assert "Prefer `cc_bridge doctor logs <agent>` as the converged diagnostics entrypoint." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_doctor_help_with_converged_subviews() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge doctor [ps|logs <agent>|storage] [--output [PATH]]" in stdout.getvalue()
    assert "cc_bridge doctor ps" in stdout.getvalue()
    assert "cc_bridge doctor logs <agent>" in stdout.getvalue()
    assert "cc_bridge doctor storage" in stdout.getvalue()
    assert "`cc_bridge ps` and `cc_bridge logs <agent>` remain compatibility entrypoints." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_doctor_storage_subview_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge doctor storage [--json]" in stdout.getvalue()
    assert "cc_bridge doctor storage --json" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_cleanup_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge cleanup" in stdout.getvalue()
    assert "Use `cc_bridge doctor storage` before cleanup" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_doctor_ps_subview_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge doctor ps" in stdout.getvalue()
    assert "Runtime diagnostics subview:" in stdout.getvalue()
    assert "`cc_bridge ps` remains a compatibility alias." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_doctor_logs_subview_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge doctor logs <agent>" in stdout.getvalue()
    assert "Runtime log diagnostics subview:" in stdout.getvalue()
    assert "`cc_bridge logs <agent>` remains a compatibility alias." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_queue_help_as_advanced_view() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge queue [--detail] <agent_name|all>" in stdout.getvalue()
    assert "Advanced backlog view:" in stdout.getvalue()
    assert "`cc_bridge pend --queue [--detail] <agent|all>` remains the equivalent weak-observer form." in stdout.getvalue()
    assert "cc_bridge queue --detail <agent_name>" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_maintenance_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge maintenance <status|tick|schedule>" in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_pend_help_with_converged_observer_modes() -> None:
    ...
    assert "usage: cc_bridge pend [--watch|--inbox|--queue] [--detail] <agent|job_id|all> [N]" in stdout.getvalue()
    assert "cc_bridge pend --watch <agent|job_id>" in stdout.getvalue()
    assert "cc_bridge pend --inbox --detail <agent>" in stdout.getvalue()
    assert "cc_bridge pend --queue --detail <agent>" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_trace_help_as_advanced_view() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge trace <submission_id|message_id|attempt_id|reply_id|job_id>" in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_cancel_help_as_job_control_view() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge cancel <job_id>" in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_inbox_help_as_supplementary_status() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge inbox [--detail] <agent_name>" in stdout.getvalue()
    assert "Weak observer compatibility entrypoint:" in stdout.getvalue()
    assert "Prefer `cc_bridge pend --inbox [--detail] <agent>` as the converged observer entrypoint." in stdout.getvalue()
    assert "cc_bridge inbox --detail <agent_name>" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_ack_help_as_advanced_recovery() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge ack <agent_name> [inbound_event_id]" in stdout.getvalue()
    assert "Advanced recovery compatibility entrypoint:" in stdout.getvalue()
    assert "Prefer `cc_bridge repair ack <agent_name> [inbound_event_id]` as the converged recovery entrypoint." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_retry_help_as_advanced_recovery() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge retry <job_id|attempt_id>" in stdout.getvalue()
    assert "Advanced recovery compatibility entrypoint:" in stdout.getvalue()
    assert "Prefer `cc_bridge repair retry <job_id|attempt_id>` as the converged recovery entrypoint." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_resubmit_help_as_advanced_recovery() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge resubmit <message_id>" in stdout.getvalue()
    assert "Advanced recovery compatibility entrypoint:" in stdout.getvalue()
    assert "Prefer `cc_bridge repair resubmit <message_id>` as the converged recovery entrypoint." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_repair_help_as_primary_advanced_recovery() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge repair <ack|retry|resubmit> ..." in stdout.getvalue()
    assert "Advanced recovery:" in stdout.getvalue()
    assert "cc_bridge repair ack <agent_name> [inbound_event_id]" in stdout.getvalue()
    assert "cc_bridge repair retry <job_id|attempt_id>" in stdout.getvalue()
    assert "cc_bridge repair resubmit <message_id>" in stdout.getvalue()
...
def test_run_cli_entrypoint_prints_repair_ack_subcommand_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge repair ack <agent_name> [inbound_event_id]" in stdout.getvalue()
    assert "Advanced recovery subcommand:" in stdout.getvalue()
    assert "`cc_bridge ack <agent_name> [inbound_event_id]` remains a compatibility alias." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_repair_retry_subcommand_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge repair retry <job_id|attempt_id>" in stdout.getvalue()
    assert "Advanced recovery subcommand:" in stdout.getvalue()
    assert "`cc_bridge retry <job_id|attempt_id>` remains a compatibility alias." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_repair_resubmit_subcommand_help() -> None:
    ...
    assert result == 0
    assert "usage: cc_bridge repair resubmit <message_id>" in stdout.getvalue()
    assert "Advanced recovery subcommand:" in stdout.getvalue()
    assert "`cc_bridge resubmit <message_id>` remains a compatibility alias." in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_prints_ask_help() -> None:
    ...
    assert (
        "cc_bridge ask [--compact] [--silence] [--chain] [--artifact-request] [--inline-request] [--artifact-reply] <target> [--] <message...>"
        in stdout.getvalue()
    )
    ...
    assert "--artifact-request force the request body into a CC_BRIDGE text artifact" in stdout.getvalue()
    assert "--inline-request keep the request body inline and disable automatic artifact spill" in stdout.getvalue()
    assert "--artifact-reply force the final reply into a CC_BRIDGE text artifact" in stdout.getvalue()
    assert "--artifact-io enable both --artifact-request and --artifact-reply" in stdout.getvalue()
    assert "cc_bridge ask --compact agent1 review latest diff" in stdout.getvalue()
    assert "cc_bridge ask --silence agent1 run smoke check" in stdout.getvalue()
    assert "cc_bridge ask --chain agent2 collect evidence for this task" in stdout.getvalue()
    assert "cc_bridge ask --chain --artifact-reply agent2 collect long evidence" in stdout.getvalue()
    legacy_chain_flag = "--" + "call" + "back"
    assert legacy_chain_flag not in stdout.getvalue()
    assert "cc_bridge ask get <job_id>    diagnostics-only: inspect one submitted job" in stdout.getvalue()
    assert stderr.getvalue() == ""
...
def test_run_cli_entrypoint_rejects_removed_provider_command() -> None:
    ...
    assert result == 2
    assert stdout.getvalue() == ""
    assert "`cc_bridge provider` has been removed" in stderr.getvalue()
    assert "Use `cc_bridge ask` for task submission/results, `cc_bridge doctor` for diagnostics, and `cc_bridge trace` for lineage details." in stderr.getvalue()
...
def test_run_cli_entrypoint_rejects_removed_mail_command() -> None:
    ...
    assert result == 2
    assert stdout.getvalue() == ""
    assert "`cc_bridge mail` has been removed" in stderr.getvalue()
    assert "Use `cc_bridge ask` for task submission/results, `cc_bridge doctor` for diagnostics, and `cc_bridge trace` for lineage details." in stderr.getvalue()
