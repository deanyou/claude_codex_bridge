...
from cc_bridge_daemon.app_runtime.service_graph import CcbdServiceGraphDependencies, build_cc_bridge_daemon_service_graph
from cc_bridge_daemon.lifecycle_report_store import CcbdShutdownReportStore, CcbdStartupReportStore
from cc_bridge_daemon.metrics import ControlPlaneMetrics
from cc_bridge_daemon.models import CcbdStartupAgentResult
from cc_bridge_daemon.project_view import ProjectViewStateStore
from cc_bridge_daemon.reload_runtime_move import run_moved_agent_runtime_updates
from cc_bridge_daemon.reload_runtime_mount import run_additive_agent_mounts
from cc_bridge_daemon.reload_apply_runtime import run_runtime_mount
from cc_bridge_daemon.services import CcbdLifecycleStore, MountManager, OwnershipGuard, SnapshotWriter
...
def test_additive_runtime_mount_reuses_retired_same_name_runtime_residue(tmp_path: Path) -> None:
    ...
    graph.registry.upsert_authority(
        replace(
            ...
            session_file=str(app.paths.cc_bridge_dir / '.codex-agent3-session'),
    ...
    assert mounted.session_file == str(app.paths.cc_bridge_dir / '.codex-agent3-session')
...
def _build_graph(app: CcbdApp, config_text: str, *, version: int):
    ...
    return build_cc_bridge_daemon_service_graph(
...
def _forbid_namespace_recreate_paths(app: CcbdApp, monkeypatch) -> None:
    ...
    monkeypatch.setattr('cc_bridge_daemon.services.project_namespace_runtime.ensure.ensure_project_namespace', _fail, raising=False)
    monkeypatch.setattr('cc_bridge_daemon.services.project_namespace_runtime.backend.kill_server', _fail, raising=False)
    monkeypatch.setattr('cc_bridge_daemon.services.project_namespace_runtime.backend.kill_window', _fail, raising=False)
    monkeypatch.setattr('cc_bridge_daemon.services.project_namespace_runtime.reflow.reflow_project_workspace', _fail, raising=False)
