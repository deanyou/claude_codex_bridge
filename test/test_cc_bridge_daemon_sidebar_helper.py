...
from cc_bridge_daemon.services.project_namespace_runtime import materialize_topology
from cc_bridge_daemon.services.project_namespace_runtime.sidebar_helper import (
    SIDEBAR_ENV_PATH,
...
def test_resolve_sidebar_helper_finds_repository_bin(tmp_path: Path) -> None:
    helper = tmp_path / 'repo' / 'bin' / 'cc_bridge-agent-sidebar'
...
def test_resolve_sidebar_helper_uses_path_as_last_discovery_source(tmp_path: Path) -> None:
    resolution = resolve_sidebar_helper(
        env={},
        which=lambda name: '/usr/local/bin/cc_bridge-agent-sidebar',
        script_root=tmp_path / 'repo',
    )

    assert resolution.path == '/usr/local/bin/cc_bridge-agent-sidebar'
...
def test_sidebar_respawn_args_replaces_symbolic_binary_with_resolved_path(tmp_path: Path) -> None:
    helper = tmp_path / 'cc_bridge-agent-sidebar'
    helper.write_text('#!/bin/sh\n', encoding='utf-8')
    helper.chmod(0o755)

    args = sidebar_respawn_args(
        ('cc_bridge-agent-sidebar', '--pane-window', 'main'),
        env={SIDEBAR_ENV_PATH: str(helper)},
        which=lambda name: None,
        script_root=tmp_path / 'repo',
    )
...
def test_sidebar_respawn_args_falls_back_to_visible_keepalive_message(tmp_path: Path) -> None:
    ...
    assert args[:2] == ('sh', '-lc')
    assert 'CC_BRIDGE sidebar helper unavailable' in args[2]
...
def test_sidebar_helper_fingerprint_tracks_runtime_binary_behind_source_wrapper(tmp_path: Path) -> None:
    root = tmp_path / 'repo'
    wrapper = root / 'bin' / 'cc_bridge-agent-sidebar'
    runtime_binary = root / 'tools' / 'cc_bridge-agent-sidebar' / 'target' / 'release' / 'cc_bridge-agent-sidebar'
...
def test_changed_sidebar_helper_respawns_only_sidebar_pane(monkeypatch, tmp_path: Path) -> None:
    pane_options = {'%1': {'@cc_bridge_sidebar_helper_id': 'sha256:old'}}
    ...
    sidebar = SimpleNamespace(launch_args=('cc_bridge-agent-sidebar', '--pane-window', 'main'))
    ...
    monkeypatch.setattr(
        ...
        lambda _backend, pane_id: ProjectNamespacePaneRecord(
            pane_id=pane_id,
            session_name='cc_bridge-project',
            window_name='main',
            role='sidebar',
    ...
    assert pane_options['%1']['@cc_bridge_sidebar_helper_id'] == 'sha256:new'
