...
pytestmark = pytest.mark.skipif(
    os.environ.get('CC_BRIDGE_PROVIDER_MEMORY_MATRIX_CHECK') != '1',
    reason='set CC_BRIDGE_PROVIDER_MEMORY_MATRIX_CHECK=1 to inspect the external provider memory matrix project',
)


DEFAULT_MATRIX_PROJECT = Path('/home/bfly/yunwei/test_cc_bridge_provider_memory_matrix')


def test_external_provider_memory_matrix_has_expected_layered_context() -> None:
    project_root = Path(os.environ.get('CC_BRIDGE_PROVIDER_MEMORY_MATRIX_PROJECT') or DEFAULT_MATRIX_PROJECT).resolve()
    if not project_root.exists():
        pytest.skip(f'external provider memory matrix project does not exist: {project_root}')

    codex = _read(project_root / '.cc-bridge' / 'agents' / 'codexer' / 'provider-state' / 'codex' / 'home' / 'AGENTS.md')
    claude = _read(
        project_root
        / '.cc-bridge'
        / 'agents'
    ...
    opencode = _read(project_root / '.cc-bridge' / 'runtime' / 'memory' / 'opencoder.md')
    gemini = _read(
        project_root
        / '.cc-bridge'
        / 'agents'
    ...
    opencode_config = json.loads(
        (project_root / '.cc-bridge' / 'agents' / 'opencoder' / 'provider-state' / 'opencode' / 'opencode.json').read_text(
            encoding='utf-8'
        )
    )
    assert opencode_config['instructions'] == [
        'AGENTS.md',
        '.cc-bridge/runtime/memory/opencoder.md',
        '.cc-bridge/runtime/skills/opencoder/opencode/ask.md',
    ]
