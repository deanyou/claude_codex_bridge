...
from cc_bridge_daemon.control_plane_transport.fake import FakeConnection, FakeControlPlaneTransport
from cc_bridge_daemon.socket_client_runtime import CcbdClientError, decode_response, recv_response_line, send_request
...
def test_fake_transport_listen_request_and_shutdown_roundtrip() -> None:
    ...
    server = CcbdSocketServer('fake://cc_bridge_daemon.sock', control_plane_transport=transport)
    calls: list[dict] = []
    ...
    try:
        ...
        request = RpcRequest(op='ping', request={'target': 'cc_bridge_daemon'})
        ...
        assert calls == [{'target': 'cc_bridge_daemon'}]
