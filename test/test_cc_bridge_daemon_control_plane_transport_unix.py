...
from cc_bridge_daemon.control_plane_transport.endpoint import endpoint_from_legacy_socket_path, endpoint_from_record, endpoint_to_record
from cc_bridge_daemon.control_plane_transport.factory import transport_for_endpoint
from cc_bridge_daemon.control_plane_transport.unix import UnixControlPlaneListener
from cc_bridge_daemon.models import CcbdLease, MountState
from cc_bridge_daemon.services.lifecycle import CcbdLifecycle, build_lifecycle
from cc_bridge_daemon.services.project_inspection import ProjectDaemonInspection
from cc_bridge_daemon.socket_client_runtime import CcbdClientError, connect_socket


def test_endpoint_legacy_projection_roundtrips() -> None:
    endpoint = endpoint_from_legacy_socket_path('C:/tmp/cc_bridge_daemon.sock')
    expected = str(Path('C:/tmp/cc_bridge_daemon.sock'))
...
def test_endpoint_record_normalizes_unix_socket_path_like_legacy_path() -> None:
    endpoint = endpoint_from_record({'kind': 'unix_socket', 'address': '  C:/tmp/cc_bridge_daemon.sock  '})

    assert endpoint == endpoint_from_legacy_socket_path('C:/tmp/cc_bridge_daemon.sock')
...
def test_transport_factory_accepts_legacy_socket_path_record(monkeypatch, tmp_path: Path) -> None:
    ...
    transport = transport_for_endpoint({'socket_path': str(tmp_path / 'cc_bridge_daemon.sock')})

    assert transport.endpoint['kind'] == 'unix_socket'
    assert str(transport.socket_path) == str(tmp_path / 'cc_bridge_daemon.sock')
...
def test_unix_listener_exposes_selectable_file_descriptor() -> None:
    ...
    listener = UnixControlPlaneListener(
        endpoint=endpoint_from_legacy_socket_path('/tmp/cc_bridge_daemon.sock'),
        socket_path=Path('/tmp/cc_bridge_daemon.sock'),
        socket=_Socket(),
...
def test_unix_connect_adapter_wraps_unsupported_platform(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr('cc_bridge_daemon.control_plane_transport.factory.os.name', 'posix')
    monkeypatch.delattr('cc_bridge_daemon.control_plane_transport.unix.socket.AF_UNIX', raising=False)

    with pytest.raises(CcbdClientError, match='unix domain sockets are not supported'):
        connect_socket(tmp_path / 'cc_bridge_daemon.sock', timeout_s=0.1)
...
def test_ownership_guard_default_probe_uses_endpoint_descriptor(monkeypatch, tmp_path: Path) -> None:
    ...
    seen: list[dict] = []
    monkeypatch.setattr(
        'cc_bridge_daemon.services.ownership.endpoint_connectable',
