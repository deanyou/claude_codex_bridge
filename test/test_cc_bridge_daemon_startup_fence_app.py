...
from cc_bridge_daemon.models import CcbdStartupReport, MountState
from cc_bridge_daemon.services.lifecycle import build_lifecycle
from cc_bridge_daemon.startup_fence import ExpectedStartupFence, StartupFenceError
...
def test_fenced_child_rejects_superseded_transaction_before_lease_or_socket_mutation(
    ...
    lifecycle_bytes = app.paths.cc_bridge_daemon_lifecycle_path.read_bytes()
    ...
    assert app.mount_manager.load_state() is None
    assert app.paths.cc_bridge_daemon_lifecycle_path.read_bytes() == lifecycle_bytes
    assert not app.paths.cc_bridge_daemon_socket_path.exists()
...
def test_fenced_child_claim_conflict_leaves_lifecycle_and_lease_unchanged(
    ...
    lifecycle_bytes = app.paths.cc_bridge_daemon_lifecycle_path.read_bytes()
    lease_bytes = app.paths.cc_bridge_daemon_lease_path.read_bytes()
    ...
    assert app.lease is None
    assert app.paths.cc_bridge_daemon_lifecycle_path.read_bytes() == lifecycle_bytes
    assert app.paths.cc_bridge_daemon_lease_path.read_bytes() == lease_bytes
    assert not app.paths.cc_bridge_daemon_socket_path.exists()
