...
def test_windows_tcp_transport_does_not_add_named_pipe_or_af_unix_branches() -> None:
    root = Path('lib/cc_bridge_daemon/control_plane_transport')
    offenders: list[str] = []
...
def test_token_secret_is_not_written_to_endpoint_or_diagnostics(tmp_path: Path) -> None:
    from cc_bridge_daemon.control_plane_transport.endpoint import endpoint_to_record, tcp_endpoint
    from cc_bridge_daemon.control_plane_transport.token_auth import TokenFile, redacted_token_diagnostics
