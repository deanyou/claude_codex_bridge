...
import cc_bridge_daemon.control_plane_transport.unix as unix_transport
...
def test_listen_refuses_to_replace_live_foreign_socket(tmp_path: Path) -> None:
    ...
    server = CcbdSocketServer(socket_path)
    try:
        with pytest.raises(RuntimeError, match='refusing to replace live cc_bridge_daemon socket'):
            server.listen()
