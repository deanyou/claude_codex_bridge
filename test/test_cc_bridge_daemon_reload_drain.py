...
from cc_bridge_daemon.reload_drain import (
    DrainBounds,
    DrainIntent,
    DrainQueue,
    DrainQueueStore,
...
def test_drain_queue_store_round_trips_to_bounded_cc_bridge_daemon_state(tmp_path: Path) -> None:
    ...
    assert app.paths.cc_bridge_daemon_reload_drain_path.name == 'reload-drain.json'
    assert app.paths.cc_bridge_daemon_reload_drain_path.exists()
