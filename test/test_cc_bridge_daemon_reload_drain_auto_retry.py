...
from cc_bridge_daemon.reload_drain import DrainIntent, DrainQueue, DrainQueueStore, plan_drain_transition
...
def test_reload_drain_auto_retry_can_be_disabled(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CC_BRIDGE_CC_BRIDGE_DAEMON_RELOAD_DRAIN_AUTO_RETRY", "0")
...
def _app_with_active_drain(project_root: Path, *, target_config: str, busy: bool) -> CcbdApp:
    ...
    (project_root / ".cc-bridge" / "cc_bridge.config").write_text(BASE_CONFIG, encoding="utf-8")
    ...
    (project_root / ".cc-bridge" / "cc_bridge.config").write_text(target_config, encoding="utf-8")
    return app
