...
from cc_bridge_daemon.start_preparation import PreparedStartAgent, _prepare_provider_launch_set
from cc_bridge_daemon.supervisor_runtime.reporting import record_startup_report
...
def test_provider_prepare_counts_attempt_completion_and_scoped_atomic_writes(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.start_preparation.provider_workspace_path_for_prepare',
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_preparation.prepare_provider_workspace', prepare_provider_workspace)
...
def test_failed_startup_report_retains_request_operation_snapshot_and_excludes_own_write(
    ...
    supervisor = SimpleNamespace(
        ...
        _paths=SimpleNamespace(
            ...
            cc_bridge_daemon_socket_placement=None,
            cc_bridge_daemon_tmux_socket_placement=None,
        ),
        _startup_report_store=SimpleNamespace(save=save_report),
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.supervisor_runtime.reporting.socket_placement_payload',
