...
def test_install_script_links_sidebar_helper() -> None:
    ...
    assert 'bin/cc_bridge-agent-sidebar' in text
    assert 'bin/build-cc_bridge-agent-sidebar' in text
    ...
    assert 'ERROR: cc_bridge-agent-sidebar binary not available' in text


def test_install_script_links_rs_helper() -> None:
    text = Path('install.sh').read_text(encoding='utf-8')

    assert 'bin/cc_bridge-rs-helper' in text
    assert 'bin/build-cc_bridge-rs-helper' in text
    ...
    assert 'cargo build --release --manifest-path "$crate_dir/Cargo.toml"' in text
    assert 'ERROR: cc_bridge-rs-helper binary not available' in text


def test_install_script_links_runtime_accelerator_when_packaged() -> None:
    text = Path('install.sh').read_text(encoding='utf-8')

    assert 'bin/cc_bridge-runtime-accelerator' in text
    assert 'bin/build-cc_bridge-runtime-accelerator' in text
    assert 'bin/build-cc_bridge-runtime-accelerator|bin/cc_bridge-runtime-accelerator' in text
...
def test_install_script_provisions_role_packs_softly() -> None:
    ...
    assert 'CC_BRIDGE_INSTALL_ROLES=0' in text
    ...
    assert 'for role_id in agentroles.archi agentroles.cc_bridge_self' in text
    assert 'provision_default_role_pack "$cc_bridge_entry" "$role_id"' in text
    ...
    assert 'set CC_BRIDGE_INSTALL_ROLES=0' in text
...
def test_sidebar_bin_wrapper_is_source_install_fallback() -> None:
    text = Path('bin/cc_bridge-agent-sidebar').read_text(encoding='utf-8')

    assert 'CC_BRIDGE_AGENT_SIDEBAR_WRAPPER' in text
    assert 'while [[ -L "$SOURCE" ]]' in text
    assert 'tools/cc_bridge-agent-sidebar/target/release/cc_bridge-agent-sidebar' in text
    assert 'while :; do sleep 3600; done' in text


def test_rs_helper_bin_wrapper_is_source_install_fallback() -> None:
    text = Path('bin/cc_bridge-rs-helper').read_text(encoding='utf-8')

    assert 'CC_BRIDGE_RS_HELPER_WRAPPER' in text
    assert 'while [[ -L "$SOURCE" ]]' in text
    assert 'tools/cc_bridge-rs-helper/target/release/cc_bridge-rs-helper' in text
    assert 'Run: bin/build-cc_bridge-rs-helper' in text
    assert 'exit 127' in text


def test_rust_sidebar_enables_terminal_mouse_capture_for_sidebar_clicks() -> None:
    text = Path('tools/cc_bridge-agent-sidebar/src/tui.rs').read_text(encoding='utf-8')
    ...
    assert '&cc_bridge_program' in text


def test_sidebar_build_script_copies_release_binary() -> None:
    text = Path('bin/build-cc_bridge-agent-sidebar').read_text(encoding='utf-8')
...
def test_rs_helper_build_script_copies_release_binary() -> None:
    text = Path('bin/build-cc_bridge-rs-helper').read_text(encoding='utf-8')
...
def test_runtime_accelerator_build_script_copies_release_binary() -> None:
    text = Path('bin/build-cc_bridge-runtime-accelerator').read_text(encoding='utf-8')

    assert 'cargo build --release --manifest-path "$WORKSPACE/Cargo.toml" -p cc_bridge-runtime-accelerator' in text
    assert 'cp -f "$TARGET_BIN" "$OUT_BIN"' in text
    assert 'Built $OUT_BIN' in text


def test_sidebar_package_script_stages_release_artifact() -> None:
    text = Path('bin/package-cc_bridge-agent-sidebar-release').read_text(encoding='utf-8')

    assert 'ARTIFACT_NAME="${CC_BRIDGE_AGENT_SIDEBAR_ARTIFACT_NAME:-cc_bridge-agent-sidebar-linux-x86_64}"' in text
...
def test_sidebar_build_script_executes_copy_path_with_fake_cargo(tmp_path: Path) -> None:
    repo = tmp_path / 'repo'
    bin_dir = repo / 'bin'
    crate_dir = repo / 'tools' / 'cc_bridge-agent-sidebar'
    fake_bin = tmp_path / 'fake-bin'
    bin_dir.mkdir(parents=True)
    crate_dir.mkdir(parents=True)
    fake_bin.mkdir()
    script = bin_dir / 'build-cc_bridge-agent-sidebar'
    script.write_text(Path('bin/build-cc_bridge-agent-sidebar').read_text(encoding='utf-8'), encoding='utf-8')
    script.chmod(0o755)
...
cat > "$crate_dir/target/release/cc_bridge-agent-sidebar" <<'BIN'
#!/usr/bin/env bash
echo sidebar-binary
BIN
chmod +x "$crate_dir/target/release/cc_bridge-agent-sidebar"
""",
        encoding='utf-8',
    )
    ...
    out_bin = bin_dir / 'cc_bridge-agent-sidebar'
...
def test_rs_helper_build_script_executes_copy_path_with_fake_cargo(tmp_path: Path) -> None:
    repo = tmp_path / 'repo'
    bin_dir = repo / 'bin'
    crate_dir = repo / 'tools' / 'cc_bridge-rs-helper'
    fake_bin = tmp_path / 'fake-bin'
    bin_dir.mkdir(parents=True)
    crate_dir.mkdir(parents=True)
    fake_bin.mkdir()
    script = bin_dir / 'build-cc_bridge-rs-helper'
    script.write_text(Path('bin/build-cc_bridge-rs-helper').read_text(encoding='utf-8'), encoding='utf-8')
    script.chmod(0o755)
...
crate_dir="$(cd "$(dirname "$manifest")" && pwd)"
mkdir -p "$crate_dir/target/release"
cat > "$crate_dir/target/release/cc_bridge-rs-helper" <<'BIN'
#!/usr/bin/env bash
echo rs-helper-binary
BIN
chmod +x "$crate_dir/target/release/cc_bridge-rs-helper"
""",
        encoding='utf-8',
    )
    fake_cargo.chmod(0o755)
    ...
    assert proc.returncode == 0, proc.stderr
    assert 'Built ' in proc.stdout
    out_bin = bin_dir / 'cc_bridge-rs-helper'
    assert out_bin.read_text(encoding='utf-8').startswith('#!/usr/bin/env bash')
    assert os.access(out_bin, os.X_OK)
...
def test_sidebar_package_script_executes_artifact_dry_run(tmp_path: Path) -> None:
    ...
    bin_dir = repo / 'bin'
    bin_dir.mkdir(parents=True)
    source_bin = bin_dir / 'cc_bridge-agent-sidebar'
    source_bin.write_text('#!/usr/bin/env bash\necho sidebar-binary\n', encoding='utf-8')
    source_bin.chmod(0o755)
    script = bin_dir / 'package-cc_bridge-agent-sidebar-release'
    script.write_text(Path('bin/package-cc_bridge-agent-sidebar-release').read_text(encoding='utf-8'), encoding='utf-8')
    script.chmod(0o755)
    ...
    artifact = repo / 'dist' / 'cc_bridge-agent-sidebar-linux-x86_64.tar.gz'
    checksum = repo / 'dist' / 'cc_bridge-agent-sidebar-linux-x86_64.tar.gz.sha256'
    ...
    assert checksum_parts[1] == 'cc_bridge-agent-sidebar-linux-x86_64.tar.gz'
    ...
    assert 'cc_bridge-agent-sidebar-linux-x86_64/bin/cc_bridge-agent-sidebar' in listing.stdout
...
def test_install_script_prefers_prebuilt_sidebar_binary(tmp_path: Path) -> None:
    ...
    crate_dir = install_prefix / 'tools' / 'cc_bridge-agent-sidebar'
    target_bin = crate_dir / 'target' / 'release' / 'cc_bridge-agent-sidebar'
    out_bin = install_prefix / 'bin' / 'cc_bridge-agent-sidebar'
    crate_dir.mkdir(parents=True)
    ...
    out_bin.write_text('# CC_BRIDGE_AGENT_SIDEBAR_WRAPPER\n', encoding='utf-8')
    out_bin.chmod(0o755)
...
""",
    ...
    assert proc.returncode == 0, proc.stderr
    assert 'Installed prebuilt cc_bridge-agent-sidebar' in proc.stdout
...
export CODEX_INSTALL_PREFIX={install_prefix}
export CODEX_BIN_DIR={tmp_path / 'bin'}
export CC_BRIDGE_SOURCE_KIND=release
{install_body}
build_sidebar_helper_if_possible
""",
        encoding='utf-8',
...
def test_install_script_rebuilds_live_sidebar_when_source_is_newer(tmp_path: Path) -> None:
    ...
    crate_dir = source_root / 'tools' / 'cc_bridge-agent-sidebar'
    target_bin = crate_dir / 'target' / 'release' / 'cc_bridge-agent-sidebar'
...
export CC_BRIDGE_SOURCE_ROOT={source_root}
{install_body}
build_sidebar_helper_if_possible
""",
        encoding='utf-8',
    ...
    assert proc.returncode == 0, proc.stderr
    assert 'Built cc_bridge-agent-sidebar' in proc.stdout
...
def test_install_script_prefers_prebuilt_rs_helper_binary(tmp_path: Path) -> None:
    install_prefix = tmp_path / 'install'
    crate_dir = install_prefix / 'tools' / 'cc_bridge-rs-helper'
    target_bin = crate_dir / 'target' / 'release' / 'cc_bridge-rs-helper'
    out_bin = install_prefix / 'bin' / 'cc_bridge-rs-helper'
    crate_dir.mkdir(parents=True)
    out_bin.parent.mkdir(parents=True)
    (crate_dir / 'Cargo.toml').write_text('[package]\nname = "cc_bridge-rs-helper"\nversion = "0.0.0"\n', encoding='utf-8')
    target_bin.parent.mkdir(parents=True)
    ...
    target_bin.chmod(0o755)
    out_bin.write_text('# CC_BRIDGE_RS_HELPER_WRAPPER\n', encoding='utf-8')
    out_bin.chmod(0o755)
    harness = tmp_path / 'harness.sh'
    install_text = Path('install.sh').read_text(encoding='utf-8')
    install_body = install_text.rsplit('main "$@"', 1)[0]
...
""",
    ...
    assert proc.returncode == 0, proc.stderr
    assert 'Installed prebuilt cc_bridge-rs-helper' in proc.stdout
    assert out_bin.read_text(encoding='utf-8') == target_bin.read_text(encoding='utf-8')
...
""",
    ...
    assert proc.returncode != 0
    assert 'ERROR: Rust toolchain required to build cc_bridge-agent-sidebar' in proc.stdout
...
def test_ci_runs_rust_sidebar_checks() -> None:
    ...
    assert 'cargo test --manifest-path tools/cc_bridge-agent-sidebar/Cargo.toml' in text
    assert 'cargo test --manifest-path tools/cc_bridge-rs-helper/Cargo.toml' in text
    assert 'bin/build-cc_bridge-agent-sidebar' in text
    assert 'bin/build-cc_bridge-rs-helper' in text
...
def test_wsl_workflows_pin_python_inside_wsl_shells() -> None:
    expected = 'export CC_BRIDGE_PYTHON=/tmp/cc_bridge-ci-py311/bin/python'
    real_workflow = Path('.github/workflows/cc_bridge_daemon-real-platform.yml').read_text(
        encoding='utf-8',
    )

    smoke_step = real_workflow.split(
        '- name: Smoke cc_bridge startup from /mnt/c in WSL',
        1,
...
def test_sidebar_release_workflow_publishes_linux_artifact() -> None:
    ...
    assert 'cargo test --manifest-path tools/cc_bridge-agent-sidebar/Cargo.toml' in text
    assert 'bin/build-cc_bridge-agent-sidebar' in text
    assert 'bin/package-cc_bridge-agent-sidebar-release' in text
    assert 'cc_bridge-agent-sidebar-linux-x86_64.tar.gz' in text
...
def test_npm_publish_workflow_skips_already_published_version() -> None:
    ...
    if '-' in version:
        ...
        assert f'default: "v{version}"' in text
    assert 'npm view "@seemseam/cc_bridge@$version" version' in text
...
def test_release_artifacts_workflow_accepts_runtime_accelerator_socket_fallback() -> None:
    ...
    assert '*/.cc-bridge/runtime-accelerator/accelerator.sock|*/cc_bridge-runtime/accelerator-*.sock)' in text
