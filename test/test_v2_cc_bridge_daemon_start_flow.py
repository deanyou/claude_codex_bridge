...
import cc_bridge_daemon.handlers.project_restart as project_restart
from cc_bridge_daemon.handlers.project_restart import RESTART_PANES_REASON, build_project_restart_panes_handler
...
from cc_bridge_daemon.startup_fence import StartupFenceError
from cc_bridge_daemon.start_flow import StartFlowSummary
from cc_bridge_daemon.start_flow_runtime.service_tmux import (
    ...
    while time.time() < deadline:
        if path.exists():
            if path.suffix != '.sock':
                return
            try:
                payload = CcbdClient(path, timeout_s=0.5).ping('cc_bridge_daemon')
...
class _FakeNamespaceTmuxBackend:
    ...
    def _tmux_run(self, args, *, capture=False, check=False, input_bytes=None, timeout=None):
        ...
        if args[:3] == ['list-panes', '-t', 'cc_bridge-repo-cc_bridge_daemon-start']:
...
def test_topology_start_uses_only_the_authoritative_cmd_pane() -> None:
    ...
    assert backend.socket_path == '/tmp/cc_bridge.sock'
    assert cmd_pane == '%7'
    assert calls == []
...
def test_tmux_layout_for_start_uses_namespace_agent_panes_when_provided() -> None:
    from cc_bridge_daemon.start_flow_runtime.service_tmux import tmux_layout_for_start
...
def test_cc_bridge_daemon_start_flow_writes_runtime_authority_via_rpc(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-start'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_flow.TmuxBackend', _FakeNamespaceTmuxBackend)

    monkeypatch.setattr('cc_bridge_daemon.start_flow.set_tmux_ui_active', lambda active: None)
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.prepare_tmux_start_layout',
        lambda context, config, targets, **kwargs: SimpleNamespace(cmd_pane_id=None, agent_panes={}),
    )
    monkeypatch.setattr('cc_bridge_daemon.start_flow.cleanup_project_tmux_orphans_by_socket', lambda **kwargs: ())
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.TmuxCleanupHistoryStore',
        lambda paths: SimpleNamespace(append=lambda event: None),
    )
    monkeypatch.setattr('cc_bridge_daemon.start_flow.resolve_agent_binding', lambda **kwargs: None)
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.ensure_agent_runtime',
        lambda *args, **kwargs: RuntimeLaunchResult(
            ...
            binding=AgentBinding(
                ...
                runtime_pid=901,
                session_file=str(project_root / '.cc-bridge' / 'demo.session.json'),
    ...
    assert runtime.tmux_socket_path == str(app.paths.cc_bridge_daemon_tmux_socket_path)
    assert runtime.binding_source.value == 'provider-session'
    assert runtime.managed_by == 'cc_bridge_daemon'
...
def test_runtime_supervisor_start_can_skip_tmux_cleanup_and_layout_for_background_mount(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-start-no-cleanup'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_flow.set_tmux_ui_active', lambda active: (_ for _ in ()).throw(AssertionError('tmux ui should be skipped')))
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.prepare_tmux_start_layout',
        lambda context, config, targets, **kwargs: (_ for _ in ()).throw(AssertionError('interactive layout should be skipped')),
    )
    monkeypatch.setattr('cc_bridge_daemon.start_flow.cleanup_project_tmux_orphans_by_socket', lambda **kwargs: (_ for _ in ()).throw(AssertionError('cleanup should be skipped')))
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.TmuxCleanupHistoryStore',
...
def test_runtime_supervisor_start_persists_startup_report(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-start-report'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    assert report.socket_placement['effective_socket_path'] == str(app.paths.cc_bridge_daemon_socket_path)
    assert report.socket_placement['socket_root_kind'] == app.paths.cc_bridge_daemon_socket_placement.root_kind
    assert report.socket_placement['tmux_effective_socket_path'] == str(app.paths.cc_bridge_daemon_tmux_socket_path)
    assert report.socket_placement['tmux_socket_root_kind'] == app.paths.cc_bridge_daemon_tmux_socket_placement.root_kind
    assert report.actions_taken == (
        f'ensure_namespace:epoch=7,session={app.paths.cc_bridge_daemon_tmux_session_name}',
        'launch_runtime:demo',
    )
...
def test_runtime_supervisor_start_defers_provider_runtime_for_herdr_namespace(
    ...
    project_root = tmp_path / 'repo-cc_bridge_daemon-start-herdr-deferred'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: SimpleNamespace(
            tmux_socket_path='',
            tmux_session_name='cc_bridge-herdr-session',
            namespace_epoch=11,
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_flow.ensure_agent_runtime', fail_if_provider_runtime_starts)
    ...
    assert report.readiness_timeline['points']['T4_requested_agents_ready']['source'] == 'cc_bridge_daemon_start_flow_provider_runtime_deferred_on_herdr'
    assert report.readiness_timeline['points']['T6_fully_warm']['status'] == 'not_reached_at_rpc_return'
    assert report.readiness_timeline['points']['T6_fully_warm']['source'] == 'cc_bridge_daemon_start_flow_provider_runtime_deferred_on_herdr'
...
def test_runtime_supervisor_starts_provider_runtime_for_herdr_assigned_pane(
    ...
    project_root = tmp_path / 'repo-cc_bridge_daemon-start-herdr-provider'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    ...
    app.project_namespace._last_topology_pane_records = {
        'herdr-pane-1': SimpleNamespace(
            ...
            session_name='cc_bridge-herdr-session',
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: SimpleNamespace(
            tmux_socket_path='',
            tmux_session_name='cc_bridge-herdr-session',
            namespace_session_name='cc_bridge-herdr-session',
            namespace_id='workspace-1',
            namespace_ipc_kind='herdr_socket',
            namespace_ipc_ref='herdr://local',
            namespace_restore_token='cc_bridge-herdr-session::workspace-1',
            namespace_epoch=11,
            workspace_window_name='main',
            workspace_window_id='win-herdr-main',
            workspace_epoch=2,
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_flow.ensure_agent_runtime', launch_provider_runtime)
    ...
    assert observed['namespace_ref'] == {
        ...
        'ipc_ref': 'herdr://local',
        'restore_token': 'cc_bridge-herdr-session::workspace-1',
    }
...
def test_runtime_supervisor_failure_report_preserves_start_correlation(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-start-failed-report'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    monkeypatch.setattr('cc_bridge_daemon.supervisor.run_start_flow', fail_start)
...
def test_runtime_supervisor_failure_report_preserves_structured_agent_timing(
    ...
    project_root = tmp_path / 'repo-cc_bridge_daemon-agent-launch-failed-report'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    def fail_launch(*args, **kwargs):
        del args, kwargs
        time.sleep(0.01)
        setattr(original_error, 'cc_bridge_startup_timings_ms', {'build_start_cmd': 5.0})
        raise original_error

    monkeypatch.setattr('cc_bridge_daemon.start_flow.ensure_agent_runtime', fail_launch)
...
def test_runtime_supervisor_start_passes_visible_layout_signature_to_namespace(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.supervisor.run_start_flow',
...
def test_runtime_supervisor_start_uses_namespace_topology_for_herdr_compact_layout(
    ...
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; agent_2:codex, agent_1:claude\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    class FakeProjectNamespace:
        ...
        def ensure(self, *, layout_signature=None, topology_plan=None, force_recreate=False, recreate_reason=None):
            ...
            return SimpleNamespace(
                ...
                namespace_ipc_kind='herdr_socket',
                namespace_ipc_ref='herdr://local',
                namespace_restore_token='cc_bridge-herdr-session::w1',
                workspace_window_name='main',
...
def test_runtime_supervisor_relabels_reused_project_namespace_pane_by_agent_name(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-relabel'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    class FakeTmuxBackend:
        ...
        def _tmux_run(self, args, *, capture=False, check=False, input_bytes=None, timeout=None):
            ...
            if args[:3] == ['display-message', '-p', '-t']:
                return SimpleNamespace(
                    returncode=0,
                    stdout=f"{args[3]}\t{app.paths.cc_bridge_daemon_tmux_session_name}\t0\tagent\tdemo\t{app.project_id}\tccbd\n",
                )
            raise AssertionError(f'unexpected tmux args: {args}')
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_flow.TmuxBackend', FakeTmuxBackend)
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.apply_cc_bridge_pane_identity',
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_flow.resolve_agent_binding', lambda **kwargs: AgentBinding(
        runtime_ref='tmux:%77',
        session_ref='session-77',
        provider='codex',
        runtime_root=str(app.paths.agent_provider_runtime_dir('demo', 'codex')),
        runtime_pid=77,
        session_file=str(project_root / '.cc-bridge' / '.codex-demo-session'),
    ...
    assert relabel_calls == [
        {
            'socket_path': str(app.paths.cc_bridge_daemon_tmux_socket_path),
            'pane_id': '%77',
            'title': 'demo',
            'agent_label': 'demo',
            'project_id': app.project_id,
...
def test_runtime_supervisor_relaunches_same_socket_binding_outside_namespace_session(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-foreign-session'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    class FakeTmuxBackend:
        ...
        def _tmux_run(self, args, *, capture=False, check=False, input_bytes=None, timeout=None):
            del capture, check, input_bytes, timeout
            if args[:3] == ['list-panes', '-t', app.paths.cc_bridge_daemon_tmux_session_name]:
                return SimpleNamespace(stdout='%0\n')
            if args[:3] == ['display-message', '-p', '-t']:
                return SimpleNamespace(
                    returncode=0,
...
def test_runtime_supervisor_bootstraps_fresh_cmd_pane_after_layout(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-fresh-cmd'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
...
def test_runtime_supervisor_project_namespace_cleanup_uses_authoritative_active_panes(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-project-cleanup'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.cleanup_project_tmux_orphans_by_socket',
        lambda **kwargs: cleanup_calls.append(kwargs) or (
            ProjectTmuxCleanupSummary(
                socket_name=str(app.paths.cc_bridge_daemon_tmux_socket_path),
                owned_panes=('%0', '%55', '%77'),
                active_panes=tuple(kwargs['active_panes_by_socket'][str(app.paths.cc_bridge_daemon_tmux_socket_path)]),
                orphaned_panes=('%77',),
                killed_panes=('%77',),
            ),
        ),
    ...
    assert cleanup_calls == [
        {
            'project_id': app.project_id,
            'active_panes_by_socket': {str(app.paths.cc_bridge_daemon_tmux_socket_path): ('%0', '%55')},
        }
    ]
...
def test_runtime_supervisor_reuses_agent_only_binding_without_cmd_namespace_match(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-agent-only-binding'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.start_flow.resolve_agent_binding',
...
def test_runtime_supervisor_reuses_agent_only_missing_binding_when_session_file_declares_no_socket(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-agent-only-missing-binding'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
...
def test_runtime_supervisor_rejects_agent_only_binding_with_provider_identity_mismatch(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-agent-only-mismatch-binding'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
...
def test_runtime_supervisor_project_namespace_start_does_not_preheal_dead_binding_before_layout(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-layout-launch'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('cmd; demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_flow.resolve_agent_binding', _resolve_agent_binding)
...
def test_cc_bridge_daemon_start_listens_and_self_probes_before_mounted_publish(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-start-order'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    with pytest.raises(CcbdClientError, match='timed out'):
        CcbdClient(app.paths.cc_bridge_daemon_socket_path, timeout_s=0.1).ping('cc_bridge_daemon')
...
def test_cc_bridge_daemon_release_unlinks_owned_socket_inside_startup_lock(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-release-socket-lock'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
...
def test_cc_bridge_daemon_same_process_restart_advances_unmounted_predecessor_generation(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-same-process-restart'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
...
def test_cc_bridge_daemon_serve_forever_accepts_ping_while_runtime_bootstrap_is_blocked(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-continuous-accept'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    monkeypatch.setattr('cc_bridge_daemon.app_runtime.lifecycle.maybe_start_runtime_accelerator', lambda root: None)
    server_thread = threading.Thread(target=run_server, daemon=True)
    server_thread.start()
    try:
        ...
        payload = CcbdClient(app.paths.cc_bridge_daemon_socket_path, timeout_s=1.0).ping('cc_bridge_daemon')
...
def test_cc_bridge_daemon_final_mounted_publish_opens_request_gate_atomically(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-atomic-mounted-gate'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    def request_ready() -> None:
        try:
            request_results.append(
                CcbdClient(app.paths.cc_bridge_daemon_socket_path, timeout_s=2.0).request('test-ready')
            )
        except BaseException as exc:
            thread_errors.append(exc)
...
def test_cc_bridge_daemon_failed_final_publish_never_opens_request_gate(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-failed-mounted-gate'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    def request_ready() -> None:
        try:
            CcbdClient(app.paths.cc_bridge_daemon_socket_path, timeout_s=3.0).request('test-ready')
        except BaseException as exc:
...
def test_second_starting_child_cannot_unlink_first_child_socket(
    ...
    project_root = tmp_path / 'repo-cc_bridge_daemon-starting-owner-race'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    first = CcbdApp(project_root)
    ...
    try:
        ...
        before = first.paths.cc_bridge_daemon_socket_path.stat()
        ...
        after = first.paths.cc_bridge_daemon_socket_path.stat()
...
def test_cc_bridge_daemon_start_rolls_back_mount_when_restore_fails(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-start-rollback'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
...
def test_cc_bridge_daemon_start_daemon_boot_adopts_existing_runtime_authority(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-daemon-boot-adopt'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
...
def test_cc_bridge_daemon_probe_failure_does_not_adopt_unmounted_runtime_generation(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-probe-failure-no-adopt'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
...
def test_cc_bridge_daemon_mounted_lifecycle_write_failure_unmounts_new_lease(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = tmp_path / 'repo-cc_bridge_daemon-mounted-write-failure'
    (project_root / '.cc-bridge').mkdir(parents=True, exist_ok=True)
    (project_root / '.cc-bridge' / 'cc_bridge.config').write_text('demo:codex\n', encoding='utf-8')
    bootstrap_project(project_root)
    app = CcbdApp(project_root)
    ...
    assert lease is not None
    assert lease.mount_state.value == 'unmounted'
    assert lease.cc_bridge_daemon_pid == app.pid
