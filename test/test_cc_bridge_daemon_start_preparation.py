...
def test_prepare_start_agents_skips_provider_preparation_for_reused_binding(
    ...
    calls: list[str] = []
    monkeypatch.setattr(
        'cc_bridge_daemon.start_preparation.prepare_provider_workspace',
        lambda **kwargs: calls.append(kwargs['agent_name']),
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.start_preparation.effective_start_command',
...
def test_provider_profile_reject_reason_accepts_agent_env_api_credentials(
    ...
    from cc_bridge_daemon.start_preparation import _provider_profile_reject_reason
...
def test_prepare_start_agents_reports_logical_window_reject_reason(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_preparation.prepare_provider_workspace', lambda **kwargs: None)
...
def test_prepare_start_agents_refreshes_cc_bridge_only_claude_permissions_when_auto_permission(
    monkeypatch,
    tmp_path: Path,
) -> None:
...
def test_prepare_start_agents_uses_hard_role_effective_command_for_claude_projection(
    ...
    monkeypatch.setattr('cc_bridge_daemon.start_preparation.prepare_provider_workspace', capture_prepare)
