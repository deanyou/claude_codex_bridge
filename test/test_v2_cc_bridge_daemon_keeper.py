...
from cc_bridge_daemon.keeper import KeeperState, KeeperStateStore, ProjectKeeper, ShutdownIntent, ShutdownIntentStore
from cc_bridge_daemon.keeper_runtime.failure_policy import (
...
from cc_bridge_daemon.reload_handoff import ReloadHandoff, ReloadHandoffStore, reload_handoff_allows_signature_mismatch
...
import cc_bridge_daemon.keeper as keeper_module
import cc_bridge_daemon.keeper_runtime.loop as keeper_loop
...
def test_project_keeper_spawns_missing_daemon(tmp_path: Path) -> None:
    ...
    keeper = ProjectKeeper(
        project_root,
        pid=777,
        spawn_cc_bridge_daemon_process_fn=lambda **kwargs: spawn_calls.append(dict(kwargs)),
    )
...
def test_keeper_releases_startup_lock_before_spawn_and_preserves_child_mounted_record(
    ...
    keeper = ProjectKeeper(project_root, pid=779, spawn_cc_bridge_daemon_process_fn=_spawn)
    ...
    store.save(
        build_lifecycle(
            ...
            control_plane_endpoint={
                'kind': 'unix_socket',
                'address': str(ctx.paths.cc_bridge_daemon_socket_path),
                'display': str(ctx.paths.cc_bridge_daemon_socket_path),
                'legacy_socket_path': str(ctx.paths.cc_bridge_daemon_socket_path),
            },
        )
    )
    state = KeeperState(
...
def test_keeper_success_does_not_overwrite_superseding_startup_transaction(tmp_path: Path) -> None:
    ...
    keeper = ProjectKeeper(project_root, pid=780, spawn_cc_bridge_daemon_process_fn=_spawn)
    inspection = _inspection(
        ctx,
        health=LeaseHealth.MISSING,
        socket_connectable=False,
...
def test_keeper_failure_does_not_overwrite_concurrent_stop_intent(tmp_path: Path) -> None:
    ...
    keeper = ProjectKeeper(project_root, pid=781, spawn_cc_bridge_daemon_process_fn=_spawn)
    inspection = _inspection(
        ctx,
        health=LeaseHealth.MISSING,
        socket_connectable=False,
...
def test_project_keeper_suppresses_restart_on_resource_exhaustion(tmp_path: Path) -> None:
    ...
    keeper = ProjectKeeper(
        project_root,
        pid=890,
        spawn_cc_bridge_daemon_process_fn=lambda **kwargs: (_ for _ in ()).throw(
...
def test_project_keeper_suppresses_restart_after_repeated_start_failures(tmp_path: Path) -> None:
    ...
    keeper = ProjectKeeper(
        project_root,
        pid=891,
        spawn_cc_bridge_daemon_process_fn=lambda **kwargs: (_ for _ in ()).throw(RuntimeError('boot loop')),
    )
    keeper._ownership_guard = _ownership_guard_stub(
        inspect=lambda: _inspection(
            ctx,
...
def test_project_keeper_preserves_namespace_epoch_when_confirming_mounted_daemon(tmp_path: Path, monkeypatch) -> None:
    ...
    ProjectNamespaceStateStore(keeper.paths).save(
        ProjectNamespaceState(
            project_id=ctx.project.project_id,
            namespace_epoch=6,
            tmux_socket_path=str(ctx.paths.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name=ctx.paths.cc_bridge_daemon_tmux_session_name,
            layout_version=3,
    ...
    monkeypatch.setattr('cc_bridge_daemon.keeper_runtime.loop.daemon_matches_project_config', lambda app: True)
    state = KeeperState(
        project_id=ctx.project.project_id,
        keeper_pid=890,
...
def test_project_keeper_does_not_promote_child_runtime_bootstrap_early(
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.keeper_runtime.loop.daemon_matches_project_config',
...
def test_project_keeper_stale_match_observation_cannot_overwrite_concurrent_stop(
    ...
    monkeypatch.setattr('cc_bridge_daemon.keeper_runtime.loop.daemon_matches_project_config', stop_during_probe)
    state = KeeperState(
        project_id=ctx.project.project_id,
        keeper_pid=893,
        started_at='2026-04-22T00:00:00Z',
...
def test_project_keeper_stale_probe_failure_cannot_overwrite_concurrent_stop(
    ...
    monkeypatch.setattr('cc_bridge_daemon.keeper_runtime.loop.daemon_matches_project_config', fail_after_stop)
    state = KeeperState(
        project_id=ctx.project.project_id,
        keeper_pid=894,
        started_at='2026-04-22T00:00:00Z',
...
def test_project_keeper_config_probe_uses_shared_control_plane_timeout(tmp_path: Path, monkeypatch) -> None:
    ...
    class _FakeClient:
        def __init__(self, socket_path, *, timeout_s=None) -> None:
            assert socket_path == ctx.paths.cc_bridge_daemon_socket_path
...
def test_project_keeper_accepts_bounded_reload_handoff_signature_window(tmp_path: Path, monkeypatch) -> None:
    ...
    _write(project_root / '.cc-bridge' / 'cc_bridge.config', 'agent1:codex, agent2:claude\n')
...
def test_project_keeper_stops_when_shutdown_intent_exists(tmp_path: Path) -> None:
    ...
    keeper = ProjectKeeper(
        ...
        spawn_cc_bridge_daemon_process_fn=lambda **kwargs: (_ for _ in ()).throw(AssertionError('keeper should not spawn')),
    )
...
def test_keeper_reconciles_moved_project_identity_before_startup_fence(
    ...
    keeper._mount_manager.mark_mounted(
        ...
        pid=999999,
        socket_path=project_root / '.cc-bridge' / 'cc_bridge_daemon' / 'old.sock',
        generation=8,
    ...
    assert lifecycle.socket_path == str(ctx.paths.cc_bridge_daemon_socket_path)
