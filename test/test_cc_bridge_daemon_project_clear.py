...
def _load_project_clear_module():
    # Avoid the existing cc_bridge_daemon.handlers package import cycle in this focused unit test.
    module_path = Path(__file__).resolve().parents[1] / 'lib' / 'cc_bridge_daemon' / 'handlers' / 'project_clear.py'
    spec = importlib.util.spec_from_file_location('cc_bridge_daemon_project_clear_for_test', module_path)
