...
import cc_bridge_daemon.start_runtime.agent_runtime as agent_runtime_module
import cc_bridge_daemon.start_runtime.agent_runtime_binding as agent_runtime_binding_module
...
from cc_bridge_daemon.start_runtime.agent_runtime import start_agent_runtime
...
def test_start_agent_runtime_records_namespace_pane_without_provider_binding() -> None:
    ...
    assert execution.agent_result.tmux_socket_path == '/tmp/cc_bridge.sock'
    ...
    assert execution.socket_name == '/tmp/cc_bridge.sock'
...
def test_start_agent_runtime_attaches_structured_failure_without_wrapping(monkeypatch) -> None:
    ...
    failed = getattr(original_error, 'cc_bridge_startup_agent_result')
...
def test_start_agent_runtime_fails_closed_when_mount_attempt_is_superseded() -> None:
    ...
    assert runtime_service.restore_calls == []
    failed = getattr(captured.value, 'cc_bridge_startup_agent_result')
    assert failed.action == 'failed'
