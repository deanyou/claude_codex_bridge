...
from cc_bridge_daemon.control_plane_transport.endpoint import endpoint_from_record, endpoint_to_record
...
from cc_bridge_daemon.control_plane_transport.factory import connect_endpoint, transport_for_legacy_socket_path
from cc_bridge_daemon.control_plane_transport.token_auth import RpcTransportAuthError, create_token_file, load_token_file, _current_windows_user
...
from cc_bridge_daemon.socket_client_runtime import decode_response, recv_response_line, send_request
from cc_bridge_daemon.socket_server import CcbdSocketServer
from cc_bridge_daemon.socket_server_runtime.loop import enqueue_connection, start_worker, stop_worker
...
def test_factory_selects_windows_tcp_for_legacy_socket_path(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr('cc_bridge_daemon.control_plane_transport.factory._is_windows', lambda: True)
    write_endpoint(
        ...
        legacy_socket_path=tmp_path / 'cc_bridge_daemon.sock',
    )

    transport = transport_for_legacy_socket_path(tmp_path / 'cc_bridge_daemon.sock')
...
def test_windows_client_without_endpoint_fails_with_endpoint_error_not_af_unix(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr('cc_bridge_daemon.control_plane_transport.factory._is_windows', lambda: True)
    client = CcbdClient(tmp_path / 'cc_bridge_daemon.sock', timeout_s=0.1)
...
def test_socket_server_prefers_windows_tcp_without_endpoint_descriptor(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr('cc_bridge_daemon.control_plane_transport.factory._is_windows', lambda: True)
    monkeypatch.setattr('cc_bridge_daemon.socket_server_runtime.server._is_windows', lambda: True)

    server = CcbdSocketServer(tmp_path / 'cc_bridge_daemon.sock')
...
def test_windows_lifecycle_does_not_synthesize_legacy_endpoint_from_socket_path(monkeypatch, tmp_path: Path) -> None:
    _patch_module_os_name(monkeypatch, 'cc_bridge_daemon.services.lifecycle', 'nt')
...
def test_windows_project_inspection_does_not_fall_back_to_legacy_endpoint(monkeypatch, tmp_path: Path) -> None:
    _patch_module_os_name(monkeypatch, 'cc_bridge_daemon.services.project_inspection', 'nt')
    ...
    inspection = ProjectDaemonInspection(
        ...
        lifecycle=SimpleNamespace(socket_path=str(tmp_path / 'cc_bridge_daemon.sock'), control_plane_endpoint=None),
    )
...
def test_create_token_file_proves_acl_convergence(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr('cc_bridge_daemon.control_plane_transport.token_auth._current_windows_user', lambda: 'DESKTOP\\User')
...
def test_tcp_listener_publishes_endpoint_and_roundtrips_ping(tmp_path: Path) -> None:
    ...
    server = CcbdSocketServer(tmp_path / 'cc_bridge_daemon.sock', control_plane_transport=transport)
    server.register_handler('ping', lambda payload: {'echo': payload})
    client = None
    try:
        server.listen()
        endpoint = read_endpoint(tmp_path / 'cc_bridge_daemon.sock')
        ...
        send_request(client, RpcRequest(op='ping', request={'target': 'cc_bridge_daemon'}))
        ...
        assert response.payload['echo'] == {'target': 'cc_bridge_daemon'}
    finally:
...
def test_bad_tcp_token_is_not_accepted_by_listener(tmp_path: Path) -> None:
    ...
    try:
        ...
        raw.sendall(
            json.dumps(
                {
                    'schema': 'cc_bridge_daemon-control-plane-token-v1',
                    'token': 'wrong-token',
                }
            ).encode('utf-8')
            + b'\n'
...
def test_windows_legacy_socket_path_rejects_invalid_tcp_endpoint_descriptor(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr('cc_bridge_daemon.control_plane_transport.factory._is_windows', lambda: True)
    endpoint_store_path(tmp_path / 'cc_bridge_daemon.sock').write_text(
...
def test_direct_windows_endpoint_rejects_non_tcp_descriptor(monkeypatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(RpcTransportAuthError) as error:
        connect_endpoint({'socket_path': str(tmp_path / 'cc_bridge_daemon.sock')}, timeout_s=0.1)
...
def test_token_store_path_rejects_generation_path_traversal(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        token_store_path(tmp_path / 'cc_bridge_daemon.sock', '../escape')

    with pytest.raises(ValueError):
        token_store_path(tmp_path / 'cc_bridge_daemon.sock', 'nested/token')
