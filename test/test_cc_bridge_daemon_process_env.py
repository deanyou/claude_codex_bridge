...
import cc_bridge_daemon.daemon_process as daemon_process
import process_background
from cc_bridge_daemon.daemon_process import _cc_bridge_daemon_env, _prepend_tool_paths, _ready_payload_matches_expected
...
def test_cc_bridge_daemon_env_prefers_current_worktree_tools(monkeypatch) -> None:
    monkeypatch.setenv('PATH', os.pathsep.join(['/usr/bin', '/bin']))
    monkeypatch.setenv('PYTHONPATH', '/stable/cc_bridge/lib:/other')

    env = _cc_bridge_daemon_env(keeper_pid=123)
    ...
    assert env['CC_BRIDGE_KEEPER_PID'] == '123'


def test_cc_bridge_daemon_env_injects_exact_startup_fence_and_drops_inherited_values(monkeypatch) -> None:
    monkeypatch.setenv(EXPECTED_STARTUP_ID_ENV, 'b' * 32)
    monkeypatch.setenv(EXPECTED_GENERATION_ENV, '99')

    env = _cc_bridge_daemon_env(
        keeper_pid=123,
...
def test_cc_bridge_daemon_env_drops_inherited_or_malformed_diagnostics_checkpoint(monkeypatch) -> None:
    monkeypatch.setenv(KEEPER_STARTUP_ACCEPTED_PERF_COUNTER_NS_ENV, '999')

    legacy_env = _cc_bridge_daemon_env(keeper_pid=123)
    malformed_env = _cc_bridge_daemon_env(
        keeper_pid=123,
        expected_startup_id='a' * 32,
        expected_generation=7,
...
def test_cc_bridge_daemon_env_rejects_partial_startup_fence() -> None:
    try:
        _cc_bridge_daemon_env(keeper_pid=123, expected_startup_id='a' * 32)
...
def test_spawn_failure_reclaims_only_spawned_child_and_closes_parent_logs(
    ...
    monkeypatch.setattr(daemon_process, '_wait_for_cc_bridge_daemon_ready', fail_wait)
    ...
    with pytest.raises(daemon_process.CcbdProcessError, match='readiness mismatch'):
        daemon_process.spawn_cc_bridge_daemon_process(
            project_root=tmp_path,
            socket_path=tmp_path / 'cc_bridge_daemon.sock',
            cc_bridge_daemon_dir=tmp_path / '.cc-bridge' / 'cc_bridge_daemon',
            timeout_s=0.01,
            expected_startup_id='a' * 32,
            expected_generation=7,
        )
    ...
    class FakeProcess:
        ...
        def wait(self, *, timeout):
            ...
            if self.wait_calls == 1:
                raise subprocess.TimeoutExpired('cc_bridge_daemon', timeout)
            return -9
