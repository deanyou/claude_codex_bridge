...
LAUNCHER = REPO_ROOT / "bin" / "_cc_bridge-python"
...
def test_launcher_prefers_install_managed_python_over_stale_inherited_state(
    ...
    installed_launcher = install_root / "bin" / "_cc_bridge-python"
    ...
    env.update(
        {
            ...
            "CC_BRIDGE_PYTHON": str(stale_inherited),
            "CC_BRIDGE_PYTHON_CACHE": str(cache_path),
    ...
    env.update(
        {
            ...
            "CC_BRIDGE_PYTHON_CACHE": str(tmp_path / "python-cache"),
            "PROBE_LOG": str(probe_log),
        }
    )
    env.pop("CC_BRIDGE_PYTHON", None)
