...
import provider_backends.codebuddy.session as codebuddy_session
...
def test_codebuddy_ensure_pane_respawns_dead_pane(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_codebuddy_ensure_pane_already_alive(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_codebuddy_ensure_pane_does_not_rediscover_different_pane(
...
def test_codebuddy_load_session_returns_none_for_inactive(tmp_path: Path) -> None:
...
def test_codebuddy_load_session_returns_none_for_missing_file(tmp_path: Path) -> None:
    """Missing session file should return None."""
    sess = codebuddy_session.load_project_session(tmp_path)
    assert sess is None


def test_codebuddy_compute_session_key_prefix(tmp_path: Path) -> None:
    ...
    key = codebuddy_session.compute_session_key(sess)
