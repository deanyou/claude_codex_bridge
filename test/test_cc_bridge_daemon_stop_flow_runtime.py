...
from cc_bridge_daemon.stop_flow_runtime.pid_cleanup import collect_pid_candidates
from cc_bridge_daemon.stop_flow_runtime.service import stop_all_project
from cli.services.kill_runtime.pid_cleanup import collect_project_authority_pid_candidates
from cc_bridge_daemon.stop_flow_runtime.pid_cleanup import terminate_runtime_pids
from cc_bridge_daemon.stop_flow_runtime.runtime_records import extra_agent_dir_names
...
def test_stop_all_project_defers_namespace_destroy_until_after_response(tmp_path: Path) -> None:
    ...
    paths = SimpleNamespace(
        agents_dir=tmp_path / '.cc-bridge' / 'agents',
        cc_bridge_daemon_socket_path=tmp_path / '.cc-bridge' / 'cc_bridge_daemon' / 'cc_bridge_daemon.sock',
        agent_dir=lambda agent_name: tmp_path / '.cc-bridge' / 'agents' / agent_name,
        agent_runtime_path=lambda agent_name: tmp_path / '.cc-bridge' / 'agents' / agent_name / 'runtime.json',
        agent_helper_path=lambda agent_name: tmp_path / '.cc-bridge' / 'agents' / agent_name / 'helper.json',
    )
...
def test_stop_all_project_removes_codex_app_server_authority_artifacts(tmp_path: Path) -> None:
    ...
    (runtime_dir / 'app-server.remote').write_text('/run/user/1000/cc_bridge-runtime/app-server-test.sock\n', encoding='utf-8')
...
def test_collect_pid_candidates_uses_runtime_root_and_force_fallback(tmp_path: Path) -> None:
    agent_dir = tmp_path / '.cc-bridge' / 'agents' / 'agent1'
...
def test_collect_project_process_candidates_matches_cc_bridge_runtime_cmdline(tmp_path: Path) -> None:
    ...
    mapping = {
        101: f'python -m provider_backends.codex.bridge --runtime-dir {cc_bridge_root / "agents/agent1/provider-runtime/codex"}',
        202: f'tmux -S {cc_bridge_root / "cc_bridge_daemon/tmux.sock"} new-session -d',
    ...
    assert candidates[101] == [cc_bridge_root]
    assert candidates[202] == [cc_bridge_root]


def test_collect_project_authority_pid_candidates_includes_cc_bridge_daemon_and_keeper_pids(tmp_path: Path) -> None:
    project_root = tmp_path / 'repo'
    cc_bridge_daemon_dir = project_root / '.cc-bridge' / 'cc_bridge_daemon'
    cc_bridge_daemon_dir.mkdir(parents=True)
    (cc_bridge_daemon_dir / 'lease.json').write_text(
        '{"record_type":"cc_bridge_daemon_lease","cc_bridge_daemon_pid":101,"keeper_pid":202}\n',
        encoding='utf-8',
    )
    (cc_bridge_daemon_dir / 'keeper.json').write_text(
        '{"record_type":"cc_bridge_daemon_keeper","keeper_pid":202}\n',
        encoding='utf-8',
    )

    candidates = collect_project_authority_pid_candidates(project_root)

    assert candidates[101] == [cc_bridge_daemon_dir / 'lease.json']
    assert candidates[202] == [cc_bridge_daemon_dir / 'lease.json', cc_bridge_daemon_dir / 'keeper.json']
...
def test_terminate_runtime_pids_reaps_helper_group_from_manifest(tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / 'repo'
    helper_path = project_root / '.cc-bridge' / 'agents' / 'agent1' / 'helper.json'
