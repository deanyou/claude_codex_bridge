...
from cc_bridge_daemon.control_plane_transport.fake import FakeControlPlaneTransport
from cc_bridge_daemon.socket_server_runtime.loop import maintenance_worker_loop, next_timeout, next_worker_timeout, post_request_tick, run_after_response_actions, run_queued_maintenance_ticks, run_tick_if_needed, start_maintenance_worker, start_worker, stop_maintenance_worker, stop_worker, worker_loop
from cc_bridge_daemon.socket_server_runtime.server import CcbdSocketServer


def _server(tmp_path) -> CcbdSocketServer:
    return CcbdSocketServer(
        tmp_path / 'cc_bridge_daemon.sock',
