...
from cc_bridge_daemon.api_models import AcceptedJobReceipt, JobStatus, SubmitReceipt, TargetKind
from cc_bridge_daemon.app import CcbdApp
from cc_bridge_daemon.app_runtime.service_graph import (
    SERVICE_GRAPH_RETAINED_COUNT_SCOPE,
    CcbdServiceGraphDependencies,
    build_cc_bridge_daemon_service_graph,
)
...
from cc_bridge_daemon.models import LeaseHealth, LeaseInspection
...
def test_service_graph_can_be_built_twice_without_writing_runtime_authority(tmp_path: Path) -> None:
    ...
    graph1 = build_cc_bridge_daemon_service_graph(_deps(1))
    graph2 = build_cc_bridge_daemon_service_graph(_deps(2))
...
def test_cc_bridge_daemon_app_bootstrap_publishes_startup_graph_fields(tmp_path: Path) -> None:
...
def test_handler_wrappers_do_not_reload_config_or_rebuild_graph_per_request(tmp_path: Path, monkeypatch) -> None:
    ...
    def _unexpected_load(*_args, **_kwargs):
        raise AssertionError('handler wrapper must not load .cc-bridge/cc_bridge.config')
    ...
    monkeypatch.setattr('cc_bridge_daemon.app_runtime.handlers.load_project_config', _unexpected_load, raising=False)
    monkeypatch.setattr('cc_bridge_daemon.app_runtime.handlers.build_cc_bridge_daemon_service_graph', _unexpected_build, raising=False)
...
def test_after_response_action_keeps_request_graph(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr('cc_bridge_daemon.handlers.project_restart.restart_project_agent_panes_in_place', _fake_restart)
...
def _project(project_root: Path) -> Path:
    ...
    (config_dir / 'cc_bridge.config').write_text('alpha:codex, beta:claude\n', encoding='utf-8')
    return project_root
