...
from cc_bridge_daemon.services.project_namespace_runtime.slot_replacement import ProjectSlotRecoveryContext
...
def test_refresh_provider_binding_replaces_missing_project_pane_inside_workspace(monkeypatch, tmp_path: Path) -> None:
    ...
    session_file = layout.cc_bridge_dir / 'agent1.session.json'
    ...
    session = _Session(
        ...
        data={
            ...
            'cc_bridge_session_id': 'cc_bridge-session-new',
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.services.runtime_runtime.refresh.resolve_project_slot_recovery_context',
        lambda **kwargs: replacement_context,
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.services.project_namespace_runtime.slot_replacement.TmuxBackend',
    ...
    assert ('%55', '@cc_bridge_session_id', 'cc_bridge-session-new') in backend.options
