...
from cc_bridge_daemon.handlers.project_compact import build_project_compact_context_handler
from cc_bridge_daemon.handlers.project_context import COMPACT_COMMANDS
...
def test_compact_targets_all_and_uses_provider_native_commands(monkeypatch) -> None:
    backend = _Backend()
    monkeypatch.setattr('cc_bridge_daemon.handlers.project_compact.TmuxBackend', lambda *, socket_path: backend)
    app = _app(
        agents={
...
def test_compact_opencode_waits_before_submit(monkeypatch) -> None:
    backend = _Backend()
    monkeypatch.setattr('cc_bridge_daemon.handlers.project_compact.TmuxBackend', lambda *, socket_path: backend)
    monkeypatch.setattr('cc_bridge_daemon.handlers.project_context.time.sleep', lambda seconds: backend.calls.append(('sleep', str(seconds))))
    app = _app(
...
def test_compact_dsh_uses_structured_api_without_pane_input(monkeypatch, tmp_path) -> None:
    backend = _Backend()
    monkeypatch.setattr(
        'cc_bridge_daemon.handlers.project_compact.TmuxBackend',
