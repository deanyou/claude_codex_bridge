...
from cc_bridge_daemon.services.health_assessment.provider_pane import assess_provider_pane
...
def test_assess_provider_pane_reports_missing_session(monkeypatch) -> None:
    binding = _binding()
    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.provider_pane.load_provider_session',
...
def test_assess_provider_pane_accepts_mux_runtime_ref_with_non_tmux_terminal(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.provider_pane.session_terminal',
        lambda session: 'mux',
    )
...
def test_assess_provider_pane_marks_foreign_tmux_pane(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.provider_pane.session_backend',
        lambda session: 'backend',
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.provider_pane.tmux_pane_state',
        lambda session, backend, pane_id: 'alive',
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.provider_pane.pane_outside_project_namespace',
        lambda **kwargs: True,
    )
