...
from cc_bridge_daemon.supervision import RuntimeSupervisionLoop, SupervisionEventStore
...
def test_runtime_supervision_loop_reflows_after_local_replacement_when_project_namespace_is_safe(tmp_path: Path) -> None:
    ...
    degraded.tmux_socket_path = str(layout.cc_bridge_daemon_tmux_socket_path)
    ...
    steady.tmux_socket_path = str(layout.cc_bridge_daemon_tmux_socket_path)
...
def test_runtime_supervision_loop_recovers_missing_pane_locally_even_when_other_agent_busy(tmp_path: Path) -> None:
    ...
    busy.tmux_socket_path = str(layout.cc_bridge_daemon_tmux_socket_path)
    registry.upsert(degraded)
...
def test_runtime_supervision_loop_keeps_healthy_cmd_slot_stable(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr('cc_bridge_daemon.supervision.cmd_slot.ProjectNamespaceController', _NamespaceController)
    monkeypatch.setattr(
        'cc_bridge_daemon.supervision.cmd_slot.inspect_project_namespace_pane',
        lambda backend, pane_id: ProjectNamespacePaneRecord(
            pane_id=pane_id,
            session_name=layout.cc_bridge_daemon_tmux_session_name,
            window_id='@2',
            role='cmd',
            slot_key='cmd',
            project_id=ctx.project_id,
...
def test_runtime_supervision_loop_keeps_healthy_herdr_cmd_slot_stable(tmp_path: Path, monkeypatch) -> None:
    ...
    class _NamespaceController:
        ...
        def load(self):
            return SimpleNamespace(
                ...
                backend_impl='herdr',
                namespace_id='w-main',
                namespace_session_name=layout.cc_bridge_daemon_tmux_session_name,
                namespace_ipc_kind='herdr_socket',
                namespace_ipc_ref='herdr://local',
                namespace_restore_token=f'{layout.cc_bridge_daemon_tmux_session_name}::w-main',
                namespace_epoch=7,
    ...
    monkeypatch.setattr('cc_bridge_daemon.supervision.cmd_slot.ProjectNamespaceController', _NamespaceController)
    monkeypatch.setattr(
        'cc_bridge_daemon.supervision.cmd_slot.build_backend',
...
def test_runtime_supervision_loop_defers_cmd_recovery_on_transient_tmux_unavailable(tmp_path: Path, monkeypatch) -> None:
    ...
    class _NamespaceController:
        ...
        def root_pane_id(self, namespace=None, *, timeout_s=None) -> str:
            ...
            raise TmuxTransientServerUnavailable('no server running on /tmp/cc_bridge-runtime/test.sock')
...
def test_runtime_supervision_loop_reflows_when_cmd_slot_is_missing(tmp_path: Path, monkeypatch) -> None:
    ...
    assert fake_backend.pane_options['%cmd']['@cc_bridge_slot'] == 'cmd'
    assert fake_backend.pane_options['%cmd']['@cc_bridge_role'] == 'cmd'
...
def test_runtime_supervision_loop_suspends_recovery_when_lifecycle_is_stopping(
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.supervision.cmd_slot.ProjectNamespaceController',
...
def test_runtime_supervision_loop_defers_transient_mount_failure(tmp_path: Path) -> None:
    ...
    assert runtime.last_failure_reason == (
        'TmuxTransientServerUnavailable: no server running on /tmp/cc_bridge-runtime/test.sock'
    )
    events = SupervisionEventStore(layout).read_all()
    assert [event.event_kind for event in events] == ['mount_started', 'mount_failed']
    assert events[1].details == {
        'reason': 'TmuxTransientServerUnavailable: no server running on /tmp/cc_bridge-runtime/test.sock'
    }
