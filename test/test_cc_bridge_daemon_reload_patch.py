...
from cc_bridge_daemon.reload_patch import build_namespace_patch_plan
...
""",
    ...
    assert {step['managed_by'] for step in patch['steps']} == {'cc_bridge_daemon'}
...
def test_cli_reload_uses_operation_timeout_for_large_runtime_graphs(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv('CC_BRIDGE_RELOAD_RPC_TIMEOUT_S', '17.5')
    monkeypatch.setattr(
        reload_module,
        'connect_current_mounted_daemon',
        lambda _context: SimpleNamespace(client=_Client()),
