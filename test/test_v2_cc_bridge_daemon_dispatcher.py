...
import cc_bridge_daemon.services.dispatcher_runtime.frontdesk_direct_handoff as direct_handoff
import pytest
...
from cc_bridge_daemon.reload_drain import DrainIntent, DrainQueueStore
from cc_bridge_daemon.services.dispatcher import JobDispatcher
from cc_bridge_daemon.services.dispatcher_runtime.frontdesk_direct_handoff import (
...
CC_BRIDGE_REQ_ID: `frontdesk-auto-1`
...
def test_runtime_service_attach_reconciles_helper_ownership_via_registry_upsert(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.services.registry.cleanup_stale_runtime_helper',
        lambda layout, runtime: seen.append(('cleanup', runtime.agent_name)) or False,
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.services.registry.sync_runtime_helper_manifest',
...
def test_dispatcher_frontdesk_direct_silence_ask_records_activation_without_rewriting_body(
    ...
    activation_path = project_root / '.cc-bridge' / 'runtime' / 'loops' / 'activations' / 'act-frontdesk-frontdesk-auto-1.json'
    ...
    assert not (project_root / '.cc-bridge' / 'runtime' / 'frontdesk-handoff').exists()
...
def test_dispatcher_frontdesk_tampered_journal_cannot_self_certify_transaction(
    ...
    transaction_path = project_root / (
        '.cc-bridge/runtime/loops/activations/'
        'act-frontdesk-frontdesk-auto-1.direct-handoff.transaction.json'
    )
    transaction = json.loads(transaction_path.read_text(encoding='utf-8'))
    if tamper_kind == 'body':
...
def test_dispatcher_frontdesk_rejects_symlink_authority_paths(
    ...
    if escape_kind == 'parent':
        (project_root / '.cc-bridge/runtime').mkdir()
        (project_root / '.cc-bridge/runtime/loops').symlink_to(outside, target_is_directory=True)
    else:
...
def test_dispatcher_frontdesk_implementation_reply_fails_boundary_guard(
    ...
    marker_path = project_root / '.cc-bridge' / 'runtime' / 'frontdesk-boundary' / f'{job_id}.json'
...
def test_dispatcher_frontdesk_short_receipt_completes_after_persisted_direct_handoff(
    ...
    assert not (project_root / '.cc-bridge' / 'runtime' / 'frontdesk-boundary').exists()
...
def test_fake_provider_can_emit_deterministic_local_markdown_reply(tmp_path: Path) -> None:
    ...
    receipt = dispatcher.submit(
        MessageEnvelope(
            ...
            to_agent='demo',
            from_actor='user',
            body='cc_bridge-local-md:matrix',
            task_id=None,
            reply_to=None,
            message_type='ask',
            delivery_scope=DeliveryScope.SINGLE,
    ...
    assert reply.startswith('# CC_BRIDGE Local Markdown matrix')
    assert '`cc_bridge-local-reply:matrix`' in reply
...
def test_dispatcher_restore_running_jobs_marks_unrecoverable_execution_incomplete(tmp_path: Path) -> None:
    ...
    assert terminal.terminal_decision is not None
    assert terminal.terminal_decision['reason'] == 'cc_bridge_daemon_restart_requires_resubmit'
