...
def test_api_key_change_links_session_and_keeps_native_history_visible(tmp_path: Path) -> None:
    ...
    assert rewritten['cc_bridge_conversation_id'] == 'cc_bridge-launch-a'
    assert rewritten['cc_bridge_authority_generation'] == 2
    assert rewritten['cc_bridge_continuity_status'] == 'continued_on_new_authority'
    ...
    assert rewritten['cc_bridge_session_history'] == [
        {
            'provider': 'codex',
...
def test_unrelated_source_config_change_does_not_rotate_codex_authority(
    ...
    runtime_dir = tmp_path / 'repo-config' / '.cc-bridge' / 'agents' / 'agent1' / 'provider-runtime' / 'codex'
...
def test_referenced_codex_auth_sidecar_change_rotates_authority(monkeypatch, tmp_path: Path) -> None:
    runtime_dir = tmp_path / 'repo-sidecar' / '.cc-bridge' / 'agents' / 'agent1' / 'provider-runtime' / 'codex'
...
def test_inherited_source_login_change_changes_private_fingerprint(monkeypatch, tmp_path: Path) -> None:
    ...
    key_path = runtime_dir.parent.parent / 'provider-state' / 'codex' / '.cc_bridge-authority-hmac-key'
...
def test_explicit_codex_api_ignores_unselected_managed_auth(monkeypatch, tmp_path: Path) -> None:
    runtime_dir = tmp_path / 'repo-explicit-auth' / '.cc-bridge' / 'agents' / 'agent1' / 'provider-runtime' / 'codex'
    runtime_dir.mkdir(parents=True)
...
def test_v855_archive_remains_resume_visible_after_authority_change(tmp_path: Path) -> None:
    ...
    assert restored['cc_bridge_conversation_id'] == 'cc_bridge-launch-a'
    assert restored['cc_bridge_authority_generation'] == 2
    assert restored['cc_bridge_resume_compatibility'] == 'linked_continuation'
    assert 'codex_session_id' not in restored
    assert restored['cc_bridge_session_history'][0]['provider_session_id'] == 'v855-hidden-session'
    assert restored['cc_bridge_session_history'][0]['provider_session_path'] == str(restored_log)
...
def test_authority_change_does_not_record_codex_path_outside_session_root(tmp_path: Path) -> None:
    ...
    assert rewritten['cc_bridge_session_history'][0]['provider_session_id'] == 'outside-session'
    assert 'provider_session_path' not in rewritten['cc_bridge_session_history'][0]
