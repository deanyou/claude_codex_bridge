...
from cc_bridge_daemon.services.health_assessment.tmux_runtime.namespace import pane_outside_project_namespace


def test_pane_outside_namespace_accepts_runtime_socket_fallback(monkeypatch) -> None:
    runtime = SimpleNamespace(project_id='proj-1', agent_name='agent1', tmux_socket_path='/tmp/cc_bridge.sock')
    namespace_store = SimpleNamespace(
        load=lambda: SimpleNamespace(tmux_socket_path='/tmp/cc_bridge.sock', tmux_session_name='sess-1')
    )

    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.tmux_runtime.namespace.backend_socket_matches',
        lambda backend, tmux_socket_path: False,
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.tmux_runtime.namespace.same_tmux_socket_path',
...
def test_pane_outside_namespace_checks_project_namespace_record(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.services.health_assessment.tmux_runtime.namespace.inspect_project_namespace_pane',
