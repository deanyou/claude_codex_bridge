...
from cc_bridge_daemon.services.project_namespace_pane import snapshot_project_namespace_panes
from cc_bridge_daemon.services.project_namespace_runtime.materialize_topology import (
...
def test_startup_pane_snapshot_serves_topology_and_binding_without_rescan() -> None:
    ...
    context = SimpleNamespace(
        backend=backend,
        current=SimpleNamespace(namespace_epoch=7, tmux_session_name='cc_bridge-demo'),
        desired_workspace_window_name='main',
        desired_session_name='cc_bridge-demo',
    )
...
@pytest.mark.parametrize(
    ...
    (
        (None, True, None),
        (_topology_record('%sibling', session_name='cc_bridge-sibling'), True, None),
        (_topology_record('%sibling-only', session_name='cc_bridge-sibling'), False, 'topology_agent_panes_changed'),
...
def test_authoritative_topology_sidebar_and_tool_duplicates_do_not_force_recreate() -> None:
    ...
    records = {
        ...
        '%sidebar-sibling': _topology_record(
            '%sidebar-sibling',
            session_name='cc_bridge-sibling',
            role='sidebar',
            slot_key='sidebar:main',
            sidebar_instance='main',
        ),
    ...
    class Backend:
        ...
        def describe_pane(self, pane_id, *, user_options):
            ...
            return {
                ...
                '@cc_bridge_slot': 'agent1' if pane_id == '%agent' else 'removed-agent',
                '@cc_bridge_window': 'main',
