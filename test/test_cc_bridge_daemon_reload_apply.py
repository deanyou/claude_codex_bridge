...
from cc_bridge_daemon.reload_apply import run_additive_reload_apply
from cc_bridge_daemon.reload_apply_runtime import run_runtime_mount
from cc_bridge_daemon.reload_handoff import ReloadHandoffStore
from cc_bridge_daemon.reload_runtime_mount import AdditiveRuntimeMountResult
...
from cc_bridge_daemon.services.project_namespace_runtime import NamespacePatchApplyResult, build_namespace_topology_plan
from cc_bridge_daemon.services.project_namespace_state import ProjectNamespaceState, ProjectNamespaceStateStore
from cc_bridge_daemon.start_flow_runtime import StartFlowSummary
...
def test_additive_reload_apply_add_agent_materializes_tmux_pane_before_mount(
    ...
    backend = _PatchFakeBackend(socket_path=str(app.paths.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(app.paths.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[app.paths.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
...
def test_additive_reload_apply_add_window_materializes_tmux_window_before_mount(
    ...
    assert result.namespace_patch['sidebar_panes'] == {'review': '%3'}
    assert result.namespace_patch['agent_panes'] == {'agent3': '%4'}
    assert ('new-window', '-d', '-t', app.paths.cc_bridge_daemon_tmux_session_name, '-n', 'review') == backend.tmux_calls[1][:6]
...
def test_reload_apply_idle_replace_uses_default_start_flow_when_not_injected(
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.reload_runtime_replace.run_additive_agent_mounts',
        fake_mounts,
    )
...
def test_project_reload_non_dry_run_view_only_publishes_and_refreshes_graph_views(
    ...
    ping = app.socket_server._handlers['ping']({'target': 'cc_bridge_daemon'})
...
def test_project_reload_non_dry_run_add_agent_publishes_after_additive_stages(
    ...
    monkeypatch.setattr(
        'cc_bridge_daemon.reload_apply_service.apply_namespace_patch',
        lambda *_args, **_kwargs: _namespace_patch_result(agent_panes={'agent3': '%3'}),
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.reload_apply_service.run_runtime_mount',
...
def test_project_reload_non_dry_run_busy_remove_blocks_without_namespace_mutation(
    ...
    assert payload['reload_drains']['retry_command'] == 'cc_bridge reload'
...
def _seed_runtime(runtime_service, agent_name: str, *, pane_id: str, provider: str = 'codex'):
    return runtime_service.attach(
        ...
        active_pane_id=pane_id,
        pane_state='alive',
        tmux_socket_path=str(runtime_service._layout.cc_bridge_daemon_tmux_socket_path),
        tmux_window_name='main',
        slot_key=agent_name,
        lifecycle_state='idle',
        managed_by='cc_bridge_daemon',
...
def _store_namespace(app: CcbdApp) -> None:
    ProjectNamespaceStateStore(app.paths).save(
        ProjectNamespaceState(
            ...
            control_window_name=app.paths.cc_bridge_daemon_tmux_control_window_name,
...
def _write_dynamic_agent_state(
    ...
    *,
    ...
    state_path = project_root / '.cc-bridge' / 'runtime' / 'agents' / agent / 'lifecycle.json'
...
def _seed_agent_pane(
    ...
    *,
    ...
    backend.pane_options[pane_id] = {
        ...
        '@cc_bridge_namespace_epoch': str(namespace_epoch),
    }
