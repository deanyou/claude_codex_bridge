...
from cc_bridge_daemon.project_view import (
    AgentActivityFacts,
...
import cc_bridge_daemon.project_view.service as project_view_service
from cc_bridge_daemon.reload_drain import DrainIntent, DrainQueueStore, plan_drain_transition
...
def _config_with_tool_window() -> ProjectConfig:
    ...
    return ProjectConfig(
        ...
        tool_windows=(ToolWindowSpec(name='files', order=0, command='cc_bridge-workbench files'),),
        entry_window='main',
    )
...
def _reply_delivery_message(
    ...
    *,
    ...
    return MessageEnvelope(
        ...
        to_agent=target,
        from_actor='system',
        body=body if body is not None else f'CC_BRIDGE_REPLY from={source_agent} reply={reply_id} status=completed job={source_job_id}\n\nOK',
...
def test_project_view_namespace_view_redacts_herdr_restore_token() -> None:
    ...
    assert 'cc_bridge-herdr::workspace-1' not in str(view)
...
def test_project_view_agent_projects_herdr_runtime_surface(tmp_path: Path) -> None:
    ...
    runtime.last_failure_reason = 'Herdr auto restore observe-only blocks CC_BRIDGE-owned recovery'
    ...
    assert projection['blocking_gaps'] == [
        'Herdr auto restore observe-only blocks CC_BRIDGE-owned recovery',
...
def test_project_view_correlates_anchored_provider_activity_by_exact_pane(
    ...
    for agent_name in config.agents:
        ...
        if agent_name == 'agent2':
            ...
            runtime.session_id = 'cc_bridge-agent2-session'
        registry.upsert(runtime)
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
    ...
    write_activity(
        ...
        cc_bridge_session_id='cc_bridge-agent2-session',
...
def test_project_view_exposes_active_reload_drains(tmp_path: Path) -> None:
    ...
    assert view['reload_drains']['retry_command'] == 'cc_bridge reload'
...
def test_project_view_ignores_codex_provider_activity_without_cc_bridge_job(tmp_path: Path) -> None:
    ...
    write_activity(
        ...
        source='codex_hook',
        event_name='UserPromptSubmit',
        cc_bridge_session_id='cc_bridge-agent1-launch',
...
def test_project_view_ignores_codex_provider_activity_failed_for_running_job(tmp_path: Path) -> None:
    ...
    registry = AgentRegistry(layout, config)
    runtime = _runtime('agent1', project_id=project_id)
    runtime.session_id = 'cc_bridge-agent1-session'
    registry.upsert(runtime)
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
    ...
    write_activity(
        ...
        source='codex_hook',
        event_name='Stop',
        cc_bridge_session_id='cc_bridge-agent1-session',
        pane_id='%1',
        workspace_path='/tmp/workspace',
    ...
    assert agent['activity_source'] == 'cc_bridge_job'
...
def test_project_view_claude_activity_includes_pane_probe(tmp_path: Path) -> None:
    ...
    registry.upsert(runtime)
    mount_manager = MountManager(layout, clock=lambda: NOW)
    mount_manager.mark_mounted(project_id=project_id, pid=123, socket_path=layout.cc_bridge_daemon_socket_path, generation=1, started_at=NOW)
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            project_id=project_id,
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-provider-claude-pane-probe',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_claude_runtime_status_reads_bound_session_with_pane_probe(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-claude-session-runtime',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_claude_pane_active_overrides_idle_activity(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-claude-pane-active',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_claude_idle_prompt_overrides_stale_active_hook(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-claude-pane-terminal-summary',
            layout_version=2,
        )
    )
...
def test_project_view_codex_runtime_status_overrides_sidebar_presentation(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-codex-runtime-sidebar',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_codex_visible_working_remains_working_after_no_progress_threshold(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-codex-runtime-stale-working',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: current_time[0])
...
def test_project_view_codex_no_active_status_without_pane_progress_becomes_free(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-codex-runtime-no-active-stale',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: current_time[0])
...
def test_project_view_codex_no_active_status_progress_resets_no_progress_timer(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-codex-runtime-progress-reset',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: current_time[0])
...
def test_project_view_codex_running_unknown_displays_start(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-codex-runtime-start',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_codex_running_ignores_session_free_without_pane_completion(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-codex-runtime-running-session-free',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
    ...
    class CodexPromptBackend(_SnapshotBackend):
        def _tmux_run(self, args: list[str], *, capture=False, check=False, timeout=None):
            self.calls.append(list(args))
            if args[:3] == ['capture-pane', '-p', '-t']:
                return type('CP', (), {'returncode': 0, 'stdout': '› CC_BRIDGE_REQ_ID: job_running_1234\n', 'stderr': ''})()
            return self._snapshot_run(args, capture=capture, check=check, timeout=timeout)
...
def test_activity_resolver_provider_free_does_not_hide_running_job() -> None:
    ...
    assert activity.source == 'cc_bridge_job'
...
def test_project_view_marks_stale_provider_activity_failed_from_pane_error(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-provider-pane-error',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_marks_codex_runtime_failed_from_http_status_error_without_activity_artifact(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-provider-http-error',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
            ...
            if args[:3] == ['capture-pane', '-p', '-t']:
                return type(
                    ...
                    (),
                    {
                        'returncode': 0,
                        'stdout': (
                            '› trigger CC_BRIDGE_HTTP_429_SMOKE and stop\n'
...
def test_project_view_does_not_use_legacy_codex_prompt_idle_status(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-provider-pane-idle',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
            ...
            if args[:3] == ['capture-pane', '-p', '-t']:
                return type(
                    ...
                    (),
                    {
                        'returncode': 0,
                        'stdout': (
                            '› print exactly CC_BRIDGE_OK and stop\n\n'
                            '• CC_BRIDGE_OK\n\n'
...
def test_project_view_returns_minimal_windows_agents_and_comms(tmp_path: Path) -> None:
    ...
    assert view['cc_bridge_daemon']['state'] == MountState.MOUNTED.value
    ...
    assert view['agents'][0]['activity_source'] == 'cc_bridge_job'
    ...
    assert view['agents'][2]['activity_source'] == 'cc_bridge_job'
...
def test_project_view_comms_folds_reply_delivery_by_reply_record_without_body_job(tmp_path: Path) -> None:
    ...
    reply_delivery = _reply_delivery_job(
        ...
        body='CC_BRIDGE_REPLY from=agent3 reply=reply_structured status=completed\n\nOK',
    )
    dispatcher._append_job(reply_delivery)
    service = ProjectViewService(
        ProjectViewDependencies(
    ...
    for index in range(3):
        ...
        dispatcher._append_job(
            _reply_delivery_job(
                ...
                body=f'CC_BRIDGE_REPLY from=agent3 reply=reply_indexed_{index} status=completed\n\nOK',
            )
        )
...
def test_reply_delivery_lookup_fallback_avoids_jsonl_list_all(tmp_path: Path, monkeypatch) -> None:
    ...
    delivery = _reply_delivery_job(
        ...
        body='CC_BRIDGE_REPLY from=agent3 reply=reply_fallback status=completed\n\nOK',
    )
...
def test_project_view_backfills_reply_delivery_source_outside_recent_tail(tmp_path: Path) -> None:
    ...
    delivery = _reply_delivery_job(
        ...
        body='CC_BRIDGE_REPLY from=agent3 reply=reply_old_source status=completed\n\nOK',
    )
    dispatcher._append_job(delivery)
    service = ProjectViewService(
        ProjectViewDependencies(
...
def test_project_view_comms_marks_agent_reply_delivery_pending(tmp_path: Path) -> None:
    ...
    delivery = _reply_delivery_job(
        ...
        body='CC_BRIDGE_REPLY from=agent3 reply=reply_delivering status=completed\n\nOK',
    )
    dispatcher._append_job(delivery)
...
def test_project_view_sequence_changes_when_content_changes(tmp_path: Path) -> None:
    ...
    assert second['view']['agents'][0]['activity_source'] == 'cc_bridge_job'
...
def test_project_view_cache_hit_skips_tmux_calls(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-cache-tmux',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_idle_cache_ttl_can_restore_legacy_cadence(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv('CC_BRIDGE_PROJECT_VIEW_IDLE_TTL_MS', '1000')
...
def test_project_view_updates_build_cache_and_tmux_metrics(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-project-view-metrics',
            layout_version=2,
        )
    )
    old_job = _job(
...
def test_project_view_consumes_sidebar_refresh_request_without_crashing(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-project-view-sidebar-refresh',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
    backend = _SidebarRefreshBackend(session_name='cc_bridge-project-view-sidebar-refresh')
    controller = ProjectNamespaceController(
        layout,
        project_id,
        backend_factory=lambda socket_path=None: backend,
...
class _SidebarRefreshBackend(_SnapshotBackend):
    ...
    def list_panes_by_user_options(self, expected: dict[str, str]) -> list[str]:
        if expected.get('@cc_bridge_role') == 'sidebar':
            return ['%90']
        return []
        ...
        if args[:3] == ['capture-pane', '-p', '-t']:
            return type(
                ...
                (),
                {
                    'returncode': 0,
                    'stdout': (
                        f'❯ CC_BRIDGE_REQ_ID: {self._job_id}\n\n'
...
def test_project_view_marks_active_window_and_agent_from_namespace_focus(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=2,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-focus',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_reads_window_and_sidebar_tmux_metadata(tmp_path: Path) -> None:
    ...
    registry = AgentRegistry(layout, config)
    mount_manager = MountManager(layout, clock=lambda: NOW)
    mount_manager.mark_mounted(project_id='proj-snap', pid=123, socket_path=layout.cc_bridge_daemon_socket_path, generation=1, started_at=NOW)
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            project_id='proj-snap',
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-snap',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_uses_rust_tmux_parser_when_enabled(tmp_path: Path, monkeypatch) -> None:
    ...
    registry = AgentRegistry(layout, config)
    mount_manager = MountManager(layout, clock=lambda: NOW)
    mount_manager.mark_mounted(project_id='proj-snap-helper', pid=123, socket_path=layout.cc_bridge_daemon_socket_path, generation=1, started_at=NOW)
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            project_id='proj-snap-helper',
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-snap-helper',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_required_tmux_parser_missing_helper_raises_without_python_fallback(
    ...
    with pytest.raises(RuntimeError, match='no Python fallback'):
        project_view_service._parse_tmux_project_view_outputs(
            ...
            sidebars_stdout='cc_bridge-snap\tmain\t%90\tproj-snap\tsidebar\tmain\tmain\n',
...
def test_project_view_captures_each_running_pane_once_per_build(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-capture-once',
            layout_version=2,
        )
    )
    old_job = _job(
...
def test_project_view_reuses_namespace_backend_within_build(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-backend-once',
            layout_version=2,
        )
    )
    old_job = _job(
    ...
    assert backend_factory_calls == [str(layout.cc_bridge_daemon_tmux_socket_path)]
...
def test_project_view_marks_provider_prompt_as_pending(tmp_path: Path) -> None:
    ...
    mount_manager = MountManager(layout, clock=lambda: NOW)
    mount_manager.mark_mounted(project_id='proj-prompt', pid=123, socket_path=layout.cc_bridge_daemon_socket_path, generation=1, started_at=NOW)
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            project_id='proj-prompt',
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-prompt',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_marks_running_job_idle_after_provider_prompt_reappears(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-idle-running',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_requires_bounded_exact_idle_observation_before_orphaning(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-claude-correlated-orphan',
            layout_version=2,
        )
    )
...
def test_project_view_resets_orphan_observation_on_runtime_binding_rotation(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-claude-orphan-binding-reset',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: '2026-05-20T11:59:00Z')
...
def test_project_view_does_not_mark_fresh_running_prompt_idle_as_recoverable(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-idle-running-fresh',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_project_view_does_not_use_legacy_codex_prompt_idle_recovery_without_anchor(tmp_path: Path) -> None:
    ...
    ProjectNamespaceStateStore(layout).save(
        ProjectNamespaceState(
            ...
            namespace_epoch=3,
            tmux_socket_path=str(layout.cc_bridge_daemon_tmux_socket_path),
            tmux_session_name='cc_bridge-idle-no-anchor',
            layout_version=2,
        )
    )
    dispatcher = JobDispatcher(layout, config, registry, clock=lambda: NOW)
...
def test_activity_resolver_core_states() -> None:
    ...
    assert queued.source == 'cc_bridge_job'
    ...
    assert running.source == 'cc_bridge_job'
    ...
    assert stale.source == 'cc_bridge_job'
...
def test_activity_resolver_treats_idle_prompt_after_request_as_agent_idle() -> None:
    activity = resolve_agent_activity(
        AgentActivityFacts(
            ...
            pane_id='%1',
            pane_state='alive',
            pane_text='❯ CC_BRIDGE_REQ_ID: job_prompt_idle_2\n\ncancelled\n\n❯ \n',
        ),
        now=NOW,
    )
...
def test_activity_resolver_ignores_fresh_job_when_provider_prompt_is_idle() -> None:
    activity = resolve_agent_activity(
        AgentActivityFacts(
            ...
            pane_id='%1',
            pane_state='alive',
            pane_text='❯ CC_BRIDGE_REQ_ID: job_prompt_idle_new\n\n❯ \n',
        ),
        now=NOW,
    )
...
def test_activity_resolver_does_not_use_job_state_without_provider_working_signal() -> None:
    activity = resolve_agent_activity(
        AgentActivityFacts(
            ...
            pane_id='%1',
            pane_state='alive',
            pane_text='❯ CC_BRIDGE_REQ_ID: job_still_waiting\n\nworking on task\n',
        ),
        now=NOW,
    )
...
def test_activity_resolver_keeps_input_stuck_detection_in_comms_not_agent_status() -> None:
    activity = resolve_agent_activity(
        AgentActivityFacts(
            ...
            pane_id='%1',
            pane_state='alive',
            pane_text='❯ CC_BRIDGE_REQ_ID: job_input_stuck\n\n  查询北京天气\n',
        ),
        now=NOW,
    )
...
def test_activity_resolver_ignores_fresh_input_stuck_job_for_agent_status() -> None:
    activity = resolve_agent_activity(
        AgentActivityFacts(
            ...
            pane_id='%1',
            pane_state='alive',
            pane_text='❯ CC_BRIDGE_REQ_ID: job_input_new\n\n  查询北京天气\n',
        ),
        now=NOW,
    )
...
def test_activity_resolver_provider_prompt_does_not_hide_running_tool() -> None:
    activity = resolve_agent_activity(
        AgentActivityFacts(
            ...
            pane_id='%1',
            pane_state='alive',
            pane_text='❯ CC_BRIDGE_REQ_ID: job_running_tool\n\nBash(sleep 60)\n⎿ Running… (10s)\n\n❯ \n',
        ),
        now=NOW,
    )
    ...
    activity = resolve_agent_activity(
        AgentActivityFacts(
            ...
            pane_text=(
                ...
                '\n'
                '─────────────────────────────────────────────────────\n'
                '❯ CC_BRIDGE_REQ_ID: job_86550847f237\n'
                '\n'
                '  再执行一次：循环50次，每次等待30s。\n'
                '─────────────────────────────────────────────────────\n'
...
def test_activity_resolver_ignores_codex_hook_activity() -> None:
    pane_text = '\n'.join(
        [
            ...
            '╰───────────────────────────────────────────────╯',
            '',
            '› trigger CC_BRIDGE_REFUSED_SMOKE and stop',
            '',
            '• Working (18s • esc to interrupt)',
            '• Messages to be submitted after next tool call (press esc to interrupt and send immediately)',
            '  ↳ trigger CC_BRIDGE_NEXT_AFTER_FAIL and stop',
            '',
            '› Use /skills to list available skills',
            '',
            '  gpt-5.5 xhigh · ~/yunwei/test_ccb2',
...
def test_activity_resolver_ignores_stale_provider_working_history() -> None:
    pane_text = '\n'.join(
        [
            ...
            '│                                               │',
            '│ model:       gpt-5.5 xhigh   /model to change │',
            '│ directory:   ~/yunwei/cc_bridge_sidebar_test        │',
            '│ permissions: YOLO mode                        │',
            '╰───────────────────────────────────────────────╯',
            '',
            '  gpt-5.5 xhigh · ~/yunwei/cc_bridge_sidebar_test',
            '',
            '',
            '',
            '',
