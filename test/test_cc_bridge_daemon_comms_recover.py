...
def _bootstrap(project_root: Path) -> ProjectContext:
    ...
    config_dir.mkdir()
    (config_dir / 'cc_bridge.config').write_text('agent1:codex, agent2:claude, agent3:codex\n', encoding='utf-8')
...
def test_project_view_marks_recoverable_and_clears_after_recovery(tmp_path: Path) -> None:
    ...
    mount_manager.mark_mounted(project_id=ctx.project_id, pid=123, socket_path=layout.cc_bridge_daemon_socket_path, generation=1, started_at=NOW)
