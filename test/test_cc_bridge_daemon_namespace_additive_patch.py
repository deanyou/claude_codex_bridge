...
from cc_bridge_daemon.services.project_namespace_runtime import (
    assert_preserved_agent_panes,
    build_namespace_topology_plan,
    snapshot_preserved_agent_panes,
)
from cc_bridge_daemon.services.project_namespace_runtime.sidebar_helper import SIDEBAR_HELPER_ID_OPTION
...
class _PatchFakeHerdrBackend:
    ...
    def __init__(self, *, session_name: str = 'cc_bridge-herdr', support_reflow: bool = True) -> None:
    ...
    def set_pane_identity(self, pane: dict[str, object], **kwargs) -> None:
        ...
        self.pane_options.setdefault(pane_id, {}).update(
            {
                '@cc_bridge_project_id': str(kwargs.get('project_id') or ''),
                '@cc_bridge_role': str(kwargs.get('role') or ''),
                '@cc_bridge_slot': str(kwargs.get('slot_key') or ''),
                '@cc_bridge_window': str(kwargs.get('window_name') or ''),
                '@cc_bridge_managed_by': str(kwargs.get('managed_by') or ''),
                '@cc_bridge_namespace_epoch': str(kwargs.get('namespace_epoch') or ''),
            }
        )
...
def test_apply_add_window_creates_only_new_window_sidebar_and_agent_panes(tmp_path: Path, monkeypatch) -> None:
    ...
    backend = _PatchFakeBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    ...
    assert ('new-window', '-d', '-t', layout.cc_bridge_daemon_tmux_session_name, '-n', 'review') in {
    ...
    assert backend.respawn_calls[0][1].startswith('CC_BRIDGE_SIDEBAR_THEME_PROFILE=default ')
    assert '--theme' not in backend.respawn_calls[0][1]
    assert backend.pane_options['%3']['@cc_bridge_role'] == 'sidebar'
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'sidebar:review'
    assert backend.pane_options['%3']['@cc_bridge_managed_by'] == 'cc_bridge_daemon'
    assert backend.pane_options['%4']['@cc_bridge_slot'] == 'agent3'
    assert backend.pane_options['%5']['@cc_bridge_slot'] == 'agent4'
    assert {backend.pane_options[pane]['@cc_bridge_window'] for pane in ('%3', '%4', '%5')} == {'review'}
...
def test_apply_herdr_append_agent_uses_v2_namespace_ref_without_tmux_runner(tmp_path: Path, monkeypatch) -> None:
    ...
    first_pane = backend.add_window('cc_bridge-herdr', 'main')
    second = backend._alloc_pane('cc_bridge-herdr', 'main')
...
"""
    ...
    second = backend._alloc_pane('cc_bridge-herdr', 'main')
    backend.window_panes['main'].append(str(second['pane_id']))
    review_pane = backend.add_window('cc_bridge-herdr', 'review')
    _seed_agent_pane(backend, first_pane, project_id='proj-1', window='main', agent='agent1')
    _seed_agent_pane(backend, str(second['pane_id']), project_id='proj-1', window='main', agent='agent2')
    ...
    assert backend.pane_options['herdr-pane-2']['@cc_bridge_window'] == 'review'
...
def test_apply_add_window_materializes_rich_alias_as_tool_pane(tmp_path: Path, monkeypatch) -> None:
    ...
    assert backend.respawn_calls[-1] == ('%5', 'CC_BRIDGE_WORKBENCH_PROFILE=rich CC_BRIDGE_WORKBENCH_FORCE_RICH=1 cc_bridge-workbench files')
    assert backend.pane_options['%5']['@cc_bridge_role'] == 'tool'
    assert backend.pane_options['%5']['@cc_bridge_slot'] == 'tool:rich'
    assert backend.pane_options['%5']['@cc_bridge_window'] == 'review'
...
def test_preserved_snapshot_and_assertion_use_fake_identity_data(tmp_path: Path) -> None:
    ...
    backend = _PatchFakeBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    first_pane = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    backend.pane_counter = 2
    _seed_agent_pane(backend, first_pane, project_id='proj-1', window='main', agent='agent1')
    _seed_agent_pane(backend, '%2', project_id='proj-1', window='main', agent='agent2')
...
def test_apply_add_window_failure_returns_partial_without_side_effect_contract(tmp_path: Path, monkeypatch) -> None:
    ...
    backend = _FailingBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    backend.pane_counter = 2
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
...
def test_apply_add_window_fails_when_preserved_agent_pane_changes(tmp_path: Path, monkeypatch) -> None:
    ...
    backend = _MovingBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    backend.pane_counter = 2
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
...
def test_apply_append_add_agent_creates_only_new_agent_pane(tmp_path: Path, monkeypatch) -> None:
    ...
    assert ('select-layout', '-E', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:main') in backend.tmux_calls
    assert backend.pane_options['%3']['@cc_bridge_project_id'] == 'proj-1'
    assert backend.pane_options['%3']['@cc_bridge_role'] == 'agent'
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'agent3'
    assert backend.pane_options['%3']['@cc_bridge_window'] == 'main'
    assert backend.pane_options['%3']['@cc_bridge_namespace_epoch'] == '3'
    assert backend.pane_options['%3']['@cc_bridge_managed_by'] == 'cc_bridge_daemon'
    assert result.diagnostics['graph_published'] is False
    assert result.diagnostics['runtime_authority_written'] is False
    assert result.diagnostics['lease_or_lifecycle_written'] is False
...
def test_apply_append_multiple_agents_reflows_between_splits_in_narrow_window(
    ...
    backend = NarrowAppendBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    backend.pane_counter = 2
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
...
def test_apply_remove_agent_kills_only_removed_agent_pane(tmp_path: Path, monkeypatch) -> None:
    ...
    assert ('select-layout', '-E', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:main') in backend.tmux_calls
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1']
...
def test_apply_remove_multiple_agents_kills_removed_panes_and_reflows(tmp_path: Path, monkeypatch) -> None:
    ...
    backend = _PatchFakeBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].extend(['%2', '%3'])
...
def test_apply_remove_agent_reflows_compact_workspace_by_namespace_window_id(tmp_path: Path, monkeypatch) -> None:
    ...
    backend = _StrictSelectBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'cc_bridge')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    backend.pane_counter = 2
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
    _seed_agent_pane(backend, '%2', project_id='proj-1', window='main', agent='agent2')
    _store_namespace(layout, project_id='proj-1', workspace_window_name='cc_bridge', workspace_window_id='@1')
    controller = ProjectNamespaceController(layout, 'proj-1', backend_factory=lambda socket_path=None: backend)
    _forbid_recreate_paths(monkeypatch)
    plan = build_reload_dry_run_plan(current, new, project_id='proj-1', current_namespace=controller.load())
    ...
    assert result.reflowed_windows == ('main',)
    assert result.reflow_errors == {}
    assert ('select-layout', '-E', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:@1') in backend.tmux_calls
    assert ('select-layout', '-E', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:main') not in backend.tmux_calls
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1']
...
def test_apply_remove_agent_reflows_window_and_restores_sidebar_width(tmp_path: Path, monkeypatch) -> None:
    ...
    backend = _PatchFakeBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] = ['%1', '%2', '%3']
    backend.pane_counter = 3
    ...
    assert backend.pane_options['%1']['@cc_bridge_slot'] == 'sidebar:main'
...
"""
    ...
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    backend.pane_counter = 2
    review_pane = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'review')
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
    _seed_agent_pane(backend, '%2', project_id='proj-1', window='main', agent='agent2')
    _seed_agent_pane(backend, review_pane, project_id='proj-1', window='review', agent='agent3')
    _store_namespace(layout, project_id='proj-1')
    ...
    assert ('select-layout', '-E', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:main') in backend.tmux_calls
    assert ('select-layout', '-E', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:review') in backend.tmux_calls
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'] == [review_pane, '%2']
    assert backend.pane_options['%2']['@cc_bridge_slot'] == 'agent2'
    assert backend.pane_options['%2']['@cc_bridge_window'] == 'review'
    assert backend.pane_options['%2']['@cc_bridge_namespace_epoch'] == '3'
...
"""
    ...
    backend = _PatchFakeBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    review_first_pane = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'review')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'].extend(['%3', '%4'])
    ...
    assert ('kill-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:review') not in backend.tmux_calls
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1', review_first_pane, '%3']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'] == ['%4']
    assert backend.pane_options[review_first_pane]['@cc_bridge_slot'] == 'helper1'
    assert backend.pane_options[review_first_pane]['@cc_bridge_window'] == 'main'
    assert backend.pane_options[review_first_pane]['@cc_bridge_namespace_epoch'] == '3'
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'helper2'
    assert backend.pane_options['%3']['@cc_bridge_window'] == 'main'
    assert backend.pane_options['%3']['@cc_bridge_namespace_epoch'] == '3'
    assert backend.pane_options['%4']['@cc_bridge_slot'] == 'helper3'
    assert backend.pane_options['%4']['@cc_bridge_window'] == 'review'
...
"""
    ...
    backend = _PatchFakeBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    review_sidebar = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'review')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'].extend(['%3', '%4'])
    backend.pane_counter = 4
    ...
    assert ('kill-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:review') in backend.tmux_calls
    assert [record['name'] for record in backend.sessions[layout.cc_bridge_daemon_tmux_session_name]] == ['main']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1', '%3', '%4']
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'zeta'
    assert backend.pane_options['%3']['@cc_bridge_window'] == 'main'
    assert backend.pane_options['%4']['@cc_bridge_slot'] == 'alpha'
    assert backend.pane_options['%4']['@cc_bridge_window'] == 'main'
...
"""
    ...
    assert result.preserved_before == {'alpha': '%4', 'main': '%1', 'zeta': '%3'}
    assert result.preserved_after == {'alpha': '%4', 'main': '%1', 'zeta': '%3'}
    assert ('new-window', '-d', '-t', layout.cc_bridge_daemon_tmux_session_name, '-n', 'archive') in {
        call[:6] for call in backend.tmux_calls
    }
    ...
    assert ('move-pane', '-v', '-s', '%4', '-t', '%3') in backend.tmux_calls
    assert ('kill-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:review') in backend.tmux_calls
    assert [record['name'] for record in backend.sessions[layout.cc_bridge_daemon_tmux_session_name]] == ['main', 'archive']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'] == ['%5', '%3', '%4']
    assert backend.pane_options['%5']['@cc_bridge_role'] == 'sidebar'
    assert backend.pane_options['%5']['@cc_bridge_slot'] == 'sidebar:archive'
    assert '%6' not in backend.pane_options
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'zeta'
    assert backend.pane_options['%3']['@cc_bridge_window'] == 'archive'
    assert backend.pane_options['%4']['@cc_bridge_slot'] == 'alpha'
    assert backend.pane_options['%4']['@cc_bridge_window'] == 'archive'
    assert review_sidebar not in backend.pane_options
...
"""
    ...
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    review_sidebar = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'review')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'].append('%3')
    backend.pane_counter = 3
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='main')
    _seed_sidebar_pane(backend, review_sidebar, project_id='proj-1', window='review')
    _seed_agent_pane(backend, '%3', project_id='proj-1', window='review', agent='zeta')
    ...
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'zeta'
    assert backend.pane_options['%3']['@cc_bridge_window'] == 'main'
    assert backend.pane_options['%4']['@cc_bridge_slot'] == 'beta'
    assert backend.pane_options['%4']['@cc_bridge_window'] == 'main'
    assert review_sidebar not in backend.pane_options
...
"""
    ...
    assert [record['name'] for record in backend.sessions[layout.cc_bridge_daemon_tmux_session_name]] == ['main', 'archive']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'] == ['%5', '%3', '%4', '%7']
    assert backend.pane_options['%5']['@cc_bridge_role'] == 'sidebar'
    assert backend.pane_options['%5']['@cc_bridge_slot'] == 'sidebar:archive'
    assert '%6' not in backend.pane_options
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'zeta'
    ...
    assert backend.pane_options['%4']['@cc_bridge_slot'] == 'alpha'
    assert backend.pane_options['%4']['@cc_bridge_window'] == 'archive'
    assert backend.pane_options['%7']['@cc_bridge_slot'] == 'beta'
    assert backend.pane_options['%7']['@cc_bridge_window'] == 'archive'
    assert review_sidebar not in backend.pane_options
...
""",
    ...
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'] == ['%3', '%2']
    assert backend.pane_options['%3']['@cc_bridge_role'] == 'sidebar'
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'sidebar:review'
...
"""
    ...
    backend.pane_options[review_sidebar]['@cc_bridge_role'] = 'sidebar'
    backend.pane_options[review_sidebar]['@cc_bridge_slot'] = 'sidebar:review'
    backend.pane_options[review_sidebar]['@cc_bridge_sidebar_instance'] = 'review'
    ...
    assert ('kill-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:review') in backend.tmux_calls
    assert [record['name'] for record in backend.sessions[layout.cc_bridge_daemon_tmux_session_name]] == ['main']
    assert backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'] == ['%1', '%3']
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'agent2'
    assert backend.pane_options['%3']['@cc_bridge_window'] == 'main'
    assert review_sidebar not in backend.pane_options
...
def test_apply_add_tool_window_creates_tool_window_sidebar_and_tool_pane(
    ...
    assert backend.respawn_calls[-2][1].startswith('CC_BRIDGE_SIDEBAR_THEME_PROFILE=light ')
    assert '--theme' not in backend.respawn_calls[-2][1]
    assert backend.respawn_calls[-1] == ('%4', 'cc_bridge-workbench files')
    assert backend.pane_options['%3']['@cc_bridge_role'] == 'sidebar'
    assert backend.pane_options['%3']['@cc_bridge_slot'] == 'sidebar:files'
    assert backend.pane_options['%4']['@cc_bridge_role'] == 'tool'
    assert backend.pane_options['%4']['@cc_bridge_slot'] == 'tool:files'
    assert backend.pane_options['%4']['@cc_bridge_window'] == 'files'
    assert result.diagnostics['runtime_authority_written'] is False
...
def test_apply_remove_tool_window_kills_only_tool_window(
    ...
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
    _seed_agent_pane(backend, '%2', project_id='proj-1', window='main', agent='agent2')
    tool_root = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'files')
    backend.pane_options[tool_root] = {
        '@cc_bridge_project_id': 'proj-1',
        '@cc_bridge_role': 'tool',
        '@cc_bridge_slot': 'tool:files',
        '@cc_bridge_window': 'files',
    ...
    assert [
        ...
        if call == ('kill-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:files')
    ] == [('kill-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:files')]
    assert [record['name'] for record in backend.sessions[layout.cc_bridge_daemon_tmux_session_name]] == ['main']
...
def test_apply_remove_tool_window_is_idempotent_when_window_already_missing(
    ...
    backend = _MissingWindowBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][0]['panes'].append('%2')
    backend.pane_counter = 2
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
...
"""
    ...
    backend = _LastPaneClosesWindowBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    review_pane = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'review')
    _seed_agent_pane(backend, '%1', project_id='proj-1', window='main', agent='agent1')
...
cc_bridge-exec = "loop-coder:codex, loop-reviewer:codex"
"""
    new_text = """version = 2
entry_window = "main"
...
"""
    ...
    backend = _PatchFakeBackend(socket_path=str(layout.cc_bridge_daemon_tmux_socket_path))
    main_pane = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'main')
    exec_pane = backend.add_window(layout.cc_bridge_daemon_tmux_session_name, 'cc_bridge-exec')
    backend.sessions[layout.cc_bridge_daemon_tmux_session_name][1]['panes'].append('%3')
    backend.pane_counter = 3
    _seed_agent_pane(backend, main_pane, project_id='proj-1', window='main', agent='frontdesk')
    _seed_agent_pane(backend, exec_pane, project_id='proj-1', window='cc_bridge-exec', agent='loop-coder')
    _seed_agent_pane(backend, '%3', project_id='proj-1', window='cc_bridge-exec', agent='loop-reviewer')
    ...
    select_call = ('select-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:@1')
    kill_call = ('kill-window', '-t', f'{layout.cc_bridge_daemon_tmux_session_name}:cc_bridge-exec')
    assert result.status == 'applied'
    assert result.removed_windows == ('cc_bridge-exec',)
...
def _forbid_recreate_paths(monkeypatch, *, allow_kill_window: bool = False) -> None:
    monkeypatch.setattr(
        'cc_bridge_daemon.services.project_namespace_runtime.ensure.ensure_project_namespace',
    ...
    for name in forbidden_backend_mutations:
        monkeypatch.setattr(
            f'cc_bridge_daemon.services.project_namespace_runtime.backend.{name}',
            lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError(f'must not call {name}')),
            raising=False,
        )
    monkeypatch.setattr(
        'cc_bridge_daemon.services.project_namespace_runtime.ensure_state.force_recreate_namespace',
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError('must not force recreate namespace')),
        raising=False,
    )
    monkeypatch.setattr(
        'cc_bridge_daemon.services.project_namespace_runtime.reflow.reflow_project_workspace',
...
def _seed_agent_pane(backend: _PatchFakeBackend, pane_id: str, *, project_id: str, window: str, agent: str) -> None:
    backend.pane_options[pane_id] = {
        ...
        '@cc_bridge_slot': agent,
        '@cc_bridge_window': window,
        '@cc_bridge_managed_by': 'cc_bridge_daemon',
        '@cc_bridge_namespace_epoch': '3',
    }
...
def _seed_sidebar_pane(backend: _PatchFakeBackend, pane_id: str, *, project_id: str, window: str) -> None:
    backend.pane_options[pane_id] = {
        '@cc_bridge_project_id': project_id,
        '@cc_bridge_role': 'sidebar',
        '@cc_bridge_slot': f'sidebar:{window}',
        '@cc_bridge_sidebar_instance': window,
        '@cc_bridge_window': window,
        '@cc_bridge_managed_by': 'cc_bridge_daemon',
        '@cc_bridge_namespace_epoch': '3',
    }
...
def _herdr_namespace_state(*, project_id: str, session_name: str = 'cc_bridge-herdr') -> ProjectNamespaceState:
    return ProjectNamespaceState(
        ...
        layout_version=3,
        layout_signature=None,
        control_window_name='cc_bridge-control',
        control_window_id='control',
...
def test_apply_add_window_normalizes_mixed_case_agent_names(tmp_path: Path, monkeypatch) -> None:
    ...
    # pane token 也应归一化（cc_bridge_slot / cc_bridge_agent 用 main_code）
    assert backend.pane_options['%4'].get('@cc_bridge_slot') == 'main_code'
    assert backend.pane_options['%4'].get('@cc_bridge_agent') == 'main_code'
