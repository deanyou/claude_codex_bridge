...
import provider_backends.qwen.session as qwen_session
...
def test_qwen_ensure_pane_respawns_dead_pane(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_qwen_ensure_pane_already_alive(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_qwen_ensure_pane_does_not_rediscover_different_pane(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    session_path = tmp_path / ".qwen-session"
    session_path.write_text(
        json.dumps({
            "cc_bridge_session_id": "test-session",
...
def test_qwen_load_session_returns_none_for_inactive(tmp_path: Path) -> None:
...
def test_qwen_load_session_returns_none_for_missing_file(tmp_path: Path) -> None:
    """Missing session file should return None."""
    sess = qwen_session.load_project_session(tmp_path)
    assert sess is None


def test_qwen_compute_session_key_prefix(tmp_path: Path) -> None:
    ...
    key = qwen_session.compute_session_key(sess)
