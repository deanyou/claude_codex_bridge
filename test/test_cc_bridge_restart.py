...
from cc_bridge_daemon.handlers.project_restart import build_project_restart_agent_handler
...
def test_project_restart_agent_handler_blocks_on_stale_role_digest(monkeypatch, tmp_path: Path) -> None:
    session = SimpleNamespace(
        data={
            'cc_bridge_role_id': 'test.locked',
            'cc_bridge_role_version': '1.0.0',
            'cc_bridge_role_digest': 'sha256:olddigest',
        },
...
def test_project_restart_agent_socket_targets_one_agent(tmp_path: Path, monkeypatch) -> None:
    ...
    thread.start()
    _wait_for_socket(app.paths.cc_bridge_daemon_socket_path)
