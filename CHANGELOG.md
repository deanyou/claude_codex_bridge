...
- Prevented incompatible remote Codex resume permission flags and made
  `cc-bridge clear` report dead panes and input delivery honestly (#346).
- Fenced Codex app-server generations, readiness checks, and shutdown cleanup
  against socket/PID races during restart (#345).
- Preserved `CC_BRIDGE_KIMI_NATIVE_TURN_TIMEOUT_S` through the daemon environment
...
- Fixed blank CC_BRIDGE Mobile conversations for OMP Agents by routing OMP through
...
- Upgrade the CC_BRIDGE host and CC_BRIDGE Mobile APK, then restart the Mobile gateway. No
...
- Added CC_BRIDGE Mobile multi-host aggregation and host display names.
...
- Includes the visible OMP ask/completion and Config V1/V2 role filtering fixes
  from v8.6.13. Restart managed Agents and CC_BRIDGE projects after upgrading; no
...
- Limited Config V1/V2 role selection to general roles and
  `agentroles.cc-bridge_self`; other workflow-specific `cc-bridge_*` roles remain reserved
  for V3.
...
- Exposed Pi and OMP native conversations in CC_BRIDGE Mobile while preserving
...
- Added per-Agent Codex `model_catalog_json` projection and model overrides,
  ...
  `CC_BRIDGE_TMUX_CONFIG` across the keeper control-plane environment (PRs #321,
  #322, and #326).
...
- Enforced UTF-8 mode at the native Windows launcher boundary instead of
  ...
  remain beta.
- Restart managed Codex Agents and CC_BRIDGE projects after upgrading. No project,
...
- Added fail-closed handling for symlinked CC_BRIDGE credential projections and
...
- Added adjustable workspace-surface opacity for local CC_BRIDGE Mobile backgrounds
...
- Added local CC_BRIDGE Mobile workspace background customization across project
...
- Restarted an already-running installed Mobile Host after a successful
  `cc-bridge update`, preserving its route, host identity, and pairing instead of
...
- No configuration, pairing, or conversation migration is required. Upgrade
  both the CC_BRIDGE host and Mobile APK for the Relay terminal fix. Native Windows
  x64 remains beta.
...
- Existing pairing and project state require no migration. Upgrade both the
  CC_BRIDGE host and Mobile APK to activate the corrected projection protocol.

## v8.6.4 (2026-08-14)

- Reworked CC_BRIDGE Mobile Agent terminals as responsive local projections: phone
...
- Added fail-closed reattachment of persisted Windows Herdr namespace and pane
  ...
  (PR #304).
- Existing pairing data requires no migration. Upgrade the CC_BRIDGE host and Mobile
  APK together to use the complete terminal viewport protocol.

## v8.6.3 (2026-08-13)

- CC_BRIDGE Mobile now turns ordinary file links under the current Agent's
  `.cc-bridge/workspaces/<agent>/...` tree into authenticated download attachments,
  including generated PDF reports.
- The workspace exception remains fail-closed: other Agent workspaces, hidden
  workspace paths, and the rest of `.cc-bridge` runtime state are not exposed.
...
- Added fail-closed, exact command approvals for project-local tool-window
  ...
  `cc-bridge config approve-commands` workflow (Issue #299).
...
- Expanded CC_BRIDGE Mobile with multi-session host terminals and Relay
...
- Added capability-driven CC_BRIDGE Mobile Provider controls with explicit
...
- Added the built-in `cc-bridge-compact` Skill and `cc-bridge compact` command with busy
...
- Added the first native Windows x64 distribution to the stable CC_BRIDGE release:
...
- Restored Pi's validated native JSONL session after CC_BRIDGE restart, while keeping
...
- Fixed project-scoped tmux socket rebinding after backend cache reuse and made
  ...
  Removed the backend import cycle that could block mobile/cc-bridge-daemon service imports.
- No configuration or conversation migration is required.

## v8.6.0-beta.3 (2026-08-11)

- Made the Windows installer's `-Yes` and `CC_BRIDGE_INSTALL_ASSUME_YES=1` modes acknowledge the guarded missing-Herdr continuation without reading interactive input.
...
- Added native PE launchers for `cc-bridge`, `ask`, `autonew`, and `ctx-transfer`, packaged as `cc-bridge-windows-x86_64.zip` with a SHA256 sidecar.
...
- Keep the required `cc-bridge-clear` control Skill and add `cc-bridge-diagnose` for every
  supported managed Agent. `$cc-bridge_diagnose <agentname>` combines authoritative
...
- Self-heal expired mailbox deliveries without replaying business work and
  forward Provider no-terminal timeout settings to `cc-bridge-daemon` (PR #288). Remove
...
- **Explicit Authority Wins Per Dimension**: CC_BRIDGE uses configured API, token,
...
- **Restart Refreshes Without Reverse Effects**: a stopped Provider generation
  is rebuilt from the current external snapshot and private projection. CC_BRIDGE
...
- **Conversation History Survives Authority Changes**: same-authority launches
  ...
  when available; otherwise CC_BRIDGE records a linked continuation while preserving
...
- **Bundled Automatic Activation**: CC_BRIDGE installs and arms `codex-reconnect`
...
- **Restart Uses Current Provider Authority**: `cc-bridge restart <agent>` now rebuilds
...
- **Network Identity Stays Private**: CC_BRIDGE Mobile requests only
...
- **General Agent Variables Reach Claude**: non-API variables from
  ...
  exported to managed Claude launches. CC_BRIDGE-managed API credentials, endpoints,
...
- **Provider Failures Are Contained**: stale CC_BRIDGE-owned Claude continuation
...
- **GUI Launches Detach Cleanly**: CC_BRIDGE Rich WezTerm starts in a new process
...
- **Wayland Cursor Compatibility Is Private**: a CC_BRIDGE-owned XCursor overlay
...
- **Long Runs Are Not Arbitrarily Truncated**: Pi pane execution has no default
  ...
  Persisted 8.5.0 jobs and `CC_BRIDGE_PI_EXECUTION_MODE=headless` retain the
...
- **Native Terminal Evidence Is Exact**: Pi `0.82.1` requires its final
  ...
  shapes cannot complete a CC_BRIDGE job.
...
- **Asset Projection Is One-Way And Owned**: managed Provider assets are
  ...
  state, and packaged Qoder `ask` / `cc-bridge-clear` controls are now guaranteed.
...
- **Required CC_BRIDGE Controls Are Always Present**: packaged `ask` and `cc-bridge-clear`
...
- **Broken Optional Skills Stay Isolated**: optional skills are projected per
  entry, so a broken external skill cannot suppress CC_BRIDGE-owned controls or
...
- **Theme Selection Is Persistent**: Config UI exposes the supported CC_BRIDGE
...
- **Dense Pairing Codes Stay Scannable**: when a QR payload cannot fit safely
  in the current terminal, CC_BRIDGE writes an owner-only PNG and prints its path
...
- **Linux Release Dependencies Are Isolated**: official release installs now
  create a CC_BRIDGE-managed Python virtual environment on Linux as well as macOS.
...
- **Encrypted Relay Transport**: CC_BRIDGE Mobile can connect through an AEAD
...
- **Route Choice Lives On The Computer**: `cc-bridge update mobile` guides Tailscale,
...
- **Config Changes Apply Predictably**: provider-affecting Config UI edits are
  recorded for the next CC_BRIDGE start instead of silently mutating active panes.
...
- **Provider Upgrades Are Managed By `cc-bridge update`**: managed panes suppress
  supported provider-native startup prompts, while `cc-bridge update` detects the
...
- **Project Provider Caches Are Retired**: Claude uses the user-installed
  ...
  copying provider software into every CC_BRIDGE project.
...
- **Kill Tolerates The Expected Socket-Close Race**: if the keeper closes cc-bridge-daemon
...
- **Marker-Only Worktrees Retire Conservatively**: CC_BRIDGE removes an owned binding
...
- **Readiness Is Proven Before Mount Publication**: cc-bridge-daemon opens a bounded
...
- **Cold And Warm Starts Do Less Repeated Work**: cc-bridge-daemon reuses validated pane,
...
- **Grok Fullscreen Overrides Minimal Mode**: explicit `--fullscreen` startup
  arguments suppress CC_BRIDGE's default `--minimal`, while default and unrelated
...
- **Desktop, npm, And Android Versions Are Synchronized**: Linux and macOS
  artifacts, `@seemseam/cc-bridge`, CLI metadata, Mobile version code, APK download
...
- **Child Rollouts Cannot Capture CC_BRIDGE Requests**: managed Codex session
...
- **Parent Turn Binding Stays Immutable**: once a CC_BRIDGE request is anchored to its
...
- **Child Collaboration Output Stays Private**: native subagent activity,
  messages, and foreign-turn completion events are excluded from CC_BRIDGE reply
...
### Grok Native CC_BRIDGE Skills

- **Ask And Clear Skills Projected Per Agent**: managed Grok homes now receive
  independently owned native `ask` and `cc-bridge-clear` skills when skill
...
- **Normal Starts Use Native Maximum Permission**: managed Grok now starts with
  ...
  providers. Safe `cc-bridge -s` starts omit both bypass mode and the CC_BRIDGE skill allow
...
- **Ask Uses The Visible Native Session**: incoming CC_BRIDGE requests are delivered
...
- **Real Cross-Agent Path Verified**: a two-Grok authenticated source-runtime
  ...
  completion, named `cc-bridge clear grok2`, and post-clear isolation checks.
...
- **Settings Launch Is Observable**: the gear action resolves the CC_BRIDGE executable
...
- **Old Project Hooks Self-Heal**: managed Claude preparation removes only the
  legacy CC_BRIDGE commands that run extensionless finish/activity Bash launchers
...
- **Invalidations Replace Active Polling**: the server-wide gateway now
  ...
  conversation changes. CC_BRIDGE Mobile refreshes authoritative REST state from
...
- **pip Has A Guarded Mirror Fallback**: install-time pip commands respect
  `CC_BRIDGE_PIP_INDEX_URL` and, on macOS TLS or network failures, retry through the
  configurable TUNA PyPI fallback. The fallback can be disabled with
  `CC_BRIDGE_PIP_FALLBACK_INDEX_URL=0`; no global pip configuration or TLS bypass is
  written.
...
- **Visual Config Control Added**: the sidebar's top-left `⚙` action and
  `cc-bridge config ui` now open a loopback-only project control panel for windows,
...
- **Blank Projects Start With One Agent**: when project and user config are
  absent, CC_BRIDGE now mounts exactly one agent named `demo` and selects the first
...
- **Claude And Gemini Hook Interpreter Fixed**: managed completion and
  activity hooks execute the CC_BRIDGE bash launchers directly, allowing their
  shebang and `_cc-bridge-python` resolver to select a compatible Python runtime and
...
- **Mobile Host Health Window Extended**: `cc-bridge update mobile` now gives
...
- **Auth Projection Manifest Added**: managed homes write
  `.cc-bridge-auth-projection.json` with source/target existence, size, and SHA256
...
- **API Authority Isolation Preserved**: explicit Codex API authority removes
  ...
  not mix.
- **WSL Codex Doctor Improved**: `cc-bridge doctor` marks Codex binaries resolved to
...
- **Missing Session Diagnostics Added**: when the official Codex session/log
  cannot be found past the no-progress window, CC_BRIDGE returns attributable
  non-success diagnostics instead of silently waiting; shutdown evidence is
  reported as provider-crashed failure.
- **Mobile Frontdesk Submit Routed Through Ask**: CC_BRIDGE Mobile frontdesk messages
  now submit cc-bridge-daemon ask jobs rather than writing directly to panes.
- **Mobile Update Watch Waits For Real Terminal State**: `cc-bridge watch` no longer
  has a default 10 second timeout, so `cc-bridge update mobile` and other long
  update flows can wait for the real terminal condition. Explicit
  `CC_BRIDGE_WATCH_TIMEOUT_S` remains supported.
- **Release Surface Synchronized**: VERSION, package.json, mobile app version
  metadata, update links, README variants, and release workflow defaults are
  aligned for 8.0.17.
...
- **Mobile Terminal Reconnect Hardened**: CC_BRIDGE Mobile terminal sessions now show
...
- **Ask Project Boundary Hardened**: `ask` routing is constrained to the
  current `.cc-bridge` project by default so provider cwd drift cannot silently send
  work into another mounted project.
- **Mobile Terminal Pane Snapshots Stabilized**: CC_BRIDGE Mobile terminal streaming
...
- **Pairing QR Scanner Safety Restored**: `cc-bridge update mobile` now keeps the
  full pairing payload and scanner-safe quiet zone in both managed and direct
  mobile gateway paths.
- **Running Project Attention Strengthened**: CC_BRIDGE Mobile now uses brighter
...
- **Pairing QR Restored For Managed Mobile Update**: `cc-bridge update mobile` now prints the pairing QR when reusing the host-wide background mobile gateway instead of only printing the pairing code.
...
- **Mobile App Link Updated**: `cc-bridge update mobile`, README links, package metadata, and the mobile release manifest now point to the 8.0.13 APK.
...
- **macOS Socket Test Fixed**: mobile host registry tests now bind their
  temporary Unix sockets under a short `/tmp/cc-bridge-sock-*` path so macOS CI does
  not fail on `AF_UNIX path too long`.
- **Mobile App Link Updated**: `cc-bridge update mobile`, README links, package
...
- **CI Smoke Roots Fixed**: tag and main test workflows now pass explicit
  `CC_BRIDGE_TEST_ROOTS` for temporary dynamic-layout smoke projects, matching the
  hardened `cc-bridge_test` source-test boundary.
- **Mobile App Link Updated**: `cc-bridge update mobile`, README links, package
...
- **Mobile Update Reconnect Stabilized**: `cc-bridge update mobile` now reuses a
...
- **Lead Conversation Refresh Fixed**: CC_BRIDGE Mobile keeps the visible agent
  selected across ProjectView refreshes, so large native Codex transcripts such
  as `cc-bridge_mobile / lead` keep loading and refreshing without drifting to the
  active tmux pane.
...
- **Running Output Highlighted**: CC_BRIDGE Mobile now highlights the active terminal-derived output bubble when an agent is working, while avoiding stale historical reply markers.
...
### CC_BRIDGE Mobile Monorepo Release

- **Mobile Source Joined The Main Repository**: the Flutter CC_BRIDGE Mobile app now
  lives under `mobile/` alongside the core CLI, cc-bridge-daemon, and mobile gateway
  source.
- **Android APK Published**: the release includes
  `cc-bridge-mobile-v8.0.0.apk` plus SHA256 and manifest assets for Android Alpha
...
- **Tailnet Onboarding Unified**: `cc-bridge update mobile` guides Tailscale setup,
...
- **Runtime Accelerator Sidecar Shipped**: release artifacts now build and
  package `bin/cc-bridge-runtime-accelerator`, so the default Codex accelerator path
...
- **Global Theme Command Added**: `cc-bridge theme` now shows or changes the global
  CC_BRIDGE UI theme, including `+` and `-` cycling across dark and light palettes.
- **Light Tmux And Sidebar Themes Added**: CC_BRIDGE-owned tmux status, pane borders,
...
- **Rich WezTerm Theme Sync Added**: generated rich WezTerm profiles read the
  same CC_BRIDGE theme preference and export matching tmux/sidebar theme variables.
- **Release Surface Synchronized**: VERSION, CLI version constants,
  package.json, release workflow defaults, README release notes, and npm
...
- **Codex Temp Log Symlink Repaired**: managed Codex startup now recreates the
  `logs_2.sqlite` symlink target parent when `/tmp/cc-bridge-codex-logs-*` cleanup
  removes it between launches.
- **Bad Symlink Fallback Hardened**: if the symlink target cannot be repaired,
  CC_BRIDGE removes the broken symlink and restores the local backup before Codex
...
- **Codex Log Redirect Migration Fixed**: managed Codex `logs_2.sqlite`
  ...
  waits for `_sqlx_migrations` before installing the CC_BRIDGE diagnostic insert
  trigger.
...
- **Codex Diagnostic Fallback Hardened**: when the temporary SQLite symlink
  path cannot be installed, CC_BRIDGE falls back to the in-place diagnostic trigger
...
- **Codex Diagnostic SQLite Churn Reduced**: managed Codex homes now install a
  ...
  with `CC_BRIDGE_CODEX_DIAGNOSTIC_LOGS=1`.
- **Release Surface Synchronized**: VERSION, CLI version constants,
  package.json, release workflow defaults, README release notes, and npm
...
- **Codex Plugin Overlay Precedence Fixed**: Codex plugin overrides now apply
  ...
  then `CC_BRIDGE_CODEX_PLUGIN_OVERRIDES_JSON` / `CC_BRIDGE_CODEX_PLUGIN_OVERRIDES` env
  overrides.
...
- **Claude Hook Inheritance Fixed**: managed Claude settings now merge inherited
  source-home hooks with CC_BRIDGE-managed finish/activity hooks so user-installed
...
- **Sidebar And Trace Diagnostics Improved**: sidebar Comms/Tips panels gain
  better scrolling and resize behavior, and `cc-bridge trace` exposes reply artifact
...
- **Z.ai CLI Provider Added**: CC_BRIDGE now includes `provider = "zai"` as an
...
- **Single Current Role Store**: Role Pack runtime lookup now follows the
  ...
  runtime authority.
- **Project Role Locks Demoted**: `.cc-bridge/role-lock.json` is no longer written or
...
- **Role-Aware Restart Guard**: provider launch session files now record role
  id, version, and digest. `cc-bridge restart <agent>` fails explicitly when the
...
- **Release Metadata Patch Fixed**: release artifact builds now patch version,
  commit, and date metadata in `cc_bridge.py` after the bash launcher split.
- **Release Surface Synchronized**: VERSION, CLI version constants,
  package.json, release workflow defaults, README release notes, and npm
...
- **Rich Launcher Closure**: plain `cc-bridge` and `cc-bridge rich` now start the
  CC_BRIDGE-managed rich WezTerm launcher unless already running inside that managed
  rich session; ordinary external WezTerm sessions no longer suppress rich
  auto-start.
- **Runtime Launcher Pinning**: command entrypoints now share the `_cc-bridge-python`
  launcher so installed and source runtimes use a consistent interpreter.
- **Default Self Window Updated**: the built-in default config keeps `cc-bridge_self`
...
- **Managed Provider Role Lookup Fixed**: CC_BRIDGE now pins role store resolution
  outside managed provider homes so provider session `HOME` rewrites no longer
  make `agentroles.*` lookups fall back to `.cc-bridge/agents/.../provider-state/.../home/.roles`.
...
- **Workbench IME Environment Normalized**: generated `cc-bridge-workbench` wrappers
...
- **Doctor Temporary Detection Fixed**: doctor runtime checks include the
  resolved `tempfile.gettempdir()` root so temporary cc-bridge-daemon implementations are
...
- **Rich Layout Alias Fixed**: `.cc-bridge/cc-bridge.config` can now use `rich` as a
  ...
  routing.
- **Rich Auto-Start Added**: after `cc-bridge update rich` installs and enables the
  bundle, plain `cc-bridge` can launch the rich workbench by default outside an
  existing rich/WezTerm session while avoiding recursive WezTerm launches.
- **Rich Disable/Uninstall Added**: `cc-bridge uninstall rich`, `cc-bridge rich uninstall`,
  and `cc-bridge rich disable` remove or disable rich mode without changing the
  normal full `cc-bridge uninstall` path.
- **Legacy Editor Cleanup Hardened**: rich updates clean CC_BRIDGE-owned legacy
...
- **Yazi Bundle Download Added**: `cc-bridge update rich` now downloads CC_BRIDGE-owned
...
- **WSL WezTerm Routing Added**: rich launchers under WSL can use
  Windows-native `wezterm.exe` while keeping CC_BRIDGE, Yazi, and preview helpers
...
- **Rich Workbench Bundle Added**: rich workbench components now install and
  update through the explicit `cc-bridge update rich` entrypoint instead of ordinary
  CC_BRIDGE install/update paths.
- **Standalone Neovim Provisioning Removed**: normal `install.sh install`,
  `cc-bridge update`, and public `cc-bridge tools ... neovim` routes no longer provision a
  standalone Neovim tool. They now direct users to `cc-bridge update rich`.
- **Rich Launch Tightened**: `cc-bridge rich` launches only an installed and enabled
  rich bundle, preventing implicit provisioning from the launch path.
- **tmux Status Simplified**: CC_BRIDGE returns to a single-line tmux status bar and
...
- **Kimi Hindsight Bridge Added**: Kimi can recall and retain Hindsight memory
  at the CC_BRIDGE execution boundary when `.hindsight/kimi.json`,
...
- **Codex Hindsight Hook Inheritance Scoped**: managed Codex homes may inherit
  ...
  CC_BRIDGE-managed activity hooks and filtering unrelated root hooks.
...
- **Next-Wave Native Providers Added**: CC_BRIDGE registers optional provider ids
...
- **Native Completion Preserved**: Qwen, Cursor, Copilot, and Pi parse
  ...
  require model-printed `CC_BRIDGE_DONE`.
- **Release Surface Synchronized**: VERSION, CLI version constants,
...
- **MiMo Run-Mode Hardened**: CC_BRIDGE now runs MiMo ask jobs through
  `mimo run --pure --format json`, avoiding external plugin/tool-call
  intermediate steps that can consume simple CC_BRIDGE ask jobs before final text.
...
- **Native Completion Detection**: Kimi, DeepCode/DeepSeek, and Antigravity
  now bind on `CC_BRIDGE_REQ_ID` and complete from provider-owned session, snapshot,
  or transcript evidence instead of requiring model-printed `CC_BRIDGE_DONE`.
- **Provider Diagnostics Hardened**: Kimi `TurnEnd` empty replies,
  ...
  diagnostics.
- **Kimi Auto-Permission Compatibility**: CC_BRIDGE now injects Kimi's current
...
- **npm Release Surface Restored**: the source tree now includes the
  `@seemseam/cc-bridge` package manifest, npm CLI runner wrappers, and a
...
- **README v7 Homepage Refreshed**: public README files now use the canonical
  v7 hero assets, npm-first install wording, and stronger `cc-bridge_self`
  positioning as CC_BRIDGE's built-in usage, config, diagnostics, and recovery
  expert.
...
- **Self-Supervision Evidence Hardened**: maintenance heartbeat now uses
  ...
  envelopes, and a self-first diagnosis path for CC_BRIDGE runtime health signals.
...
- **Role Pack Refresh Tightened**: default Role Pack install and project role
  lock refresh paths now handle `agentroles.archi` and `agentroles.cc-bridge_self`
  more consistently during update/startup.

## v7.4.1 (2026-06-11)

### Maintenance Heartbeat And cc-bridge_self Defaults
...
- **Maintenance Heartbeat Runner Hardened**: adds the project-scoped
  ...
  enabled.
- **cc-bridge_self Default Alignment**: blank-project built-in config now includes
  `cc-bridge_self:codex` bound to canonical `agentroles.cc-bridge_self`, and install/update
  provisioning refreshes the recommended CC_BRIDGE self role without changing
  existing custom project configs.
- **Role Identity Compatibility Fixed**: CC_BRIDGE source now matches the
  `agent-roles-spec` catalog identity `agentroles.cc-bridge_self` and only treats
  `agentrole.cc-bridge_self` as legacy input compatibility.
- **Config Authority And Prompt Delivery Hardened**: generated config guidance
  keeps one role-binding authority, Role Pack hook paths use the correct CC_BRIDGE
  binary/project root, and Codex prompt delivery acceptance guards reduce false
  completion.
- **cc-bridge_self Expert Knowledge Docs Added**: adds the CC_BRIDGE self expert manual,
  plan-tree decisions, and test coverage for the role's expert reference and
  communication recovery guidance.

## v7.4.0 (2026-06-10)

### cc-bridge_self Maintenance Role

- **CC_BRIDGE Self Role Added**: `agentroles.cc-bridge_self` is the dedicated CC_BRIDGE
  self-maintenance role for runtime diagnosis, guarded recovery, chain repair,
  CC_BRIDGE configuration ownership, and single-agent restart assistance.
- **Private cc-bridge-config Ownership**: full `cc-bridge-config` now lives inside the
  `cc-bridge_self` Role Pack instead of being inherited by every managed agent.
- **Recommended Role Provisioning**: install/update Role Pack provisioning now
  ...
  `agentroles.cc-bridge_self`, by default.
- **Project Adoption Guidance**: README guidance now strongly recommends adding
  `agentroles.cc-bridge_self:codex` to CC_BRIDGE projects that should have a dedicated
...
- **AGY Execution Adapter Added**: Antigravity (`agy`) now has a
  ...
  polling, and supporting docs, so it can participate as a managed CC_BRIDGE
  provider without relying on the older launcher path.
- **Project Tmux History Preserved**: CC_BRIDGE-managed project tmux sessions now
...
- **Provider Memory Ownership Policy Added**: Claude, Codex, and OpenCode
  managed contexts now exclude provider-native project memory from the CC_BRIDGE
...
- **Provider User Memory Filtered**: provider user memory is filtered only at
  the provider-user-memory source layer, removing legacy CC_BRIDGE install marker
  blocks and old collaboration sections without rewriting user-owned files.
- **Shared Memory Template Simplified**: the default `.cc-bridge/cc-bridge_memory.md`
  template is updated to v5 and no longer repeats the Ask Communication block
  already supplied through CC_BRIDGE managed agent memory.
- **Seed-Aware Shared Memory Migration Added**: CC_BRIDGE upgrades only old,
...
- **Claude Route-Mode Install Hardened**: route-mode installs no longer write
  `~/.claude/rules/cc-bridge-config.md`. Install/uninstall remove only CC_BRIDGE-marked
...
- **Source Runtime Tmux UI Import Cycle Fixed**: tmux UI version detection no
  ...
  startup paths import-safe under `cc-bridge_test`.

## v7.3.5 (2026-06-07)

### Tmux Border Hook Hotfix

- **Release Path Hook Fixed**: `config/cc-bridge-tmux-on.sh` now resolves stable
  ...
  from storing temporary `/tmp/cc-bridge-v...-release.../config/cc-bridge-border.sh`
  paths.
- **Border Hook Guarded**: Python runtime hook generation now writes
  ...
  `cc-bridge-border.sh ... returned 127` while switching panes.
- **Post-Update Tmux UI Refresh Added**: `cc-bridge update` now best-effort refreshes
...
- v7.3.4 was withdrawn because its tmux border hook could persist temporary
  release paths such as `/tmp/cc-bridge-v...-release.../config/cc-bridge-border.sh` into
...
- **Archi Role Tooling Simplified**: `agentroles.archi` now uses the global
  `@seemseam/archi` npm package as the single Architec tool source. CC_BRIDGE no
...
- **Archi Doctor Aligned With Bundled Capabilities**: `cc-bridge roles install`,
  `cc-bridge roles update`, and `cc-bridge roles doctor agentroles.archi` now report and
  validate the npm package, the `archi` CLI, and its bundled Hippo/llmgateway
  capabilities instead of CC_BRIDGE-managed Python tool internals.
- **Legacy `cc-bridge-arch` Forwarder Updated**: `bin/cc-bridge-arch` forwards directly to
...
- **Guarded Source Test Entrypoint Added**: `cc-bridge_test` provides a guarded
  source-checkout entrypoint for isolated development validation without
  modifying or shadowing the installed CC_BRIDGE runtime.
- **OpenCode Autoupdate Disabled In Managed Panes**: generated `opencode.json`
  ...
  `OPENCODE_DISABLE_AUTOUPDATE=true` so CC_BRIDGE-owned panes do not self-update
  under the workspace runtime.
- **Config Skill Refined**: inherited `cc-bridge-config` skills now support
...
- **Blank Install Role Pack Provisioning Fixed**: release and source installs
  ...
  `cc-bridge roles update agentroles.archi` reports that the role is not installed.
  The installer falls back to `cc-bridge roles install agentroles.archi`, allowing
  the first install to provision `.roles/installed` and the `cc-bridge-archi`
...
- **Agent Roles Store Flow Simplified**: CC_BRIDGE now uses the external Agent Roles
  manager as the only Role Pack writer and reads installed roles only from
  `.roles/installed`. The old CC_BRIDGE-owned writer and
  `CC_BRIDGE_AGENT_ROLES_MANAGER` rollback switch were removed.
- **Legacy Role Store Is Migration-Only**: existing `$XDG_DATA_HOME/cc-bridge/roles`
...
- **Role Add Aligned With Agent Roles**: `cc-bridge roles add` now auto-installs
  missing system-source roles through `agent-roles --path`, preserving
  `cc-bridge.archi` compatibility while writing canonical `agentroles.archi`
  bindings and locks.
- **Ask Artifact Transport Added**: `ask` and `cc-bridge ask` now support
  `--artifact-request`, `--artifact-reply`, and `--artifact-io` so large or
  explicitly artifact-backed request/reply bodies can be stored as CC_BRIDGE text
...
- **OpenCode Clear Timing Fixed**: `cc-bridge clear` adds an OpenCode-only submit
...
- **Shared Workspace Controls Added**: `.cc-bridge/cc-bridge.config` supports
...
- **Provider Command Templates Added**: agent config supports
  `provider_command_template`, where `{command}` expands to CC_BRIDGE's normal
...
- **Config Skill Updated**: inherited `cc-bridge-config` skills now default to a
...
- **Source Development Isolation Preserved**: project memory and release
  guidance keep source checkout changes isolated from the system-installed CC_BRIDGE;
  development runtime testing should use `cc-bridge_test` from external test projects
  such as `test_ccb2`.
...
- **Legacy Store Migration Added**: existing `$XDG_DATA_HOME/cc-bridge/roles` installed snapshots are copied into `.roles/installed` at Role Pack management boundaries without deleting the old store, preserving existing project lock digest resolution.
- **Path Update Aligned**: `cc-bridge roles update --path ...` now also routes through the Agent Roles manager and writes `.roles/installed` instead of the legacy CC_BRIDGE store.
- **Sync Validation Hardened**: malformed `agent-roles sync --json` role rows now fail closed, while `cc-bridge roles sync --with-tools` composes manager-owned payload sync with CC_BRIDGE-owned tool hook execution.
- **Role Config Guidance Updated**: inherited `cc-bridge-config` skill docs now describe `.roles/installed` as the default package store while keeping `.cc-bridge/cc-bridge.config` limited to canonical role ids.

## v7.2.11 (2026-06-04)

### Superseded Agent Roles Opt-In Preview

- Superseded by v7.2.12 after the release direction changed from an opt-in `CC_BRIDGE_AGENT_ROLES_MANAGER=1` preview to a default-on Agent Roles manager migration. Do not use v7.2.11 as the recommended release.

## v7.2.10 (2026-06-04)

### Role Pack Post-Update Hotfix

- **Post-Update Handoff Fixed**: `cc-bridge update` now installs the new release, smoke-checks the installed entrypoint, and delegates Role Pack plus Neovim provisioning to the newly installed `cc-bridge __post-update` command instead of continuing with the old updater process.
- **Legacy Role Store Repair Added**: installed `cc-bridge.archi` metadata is repaired under canonical `agentroles.archi`, and stale removed source paths fall back to the current catalog role source.
- **Forced Provisioning Failure Propagates**: `CC_BRIDGE_INSTALL_ROLES=1`, `CC_BRIDGE_INSTALL_NEOVIM=1`, and `CC_BRIDGE_POST_UPDATE_REQUIRED=1` now make post-update subprocess failures fail the parent update, while optional post-update provisioning remains a warning after the core update succeeds.
- **Role Config Guidance Aligned**: inherited `cc-bridge-config` skill docs now generate `agentroles.archi` bindings and mention `cc-bridge.archi` only as a legacy migration alias.

## v7.2.9 (2026-06-04)

### Agent Roles Catalog Release

- **Agent Roles Catalog Added**: CC_BRIDGE now consumes `agentroles.archi` from the external `agent-roles-spec` catalog instead of shipping production role content inside the CC_BRIDGE source tree.
- **Catalog Role Lifecycle Hardened**: `cc-bridge roles list/install/update/sync/add/doctor` now works with catalog roles, installed-role metadata, project locks, digest pinning, and explicit re-add semantics.
- **Runtime Projection Preserved**: role memory, adapter memory, provider skills, and CC_BRIDGE adapter tool hooks project into managed Codex/Claude homes for `agentroles.archi`.
- **Update Flow Improved**: `install.sh install` and `cc-bridge update` handle catalog role refreshes, report newly available roles, and keep non-interactive follow-up commands explicit.
- **Compatibility Alias Kept**: legacy `cc-bridge.archi` inputs resolve to `agentroles.archi` while project config and locks use the canonical catalog role id.
- **Source Runtime Guard Fixed**: source checkout commands that pass `--project` now validate the target project against allowed test roots, restoring CC_BRIDGE_DAEMON communication smoke checks launched from the source checkout.
- **Official Smoke Roots Fixed**: real-platform soak, fastpath, and storage cleanup smoke checks now pass their generated test roots through `CC_BRIDGE_SOURCE_ALLOWED_ROOTS`.
- **WSL Mounted Startup Smoke Fixed**: the main Tests workflow now passes the generated `/mnt/c/Temp` startup-smoke project through `CC_BRIDGE_SOURCE_ALLOWED_ROOTS`.
...
- Superseded by v7.2.7 after the release gate found that the WSL mounted startup smoke in the main Tests workflow also needed its generated `/mnt/c/Temp` project in `CC_BRIDGE_SOURCE_ALLOWED_ROOTS`.

## v7.2.5 (2026-06-04)

### Superseded Source Runtime Guard Hotfix

- Superseded by v7.2.6 after the release gate found that official soak, fastpath, and storage cleanup smoke checks needed explicit generated test roots in `CC_BRIDGE_SOURCE_ALLOWED_ROOTS`.

## v7.2.4 (2026-06-04)

### Superseded Agent Roles Catalog Release

- Superseded by v7.2.5 after the release gate found that source checkout `--project` commands were rejected from the source cwd during CC_BRIDGE_DAEMON real platform smoke checks.

## v7.2.3 (2026-06-03)

### Root Install Support Validation Hotfix

- **Root Install Support Preserved**: keeps the v7.2.2 root install confirmation gate, install identity metadata, and `cc-bridge doctor` runtime identity diagnostics intact.
...
- **Root Install Confirmation Added**: `install.sh install` now refuses root execution by default, requires interactive `yes` confirmation, and requires `CC_BRIDGE_ALLOW_ROOT_INSTALL=1` for non-interactive root installs.
...
- **Install Identity Metadata Added**: install metadata now records root status, install user, and sudo user details so future diagnostics can explain how CC_BRIDGE was installed.
- **Doctor Runtime Identity Diagnostics Added**: `cc-bridge doctor` reports runtime user, owner, root state, and warns when root runs inside a non-root project.
...
- **Role Pack UX Added**: `cc-bridge roles add cc-bridge.archi:codex` is now the primary project entry point; configs keep the shorthand `cc-bridge.archi:codex`, while runtime resolves it to the local `archi` agent and projects role memory plus provider skills.
- **Bundled `cc-bridge.archi` Role Added**: the first built-in Role Pack provides an Architec-backed architecture reviewer with role memory, Codex/Claude skills, and CC_BRIDGE-managed Architec tooling.
- **Role Dependency Flow Simplified**: `cc-bridge roles install cc-bridge.archi` and `cc-bridge roles update cc-bridge.archi` install or refresh both role assets and dependencies by default; `install.sh install` and `cc-bridge update` prompt interactive users and show `cc-bridge roles update cc-bridge.archi` as the non-interactive follow-up.
- **Managed Tool Windows Added**: CC_BRIDGE config now supports `[tool_windows.<name>]` for non-agent windows such as the bundled `neovim` tool, including sidebar/project-view rows, reload add/remove support, and `cc-bridge tools install/doctor neovim` provisioning.
...
- **CC_BRIDGE Config Skill Updated**: inherited Codex/Claude `cc-bridge-config` source docs and references now expose all three sidebar view height parameters when generating or migrating windows topology.

## v7.1.0 (2026-05-30)

### Dynamic Reload Release

- **Config Reload Preview Added**: after editing `.cc-bridge/cc-bridge.config`, use `cc-bridge reload --dry-run` to preview the daemon-side plan before changing tmux state.
- **Dynamic Reload Apply Added**: `cc-bridge reload` can dynamically add agents, add windows, unload idle agents, and remove idle windows under the existing `cc-bridge-daemon` daemon without restarting unrelated panes.
...
- **Provider Activity Hooks Added**: CC_BRIDGE now records provider-native activity evidence from hook artifacts so the sidebar can distinguish active, pending, idle, and failed provider work more accurately than pane text alone.
...
- **Sidebar Controls Preserved**: the top-right sidebar controls remain `↻` and `×`; `×` performs project-level `cc-bridge kill`, while `q` and `Esc` exit only the sidebar pane.
- **Tmux Vim Controls Documented And Applied**: CC_BRIDGE-managed tmux keeps `mode-keys vi`, copy-mode `v`/`C-v`/`y`, `prefix+h/j/k/l` pane focus, and `prefix+H/J/K/L` pane resizing.
...
- **Agent Context Clear Command Added**: `cc-bridge clear [agent...]` sends provider-native `/clear` to all or selected mounted agent panes without deleting project state or restarting runtimes, with `all` and unknown-agent validation handled through `cc-bridge-daemon`.
...
- **CC_BRIDGE Clear Skill Added**: Claude and Codex inherited skills now include `cc-bridge-clear`, installers project the skill, and Claude managed settings allow `Bash(cc-bridge clear *)`.
...
- **Pane Restart And Click Routing Added**: `project_restart_panes` and hidden `cc-bridge __sidebar-click` paths now let sidebar actions refresh project panes and restore focus through `cc-bridge-daemon`.
- **Sidebar Width Sync Hardened**: sidebar widths now accept integer column sizes, drag-resize uses `resize-pane -M`, `after-resize-pane` syncs widths across windows, and the global `window-resized` hook reapplies the stored width for the active CC_BRIDGE session instead of learning tmux's temporary compression.

## v7.0.6 (2026-05-24)

### macOS Release Test Smoke Hotfix

- **macOS Release Install Smoke Fixed**: GitHub Tests now prebuild a host-runnable `bin/cc-bridge-agent-sidebar` before simulating a release install from a source checkout, matching the packaged release shape.
- **v7.0.5 Hotfixes Preserved**: this release includes the Claude keychain service override, macOS `cc-bridge update` sidebar helper preservation, and explicit sidebar rebuild failure handling from v7.0.5.

## v7.0.5 (2026-05-24)

### Claude Keychain And macOS Update Hotfix

- **Claude Keychain Override Added**: `CC_BRIDGE_KEYCHAIN_SERVICE_OVERRIDE` can bind managed Claude materialization to a specific macOS Keychain service, and control-plane environment handling preserves the override.
- **macOS Update Preserves Sidebar Helper**: `cc-bridge update` staging now skips line-ending normalization for binary files, so `bin/cc-bridge-agent-sidebar` is not corrupted during macOS updates.
...
- **Inherited Skill Install Cleanup Improved**: `cc-bridge-config` is now the canonical inherited skill name, legacy `cc-bridge_config` and obsolete helper skills are removed silently during install, and useful tool packages were refreshed.

## v7.0.3 (2026-05-23)

### macOS Sidebar Universal Binary Hotfix

- **macOS Sidebar Runs Natively**: the macOS release artifact now builds `cc-bridge-agent-sidebar` for both `x86_64-apple-darwin` and `aarch64-apple-darwin`, then combines them with `lipo` into the shipped `bin/cc-bridge-agent-sidebar` universal binary.
...
- **Sidebar Install Recovery Hardened**: installers smoke-test existing and prebuilt `cc-bridge-agent-sidebar` binaries before trusting them, rebuild locally with cargo when needed, and source wrappers now resolve symlinks before locating the repo target binary.
...
- **macOS Checksum Portability Fixed**: `bin/package-cc-bridge-agent-sidebar-release` now writes sidebar artifact SHA256 files with `sha256sum`, macOS `shasum -a 256`, or a `python3` fallback, restoring macOS GitHub Tests for the v7 release line.

## v7.0.0 (2026-05-23)

### Native Sidebar Control Release

- **Native CC_BRIDGE Sidebar Added**: adds the Rust `cc-bridge-agent-sidebar` helper with per-window project view, fixed gray sidebar identity, colored provider/runtime activity status, mouse/keyboard focus switching, and release packaging hooks.
- **Comms Tracking Split From Agent Activity**: top agent rows now reflect real provider pane/runtime activity, while the bottom Comms section remains the CC_BRIDGE ask/job tracking and recovery surface.
- **Window Topology Config Added**: `cc-bridge_config` docs and skills now cover `version = 2` `[windows]` syntax; explicit windows mount multiple named tmux windows with sidebar panes.
...
- **Long CC_BRIDGE Text Artifacts Added**: oversized ask bodies, terminal replies, notices, and callback continuation text spill to bounded UTF-8 artifacts under `.cc-bridge/cc-bridge-daemon/artifacts/text/`, with previews and diagnostics bundle coverage.
...
- **Config Source Fixes Included**: the current release package includes explicit config source kinds for built-in defaults, user `~/.cc-bridge/cc-bridge.config`, and project `.cc-bridge/cc-bridge.config`, with project config taking highest priority.
- **Kill Cleanup Ordering Included**: `stop_all` defers project tmux namespace destruction until after the socket response finalizer so `cc-bridge kill` and `cc-bridge kill -f` can complete cleanup from inside a CC_BRIDGE pane.
- **Managed Tmux Policy Follow-Up Added**: isolated managed tmux sessions now explicitly enable CC_BRIDGE-owned `mouse on` and `set-clipboard on` policy in project namespaces and detached tmux paths.

## v6.2.7 (2026-05-21)

### Config Source And Stop Cleanup Release

- **Three-Layer Config Sources Added**: config resolution now reports explicit source kinds for built-in defaults, user config at `~/.cc-bridge/cc-bridge.config`, and project config at `.cc-bridge/cc-bridge.config`, with project config taking highest priority.
- **Config Validate Output Clarified**: `cc-bridge config validate` now surfaces `config_source_kind` and `used_builtin_default`, and README/docs/`cc-bridge_config` skill guidance describe the active config layer instead of assuming only project config.
- **Kill Cleanup Ordering Fixed**: `stop_all` now defers project tmux namespace destruction until after the socket response finalizer, so `cc-bridge kill` and `cc-bridge kill -f` launched from a CC_BRIDGE tmux pane can finish daemon cleanup before their pane is destroyed.

## v6.2.6 (2026-05-20)

### Tmux Isolation And Startup Hardening Release

- **CC_BRIDGE Tmux Uses Isolated Config By Default**: managed tmux commands now run with `tmux -f /dev/null ...`, with `CC_BRIDGE_TMUX_CONFIG` available for explicit managed overrides, so user `~/.tmux.conf` plugins and hooks cannot alter CC_BRIDGE pane topology.
- **Source Install Startup Path Hardened**: source/dev installs now use a Python wrapper, honor `CC_BRIDGE_PYTHON_BIN`, run post-install entrypoint smoke checks, and keep Droid MCP registration bounded by a timeout.
...
- **Managed Memory Sources Preserved**: provider user memory from real `~/.claude/CLAUDE.md`, `.cc-bridge/cc-bridge_memory.md`, and per-agent `.cc-bridge/agents/<agent>/memory.md` still project into the managed Claude home.
...
- **Installer TOML Parser Dependency Added**: Linux/macOS and Windows installers now auto-install `tomli>=2.0.0` when no `tomllib`/`tomli`/`toml` reader is available, support `CC_BRIDGE_INSTALL_TOMLI=0`, and install `tomli` inside the managed venv before optional watchdog.
...
### Inherited CC_BRIDGE Config Skill Release

- **Inherited `cc-bridge_config` Skill Added**: Claude and Codex installs now inherit a `cc-bridge_config` skill for designing `.cc-bridge/cc-bridge.config`, choosing roles/providers/worktree layout, and updating shared plus per-agent CC_BRIDGE memory.
- **Inherited Skill Layout Consolidated**: CC_BRIDGE-owned inherited skills now live under `inherit_skills/`; optional `useful_tools/` remain user-installable tools and are not inherited by default.
...
- **Config Memory Routing Clarified**: `cc-bridge_config` memory patterns prefer direct owner-to-next-owner callback handoffs and separate root work packages for parallel chains, without claiming single-task multi-callback fan-in.

## v6.2.0 (2026-05-17)

### Callback Ask Chain Release

- **Callback Ask Chains Added**: `cc-bridge ask --chain <agent>` lets an active agent delegate work whose result is needed before finishing the original task; CC_BRIDGE resumes the parent as a continuation task when the child reply is ready.
- **Nested Ask Guardrails Enforced**: plain nested `ask` from an active CC_BRIDGE task is rejected; use `--chain` for needed child results or `--silence` for independent no-result-needed work.
...
- **Forced Kill Finalization Survives Client Disconnects**: `cc-bridge kill -f` now still queues daemon finalization when the requesting pane disappears before the socket response is written.
- **Project-Scoped Kill Cleanup Hardened**: kill cleanup preserves full tmux socket paths, reads lifecycle owner/keeper PID authority, and scopes process fallback matching to CC_BRIDGE control-plane commands for the same project.
- **Stale Execution Residue Cleared**: cc-bridge-daemon startup and late provider updates now clear execution files for cancelled, completed, or missing jobs so `doctor` no longer reports stale work as active or recoverable authority.
...
- **Managed Claude Follows Source Active Version**: when the source home uses the standard Claude Code layout, CC_BRIDGE now detects `~/.local/bin/claude -> ~/.local/share/claude/versions/<version>/claude` and makes managed Claude use that active version.
- **Active Version Is Cached Safely**: the selected source-home Claude version is copied into the CC_BRIDGE provider cache, then managed `.local/bin/claude` points at the cached copy instead of selecting another version already present in shared cache.
...
- **Managed Ask Skills Project Across Providers**: Claude inherited `skills/` and `commands/` now use CC_BRIDGE projected assets instead of copy-sync, so managed agents inherit system-installed ask skills without duplicating user provider homes.
...
- **Ask Replies Are Guided By Default**: `cc-bridge ask` now injects concise reply guidance, adds `--compact` for distilled answers and `--silence` for silent-on-success asks.

## v6.1.18 (2026-05-15)

### Heartbeat Timeout And Useful Tools Release

- **Heartbeat Timeout Now Terminalizes Stalled Jobs**: running-job heartbeat observations stay internal until three no-progress intervals, then CC_BRIDGE emits one terminal `heartbeat_timeout` reply that recommends sending a small communication test before another large task.
...
- **Claude Completion Request Binding Fixed**: Claude Stop hooks now resolve the current outer `CC_BRIDGE_REQ_ID` from structured transcript/user prompt records, so forwarded text or tool output containing older request ids cannot write completion events to the wrong job.
- **Codex Session Resume Stabilized**: Codex memory projection fingerprint changes are now diagnostic freshness metadata, not conversation identity; restart no longer archives or resets the Codex session just because `.cc-bridge/cc-bridge_memory.md` changed.
...
- **Ask Handoff Guidance Stabilized**: generated managed-memory bundles now include CC_BRIDGE-owned submit-only ask coordination rules, so stale `.cc-bridge/cc-bridge_memory.md` text cannot reintroduce polling/waiting behavior after restart.
- **Project Memory Template Tightened**: new `.cc-bridge/cc-bridge_memory.md` files now describe `/ask` as a fire-and-forget handoff and avoid obsolete `cc-bridge -h` guidance in the managed memory seed.
- **Claude ccswitch Route Inheritance Fixed**: managed Claude startup now prefers the source-home `~/.claude/settings.json` `ANTHROPIC_BASE_URL` over a stale caller-shell `ANTHROPIC_BASE_URL`, so ccswitch route changes take effect after `cc-bridge kill && cc-bridge`.
...
- **Remote Kill Fully Stops cc-bridge-daemon**: `cc-bridge kill` now snapshots the active `cc-bridge-daemon` and keeper pids before remote `stop_all`, then waits for those exact control-plane processes to exit instead of trusting `phase=unmounted` alone.
- **Cleanup Works Immediately After Kill**: remote kill now finalizes lifecycle state to `phase=unmounted` / `desired_state=stopped`, so `cc-bridge cleanup` can run right after `cc-bridge kill` without requiring a second kill.
...
- **Storage Diagnostics Hardened**: `cc-bridge doctor storage` now classifies the managed Claude `Library/Keychains` symlink as secret auth state instead of unknown out-of-bounds residue
...
- **Claude Cleanup Rollback Fix Included**: keeps the active Claude Code version plus one rollback version during `cc-bridge cleanup`

## v6.1.10 (2026-05-13)

### Claude Cleanup Rollback Hotfix

- **Claude Rollback Cache Preserved**: `cc-bridge cleanup` now keeps the active Claude Code version plus one rollback version while pruning older rebuildable version-cache entries
...
- **Cleanup Reclaims Runtime Residue**: `cc-bridge cleanup` now prunes old Claude shared versions, Gemini shared cache content, rebuildable Claude caches, and stale pane crash logs while preserving session/auth authority
- **Kill Shutdown Reliability Hardened**: `cc-bridge kill` now snapshots old `cc-bridge-daemon`/keeper pids before shutdown, waits for them to really exit, treats Linux zombies as dead, and avoids killing a newer backend generation
...
- **Codex Shared Memory Refresh Fixed**: Codex startup now records the managed `AGENTS.md` memory projection fingerprint and skips stale `resume` bindings when `.cc-bridge/cc-bridge_memory.md` changes
...
- **Start/Maintenance Race Fixed**: cc-bridge-daemon now prevents heartbeat maintenance from mutating project tmux panes while a start request is laying out and launching agents
- **Project Memory Anchor Tightened**: CC_BRIDGE no longer creates, imports, or depends on project-root `CC_BRIDGE.md`; `.cc-bridge/cc-bridge_memory.md` is the only shared CC_BRIDGE memory anchor
...
- **Shared Project Memory Landed**: `.cc-bridge/cc-bridge_memory.md` is now the shared project memory anchor injected into managed Claude, Codex, Gemini, and OpenCode agents during startup
- **Per-Agent Memory Layer Added**: `.cc-bridge/agents/<agent>/memory.md` now participates as an agent-private overlay on top of the shared project file
...
- **Storage Audit Expanded**: `cc-bridge doctor storage` now reports explicit storage classes for provider authority, sessions, secrets, workspaces, user content, projected config, rebuildable cache, and startup authority bundles
- **Safe Cleanup Added**: `cc-bridge cleanup` now holds the project lifecycle lock, refuses active `cc-bridge-daemon` or pending ask jobs, prunes old Claude version caches conservatively, and removes only safe Gemini rebuildable caches
...
### CC_BRIDGE_DAEMON Ask Stability And Observer Convergence

- **Ask Submit Fastpath Stabilized**: `cc-bridge ask` now returns bounded receipts without waiting on provider readiness, mailbox history projection, or long maintenance ticks; real Linux fastpath stress validated 60 queued asks with p95 submit latency under 250ms
...
- **Real Platform Validation Added**: new GitHub Actions coverage runs macOS and WSL cc-bridge-daemon/ask smoke tests, communication matrix, short soak, and fastpath stress with stub providers; Linux local validation covered full pytest, comm matrix, soak, and fastpath stress

## v6.0.29 (2026-05-07)

### WSL Runtime State Relocation

- **Runtime State Moved Off Mounted Drives**: on WSL projects rooted under `/mnt/<drive>/...`, project authority remains in `.cc-bridge` while `cc-bridge-daemon/` and agent runtime state relocate to a local Linux state root with explicit marker files
- **Diagnostics and Bundle Mapping Updated**: doctor output and support bundles now expose the project anchor, runtime-state root, relocation reason, and logical `.cc-bridge` archive paths for relocated runtime files
...
- **Socket Server Accept Path Decoupled**: cc-bridge-daemon now accepts connections separately from a serialized worker lane, so one slow or incomplete client request no longer blocks new control-plane probes or heartbeats
...
- **Foreground Attach Timeout Split**: interactive `cc-bridge` startup now uses foreground-attach-specific RPC and target-ready budgets instead of reusing the short daemon probe timeout
- **macOS Attach Race Reduced**: foreground attach now tolerates slower post-start `cc-bridge-daemon` ping and tmux namespace/window visibility on macOS without redefining daemon startup success
...
- **Managed Isolation Preserved**: transport inheritance is centralized and does not allow caller-global `CODEX_HOME`, `GEMINI_ROOT`, `CLAUDE_PROJECTS_ROOT`, or `CC_BRIDGE_CALLER_*` runtime authority to override agent-scoped managed state
...
- **Opencode Session Detection Hardened**: opencode now treats env-session mode as active only when its provider-specific runtime env is present, avoiding stale generic `CC_BRIDGE_SESSION_ID` contamination
...
- **macOS Socket Test Race Fixed**: cc-bridge-daemon socket tests now wait for the daemon socket to answer ping requests before issuing RPCs, avoiding macOS runner readiness races
...
- **Default Config Startup Fixed**: keeper startup now treats a missing `.cc-bridge/cc-bridge.config` as a request to use the built-in default project config instead of exiting before `cc-bridge-daemon` can mount
...
- **Claude Official Login Source Home Fixed**: managed Claude startup now treats `.cc-bridge/agents/*/provider-state/*/home` as an isolated runtime home, not the user's source home, so official browser-login credentials are copied from the real account home
...
- **Claude Official Login Projection**: managed Claude homes now project Claude Code official login credentials from `.claude/.credentials.json`, so browser-login-backed auth can be inherited into isolated CC_BRIDGE runtimes instead of only API-token-based settings auth
...
- **Foreground Attach UX Hardening**: interactive `cc-bridge` startup now seeds tmux namespace creation from the real terminal viewport and issues a best-effort client refresh after attach so first paint matches the current terminal size without manual redraw
...
- **Source Dev Install Mode**: installs from a git checkout now stay linked to the live source tree, skip startup auto-update prompts, and can switch to a managed release install through `cc-bridge update`
- **Agent API / Model Shortcuts**: `.cc-bridge/cc-bridge.config` now accepts flat per-agent `key`, `url`, and `model` shortcuts so common provider overrides stay concise
...
- **Cached Startup Update Prompt**: interactive foreground `cc-bridge` start can now read install-scoped cached release metadata and offer an upgrade prompt only when a newer stable release is already known locally
...
- **Project Lifecycle Regression Coverage**: added backend/state regression tests for the absent-server cold-start path so real `cc-bridge -> ping -> kill` lifecycle flows remain covered
...
- **Mounted-State Read Path Fixes**: `ping cc-bridge-daemon` and `ping <agent>` now read mounted/runtime state from current authority instead of drifting to stale failure views after restart or recovery
- **Shutdown Transaction Hardening**: `cc-bridge kill` and `cc-bridge kill -f` now terminalize non-terminal jobs inside the same shutdown transaction so in-flight work cannot reappear as restore or auto-retry authority after restart
...
- **Foreground Kill Lifecycle Fix**: `cc-bridge kill` no longer leaves interactive `cc-bridge` reporting a false foreground-attach failure after the project tmux namespace is intentionally destroyed
...
- **Provider Control-Plane Isolation Tightened**: project-scoped control-plane processes now scrub inherited provider runtime markers more strictly so agent runtime identity does not leak into `cc-bridge`, keeper, or `cc-bridge-daemon`
...
- **Agent Isolation Stability**: strengthened managed agent isolation so Codex, Claude, and Gemini agent sessions stay bound to their own project-scoped provider state under `.cc-bridge`
...
- **Release Tarball Upgrade Fix**: `cc-bridge update` now resolves the extracted release directory correctly instead of treating the `.tar.gz` filename as a directory path
...
- **Release Build Hygiene**: Linux release packaging now ignores local `.cc-bridge-requests/` mailbox residue so official builds are not blocked by runtime leftovers

## v6.0.2 (2026-04-17)

### 🔁 Agent Routing & Install Guardrails

- **Caller Attribution Fix**: `cc-bridge ask` now preserves the originating agent identity so replies route back to the correct mailbox instead of drifting to `user` or `cmd`
...
- **Infinite Parallel Agent Edition**: CC_BRIDGE v6 establishes the runtime foundation for effectively unbounded multi-agent delegation inside one project
...
- **User Workflow Reduced**: Public startup and rebuild flow is now intentionally centered on `cc-bridge`, `cc-bridge -s`, `cc-bridge -n`, `cc-bridge kill`, and `cc-bridge kill -f`
- **Control Plane Retained**: `ask`, `ping`, `pend`, and `watch` remain available for model-side coordination without dominating user help
- **Safe Rebuild Semantics**: Legacy project runtime state is rebuilt from `.cc-bridge/cc-bridge.config`, while current 6.x projects retain an explicit runtime marker

### 🌳 Workspace & Recovery

- **Default Inplace Workspaces**: Agents now default to `inplace`; isolated branches are opt-in via `agent:provider(worktree)`
- **Worktree Reconciliation**: Added stable handling for added, removed, renamed, dirty, missing, and unmerged worktree agents during start, kill, and `cc-bridge -n`
...
- **Linux Release Path**: `cc-bridge update` for the 6.x line is now aligned to Linux/WSL release assets instead of source snapshots
...
- **Public CLI Simplified**: User-facing startup flow is now centered on `cc-bridge`, `cc-bridge -s`, `cc-bridge -n`, `cc-bridge kill`, and `cc-bridge kill -f`
- **Explicit Worktree Opt-In**: Compact `cc-bridge.config` entries now default to `workspace_mode='inplace'`; isolated branches require `agent:provider(worktree)`
...
- **Reset Rebuilds Cleanly**: `cc-bridge -n` rebuilds project runtime state while preserving `.cc-bridge/cc-bridge.config`
...
- **Kill Warnings**: `cc-bridge kill` now warns clearly when project worktree agents still have unmerged or dirty state that needs user attention
...
- **askd Startup Guardrails**: `bin/ask` now respects `CC_BRIDGE_ASKD_AUTOSTART=0` and scrubs inherited daemon lifecycle env before spawning askd
- **Claude Session Backfill**: `cc-bridge` startup again backfills `work_dir` and `work_dir_norm` into existing `.claude-session` files
...
- **Async Guardrail**: Added global mandatory turn-stop rule to `claude-md-cc-bridge.md` to prevent Claude from polling after async `ask` submission
- **Marker Consistency**: `bin/ask` now emits `[CC_BRIDGE_ASYNC_SUBMITTED provider=xxx]` matching all other provider scripts
- **SKILL.md DRY**: Ask skill rules reference global guardrail with local fallback, eliminating duplicate maintenance
- **Command References**: Fixed `/ping` → `/cping` and `ping` → `cc-bridge-ping` in docs

## v5.2.4 (2026-02-11)

### 🔧 Bug Fixes

- **Explicit CC_BRIDGE_CALLER**: `bin/ask` no longer defaults to `"claude"` when `CC_BRIDGE_CALLER` is unset; exits with an error instead
- **SKILL.md template**: Ask skill execution template now explicitly passes `CC_BRIDGE_CALLER=claude`

## v5.2.3 (2026-02-09)

### 🚀 Project-Local History + Legacy Compatibility

- **Local History**: Context exports now save to `./.cc-bridge/history/` per project
- **CWD Scope**: Auto transfer runs only for the current working directory
- **Legacy Migration**: Auto-detect `.cc-bridge_config` and upgrade to `.cc-bridge` when possible
...
- **Auto Context Export**: Previous Claude session is extracted to `./.cc-bridge/history/claude-<timestamp>-<old_id>.md`
...
- **askd Lifecycle**: askd is bound to CC_BRIDGE lifecycle to avoid stale daemons
- **Mounted Detection**: `cc-bridge-mounted` now uses ping-based detection across all platforms
- **State File Lookup**: `askd_client` falls back to `CC_BRIDGE_RUN_DIR` for daemon state files
...
- Environment overrides: `CC_BRIDGE_ASK_FOREGROUND=1` / `CC_BRIDGE_ASK_BACKGROUND=1`
- Foreground runs sync and suppresses completion hook unless `CC_BRIDGE_COMPLETION_HOOK_ENABLED` is set
- `CC_BRIDGE_CALLER` now defaults to `codex` in Codex sessions when unset
...
| `cping`, `gping`, `oping`, `dping`, `lping` | `cc-bridge-ping <provider>` (skill: `/cping`) |
