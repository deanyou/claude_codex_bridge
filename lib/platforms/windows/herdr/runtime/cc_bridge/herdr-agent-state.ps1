# Herdr agent-state hook for CC_BRIDGE (Claude Code Bridge)
# Place this file at: C:\Users\Administrator\.cc-bridge\hooks\herdr-agent-state.ps1
#
# Herdr calls this hook to discover CC_BRIDGE-managed agents in the current
# project.  The hook queries cc-bridge-daemon via its control-plane socket and emits
# JSON with one agent entry per CC_BRIDGE-managed pane.
#
...
# Locate the cc-bridge-daemon control-plane token and socket.
# The state root follows CC_BRIDGE_RUNTIME_STATE_HOME / project_id / cc-bridge-daemon.
$cc-bridgeDir = Join-Path $ProjectRoot '.cc-bridge'
$refPath = Join-Path $cc-bridgeDir 'runtime-root-ref.json'
...
$cc-bridge-daemonDir = Join-Path $stateRoot 'cc-bridge-daemon'
$endpointPath = Join-Path $cc-bridge-daemonDir 'control-plane-endpoint.json'
...
# Query cc-bridge-daemon for agent state via TCP control plane.
$agents = @()
$agentIndexByName = @{}
try {
    ...
    $auth = (@{ schema = "cc-bridge-daemon-control-plane-token-v1"; token = $authToken } | ConvertTo-Json -Compress) + "`n"
    $writer.Write($auth)
    $writer.Flush()
    $ack = $reader.ReadLine()
    if (-not $ack) {
        throw "cc-bridge-daemon auth ack missing"
    }
    ...
    # cc-bridge-daemon not reachable; return empty.
}
...
if ($agents.Count -eq 0) {
    try {
        ...
        $request = (@{ api_version = 2; op = "ping"; request = @{ target = "cc-bridge-daemon" } } | ConvertTo-Json -Compress -Depth 4) + "`n"
        $writer.Write($request)
        $writer.Flush()
        ...
        # Older or unreachable cc-bridge-daemon; return any project_view results or empty.
    }
}
