...
def spawn_cc_bridge_daemon_process(
    *,
    project_root: Path,
    socket_path: Path,
    cc_bridge_daemon_dir: Path,
    timeout_s: float,
    ...
    cc_bridge_daemon_dir.mkdir(parents=True, exist_ok=True)
    with open(cc_bridge_daemon_dir / 'cc_bridge_daemon.stdout.log', 'ab') as stdout_log, open(
        cc_bridge_daemon_dir / 'cc_bridge_daemon.stderr.log',
        'ab',
    ) as stderr_log:
        ...
        try:
            _wait_for_cc_bridge_daemon_ready(
                process=process,
...
def _wait_for_cc_bridge_daemon_ready(
    *,
    ...
    while time.time() < deadline:
        if socket_path.exists():
            try:
                payload = CcbdClient(socket_path, timeout_s=CONTROL_PLANE_RPC_TIMEOUT_S).ping('cc_bridge_daemon')
                if _ready_payload_matches_expected(
                    ...
                    return
                last_error = 'cc_bridge_daemon readiness identity does not match expected startup transaction'
        ...
        if process.poll() is not None:
            if socket_path.exists():
                try:
                    ...
                    last_error = str(exc)
            raise CcbdProcessError(f'cc_bridge_daemon exited before ready with code {process.returncode}')
        time.sleep(0.05)
    raise CcbdProcessError(last_error or 'timed out waiting for cc_bridge_daemon to become ready')


def _cc_bridge_daemon_env(
    *,
    keeper_pid: int | None,
    expected_startup_id: str | None = None,
    expected_generation: int | None = None,
    ...
    if keeper_pid is not None and keeper_pid > 0:
        env['CC_BRIDGE_KEEPER_PID'] = str(int(keeper_pid))
    return env
...
__all__ = ['CcbdProcessError', 'spawn_cc_bridge_daemon_process']
