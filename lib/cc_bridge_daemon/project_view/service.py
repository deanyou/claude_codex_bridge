...
from platforms.windows.herdr.cc_bridge_daemon_surface_projection import (
    build_herdr_runtime_surface_projection,
    build_herdr_surface_projection,
)
from cc_bridge_daemon.models import MountState
from cc_bridge_daemon.reload_drain_status import reload_drain_revision, reload_drain_status_payload
from cc_bridge_daemon.project_focus.tmux import backend_for_namespace, refresh_sidebar_panes
from cc_bridge_daemon.services.project_namespace_state_runtime.namespace_projection import (
    NAMESPACE_BACKEND_FAMILY,
    redacted_namespace_projection,
    resolved_namespace_backend_family,
)
from cc_bridge_daemon.services.dispatcher_runtime import comms_recoverability_for_job
...
def build_project_view(
    ...
    *,
    ...
    return {
        ...
        'cc_bridge_daemon': _cc_bridge_daemon_view(lease),
...
def _project_view_ttl_ms(deps: ProjectViewDependencies) -> int:
    if deps.cache_ttl_ms is not None:
        ...
        ttl_ms = _env_int('CC_BRIDGE_PROJECT_VIEW_TTL_MS', PROJECT_VIEW_TTL_MS)
    else:
        ttl_ms = _env_int('CC_BRIDGE_PROJECT_VIEW_IDLE_TTL_MS', PROJECT_VIEW_IDLE_TTL_MS)
    return max(0, ttl_ms)
...
def _provider_control_record(
    *,
    ...
    return {
        ...
        # CC_BRIDGE launches both providers through managed tmux processes. A model
...
def _cc_bridge_daemon_view(lease) -> dict[str, object]:
    return {
...
def _collect_tmux_project_view_facts(context: _ProjectViewBuildContext) -> tuple[dict[str, object], dict[str, dict[str, object]]]:
    ...
    focus_cp = _tmux_run_best_effort(
        ...
        [
            ...
            '#{window_name}\t#{pane_id}\t#{@cc_bridge_role}\t#{@cc_bridge_slot}',
        ],
    ...
    sidebars_cp = _tmux_run_best_effort(
        ...
        [
            ...
            '-a',
            '-F',
            '#{session_name}\t#{window_name}\t#{pane_id}\t#{@cc_bridge_project_id}\t#{@cc_bridge_role}\t#{@cc_bridge_sidebar_instance}\t#{@cc_bridge_window}',
        ],
        metrics_context=context.metrics_context,
    )
...
def _parse_tmux_project_view_outputs(
    *,
    ...
    mode = str(os.environ.get('CC_BRIDGE_RUST_PROJECT_VIEW', '')).strip().lower()
    ...
    try:
        ...
        if required:
            raise RuntimeError(
                'project_view.tmux.parse requires cc_bridge-rs-helper; no Python fallback is available for this path'
            ) from exc
