...
def provider_activity_evidence(
    *,
    ...
    return read_activity_evidence(
        ...
        agent_name=agent_name,
        provider=provider,
        cc_bridge_session_id=_runtime_cc_bridge_session_id(runtime),
...
def _runtime_cc_bridge_session_id(runtime: object | None) -> str | None:
    if runtime is None:
        return None
    text = str(getattr(runtime, 'cc_bridge_session_id', '') or '').strip()
    if text:
        return text
    return None
