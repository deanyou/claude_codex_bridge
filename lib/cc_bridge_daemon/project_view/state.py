...
_PROJECT_VIEW_STATE_RECORD_TYPE = 'cc_bridge_daemon_project_view_state'
...
class ProjectViewStateStore:
    ...
    def load(self) -> ProjectViewState:
        path = self._layout.cc_bridge_daemon_project_view_state_path
        if not path.exists():
    ...
    def _save(self, state: ProjectViewState) -> None:
        self._store.save(
            self._layout.cc_bridge_daemon_project_view_state_path,
            state,
