...
from cc_bridge_daemon.api_models import RpcRequest, RpcResponse
    ...
    try:
        ...
        with server._bootstrap_gate_lock:
            if server._stop_event.is_set() and request.op != 'shutdown':
                response = RpcResponse.failure('cc_bridge_daemon request serving is stopping')
            else:
                ...
                if bootstrap_restricted and request.op != 'ping':
                    response = RpcResponse.failure('cc_bridge_daemon bootstrap accepts ping only')
...
def _recv_request_line(conn) -> bytes:
    ...
    while b'\n' not in raw:
        ...
        if len(raw) > _MAX_REQUEST_BYTES:
            raise ValueError('cc_bridge_daemon request exceeds maximum size')
    return raw
