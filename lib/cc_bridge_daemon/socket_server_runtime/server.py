...
from cc_bridge_daemon.control_plane_transport import endpoint_from_legacy_socket_path
from cc_bridge_daemon.control_plane_transport.factory import transport_for_legacy_socket_path
...
class CcbdSocketServer:
    ...
    def begin_runtime_bootstrap(self) -> None:
        with self._bootstrap_gate_lock:
            if self._server is None:
                raise RuntimeError('cc_bridge_daemon runtime bootstrap requires a listening socket')
            if self._stop_event.is_set():
                raise RuntimeError('cc_bridge_daemon runtime bootstrap cannot begin while stopping')
            if self._bootstrap_probe_active or self._runtime_bootstrap_active:
                raise RuntimeError('cc_bridge_daemon runtime bootstrap is already active')
            self._runtime_bootstrap_active = True

    def finish_runtime_bootstrap(self, publish_ready) -> None:
        if not callable(publish_ready):
            raise TypeError('cc_bridge_daemon runtime bootstrap completion requires a publication callback')
        with self._bootstrap_gate_lock:
            try:
    ...
    def _assert_runtime_bootstrap_can_finish_locked(self) -> None:
        if not self._runtime_bootstrap_active:
            raise RuntimeError('cc_bridge_daemon runtime bootstrap is not active')
        if self._bootstrap_probe_active:
            raise RuntimeError('cc_bridge_daemon bootstrap self-probe is still active')
        if self._server is None:
            raise RuntimeError('cc_bridge_daemon listening socket is unavailable')
        if self._stop_event.is_set():
            raise RuntimeError('cc_bridge_daemon request serving stopped during runtime bootstrap')
        worker_error = self._peek_worker_error()
        if worker_error is not None:
            raise RuntimeError(f'cc_bridge_daemon request serving failed during runtime bootstrap: {worker_error}')
        worker = self._worker_thread
        if worker is None or not worker.is_alive():
            raise RuntimeError('cc_bridge_daemon request worker is not running during runtime bootstrap')
