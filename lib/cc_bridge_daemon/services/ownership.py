...
from cc_bridge_daemon.control_plane_transport.endpoint import endpoint_from_legacy_socket_path
from cc_bridge_daemon.control_plane_transport.factory import endpoint_connectable
...
class OwnershipGuard:
    ...
    def startup_lock(self):
        lock_path = self._layout.cc_bridge_daemon_dir / 'startup.lock'
    ...
    def verify_or_takeover(self, *, project_id: str, pid: int, socket_path: str | Path) -> int:
        ...
        raise OwnershipConflictError(
            f'cc_bridge_daemon lease is held by pid={current.cc_bridge_daemon_pid} generation={current.generation}: {inspection.reason}'
        )
    ...
    def assert_expected_claim_allowed(
        ...
        *,
        ...
        if generation <= 0:
            raise OwnershipConflictError('expected cc_bridge_daemon generation must be positive')
        current = self._mount_manager.load_state()
        if current is None:
            return
        ...
        if current.project_id != project_id:
            if current.mount_state is MountState.UNMOUNTED and current_generation < generation:
                return
            raise OwnershipConflictError(
                'cc_bridge_daemon lease project_id mismatch for expected claim: '
                f'expected {project_id}, found {current.project_id}'
            )
        if current_generation >= generation:
            raise OwnershipConflictError(
                'cc_bridge_daemon expected generation is already held or superseded: '
        ...
        if inspection.takeover_allowed:
            return
        raise OwnershipConflictError(
            'cc_bridge_daemon predecessor lease does not allow expected claim: '
            f'pid={current.cc_bridge_daemon_pid} generation={current_generation}: {inspection.reason}'
        )
    ...
    def _lease_signals(
        ...
        *,
        ...
        pid_alive = self._pid_exists(lease.cc_bridge_daemon_pid)
    ...
    def _same_holder(self, lease: CcbdLease, *, pid: int, socket_path: str | Path) -> bool:
        ...
        return lease.cc_bridge_daemon_pid == pid and current_socket == desired_socket
