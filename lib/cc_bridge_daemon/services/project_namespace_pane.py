...
class ProjectNamespacePaneRecord:
    ...
    cc_bridge_window: str | None = None
    ...
    def _matches_window_name(self, expected: str) -> bool:
        if self.cc_bridge_window is not None:
            return self._matches_field(self.cc_bridge_window, expected)
...
def inspect_project_namespace_pane(backend, pane_id: str) -> ProjectNamespacePaneRecord | None:
    ...
    return ProjectNamespacePaneRecord(
        ...
        role=_clean(details.get('@cc_bridge_role')),
        slot_key=_clean(details.get('@cc_bridge_slot')),
        cc_bridge_window=_clean(details.get('@cc_bridge_window')),
        sidebar_instance=_clean(details.get('@cc_bridge_sidebar_instance')),
        sidebar_helper_id=_clean(details.get('@cc_bridge_sidebar_helper_id')),
        agent_label=_clean(details.get('@cc_bridge_agent')),
        label_style=_clean(details.get('@cc_bridge_label_style')),
        border_style=_clean(details.get('@cc_bridge_border_style')),
        active_border_style=_clean(details.get('@cc_bridge_active_border_style')),
        cc_bridge_session_id=_clean(details.get('@cc_bridge_session_id')),
        window_width=_clean_int(details.get('window_width')),
        pane_width=_clean_int(details.get('pane_width')),
        project_id=_clean(details.get('@cc_bridge_project_id')),
        managed_by=_clean(details.get('@cc_bridge_managed_by')),
        namespace_epoch=_clean_int(details.get('@cc_bridge_namespace_epoch')),
...
def _decode_tmux_pane_description(line: str) -> dict[str, str] | None:
    ...
    if len(parts) == 7:
        return {
            ...
            '@cc_bridge_role': parts[3].strip(),
            '@cc_bridge_slot': parts[4].strip(),
            '@cc_bridge_window': '',
            '@cc_bridge_project_id': parts[5].strip(),
            '@cc_bridge_managed_by': parts[6].strip(),
            '@cc_bridge_namespace_epoch': '',
        }
    if len(parts) == 10:
        return {
            ...
            '@cc_bridge_role': parts[5].strip(),
            '@cc_bridge_slot': parts[6].strip(),
            '@cc_bridge_window': '',
            '@cc_bridge_project_id': parts[7].strip(),
            '@cc_bridge_managed_by': parts[8].strip(),
            '@cc_bridge_namespace_epoch': parts[9].strip(),
        }
    if len(parts) == 11:
        return {
            'pane_id': parts[0].strip(),
            ...
            '@cc_bridge_role': parts[5].strip(),
            '@cc_bridge_slot': parts[6].strip(),
            '@cc_bridge_window': parts[7].strip(),
            '@cc_bridge_sidebar_instance': '',
            '@cc_bridge_project_id': parts[8].strip(),
            '@cc_bridge_managed_by': parts[9].strip(),
            '@cc_bridge_namespace_epoch': parts[10].strip(),
        }
    if len(parts) == 12:
        return {
            'pane_id': parts[0].strip(),
            ...
            '@cc_bridge_slot': parts[6].strip(),
            '@cc_bridge_window': parts[7].strip(),
            '@cc_bridge_sidebar_instance': parts[8].strip(),
            '@cc_bridge_sidebar_helper_id': '',
            '@cc_bridge_project_id': parts[9].strip(),
            '@cc_bridge_managed_by': parts[10].strip(),
            '@cc_bridge_namespace_epoch': parts[11].strip(),
        }
    if len(parts) == 13:
        return {
            'pane_id': parts[0].strip(),
            ...
            '@cc_bridge_window': parts[7].strip(),
            '@cc_bridge_sidebar_instance': parts[8].strip(),
            '@cc_bridge_sidebar_helper_id': parts[9].strip(),
            '@cc_bridge_project_id': parts[10].strip(),
            '@cc_bridge_managed_by': parts[11].strip(),
            '@cc_bridge_namespace_epoch': parts[12].strip(),
        }
    ...
    result = {
        ...
        '@cc_bridge_agent': parts[14].strip(),
        '@cc_bridge_label_style': parts[15].strip(),
        '@cc_bridge_border_style': parts[16].strip(),
        '@cc_bridge_active_border_style': parts[17].strip(),
        '@cc_bridge_session_id': parts[18].strip(),
    }
    if len(parts) == 21:
