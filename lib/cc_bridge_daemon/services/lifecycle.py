...
from cc_bridge_daemon.models import MountState, SCHEMA_VERSION
from storage.json_store import JsonStore
from storage.paths import PathLayout


_RECORD_TYPE = 'cc_bridge_daemon_lifecycle'
...
class CcbdLifecycleStore:
    ...
    def load(self) -> CcbdLifecycle | None:
        path = self._layout.cc_bridge_daemon_lifecycle_path
        if not path.exists():
            return None
        return self._store.load(path, loader=CcbdLifecycle.from_record)

    def save(self, lifecycle: CcbdLifecycle) -> None:
        self._store.save(self._layout.cc_bridge_daemon_lifecycle_path, lifecycle, serializer=lambda value: value.to_record())
