...
from cc_bridge_daemon.models import CcbdLease, MountState, SCHEMA_VERSION
from cc_bridge_daemon.system import current_uid, parse_utc_timestamp, read_boot_id, utc_now
...
class MountManager:
    ...
    def load_state(self) -> CcbdLease | None:
        path = self._layout.cc_bridge_daemon_lease_path
        if not path.exists():
            return None
    ...
    def mark_mounted(
        ...
        *,
        ...
        self._store.save(self._layout.cc_bridge_daemon_lease_path, lease, serializer=lambda value: value.to_record())
        return lease
    ...
    def refresh_heartbeat(
        ...
        *,
        ...
        if lease is None:
            raise RuntimeError('cc_bridge_daemon lease does not exist')
        ...
        self._store.save(self._layout.cc_bridge_daemon_lease_path, updated, serializer=lambda value: value.to_record())
        return updated
    ...
    def update_config_signature(
        ...
        *,
        ...
        if lease is None:
            raise RuntimeError('cc_bridge_daemon lease does not exist')
        if lease.mount_state is not MountState.MOUNTED:
            raise RuntimeError('cc_bridge_daemon lease is not mounted')
        self._assert_expected_holder(
            lease,
            expected_pid=expected_pid,
            expected_daemon_instance_id=expected_daemon_instance_id,
        ...
        if int(lease.generation) != int(expected_generation):
            raise RuntimeError(
                'cc_bridge_daemon lease generation changed: '
    ...
    def _assert_expected_holder(
        ...
        *,
        ...
        if expected_pid is not None and int(lease.cc_bridge_daemon_pid) != int(expected_pid):
            raise RuntimeError(
                f'cc_bridge_daemon lease holder changed: expected pid={expected_pid}, found pid={lease.cc_bridge_daemon_pid}'
            )
        ...
        if expected_instance:
            ...
            if current_instance != expected_instance:
                raise RuntimeError(
                    'cc_bridge_daemon lease holder changed: '
...
def _heartbeat_write_interval_s() -> float:
    raw = os.environ.get('CC_BRIDGE_CC_BRIDGE_DAEMON_HEARTBEAT_WRITE_INTERVAL_S')
    if raw is None:
...
def _lease_from_record(record: dict) -> CcbdLease:
    ...
    if record.get('record_type') != 'cc_bridge_daemon_lease':
        raise ValueError("record_type must be 'cc_bridge_daemon_lease'")
    return CcbdLease(
        project_id=record['project_id'],
        cc_bridge_daemon_pid=int(record['cc_bridge_daemon_pid']),
