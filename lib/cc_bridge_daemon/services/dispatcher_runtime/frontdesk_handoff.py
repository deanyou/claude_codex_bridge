...
def enforce_frontdesk_boundary(
    ...
    *,
    ...
    payload = {
        'schema_version': 1,
        'record_type': 'cc_bridge_frontdesk_boundary_violation',
        'status': 'blocked',
...
def _has_persisted_direct_handoff(dispatcher, current: JobRecord) -> bool:
    ...
    return bool(
        activation.get('record_type') == 'cc_bridge_loop_frontdesk_planner_activation'
...
def _boundary_marker_path(dispatcher, job_id: str) -> Path:
    path = Path(dispatcher._layout.project_root) / '.cc-bridge' / 'runtime' / 'frontdesk-boundary' / f'{job_id}.json'
