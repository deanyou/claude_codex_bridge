...
from cc_bridge_daemon.api_models import AcceptedJobReceipt, DeliveryScope, JobStatus, MessageEnvelope, SubmitReceipt
...
_REQUEST_ID_RE = re.compile(r'(?mi)^\s*CC_BRIDGE_REQ_ID\s*:\s*`?([^`\n]+?)`?\s*$')
_TRANSACTION_SCHEMA = 'cc_bridge.frontdesk.direct_handoff_admission_transaction.v1'
    ...
    for path in sorted(root.glob('*.direct-handoff.transaction.json')):
        ...
        try:
            ...
            atomic_write_json(
                ...
                {
                    'schema': 'cc_bridge.frontdesk.direct_handoff_admission_recovery_error.v1',
                    'record_type': 'cc_bridge_frontdesk_direct_handoff_admission_recovery_error',
...
def _validate_shape(dispatcher, request: MessageEnvelope) -> None:
    ...
    if request_id_match is None:
        raise dispatcher._dispatch_error('frontdesk planner handoff requires CC_BRIDGE_REQ_ID in intake evidence')
    request_id = request_id_match.group(1).strip()
    if request_id != match.group(1):
        raise dispatcher._dispatch_error('frontdesk task id must match the CC_BRIDGE_REQ_ID intake field')
...
def _job_import_settled(context, job_id: str) -> bool:
    path = Path(context.project.project_root) / '.cc-bridge' / 'runtime' / 'role-output-imports.jsonl'
    try:
...
def _context(dispatcher):
    ...
    return SimpleNamespace(
        ...
        project=SimpleNamespace(
            ...
            config_dir=layout.cc_bridge_dir,
            project_id=layout.project_id,
            source='cc_bridge_daemon-frontdesk-direct-ask',
        ),
    )
