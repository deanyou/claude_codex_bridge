    ...
    *,
    ...
    if _force_reply_artifact(current):
        ...
        if _is_kimi_no_captured_reply(current, decision, diagnostics):
            reply = artifact_stub(
                prefix=(
                    f'CC_BRIDGE completion reply for job {current.job_id} has no captured Kimi provider reply; '
            ...
            reply = artifact_stub(
                prefix=(
                    f'CC_BRIDGE completion reply for job {current.job_id} has no captured '
            ...
            reply = artifact_stub(
                prefix=f'CC_BRIDGE completion reply for job {current.job_id} was stored as an artifact by --artifact-reply.',
                artifact=artifact,
                include_preview=False,
            )
        ...
        reply, artifact = maybe_spill_text(
            ...
            kind='completion-reply',
            owner_id=current.job_id,
            prefix=f'CC_BRIDGE completion reply for job {current.job_id} is larger than 4 KiB and was stored as an artifact.',
            now=finished_at,
        )
    if artifact is None:
        return decision
