    ...
    for path in sorted(root.glob('*.json')):
        ...
        try:
            ...
            atomic_write_json(
                ...
                {
                    'schema': 'cc_bridge.detailer.replan_recovery_error.v1',
                    'record_type': 'cc_bridge_detailer_replan_recovery_error',
...
def _prepare(
    ...
    *,
    ...
    if existing is not None:
        ...
        intent = {
            'schema': 'cc_bridge.detailer.replan_intent.v1',
            'record_type': 'cc_bridge_detailer_replan_intent',
            'status': 'prepared',
    ...
    if activation is None:
        ...
        activation = {
            'schema_version': 1,
            'record_type': 'cc_bridge_loop_detailer_planner_replan_activation',
...
def _validate_source_job(
    ...
    *,
    ...
    if role and role != 'agentroles.cc_bridge_task_detailer':
...
def _planner_request(request: MessageEnvelope, activation: Mapping[str, object]) -> MessageEnvelope:
    ...
    body = json.dumps({
        'schema': 'cc_bridge.detailer.planner_activation.v1',
...
def _context(dispatcher):
    ...
    return SimpleNamespace(
        ...
        project=SimpleNamespace(
            ...
            source='cc_bridge_daemon-detailer-replan-direct-ask',
        ),
    )


def _intent_root(dispatcher) -> Path:
    return Path(dispatcher._layout.project_root) / '.cc-bridge' / 'runtime' / 'detailer-replan'
