...
from cc_bridge_daemon.api_models import DeliveryScope, JobRecord, MessageEnvelope, TargetKind
...
def _validate_request_body_artifact(dispatcher, request: MessageEnvelope) -> None:
    if request.body_artifact:
        ...
        return
    if should_spill_text(request.body, threshold_bytes=TEXT_ARTIFACT_SPILL_BYTES):
        raise dispatcher._dispatch_error('ask body exceeds 4 KiB and must be submitted with a CC_BRIDGE body artifact')
