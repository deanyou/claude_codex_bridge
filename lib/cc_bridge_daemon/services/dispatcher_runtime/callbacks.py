...
def validate_nested_ask_request(dispatcher, request: MessageEnvelope) -> None:
    ...
    raise dispatcher._dispatch_error(
        'plain ask from an active CC_BRIDGE task requires --chain when the child result is needed, '
...
def _validate_callback_continuation_target(dispatcher, *, parent, child_agent: str) -> None:
    ...
    if target and upstream and target == upstream:
        raise dispatcher._dispatch_error(
            ...
            f'is not allowed for chain edge {edge.edge_id}; finish the current response and CC_BRIDGE will deliver it upstream'
        )
...
def _callback_failure_reply(*, edge: CallbackEdgeRecord, reason: str, detail: str) -> str:
    ...
    return (
        f'CC_BRIDGE chain failed for delegated task {edge.child_job_id} '
...
def _continuation_request(dispatcher, *, edge: CallbackEdgeRecord, child_job, decision: CompletionDecision) -> MessageEnvelope:
    ...
    body, body_artifact = maybe_spill_text(
        ...
        prefix=f'CC_BRIDGE result-chain continuation {edge.edge_id} is larger than 4 KiB and was stored as an artifact.',
    )
...
def _continuation_body(*, edge: CallbackEdgeRecord, child_job, decision: CompletionDecision) -> str:
    ...
    parts.extend(
        [
            ...
            'Do not call ask, --chain, or --silence to the original caller; CC_BRIDGE will deliver this chain result upstream.',
        ]
    )
    return '\n'.join(parts)
...
def _callback_body_summary(request: MessageEnvelope) -> str:
    body = _strip_cc_bridge_guidance(request.body)
...
def _strip_cc_bridge_guidance(body: str) -> str:
    indexes = [
        index
