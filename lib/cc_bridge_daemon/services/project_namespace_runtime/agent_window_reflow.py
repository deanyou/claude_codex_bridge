...
from cc_bridge_daemon.reload_additive_agents import window_agent_names, window_map
...
def _observed_panes(runner, *, window_target: str, timeout_s: float | None) -> tuple[tuple[_ObservedPane, ...], str | None]:
    completed = runner(
        [
            ...
            window_target,
            '-F',
            '#{pane_id}\t#{pane_index}\t#{pane_left}\t#{pane_top}\t#{pane_width}\t#{pane_height}\t#{@cc_bridge_role}\t#{@cc_bridge_slot}\t#{@cc_bridge_window}\t#{@cc_bridge_sidebar_instance}',
        ],
        check=False,
        capture=True,
        timeout=timeout_s,
