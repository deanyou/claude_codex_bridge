...
SIDEBAR_BINARY_NAME = 'cc-bridge-agent-sidebar'
SIDEBAR_ENV_PATH = 'CC_BRIDGE_AGENT_SIDEBAR_BIN'
SIDEBAR_HELPER_ID_OPTION = '@cc_bridge_sidebar_helper_id'
_SIDEBAR_WRAPPER_MARKER = b'CC_BRIDGE_AGENT_SIDEBAR_WRAPPER'
...
def missing_sidebar_respawn_args(reason: str | None = None) -> tuple[str, ...]:
    message = 'CC_BRIDGE sidebar helper unavailable'
    ...
    body = (
        ...
        "printf '%s\\n' 'Build or install bin/cc-bridge-agent-sidebar, or set CC_BRIDGE_AGENT_SIDEBAR_BIN.'; "
...
def _sidebar_runtime_binary(path: Path, *, script_root: Path) -> Path:
    ...
    if _SIDEBAR_WRAPPER_MARKER not in prefix:
        return path
    candidate = script_root / 'tools' / 'cc-bridge-agent-sidebar' / 'target' / 'release' / SIDEBAR_BINARY_NAME
