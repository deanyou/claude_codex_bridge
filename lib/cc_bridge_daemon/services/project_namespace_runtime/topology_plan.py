...
def build_namespace_topology_plan(config, *, cc_bridge_daemon_socket_path: str | None = None, project_root: str | None = None) -> NamespaceTopologyPlan:
    sidebar_enabled = (
...
def _window_plan(window, *, sidebar, cc_bridge_daemon_socket_path: str | None, project_root: str | None) -> NamespaceWindowPlan:
...
def _tool_window_plan(tool, *, order_offset: int, sidebar, cc_bridge_daemon_socket_path: str | None, project_root: str | None) -> NamespaceWindowPlan:
    sidebar_plan = _sidebar_plan(
        sidebar,
...
def _sidebar_plan(sidebar, *, window_name: str, cc_bridge_daemon_socket_path: str | None, project_root: str | None) -> SidebarPanePlan | None:
    return (
        SidebarPanePlan(
            mode=sidebar.mode,
            width=sidebar.width,
            ...
            launch_args=_sidebar_launch_args(cc_bridge_daemon_socket_path=cc_bridge_daemon_socket_path, project_root=project_root, window_name=window_name),
        )
...
def _sidebar_launch_args(*, cc_bridge_daemon_socket_path: str | None, project_root: str | None, window_name: str) -> tuple[str, ...]:
    args = ['cc-bridge-agent-sidebar']
    if cc_bridge_daemon_socket_path:
        args.extend(['--cc_bridge_daemon-socket', str(cc_bridge_daemon_socket_path)])
    if project_root:
