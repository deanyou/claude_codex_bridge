...
def resolve_project_slot_recovery_context(
    *,
    ...
    if runtime_socket and not same_tmux_socket_path(runtime_socket, namespace.tmux_socket_path):
        return None
    if not runtime_socket and str(getattr(runtime, 'managed_by', '') or '').strip() != 'cc_bridge_daemon':
        return None
    try:
...
def inject_project_slot_recovery_hints(session, context: ProjectSlotRecoveryContext | None) -> None:
    ...
    if not isinstance(data, dict):
        return
    data['cc_bridge_project_id'] = context.project_id
    data['cc_bridge_slot'] = context.slot_key
    data['cc_bridge_managed_by'] = 'cc_bridge_daemon'
    data['cc_bridge_namespace_epoch'] = str(context.namespace_epoch)
    data['cc_bridge_replacement_parent_pane'] = context.workspace_root_pane_id
