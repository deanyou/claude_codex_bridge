...
def destroy_project_namespace(controller, *, reason: str):
    ...
    controller._layout.cc_bridge_daemon_dir.mkdir(parents=True, exist_ok=True)
    current = controller._state_store.load()
    occurred_at = controller._clock()
    tmux_socket_path = str(current.tmux_socket_path) if current is not None else str(controller._layout.cc_bridge_daemon_tmux_socket_path)
    tmux_session_name = str(current.tmux_session_name) if current is not None else controller._layout.cc_bridge_daemon_tmux_session_name
    ...
    next_state = build_destroyed_state(
        ...
        control_window_name=(
            ...
            else controller._layout.cc_bridge_daemon_tmux_control_window_name
        ),
        workspace_window_name=(
            str(current.workspace_window_name)
            if current is not None and current.workspace_window_name
            else controller._layout.cc_bridge_daemon_tmux_workspace_window_name
        ),
    )
