...
from cc_bridge_daemon.services.project_namespace_pane import (
...
def refresh_topology_ui_for_project(
    ...
    *,
    ...
    if callable(getattr(context.backend, '_tmux_run', None)):
        apply_project_tmux_ui(
            tmux_socket_path=context.desired_socket_path,
            cc_bridge_daemon_socket_path=str(controller._layout.cc_bridge_daemon_socket_path),
            tmux_session_name=context.desired_session_name,
            backend=context.backend,
        )
    ...
    *,
    ...
    for window in tuple(getattr(topology_plan, 'windows', ()) or ()):
        for agent_name in tuple(getattr(window, 'agent_names', ()) or ()):
            expected = {
                '@cc_bridge_project_id': controller._project_id,
                '@cc_bridge_role': 'agent',
                '@cc_bridge_slot': str(agent_name),
                '@cc_bridge_window': str(window.name),
    ...
    *,
    ...
    matches = _matching_pane_ids(
        ...
        context.backend,
        {
            ...
            '@cc_bridge_role': 'cmd',
            '@cc_bridge_slot': 'cmd',
            '@cc_bridge_window': cmd_windows[0],
            '@cc_bridge_managed_by': 'cc_bridge_daemon',
        },
    ...
    if resolved_records is None:
        candidate_ids = _list_panes_by_user_options(
            context.backend,
            {
                '@cc_bridge_project_id': controller._project_id,
                '@cc_bridge_managed_by': 'cc_bridge_daemon',
                '@cc_bridge_namespace_epoch': str(resolved_epoch or ''),
            },
        )
        resolved_records = {}
            ...
            expected.append(
                {
                    ...
                    '@cc_bridge_slot': f'sidebar:{window_name}',
                    '@cc_bridge_window': window_name,
                    '@cc_bridge_sidebar_instance': window_name,
                    '@cc_bridge_managed_by': 'cc_bridge_daemon',
                }
            )
    ...
    *,
    ...
    if pane_records is not None:
        observed_windows = {
            ...
            and str(record.managed_by or '').strip() == 'cc_bridge_daemon'
        ...
        for window in windows:
            matches = _matching_pane_ids(
                ...
                context.backend,
                {
                    '@cc_bridge_project_id': controller._project_id,
                    '@cc_bridge_role': 'sidebar',
                    '@cc_bridge_slot': f'sidebar:{window.name}',
                    '@cc_bridge_window': str(window.name),
                    '@cc_bridge_sidebar_instance': str(window.name),
                    '@cc_bridge_managed_by': 'cc_bridge_daemon',
                },
                tmux_session_name=resolved_session,
                namespace_epoch=resolved_epoch,
...
def _rename_legacy_workspace_if_needed(controller, context, *, first_window_name: str, timeout_s: float | None) -> None:
    legacy_name = str(getattr(controller._layout, 'cc_bridge_daemon_tmux_workspace_window_name', '') or '').strip()
    if context.current is not None:
...
def _materialize_agent_layout(
    ...
    *,
    ...
    def assign_leaf(item: str, pane_id: str) -> None:
        ...
        # Main_Code，而 CC_BRIDGE 内部 config key / topology agent_names 是 normalized
...
def _list_sidebar_geometry_records(
    ...
    *,
    ...
    if pane_records is not None:
        ...
        for pane_id, record in pane_records.items():
            ...
            if not project_id and (
                ...
                or record.managed_by != 'cc_bridge_daemon'
                or not record.alive
            ):
                continue
    ...
    for line in (getattr(cp, 'stdout', '') or '').splitlines():
        ...
        if pane_session != session_name or role != 'sidebar' or managed_by != 'cc_bridge_daemon':
            continue
...
def _session_sidebar_width_override(backend, session_name: str) -> int:
    ...
    try:
        cp = runner(
            ['show-option', '-qv', '-t', session_name, '@cc_bridge_sidebar_width_cells'],
            capture=True,
            check=False,
            timeout=0.5,
        )
...
def _set_session_sidebar_sync_guard(backend, session_name: str, *, enabled: bool) -> None:
    ...
    if not callable(runner):
        return
    args = (
        ['set-option', '-t', session_name, '@cc_bridge_sidebar_sync_guard', '1']
        if enabled
        else ['set-option', '-u', '-t', session_name, '@cc_bridge_sidebar_sync_guard']
    )
    try:
...
def _matching_pane_ids(
    ...
    *,
    ...
    project_id = str(expected.get('@cc_bridge_project_id', '') or '').strip()
    ...
    if namespace_epoch is not None:
        candidate_expected['@cc_bridge_namespace_epoch'] = str(namespace_epoch)
    ...
    for pane_id, record in candidates:
        ...
        if not record.matches_authoritative_topology(
            tmux_session_name=resolved_session,
            project_id=project_id,
            role=expected.get('@cc_bridge_role'),
            slot_key=expected.get('@cc_bridge_slot'),
            window_name=expected.get('@cc_bridge_window'),
            sidebar_instance=expected.get('@cc_bridge_sidebar_instance'),
            managed_by=str(expected.get('@cc_bridge_managed_by', 'cc_bridge_daemon') or 'cc_bridge_daemon'),
            namespace_epoch=namespace_epoch,
...
def _pane_record_option(record: ProjectNamespacePaneRecord, option: str) -> str:
    field_names = {
        ...
        '@cc_bridge_role': 'role',
        '@cc_bridge_slot': 'slot_key',
        '@cc_bridge_window': 'cc_bridge_window',
        '@cc_bridge_sidebar_instance': 'sidebar_instance',
        '@cc_bridge_managed_by': 'managed_by',
        '@cc_bridge_namespace_epoch': 'namespace_epoch',
    }
...
def _respawn_sidebar(backend, pane_id: str, launch_args: tuple[str, ...], *, cwd: str) -> None:
    ...
    command = f'CC_BRIDGE_SIDEBAR_THEME_PROFILE={shlex.quote(tmux_theme_profile())} {command}'
