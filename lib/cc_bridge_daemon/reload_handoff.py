...
_RECORD_TYPE = 'cc_bridge_daemon_reload_handoff'
...
class ReloadHandoffStore:
    ...
    def load(self) -> ReloadHandoff | None:
        path = self.layout.cc_bridge_daemon_reload_handoff_path
        if not path.exists():
            return None
    ...
    def save(self, handoff: ReloadHandoff) -> None:
        self.store.save(
            self.layout.cc_bridge_daemon_reload_handoff_path,
            handoff,
    ...
    def clear(self) -> None:
        try:
            self.layout.cc_bridge_daemon_reload_handoff_path.unlink()
...
def _matches_current_holder(app, handoff: ReloadHandoff) -> bool:
    ...
    if lease is None:
        return False
    if int(getattr(lease, 'cc_bridge_daemon_pid', 0) or 0) != int(handoff.daemon_pid):
        return False
