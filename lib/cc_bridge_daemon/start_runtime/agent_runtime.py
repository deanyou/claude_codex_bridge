...
def start_agent_runtime(
    *,
    ...
    try:
        ...
        tmux_window_name = (
            window_name
            or getattr(namespace_record, 'cc_bridge_window', None)
...
def _pane_identity_is_current(
    ...
    *,
    ...
    if any(str(getattr(record, field, '') or '').strip() != value for field, value in expected.items()):
        return False
    if window_name is not None and str(getattr(record, 'cc_bridge_window', '') or '').strip() != window_name:
        return False
    if namespace_epoch is not None and getattr(record, 'namespace_epoch', None) != int(namespace_epoch):
        return False
    cc_bridge_session_id = str(getattr(binding, 'cc_bridge_session_id', '') or '').strip()
    if cc_bridge_session_id and str(getattr(record, 'cc_bridge_session_id', '') or '').strip() != cc_bridge_session_id:
        return False
    return True
...
def _attach_failed_agent_result(exc: Exception, result: CcbdStartupAgentResult) -> None:
    try:
        setattr(exc, 'cc_bridge_startup_agent_result', result)
        setattr(exc, 'cc_bridge_startup_timings_ms', dict(result.timings_ms or {}))
    except Exception:
        return
