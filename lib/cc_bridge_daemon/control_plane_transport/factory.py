...
def transport_for_endpoint(endpoint: EndpointRef | dict | str | Path):
    ...
    if _is_windows() and isinstance(endpoint, dict) and resolved['kind'] != 'tcp_loopback':
        raise RpcTransportAuthError('endpoint-invalid', 'cc_bridge_daemon Windows control-plane endpoint must use tcp_loopback')
    ...
    raise RuntimeError(f'unsupported cc_bridge_daemon control-plane endpoint: {resolved["kind"]}')
