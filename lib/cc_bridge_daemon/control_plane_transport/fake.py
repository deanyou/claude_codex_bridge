...
from cc_bridge_daemon.socket_server_runtime.loop import enqueue_connection, start_worker
    ...
    def recv(self, size: int) -> bytes:
        ...
        while not self._incoming:
            ...
            if deadline is not None and time.monotonic() >= deadline:
                raise TimeoutError('fake cc_bridge_daemon connection recv timed out')
            time.sleep(0.01)
...
class FakeControlPlaneListener:
    ...
    def enqueue(self, connection: FakeConnection) -> None:
        if self.closed:
            raise OSError('fake cc_bridge_daemon listener is closed')
    ...
    def accept(self):
        ...
        try:
            ...
            raise TimeoutError('fake cc_bridge_daemon listener has no pending connection') from exc
    ...
    def fileno(self) -> int:
        raise OSError('fake cc_bridge_daemon listener has no file descriptor')
...
class FakeControlPlaneTransport:
    def __init__(self, endpoint: EndpointRef | None = None) -> None:
        self.endpoint = endpoint or {
            'kind': 'unix_socket',
            'address': 'fake://cc_bridge_daemon',
            'display': 'fake://cc_bridge_daemon',
