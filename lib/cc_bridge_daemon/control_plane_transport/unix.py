    ...
    def listen(self) -> 'UnixControlPlaneListener':
        ...
        try:
            ...
            if bound_socket_stat is None:
                raise RuntimeError('cc_bridge_daemon bound socket inode is unavailable')
...
def remove_stale_socket_path(path: Path) -> None:
    ...
    if not stat.S_ISSOCK(initial.st_mode):
        raise RuntimeError(f'refusing to replace non-socket cc_bridge_daemon path: {path}')
    if socket_path_connectable(path):
        raise RuntimeError(f'refusing to replace live cc_bridge_daemon socket: {path}')
    try:
    ...
    if current_identity != initial_identity or not stat.S_ISSOCK(current.st_mode):
        raise RuntimeError(f'cc_bridge_daemon socket path changed during stale cleanup: {path}')
    path.unlink()
...
def socket_path_connectable(path: Path, *, timeout_s: float = 0.1) -> bool:
    ...
    try:
        ...
        if exc.errno in {errno.ENOENT, errno.ECONNREFUSED}:
            return False
        raise RuntimeError(f'cannot prove existing cc_bridge_daemon socket is stale: {path}: {exc}') from exc
    finally:
        probe.close()
...
def _probe_client_path(nonce: str) -> Path:
    name = f'cc_bridge-probe-{os.getpid()}-{nonce[:12]}.sock'
