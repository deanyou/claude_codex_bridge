...
_AUTH_MARKER = 'cc_bridge_daemon-control-plane-token-v1'
_AUTH_ACK_MARKER = 'cc_bridge_daemon-control-plane-token-ack-v1'
...
def load_token_file(path: str | Path) -> TokenFile:
    ...
    try:
        ...
        raise RpcTransportAuthError('token-missing', 'cc_bridge_daemon control-plane token is missing') from exc
    except PermissionError as exc:
        raise RpcTransportAuthError('token-unreadable', 'cc_bridge_daemon control-plane token is unreadable') from exc
    if not isinstance(payload, dict) or payload.get('schema') != _AUTH_MARKER:
        raise RpcTransportAuthError('token-invalid', 'cc_bridge_daemon control-plane token file is invalid')
    token = str(payload.get('token') or '').strip()
    generation = str(payload.get('generation') or '').strip()
    if not token or not generation:
        raise RpcTransportAuthError('token-invalid', 'cc_bridge_daemon control-plane token file is incomplete')
    return TokenFile(
        token_ref=str(target),
        token=token,
        generation=generation,
...
def _recv_auth_ack(sock) -> None:
    try:
        ...
        raise RpcTransportAuthError('not-same-user', 'cc_bridge_daemon auth handshake was rejected') from exc
    payload = _decode_auth_payload(line)
    if payload.get('schema') != _AUTH_ACK_MARKER or payload.get('ok') is not True:
        raise RpcTransportAuthError('handshake-failed', 'cc_bridge_daemon auth ack is invalid')
    if remainder:
        raise RpcTransportAuthError('handshake-failed', 'cc_bridge_daemon auth ack contains unexpected data')
