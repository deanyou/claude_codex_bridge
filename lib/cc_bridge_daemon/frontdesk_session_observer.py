...
_REQUEST_ID_RE = re.compile(
    r'(?mi)^\s*CC_BRIDGE_REQ_ID\s*:\s*`?([A-Za-z0-9][A-Za-z0-9_-]{0,79})`?\s*$'
)
...
def _request_context_for_turn(session_path: Path, turn_id: str) -> dict[str, object]:
    ...
    for message in user_messages:
        match = re.match(
            r'(?is)^\s*CC_BRIDGE_REQ_ID\s*:\s*`?([A-Za-z0-9][A-Za-z0-9_-]{0,79})`?(?:\s|$)',
            message,
        )
        if match:
...
def _normalize_intake_request_id(reply: str, request_id: str) -> str:
    ...
    if not without_ids:
        return f'CC_BRIDGE_REQ_ID: {request_id}'
    lines = without_ids.splitlines()
    insert_at = 1 if lines else 0
    lines[insert_at:insert_at] = ['', f'CC_BRIDGE_REQ_ID: {request_id}']
    return '\n'.join(lines).strip()


def _state_path(app) -> Path:
    return Path(app.paths.project_root) / '.cc-bridge' / 'runtime' / 'frontdesk-session-observer' / 'state.json'
...
def _record_state(app, payload: dict[str, object]) -> dict[str, object]:
    record = {
        'schema_version': 1,
        'record_type': 'cc_bridge_frontdesk_session_observer',
