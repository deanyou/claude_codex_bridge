...
def run_start_flow(
    *,
            ...
            try:
                ...
                if readiness_recorder is not None:
                    ...
                    readiness_recorder.mark(
                        ...
                        source='cc_bridge_daemon_start_flow_agent_failure',
                ...
                failed_result = getattr(exc, 'cc_bridge_startup_agent_result', None)
                ...
                try:
                    setattr(exc, 'cc_bridge_startup_agent_results', failure_results)
                except Exception:
                    pass
                raise
    ...
    if readiness_recorder is not None:
        if provider_runtime_deferred:
            readiness_recorder.mark(
                ...
                source='cc_bridge_daemon_start_flow_provider_runtime_deferred_on_herdr',
                agents=targets,
                now_ns=time.perf_counter_ns(),
            )
        else:
            readiness_recorder.mark(
                'T4_requested_agents_ready',
                source='cc_bridge_daemon_start_flow_authority_committed',
                agents=targets,
                now_ns=time.perf_counter_ns(),
            )
        ...
        if provider_runtime_deferred:
            ...
            readiness_recorder.mark(
                'T6_fully_warm',
                status='not_reached_at_rpc_return',
                source='cc_bridge_daemon_start_flow_requested_subset',
                agents=targets,
                now_ns=time.perf_counter_ns(),
            )
    ...
    return StartFlowSummary(
        ...
        project_id=project_id,
        started=targets,
        socket_path=str(paths.cc_bridge_daemon_socket_path),
...
def _herdr_assigned_pane_ref(
    *,
    ...
    if not session_name:
        return None
    window_name = (
        getattr(record, 'cc_bridge_window', None)
