...
class CcbdClient:
    ...
    def request(self, op: str, payload: dict | None = None) -> dict:
        ...
        try:
            ...
            sock.close()
        if not raw:
            raise CcbdClientError('empty response from cc_bridge_daemon')
        response = decode_response(raw)
        if not response.ok:
            raise CcbdClientError(response.error or 'cc_bridge_daemon request failed')
...
def _resolve_timeout(explicit: float | None) -> float:
    if explicit is not None:
        try:
            ...
            return 3.0
    for env_name in ('CC_BRIDGE_CC_BRIDGE_DAEMON_CLIENT_TIMEOUT_S',):
