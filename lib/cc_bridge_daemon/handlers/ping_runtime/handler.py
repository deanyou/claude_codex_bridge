...
from .payloads import build_agent_payload, build_cc_bridge_daemon_payload
    ...
    def handle(payload: dict) -> dict:
        ...
        try:
            ...
            if target in {'', 'cc_bridge_daemon'}:
                ...
                response = build_cc_bridge_daemon_payload(
            ...
            if target == 'all':
                return {
                    'project_id': project_id,
                    'cc_bridge_daemon_state': str(getattr(inspection, 'phase', '') or '').strip()
