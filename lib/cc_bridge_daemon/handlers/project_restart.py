...
from cc_bridge_daemon.services.start_policy import recovery_start_options
    ...
    def handle(payload: dict) -> tuple[dict, object]:
        ...
        def _after_response() -> None:
            try:
                ...
                # Keep cc_bridge_daemon alive; the supervision loop can repair failed panes later.
                return
...
def _restart_start_options(app) -> tuple[bool, bool]:
    ...
    try:
        ...
        policy = None
    if policy is None:
        # Match the normal `cc_bridge start` defaults when no recovery policy exists.
        return True, True
...
def _session_role_evidence(*, session) -> dict[str, str] | None:
    ...
    if not isinstance(data, dict):
        return None
    role_id = _clean(data.get('cc_bridge_role_id'))
    if not role_id:
        return None
    return {
        'id': role_id,
        'version': _clean(data.get('cc_bridge_role_version')) or '',
        'digest': _clean(data.get('cc_bridge_role_digest')) or '',
    }
