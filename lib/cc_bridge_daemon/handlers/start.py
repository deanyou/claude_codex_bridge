...
from cc_bridge_daemon.startup_identity import resolve_startup_run_id
    ...
    def handle(payload: dict) -> dict:
        ...
        if readiness_recorder is not None:
            readiness_recorder.mark(
                'rpc_accepted',
                source='cc_bridge_daemon_start_handler',
                now_ns=rpc_accepted_ns,
            )
...
def _trusted_keeper_startup_checkpoint(app):
    ...
    try:
        if (
            ...
            or int(lease.cc_bridge_daemon_pid) != int(app.pid)
