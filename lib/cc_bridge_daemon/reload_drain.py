...
_QUEUE_RECORD_TYPE = 'cc_bridge_daemon_reload_drain_queue'
...
class DrainQueueStore:
    ...
    def load(self) -> DrainQueue:
        path = self._layout.cc_bridge_daemon_reload_drain_path
        if not path.exists():
    ...
    def save(self, queue: DrainQueue) -> None:
        self._store.save(self._layout.cc_bridge_daemon_reload_drain_path, queue, serializer=lambda value: value.to_record())
