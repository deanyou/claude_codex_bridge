...
class KeeperStateStore:
    ...
    def load(self) -> KeeperState | None:
        path = self._layout.cc_bridge_daemon_keeper_path
        if not path.exists():
            return None
    ...
    def save(self, state: KeeperState) -> None:
        ...
        if current is None and self._layout.cc_bridge_daemon_keeper_path.exists():
            current = self.load()
        if current is not None and _keeper_state_write_can_skip(current, state):
            return
        self._store.save(self._layout.cc_bridge_daemon_keeper_path, state, serializer=lambda value: value.to_record())
        self._last_saved = state
...
class ShutdownIntentStore:
    ...
    def load(self) -> ShutdownIntent | None:
        path = self._layout.cc_bridge_daemon_shutdown_intent_path
        if not path.exists():
            return None
        return self._store.load(path, loader=ShutdownIntent.from_record)

    def save(self, intent: ShutdownIntent) -> None:
        self._store.save(self._layout.cc_bridge_daemon_shutdown_intent_path, intent, serializer=lambda value: value.to_record())

    def clear(self) -> None:
        try:
            self._layout.cc_bridge_daemon_shutdown_intent_path.unlink()
        except FileNotFoundError:
            pass


def _keeper_state_write_interval_s() -> float:
    raw = os.environ.get('CC_BRIDGE_KEEPER_STATE_WRITE_INTERVAL_S')
    if raw is None:
