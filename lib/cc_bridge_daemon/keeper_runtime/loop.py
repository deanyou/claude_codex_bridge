...
from cc_bridge_daemon.reload_handoff import reload_handoff_allows_signature_mismatch
from cc_bridge_daemon.services.project_namespace_state import ProjectNamespaceStateStore
from cc_bridge_daemon.services.lifecycle import current_socket_inode, lifecycle_from_inspection
...
from cc_bridge_daemon.startup_policy import CONTROL_PLANE_RPC_TIMEOUT_S, STARTUP_TRANSACTION_TIMEOUT_S
...
def run_forever(app, *, poll_interval: float = 0.5, start_timeout_s: float = STARTUP_TRANSACTION_TIMEOUT_S) -> int:
    lock_path = app.paths.cc_bridge_daemon_dir / 'keeper.lock'
...
def stale_restart_state(app, *, state: KeeperState, inspection, occurred_at: str) -> KeeperState | None:
    ...
    if inspection.heartbeat_fresh:
        return None
    pid = int(inspection.lease.cc_bridge_daemon_pid or 0)
    if pid > 0:
...
def daemon_matches_project_config(app) -> bool:
    ...
    payload = CcbdClient(app.paths.cc_bridge_daemon_socket_path, timeout_s=CONTROL_PLANE_RPC_TIMEOUT_S).ping('cc_bridge_daemon')
    ...
    if actual_signature:
        ...
        # project daemon. Explicit `cc_bridge reload` owns applying disk config drift.
        return True
...
def request_shutdown(app) -> None:
    client = CcbdClient(app.paths.cc_bridge_daemon_socket_path, timeout_s=CONTROL_PLANE_RPC_TIMEOUT_S)
    try:
        client.stop_all(force=False)
    except CcbdClientError:
        inspection = app._ownership_guard.inspect()
        if inspection.lease is not None and inspection.pid_alive:
            app._terminate_pid_tree(int(inspection.lease.cc_bridge_daemon_pid or 0), timeout_s=1.0)
...
def _startup_promotion_owned_by_child(lifecycle, lease) -> bool:
    ...
    try:
        return (
            ...
            and int(lifecycle.owner_pid or 0) == int(lease.cc_bridge_daemon_pid)
...
def _same_observed_lease(current, observed) -> bool:
    ...
    try:
        return (
            ...
            and int(current.cc_bridge_daemon_pid) == int(observed.cc_bridge_daemon_pid)
...
def cleanup_transient_keeper_files(app, *, lock_path: Path) -> None:
    for path in (
        app.paths.cc_bridge_daemon_keeper_path,
        app.paths.cc_bridge_daemon_reload_handoff_path,
        app.paths.cc_bridge_daemon_dir / 'keeper.stdout.log',
        app.paths.cc_bridge_daemon_dir / 'keeper.stderr.log',
        Path(lock_path),
    ):
        try:
            path.unlink()
            ...
            continue
    for path in (app.paths.cc_bridge_daemon_dir,):
        try:
            path.rmdir()
        except OSError:
            continue
