...
from cc_bridge_daemon.services.project_namespace_runtime.backend import build_backend, remember_namespace_state_ref
...
def _inspect_herdr_cmd_record(ctx: RuntimeSupervisionContext, backend, namespace):
    ...
    expected = {
        ...
        '@cc_bridge_namespace_epoch': str(getattr(namespace, 'namespace_epoch', '') or ''),
    }
    if window_name:
        expected['@cc_bridge_window'] = window_name
    try:
