...
from cc_bridge_daemon.reload_drain import DrainQueueStore
from cc_bridge_daemon.restore_report_store import CcbdRestoreReportStore
from cc_bridge_daemon.services import (
    CcbdLifecycleStore,
    JobHeartbeatService,
    MountManager,
    OwnershipGuard,
...
def initialize_app(
    ...
    *,
    ...
    # cc_bridge_daemon is non-interactive and is the authority that materializes project
    ...
    _publish_mobile_gateway_project(app.project_id, app.project_root, app.paths.cc_bridge_daemon_socket_path, clock=clock)
    sweep_expired_text_artifacts(app.paths)
    app.clock = clock
    app.pid = pid or os.getpid()
    config = load_project_config(app.project_root).config
    keeper_pid = str(os.environ.get('CC_BRIDGE_KEEPER_PID') or '').strip()
    ...
    service_graph = build_cc_bridge_daemon_service_graph(
    ...
    publish_cc_bridge_daemon_service_graph(app, service_graph)
    ...
    app.socket_server = CcbdSocketServer(app.paths.cc_bridge_daemon_socket_path)
