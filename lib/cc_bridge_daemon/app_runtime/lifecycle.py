...
from cc_bridge_daemon.models import CcbdShutdownReport, CcbdStartupReport, MountState, cleanup_summaries_from_objects
from cc_bridge_daemon.system import process_exists
from cc_bridge_daemon.reload_drain_auto_retry import tick_reload_drain_auto_retry
from cc_bridge_daemon.services.dispatcher_runtime.frontdesk_direct_handoff import recover_frontdesk_direct_handoffs
from cc_bridge_daemon.services.dispatcher_runtime.detailer_replan_handoff import recover_detailer_replan_handoffs
from cc_bridge_daemon.services.lifecycle import build_lifecycle, current_socket_inode
from cc_bridge_daemon.startup_fence import StartupFenceError, validate_expected_startup_lifecycle
from cc_bridge_daemon.startup_policy import CONTROL_PLANE_RPC_TIMEOUT_S
from cc_bridge_daemon.stop_flow import build_shutdown_runtime_snapshots
...
DEFAULT_CC_BRIDGE_DAEMON_POLL_INTERVAL_S = 1.0
            ...
            if expected_fence is None:
                ...
                if lifecycle is not None:
                    ...
                    if (
                        ...
                        # The previous cc_bridge_daemon crashed and left the lifecycle
...
def idle_full_heartbeat_interval_s() -> float:
    try:
        return max(0.0, float(os.environ.get('CC_BRIDGE_CC_BRIDGE_DAEMON_IDLE_FULL_HEARTBEAT_INTERVAL_S', DEFAULT_IDLE_FULL_HEARTBEAT_INTERVAL_S)))
    except Exception:
...
def serve_forever(app, *, poll_interval: float = DEFAULT_CC_BRIDGE_DAEMON_POLL_INTERVAL_S) -> None:
        ...
        with app.ownership_guard.startup_lock():
            ...
            report = CcbdStartupReport(
                ...
                socket_placement={
                    ...
                    **socket_placement_payload(app.paths.cc_bridge_daemon_socket_placement),
                    **socket_placement_payload(app.paths.cc_bridge_daemon_tmux_socket_placement, prefix='tmux'),
                },
...
def effective_poll_interval(poll_interval: float) -> float:
    try:
        ...
        requested = DEFAULT_CC_BRIDGE_DAEMON_POLL_INTERVAL_S
    try:
        minimum = float(os.environ.get('CC_BRIDGE_CC_BRIDGE_DAEMON_MIN_POLL_INTERVAL_S', '0'))
    except Exception:
        minimum = 0.0
    ...
    *,
    ...
    app.lifecycle_store.save(
        lifecycle.with_phase(
            ...
            socket_path=str(app.paths.cc_bridge_daemon_socket_path),
            socket_inode=current_socket_inode(app.paths.cc_bridge_daemon_socket_path),
            control_plane_endpoint=_current_control_plane_endpoint(app),
...
def _validate_legacy_startup_transaction(lifecycle) -> None:
    ...
    if phase == 'stopping':
        raise StartupFenceError('cannot start cc_bridge_daemon while lifecycle is stopping')
...
def _validate_starting_owner(app, lifecycle, *, generation: int) -> None:
    checks = (
        ...
        (
            ...
            == str(Path(app.paths.cc_bridge_daemon_socket_path)),
...
def _validate_bootstrap_readiness_payload(app, payload) -> None:
    if not isinstance(payload, dict):
        raise RuntimeError('cc_bridge_daemon bootstrap self-ping payload is invalid')
    expected_fence = getattr(app, 'expected_startup_fence', None)
    ...
    for valid, reason in checks:
        if not valid:
            raise RuntimeError(f'cc_bridge_daemon bootstrap self-ping rejected: {reason}')
    if expected_fence is None:
        return
    fenced_checks = (
        (
...
def _validate_lease_holder(app, *, generation: int) -> None:
    ...
    checks = (
        ...
        (int(lease.cc_bridge_daemon_pid) == int(app.pid), 'pid mismatch'),
        (
            str(lease.daemon_instance_id or '') == str(app.daemon_instance_id or ''),
            'daemon_instance_id mismatch',
        ),
        (
            str(Path(lease.socket_path)) == str(Path(app.paths.cc_bridge_daemon_socket_path)),
            'socket_path mismatch',
        ),
...
def _lease_is_app_generation(app, lease, *, generation: int, mount_state: MountState) -> bool:
    ...
    try:
        return (
            ...
            and int(lease.cc_bridge_daemon_pid) == int(app.pid)
            and str(lease.daemon_instance_id or '') == str(app.daemon_instance_id or '')
            and str(Path(lease.socket_path)) == str(Path(app.paths.cc_bridge_daemon_socket_path))
        )
    except (AttributeError, TypeError, ValueError):
        return False
...
def _can_release_lifecycle_without_lease_locked(app) -> bool:
    ...
    if str(Path(getattr(lifecycle, 'socket_path', '') or '')) != str(
        Path(app.paths.cc_bridge_daemon_socket_path)
    ):
        return False
    expected_fence = getattr(app, 'expected_startup_fence', None)
    if expected_fence is not None:
