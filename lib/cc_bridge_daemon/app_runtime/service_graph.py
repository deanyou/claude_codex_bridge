...
from cc_bridge_daemon.project_focus import ProjectFocusDependencies, ProjectFocusService
from cc_bridge_daemon.project_view import ProjectViewDependencies, ProjectViewService
from cc_bridge_daemon.services import AgentRegistry, HealthMonitor, JobDispatcher, RuntimeService
from cc_bridge_daemon.supervision import RuntimeSupervisionLoop
...
def build_cc_bridge_daemon_service_graph(deps: CcbdServiceGraphDependencies) -> CcbdServiceGraph:
...
def current_cc_bridge_daemon_service_graph(app) -> CcbdServiceGraph:
    graph = getattr(app, 'service_graph', None)
    if graph is None:
        raise RuntimeError('cc_bridge_daemon service graph is not published')
    return graph


def publish_cc_bridge_daemon_service_graph(app, graph: CcbdServiceGraph) -> CcbdServiceGraph:
...
__all__ = [
    ...
    'current_cc_bridge_daemon_service_graph',
    'publish_cc_bridge_daemon_service_graph',
]
