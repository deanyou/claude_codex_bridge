...
def write_ask_usage(
    ...
    *,
    ...
    print("      --artifact-request force the request body into a CC_BRIDGE text artifact", file=out)
    print("      --inline-request keep the request body inline and disable automatic artifact spill", file=out)
    print("      --artifact-reply force the final reply into a CC_BRIDGE text artifact", file=out)
