...
def cc_bridge_daemon_summary(*, local, stores: dict[str, object], errors: list[str], remote: dict | None = None) -> dict:
    implementation = _implementation_summary(getattr(local, 'cc_bridge_daemon_pid', None))
    ...
    return {
        ...
        'pid': local.cc_bridge_daemon_pid,
...
def _implementation_summary(pid: object) -> dict[str, object]:
    ...
    if root is None:
        return {
            'root': None,
            'status': 'unknown',
            'reason': 'cc_bridge_daemon_entrypoint_not_found_in_cmdline',
    ...
    if _path_is_temporary(root):
        return {
            'root': str(root),
            'status': 'degraded',
            'reason': 'cc_bridge_daemon_implementation_root_is_temporary',
            'cmdline': shlex.join(cmdline),
        }
    return {
        'root': str(root),
        'status': 'ok',
        'reason': 'cc_bridge_daemon_implementation_root_is_durable',
        'cmdline': shlex.join(cmdline),
    }
...
def _implementation_root_from_cmdline(cmdline: tuple[str, ...]) -> Path | None:
    for arg in cmdline:
        path = Path(arg)
        if path.parts[-3:] in {('lib', 'cc_bridge_daemon', 'main.py'), ('lib', 'cc_bridge_daemon', 'keeper_main.py')}:
            try:
...
__all__ = ['cc_bridge_daemon_summary']
