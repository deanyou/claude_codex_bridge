...
const REQ_ID_PREFIX: &str = "CC_BRIDGE_REQ_ID:";
const DONE_PREFIX: &str = "CC_BRIDGE_DONE:";
...
pub fn handle_request(request: AcceleratorRequest) -> AcceleratorResponse {
    match request.method.as_str() {
        "ping" => AcceleratorResponse::ok(json!({
            ...
            "service": "cc_bridge-runtime-accelerator",
            "status": "ok",
        })),
...
where
    F: Fn() -> bool,
{
    anyhow::bail!("cc_bridge-runtime-accelerator socket server requires Unix sockets")
}
...
pub fn classify_process(command: &str, args: &str) -> String {
    ...
    if text.contains("cc_bridge-runtime-accelerator") {
    ...
    } else if text.contains("cc_bridge_daemon") || text.contains("cc_bridge-daemon") {
        "cc_bridge_daemon".to_string()
...
fn runtime_socket_roots() -> Vec<PathBuf> {
    ...
    if let Ok(raw) = std::env::var("XDG_RUNTIME_DIR") {
        if !raw.trim().is_empty() {
            roots.push(PathBuf::from(raw).join("cc_bridge-runtime"));
        }
    }
    roots.push(PathBuf::from("/tmp").join("cc_bridge-runtime"));
    let temp_root = std::env::temp_dir().join("cc_bridge-runtime");
...
mod tests {
    ...
    fn ping_uses_daemon_like_response_shape() {
        ...
        assert_eq!(result["service"], "cc_bridge-runtime-accelerator");
    ...
    fn classifies_hot_loop_processes() {
        ...
        assert_eq!(classify_process("cc_bridge_daemon", "--project /repo"), "cc_bridge_daemon");
        assert_eq!(classify_process("codex", "--sandbox danger"), "codex_cli");
    }

    #[test]
    fn default_socket_lives_under_project_cc_bridge() {
        assert_eq!(
            default_socket_path(Path::new("/repo")),
            PathBuf::from("/repo/.cc-bridge/runtime-accelerator/accelerator.sock")
        );
    }
    ...
    fn codex_observe_reads_anchor_reply_and_terminal_boundary() {
        ...
        std::fs::write(
            ...
            [
                ...
                json!({
                    "type": "event_msg",
                    "payload": {"type": "user_message", "message": "CC_BRIDGE_REQ_ID: req-1\nhello", "turn_id": "turn-1"}
                })
                .to_string(),
                json!({
                    "type": "response_item",
    ...
    fn codex_observe_task_complete_error_fails_turn_without_success_boundary() {
        let path = std::env::var_os("CC_BRIDGE_TEST_TMPDIR")
            .map(PathBuf::from)
            .unwrap_or_else(std::env::temp_dir)
            .join(format!(
                "cc_bridge-runtime-accelerator-{}-task-complete-error.jsonl",
                std::process::id()
            ));
        std::fs::write(
            &path,
    ...
    fn unique_test_path(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "cc_bridge-runtime-accelerator-{}-{name}",
            std::process::id()
        ))
    }
}
