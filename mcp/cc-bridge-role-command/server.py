#!/usr/bin/env python3
"""Narrow MCP transport for provider-enforced CC_BRIDGE role commands."""
...
SERVER_INFO = {'name': 'cc_bridge-role-command', 'version': '0.1.0'}
FRONTDESK_TOOL_NAME = 'cc_bridge_frontdesk_ask_planner'
DETAILER_TOOL_NAME = 'cc_bridge_task_detailer_replan_planner'
...
_TOOL_DEFS = [
    {
        ...
        'inputSchema': {
            ...
            'properties': {
                'request_id': {
                    'type': 'string',
                    'description': 'The CC_BRIDGE request id used for exact-once handoff.',
                },
                'evidence': {
                    'type': 'string',
    ...
    },
    {
        ...
        'inputSchema': {
            ...
            'properties': {
                ...
                'request': {
                    'type': 'string',
                    'description': 'Exact cc_bridge.detailer.replan_request.v1 JSON authored by Task Detailer.',
                },
            },
...
def _project_root() -> Path:
    ...
    if not raw:
        raise RuntimeError('managed role command is missing CC_BRIDGE_CALLER_PROJECT_ROOT')
    root = Path(raw).expanduser().resolve()
    if not (root / '.cc-bridge' / 'cc_bridge.config').is_file():
...
def _validate_frontdesk_request(request_id: str, evidence: str) -> None:
    ...
    if f'CC_BRIDGE_REQ_ID: {request_id}' not in evidence:
        raise RuntimeError('evidence CC_BRIDGE_REQ_ID must match request_id')
...
def submit_task_detailer_replan(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        actor = str(os.environ.get('CC_BRIDGE_CALLER_ACTOR') or '').strip().lower()
        if actor not in {'task_detailer', 'cc_bridge_task_detailer'}:
        ...
        if not isinstance(request, dict) or request.get('schema') != 'cc_bridge.detailer.replan_request.v1':
            raise RuntimeError('request must use cc_bridge.detailer.replan_request.v1')
