...
def build_context_for(work_dir: str | None):
    ...
    command = ParsedPingCommand(project=None, target='cc_bridge_daemon')
...
def ping_agent(args: dict[str, Any]) -> dict[str, Any]:
    target = (_optional_text(args, "target") or "cc_bridge_daemon").lower()
    work_dir = _optional_text(args, "work_dir")
    try:
        context = build_context_for(work_dir)
...
_TOOL_HANDLERS = {
    "cc_bridge_ask_agent": lambda args, caller: submit_task(args, caller=caller),
    "cc_bridge_pend_agent": lambda args, caller: pend_task(args),
    "cc_bridge_ping_agent": lambda args, caller: ping_agent(args),
}
